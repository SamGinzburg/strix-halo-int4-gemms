	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk               ; -- Begin function amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk,@function
amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk:                      ; @amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 80 0                          ; generate_amdgcn.py:80:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[16:17], s[0:1], 0x30
	s_load_b32 s19, s[0:1], 0x38
.Ltmp0:
	.loc	1 90 16 prologue_end            ; generate_amdgcn.py:90:16
	s_abs_i32 s6, s2
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_and_b32_e32 v39, 15, v0
	v_dual_mov_b32 v8, 0 :: v_dual_lshlrev_b32 v37, 2, v0
	.loc	1 101 14                        ; generate_amdgcn.py:101:14
	v_lshrrev_b32_e32 v46, 4, v0
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v47, 2, v0
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshlrev_b32_e32 v34, 3, v39
	v_dual_mov_b32 v6, v8 :: v_dual_and_b32 v33, 12, v37
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_bfe_i32 v9, v0, 5, 1
	v_mov_b32_e32 v24, v8
	v_dual_mov_b32 v7, v8 :: v_dual_and_b32 v36, 0x60, v0
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_lshlrev_b32_e32 v41, 3, v0
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_and_b32_e32 v9, 0x88, v9
	v_bfe_i32 v42, v0, 3, 1
	v_dual_mov_b32 v21, v8 :: v_dual_lshlrev_b32 v38, 4, v0
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:88:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s17, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s7, s16, 63
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s9, s7, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_lshr_b32 s5, s5, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_lshr_b32 s9, s9, 26
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s7, s7, s9
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s4, s4, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s7, s7, 6
.Ltmp11:
	.loc	1 89 24 is_stmt 1               ; generate_amdgcn.py:89:24
	s_lshl_b32 s14, s4, 2
	v_mov_b32_e32 v20, v8
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_abs_i32 s4, s14
	v_mov_b32_e32 v19, v8
	s_cvt_f32_u32 s5, s4
	s_sub_i32 s8, 0, s4
	v_mov_b32_e32 v18, v8
	v_mov_b32_e32 v17, v8
	v_rcp_iflag_f32_e32 v1, s5
	v_mov_b32_e32 v32, v8
	v_mov_b32_e32 v31, v8
	v_mov_b32_e32 v30, v8
	v_mov_b32_e32 v29, v8
	v_mov_b32_e32 v28, v8
	v_mov_b32_e32 v27, v8
	v_mov_b32_e32 v26, v8
	v_mov_b32_e32 v25, v8
	v_mov_b32_e32 v16, v8
	v_readfirstlane_b32 s5, v1
	v_mov_b32_e32 v15, v8
	v_mov_b32_e32 v14, v8
	v_mov_b32_e32 v13, v8
	v_mov_b32_e32 v12, v8
	s_mul_f32 s5, s5, 0x4f7ffffe
	v_mov_b32_e32 v11, v8
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshrrev_b32_e32 v40, 1, v36
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_and_b32_e32 v10, 0x37c, v37
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_cvt_u32_f32 s5, s5
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(VALU_DEP_1)
	s_mul_i32 s8, s8, s5
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_xor_b32_e32 v44, v9, v10
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_mul_hi_u32 s8, s5, s8
	v_mov_b32_e32 v10, v8
	s_add_i32 s5, s5, s8
	s_xor_b32 s8, s2, s14
	s_mul_hi_u32 s5, s6, s5
	s_ashr_i32 s8, s8, 31
	s_mul_i32 s9, s5, s4
	v_mov_b32_e32 v9, v8
	s_sub_i32 s6, s6, s9
	s_add_i32 s9, s5, 1
	s_sub_i32 s10, s6, s4
	s_cmp_ge_u32 s6, s4
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v44, 0, v44
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_cselect_b32 s5, s9, s5
	s_cselect_b32 s6, s10, s6
	s_add_i32 s9, s5, 1
	s_cmp_ge_u32 s6, s4
	s_cselect_b32 s4, s9, s5
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s4, s4, s8
	s_sub_i32 s15, s4, s8
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 91 19                         ; generate_amdgcn.py:91:19
	s_lshl_b32 s16, s15, 2
	.loc	1 93 29                         ; generate_amdgcn.py:93:29
	s_mul_i32 s15, s15, s14
	.loc	1 92 31                         ; generate_amdgcn.py:92:31
	s_sub_i32 s4, s7, s16
	.loc	1 93 29                         ; generate_amdgcn.py:93:29
	s_sub_i32 s2, s2, s15
	.loc	1 92 20                         ; generate_amdgcn.py:92:20
	s_min_i32 s18, s4, 4
	s_load_b256 s[4:11], s[0:1], 0x0
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_abs_i32 s21, s18
	s_abs_i32 s14, s2
	s_cvt_f32_u32 s12, s21
	s_load_b64 s[0:1], s[0:1], 0x20
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s12
	v_readfirstlane_b32 s12, v1
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	v_mad_u64_u32 v[1:2], null, s17, v46, v[34:35]
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_and_b32_e32 v35, 0x80, v0
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_mul_f32 s12, s12, 0x4f7ffffe
	s_cvt_u32_f32 s20, s12
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[12:13], s[6:7]
	s_sub_i32 s6, 0, s21
	s_mov_b32 s7, 0x31027000
	s_mul_i32 s6, s6, s20
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s6, s20, s6
	s_add_i32 s20, s20, s6
	s_xor_b32 s6, s2, s18
	s_mul_hi_u32 s15, s14, s20
	s_ashr_i32 s20, s6, 31
	s_mul_i32 s22, s15, s21
	s_mov_b32 s6, 0x7ffffffe
	s_sub_i32 s14, s14, s22
	s_add_i32 s22, s15, 1
	s_sub_i32 s23, s14, s21
	s_cmp_ge_u32 s14, s21
	s_cselect_b32 s15, s22, s15
	s_cselect_b32 s14, s23, s14
	s_add_i32 s22, s15, 1
	s_cmp_ge_u32 s14, s21
	s_cselect_b32 s14, s22, s15
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_lshr_b32 s15, s19, 31
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_xor_b32 s22, s14, s20
	.loc	1 108 23                        ; generate_amdgcn.py:108:23
	s_lshl_b32 s23, s3, 4
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_add_i32 s3, s19, s15
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_sub_i32 s15, s22, s20
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_ashr_i32 s21, s3, 1
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_mul_i32 s18, s15, s18
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	v_mad_u64_u32 v[2:3], null, s21, v47, v[33:34]
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_sub_i32 s2, s2, s18
	.loc	1 108 18                        ; generate_amdgcn.py:108:18
	v_or_b32_e32 v4, s23, v33
	.loc	1 100 14                        ; generate_amdgcn.py:100:14
	s_lshl_b32 s3, s15, 7
	.loc	1 93 13                         ; generate_amdgcn.py:93:13
	s_add_i32 s15, s2, s16
	.loc	1 108 18                        ; generate_amdgcn.py:108:18
	v_or_b32_e32 v5, s23, v46
	.loc	1 99 14                         ; generate_amdgcn.py:99:14
	s_lshl_b32 s16, s15, 6
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cmp_lt_i32 s19, 2
	.loc	1 109 23                        ; generate_amdgcn.py:109:23
	v_cmp_gt_i32_e32 vcc_lo, s21, v4
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	s_mul_i32 s15, s16, s21
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cselect_b32 s18, -1, 0
	s_cmp_gt_i32 s19, 1
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	s_mul_i32 s14, s17, s23
	.loc	1 110 23                        ; generate_amdgcn.py:110:23
	v_cmp_gt_i32_e64 s2, s21, v5
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	v_add3_u32 v2, s23, s15, v2
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cselect_b32 s15, -1, 0
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	v_add3_u32 v1, s14, s3, v1
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_and_b32 vcc_lo, s15, vcc_lo
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_and_b32 s5, s5, 0xffff
	v_cndmask_b32_e32 v2, 0x80000000, v2, vcc_lo
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_and_b32 vcc_lo, s15, s2
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_mov_b32 s14, s6
	v_cndmask_b32_e32 v1, 0x80000000, v1, vcc_lo
	s_mov_b32 s15, s7
	s_and_b32 s13, s13, 0xffff
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	buffer_load_b32 v45, v2, s[4:7], 0 offen
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	buffer_load_b64 v[48:49], v1, s[12:15], 0 offen
	v_mov_b32_e32 v3, v8
	v_mov_b32_e32 v5, v8
	v_mov_b32_e32 v4, v8
	v_mov_b32_e32 v2, v8
	v_mov_b32_e32 v1, v8
	v_mov_b32_e32 v23, v8
	v_dual_mov_b32 v22, v8 :: v_dual_add_nc_u32 v43, 0, v41
	v_and_b32_e32 v41, 0x88, v42
	v_lshlrev_b32_e32 v42, 1, v35
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cmpk_lt_i32 s19, 0x102
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_waitcnt vmcnt(1)
	ds_store_b32 v44, v45 offset:2048
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_waitcnt vmcnt(0)
	ds_store_b64 v43, v[48:49]
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cbranch_scc1 .LBB0_3
; %bb.1:                                ; %.lr.ph
	s_add_i32 s2, s23, 0x80
	v_dual_mov_b32 v9, 0 :: v_dual_lshlrev_b32 v2, 4, v0
	v_add_nc_u32_e32 v1, s2, v46
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:106:32 ]
	s_add_i32 s14, s21, 0x7f
	v_add_nc_u32_e32 v4, 0, v39
	.loc	2 43 12 is_stmt 0               ; standard.py:43:12 @[ generate_amdgcn.py:106:32 ]
	s_lshr_b32 s14, s14, 7
	v_dual_mov_b32 v11, v9 :: v_dual_and_b32 v2, 0x70, v2
.Ltmp13:
	.loc	1 106 9 is_stmt 1               ; generate_amdgcn.py:106:9
	v_sub_nc_u32_e64 v3, s14, 2 clamp
	v_mov_b32_e32 v10, v9
	v_mul_lo_u32 v1, s17, v1
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_3) | instid1(VALU_DEP_3)
	v_or3_b32 v2, v41, v42, v2
	s_lshl_b32 s14, s22, 7
	v_readfirstlane_b32 s19, v3
	v_dual_mov_b32 v12, v9 :: v_dual_add_nc_u32 v3, s16, v47
	v_xor_b32_e32 v5, 8, v2
	v_sub_nc_u32_e32 v45, s21, v33
	v_add3_u32 v1, v1, s14, v34
	s_delay_alu instid0(VALU_DEP_4)
	v_mad_u64_u32 v[33:34], null, s21, v3, v[33:34]
	s_lshl_b32 s15, s20, 7
	v_sub_nc_u32_e32 v46, s21, v46
	v_subrev_nc_u32_e32 v47, s15, v1
	v_dual_mov_b32 v15, v9 :: v_dual_add_nc_u32 v34, 0, v2
	v_dual_mov_b32 v25, v9 :: v_dual_add_nc_u32 v48, 0, v5
	v_mov_b32_e32 v13, v9
	v_dual_mov_b32 v14, v9 :: v_dual_add_nc_u32 v49, v4, v40
	v_mov_b32_e32 v16, v9
	v_mov_b32_e32 v26, v9
	v_mov_b32_e32 v27, v9
	v_mov_b32_e32 v28, v9
	v_mov_b32_e32 v29, v9
	v_mov_b32_e32 v30, v9
	v_mov_b32_e32 v31, v9
	v_mov_b32_e32 v32, v9
	v_mov_b32_e32 v17, v9
	v_mov_b32_e32 v18, v9
	v_mov_b32_e32 v19, v9
	v_mov_b32_e32 v20, v9
	v_mov_b32_e32 v21, v9
	v_mov_b32_e32 v22, v9
	v_mov_b32_e32 v23, v9
	v_mov_b32_e32 v24, v9
	v_mov_b32_e32 v1, v9
	v_mov_b32_e32 v2, v9
	v_mov_b32_e32 v3, v9
	v_mov_b32_e32 v4, v9
	v_mov_b32_e32 v5, v9
	v_mov_b32_e32 v6, v9
	v_mov_b32_e32 v7, v9
	v_mov_b32_e32 v8, v9
	s_add_i32 s19, s19, 1
	s_lshl_b32 s20, s17, 7
	s_mov_b32 s14, s6
	s_mov_b32 s15, s7
.LBB0_2:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 110 23                        ; generate_amdgcn.py:110:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v46
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v50, s2, v33
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_add_i32 s19, s19, -1
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_cndmask_b32_e32 v51, 0x80000000, v47, vcc_lo
	.loc	1 109 23                        ; generate_amdgcn.py:109:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v45
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	v_add_nc_u32_e32 v47, s20, v47
	s_addk_i32 s2, 0x80
	s_cmp_lg_u32 s19, 0
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_cndmask_b32_e32 v50, 0x80000000, v50, vcc_lo
	buffer_load_b32 v68, v50, s[4:7], 0 offen
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	buffer_load_b64 v[58:59], v51, s[12:15], 0 offen
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	ds_load_u8 v60, v49 offset:1664
	ds_load_u8 v61, v49 offset:1536
	ds_load_u8 v62, v49 offset:1920
	ds_load_u8 v63, v49 offset:1792
	ds_load_u8 v64, v49 offset:1152
	ds_load_u8 v65, v49 offset:1024
	ds_load_u8 v66, v49 offset:1408
	ds_load_u8 v67, v49 offset:1280
	ds_load_u8 v69, v49 offset:640
	ds_load_u8 v70, v49 offset:512
	ds_load_u8 v71, v49 offset:896
	ds_load_u8 v72, v49 offset:768
	ds_load_u8 v73, v49 offset:960
	ds_load_u8 v74, v49 offset:832
	ds_load_u8 v75, v49 offset:704
	ds_load_u8 v76, v49 offset:576
	ds_load_u8 v77, v49 offset:128
	ds_load_u8 v78, v49
	ds_load_u8 v79, v49 offset:384
	ds_load_u8 v80, v49 offset:256
	ds_load_u8 v81, v49 offset:448
	ds_load_u8 v82, v49 offset:320
	ds_load_u8 v83, v49 offset:192
	ds_load_u8 v84, v49 offset:64
	ds_load_u8 v85, v49 offset:1984
	ds_load_u8 v86, v49 offset:1856
	ds_load_u8 v87, v49 offset:1728
	ds_load_u8 v88, v49 offset:1600
	ds_load_u8 v89, v49 offset:1472
	ds_load_u8 v90, v49 offset:1344
	ds_load_u8 v91, v49 offset:1216
	ds_load_u8 v92, v49 offset:1088
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_2addr_stride64_b64 v[50:53], v34 offset0:4 offset1:5
	ds_load_2addr_stride64_b64 v[54:57], v48 offset0:4 offset1:5
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v69, v70, v69, 0xc0c0004
	s_waitcnt lgkmcnt(22)
	v_perm_b32 v70, v72, v71, 0xc0c0004
	v_perm_b32 v66, v67, v66, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v71, v78, v77, 0xc0c0004
	v_perm_b32 v78, v65, v64, 0xc0c0004
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v72, v80, v79, 0xc0c0004
	v_perm_b32 v64, v76, v75, 0xc0c0004
	v_perm_b32 v65, v74, v73, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v73, v82, v81, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v67, v84, v83, 0xc0c0004
	v_perm_b32 v77, v61, v60, 0xc0c0004
	v_perm_b32 v62, v63, v62, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v75, v86, v85, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v74, v88, v87, 0xc0c0004
	v_lshl_or_b32 v61, v70, 16, v69
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v79, v90, v89, 0xc0c0004
	v_lshl_or_b32 v60, v72, 16, v71
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v76, v92, v91, 0xc0c0004
	v_lshl_or_b32 v65, v65, 16, v64
	v_lshl_or_b32 v64, v73, 16, v67
	v_lshl_or_b32 v63, v62, 16, v77
	v_lshl_or_b32 v62, v66, 16, v78
	v_lshl_or_b32 v67, v75, 16, v74
	v_lshl_or_b32 v66, v79, 16, v76
	s_waitcnt lgkmcnt(1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[60:61], v[50:51], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[25:32], v[64:65], v[50:51], v[25:32] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[17:24], v[60:61], v[52:53], v[17:24] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[1:8], v[64:65], v[52:53], v[1:8] neg_lo:[1,1,0]
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_waitcnt lgkmcnt(0)
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_wmma_i32_16x16x16_iu4 v[9:16], v[62:63], v[54:55], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[25:32], v[66:67], v[54:55], v[25:32] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[17:24], v[62:63], v[56:57], v[17:24] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[1:8], v[66:67], v[56:57], v[1:8] neg_lo:[1,1,0]
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_barrier
	s_waitcnt vmcnt(1)
	ds_store_b32 v44, v68 offset:2048
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_waitcnt vmcnt(0)
	ds_store_b64 v43, v[58:59]
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cbranch_scc1 .LBB0_2
.LBB0_3:                                ; %._crit_edge
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_and_not1_b32 vcc_lo, exec_lo, s18
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_cbranch_vccnz .LBB0_6
; %bb.4:                                ; %._crit_edge._crit_edge
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshlrev_b32_e32 v33, 4, v0
	s_cbranch_execz .LBB0_7
; %bb.5:
	.loc	1 0 9 is_stmt 0                 ; generate_amdgcn.py:0:9
	s_delay_alu instid0(VALU_DEP_1)
	v_mov_b32_e32 v38, v33
	s_branch .LBB0_8
.LBB0_6:
                                        ; implicit-def: $vgpr33
.LBB0_7:
	.loc	1 131 22 is_stmt 1              ; generate_amdgcn.py:131:22
	v_add3_u32 v33, 0, v39, v40
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_and_b32_e32 v34, 0x70, v38
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	ds_load_u8 v45, v33 offset:640
	ds_load_u8 v46, v33 offset:512
	ds_load_u8 v47, v33 offset:896
	ds_load_u8 v48, v33 offset:768
	ds_load_u8 v49, v33 offset:128
	ds_load_u8 v50, v33
	ds_load_u8 v51, v33 offset:384
	ds_load_u8 v52, v33 offset:256
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_or3_b32 v34, v41, v42, v34
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	ds_load_u8 v53, v33 offset:960
	ds_load_u8 v54, v33 offset:832
	ds_load_u8 v55, v33 offset:704
	ds_load_u8 v56, v33 offset:576
	ds_load_u8 v57, v33 offset:448
	ds_load_u8 v58, v33 offset:320
	ds_load_u8 v59, v33 offset:192
	ds_load_u8 v60, v33 offset:64
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v41, 0, v34
	v_xad_u32 v61, v34, 8, 0
	ds_load_2addr_stride64_b64 v[41:44], v41 offset0:4 offset1:5
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(15)
	v_perm_b32 v34, v46, v45, 0xc0c0004
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v45, v48, v47, 0xc0c0004
	s_waitcnt lgkmcnt(11)
	v_perm_b32 v46, v50, v49, 0xc0c0004
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v47, v52, v51, 0xc0c0004
	ds_load_u8 v49, v33 offset:1664
	ds_load_u8 v50, v33 offset:1536
	ds_load_u8 v51, v33 offset:1152
	ds_load_u8 v52, v33 offset:1024
	ds_load_u8 v62, v33 offset:1408
	ds_load_u8 v63, v33 offset:1280
	ds_load_u8 v64, v33 offset:1920
	ds_load_u8 v65, v33 offset:1792
	ds_load_u8 v66, v33 offset:1984
	ds_load_u8 v67, v33 offset:1856
	ds_load_u8 v68, v33 offset:1728
	ds_load_u8 v69, v33 offset:1600
	ds_load_u8 v70, v33 offset:1472
	ds_load_u8 v71, v33 offset:1344
	ds_load_u8 v72, v33 offset:1216
	ds_load_u8 v73, v33 offset:1088
	v_lshl_or_b32 v34, v45, 16, v34
	v_lshl_or_b32 v33, v47, 16, v46
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_2addr_stride64_b64 v[45:48], v61 offset0:4 offset1:5
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(15)
	v_perm_b32 v61, v50, v49, 0xc0c0004
	v_perm_b32 v49, v56, v55, 0xc0c0004
	v_perm_b32 v50, v54, v53, 0xc0c0004
	v_perm_b32 v53, v60, v59, 0xc0c0004
	v_perm_b32 v54, v58, v57, 0xc0c0004
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v51, v52, v51, 0xc0c0004
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v55, v65, v64, 0xc0c0004
	v_perm_b32 v56, v63, v62, 0xc0c0004
	v_lshl_or_b32 v50, v50, 16, v49
	v_lshl_or_b32 v49, v54, 16, v53
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v53, v69, v68, 0xc0c0004
	v_perm_b32 v54, v67, v66, 0xc0c0004
	s_waitcnt lgkmcnt(3)
	v_perm_b32 v58, v71, v70, 0xc0c0004
	v_wmma_i32_16x16x16_iu4 v[9:16], v[33:34], v[41:42], v[9:16] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(1)
	v_perm_b32 v57, v73, v72, 0xc0c0004
	v_lshl_or_b32 v52, v55, 16, v61
	v_lshl_or_b32 v51, v56, 16, v51
	v_wmma_i32_16x16x16_iu4 v[25:32], v[49:50], v[41:42], v[25:32] neg_lo:[1,1,0]
	v_lshl_or_b32 v42, v54, 16, v53
	v_lshl_or_b32 v41, v58, 16, v57
	v_wmma_i32_16x16x16_iu4 v[17:24], v[33:34], v[43:44], v[17:24] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[1:8], v[49:50], v[43:44], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[51:52], v[45:46], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[25:32], v[41:42], v[45:46], v[25:32] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[17:24], v[51:52], v[47:48], v[17:24] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[1:8], v[41:42], v[47:48], v[1:8] neg_lo:[1,1,0]
.LBB0_8:
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v33, 3, v35
	.loc	1 142 14                        ; generate_amdgcn.py:142:14
	s_and_b32 s5, s9, 0xffff
	s_mov_b32 s7, 0x31027000
	s_mov_b32 s6, 0x7ffffffe
	s_mov_b32 s4, s8
	.loc	1 142 22 is_stmt 0              ; generate_amdgcn.py:142:22
	v_or3_b32 v33, v33, v39, s16
	.loc	1 100 32 is_stmt 1              ; generate_amdgcn.py:100:32
	v_bfe_u32 v34, v0, 4, 1
	v_and_b32_e32 v55, 0x7f, v0
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v61, v1
	v_cvt_f32_i32_e32 v62, v2
	.loc	1 142 22                        ; generate_amdgcn.py:142:22
	v_lshlrev_b32_e32 v33, 1, v33
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v63, v3
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_and_b32_e32 v1, 14, v0
	v_and_b32_e32 v2, 0x78, v37
	v_and_b32_e32 v3, 1, v0
	.loc	1 142 14                        ; generate_amdgcn.py:142:14
	v_or_b32_e32 v39, 64, v33
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_bfe_i32 v0, v0, 0, 1
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v67, v7
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshl_or_b32 v2, v36, 3, v2
	.loc	1 142 14                        ; generate_amdgcn.py:142:14
	s_clause 0x1
	buffer_load_u16 v33, v33, s[4:7], 0 offen
	buffer_load_u16 v39, v39, s[4:7], 0 offen
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshlrev_b32_e32 v7, 11, v1
	v_and_b32_e32 v0, 0x840, v0
	.loc	1 143 22                        ; generate_amdgcn.py:143:22
	v_or3_b32 v34, v34, v40, s3
	.loc	1 143 14 is_stmt 0              ; generate_amdgcn.py:143:14
	s_and_b32 s5, s11, 0xffff
	s_mov_b32 s4, s10
	.loc	1 144 9 is_stmt 1               ; generate_amdgcn.py:144:9
	v_cmp_eq_u32_e32 vcc_lo, 0, v35
	v_xor_b32_e32 v0, v2, v0
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v56, 7, v35
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v65, v5
	v_cvt_f32_i32_e32 v66, v6
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_cndmask_b32_e64 v5, 0x840, 0, vcc_lo
	v_or3_b32 v0, v0, v7, v35
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	v_mul_lo_u32 v6, s17, v56
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v57, v18
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_mul_i32 s2, s16, s17
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v58, v20
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_add_nc_u32_e32 v83, 0, v0
	v_xad_u32 v84, v0, 8, 0
	v_xad_u32 v85, v0, 16, 0
	v_xad_u32 v86, v0, 24, 0
	v_xad_u32 v87, v0, 32, 0
	v_xad_u32 v88, v0, 40, 0
	v_xad_u32 v89, v0, 48, 0
	v_xad_u32 v90, v0, 56, 0
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_add_i32 s2, s2, s3
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v60, v24
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	v_add3_u32 v36, s2, v55, v6
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v64, v4
	v_cvt_f32_i32_e32 v68, v8
	v_cvt_f32_i32_e32 v9, v9
	v_cvt_f32_i32_e32 v59, v22
	v_cvt_f32_i32_e32 v11, v11
	v_cvt_f32_i32_e32 v10, v10
	v_cvt_f32_i32_e32 v12, v12
	v_cvt_f32_i32_e32 v14, v14
	v_cvt_f32_i32_e32 v16, v16
	v_cvt_f32_i32_e32 v25, v25
	v_cvt_f32_i32_e32 v27, v27
	v_cvt_f32_i32_e32 v17, v17
	v_cvt_f32_i32_e32 v13, v13
	v_cvt_f32_i32_e32 v15, v15
	v_cvt_f32_i32_e32 v26, v26
	v_cvt_f32_i32_e32 v28, v28
	v_cvt_f32_i32_e32 v31, v31
	v_cvt_f32_i32_e32 v19, v19
	v_cvt_f32_i32_e32 v30, v30
	v_cvt_f32_i32_e32 v32, v32
	v_cvt_f32_i32_e32 v21, v21
	v_cvt_f32_i32_e32 v23, v23
	v_cvt_f32_i32_e32 v29, v29
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_lshl_b32 s8, s17, 2
	s_mul_i32 s9, s17, 6
	s_lshl_b32 s10, s17, 3
	s_mul_i32 s11, s17, 10
	s_mul_i32 s12, s17, 12
	s_mul_i32 s13, s17, 14
	s_lshl_b32 s14, s17, 4
	s_mul_i32 s15, s17, 18
	s_mul_i32 s16, s17, 20
	s_mul_i32 s18, s17, 22
	s_mul_i32 s19, s17, 24
	s_mul_i32 s20, s17, 26
	s_mul_i32 s21, s17, 28
	s_mul_i32 s22, s17, 30
	s_lshl_b32 s23, s17, 5
	s_mul_i32 s24, s17, 34
	s_mul_i32 s25, s17, 36
	s_mul_i32 s26, s17, 38
	s_mul_i32 s27, s17, 40
	s_mul_i32 s28, s17, 42
	s_mul_i32 s29, s17, 44
	s_mul_i32 s30, s17, 46
	s_mul_i32 s31, s17, 48
	s_mul_i32 s33, s17, 50
	s_mul_i32 s34, s17, 52
	.loc	1 218 9 is_stmt 0               ; generate_amdgcn.py:218:9
	s_and_b32 s1, s1, 0xffff
	s_mov_b32 s2, s6
	s_mov_b32 s3, s7
	v_add_lshl_u32 v55, v36, s9, 2
	v_add_lshl_u32 v56, v36, s10, 2
	v_add_lshl_u32 v69, v36, s11, 2
	v_add_lshl_u32 v70, v36, s12, 2
	v_add_lshl_u32 v71, v36, s13, 2
	v_add_lshl_u32 v72, v36, s14, 2
	v_add_lshl_u32 v73, v36, s15, 2
	v_add_lshl_u32 v74, v36, s16, 2
	v_add_lshl_u32 v75, v36, s18, 2
	v_add_lshl_u32 v76, v36, s19, 2
	v_add_lshl_u32 v77, v36, s20, 2
	v_add_lshl_u32 v78, v36, s21, 2
	v_add_lshl_u32 v79, v36, s22, 2
	v_add_lshl_u32 v80, v36, s23, 2
	v_add_lshl_u32 v81, v36, s24, 2
	v_add_lshl_u32 v82, v36, s25, 2
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_mul_i32 s35, s17, 54
	s_mul_i32 s36, s17, 56
	s_mul_i32 s37, s17, 58
	s_mul_i32 s38, s17, 60
	.loc	1 142 14 is_stmt 1              ; generate_amdgcn.py:142:14
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v0, 16, v33
	.loc	1 143 22                        ; generate_amdgcn.py:143:22
	v_lshlrev_b32_e32 v34, 1, v34
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshlrev_b32_e32 v3, 6, v3
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	v_or_b32_e32 v40, 4, v34
	v_or_b32_e32 v41, 8, v34
	v_or_b32_e32 v42, 12, v34
	v_or_b32_e32 v43, 16, v34
	v_or_b32_e32 v44, 20, v34
	v_or_b32_e32 v45, 24, v34
	v_or_b32_e32 v46, 28, v34
	s_clause 0x7
	buffer_load_u16 v47, v34, s[4:7], 0 offen
	buffer_load_u16 v40, v40, s[4:7], 0 offen
	buffer_load_u16 v41, v41, s[4:7], 0 offen
	buffer_load_u16 v42, v42, s[4:7], 0 offen
	buffer_load_u16 v43, v43, s[4:7], 0 offen
	buffer_load_u16 v44, v44, s[4:7], 0 offen
	buffer_load_u16 v45, v45, s[4:7], 0 offen
	buffer_load_u16 v46, v46, s[4:7], 0 offen
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_xor_b32_e32 v2, v5, v3
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	s_waitcnt vmcnt(3)
	v_lshlrev_b32_e32 v6, 16, v43
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v18, 16, v46
	v_lshlrev_b32_e32 v7, 16, v44
	v_lshlrev_b32_e32 v3, 16, v40
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v43, v0, v3
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	v_or_b32_e32 v48, 0x80, v34
	v_or_b32_e32 v49, 0x84, v34
	v_or_b32_e32 v50, 0x88, v34
	v_or_b32_e32 v51, 0x8c, v34
	v_or_b32_e32 v52, 0x90, v34
	v_or_b32_e32 v53, 0x94, v34
	v_or_b32_e32 v54, 0x98, v34
	v_or_b32_e32 v34, 0x9c, v34
	s_clause 0x7
	buffer_load_u16 v48, v48, s[4:7], 0 offen
	buffer_load_u16 v49, v49, s[4:7], 0 offen
	buffer_load_u16 v50, v50, s[4:7], 0 offen
	buffer_load_u16 v51, v51, s[4:7], 0 offen
	buffer_load_u16 v52, v52, s[4:7], 0 offen
	buffer_load_u16 v53, v53, s[4:7], 0 offen
	buffer_load_u16 v54, v54, s[4:7], 0 offen
	buffer_load_u16 v34, v34, s[4:7], 0 offen
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_lshl_b32 s5, s17, 1
	s_mul_i32 s4, s17, 62
	.loc	1 218 9 is_stmt 0               ; generate_amdgcn.py:218:9
	v_add_lshl_u32 v37, v36, s5, 2
	.loc	1 143 14 is_stmt 1              ; generate_amdgcn.py:143:14
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v34, 16, v34
	v_lshlrev_b32_e32 v20, 16, v48
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshlrev_b32_e32 v1, 2, v1
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	v_lshlrev_b32_e32 v24, 16, v50
	v_lshlrev_b32_e32 v40, 16, v53
	v_lshlrev_b32_e32 v5, 16, v42
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_and_b32_e32 v4, 0x700, v38
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	v_lshlrev_b32_e32 v8, 16, v45
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v50, v0, v20
	.loc	1 218 9                         ; generate_amdgcn.py:218:9
	v_add_lshl_u32 v38, v36, s8, 2
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v45, v0, v5
	.loc	1 144 9 is_stmt 0               ; generate_amdgcn.py:144:9
	v_or3_b32 v1, v4, v1, v2
	s_delay_alu instid0(VALU_DEP_1)
	v_add_nc_u32_e32 v91, 0, v1
	v_xad_u32 v92, 0x1008, v1, 0
	v_xad_u32 v93, 0x2010, v1, 0
	v_xad_u32 v94, 0x3018, v1, 0
	v_xad_u32 v95, 0x4020, v1, 0
	v_xad_u32 v96, 0x5028, v1, 0
	v_xad_u32 v97, 0x6030, v1, 0
	v_xad_u32 v98, 0x7038, v1, 0
	.loc	1 142 14 is_stmt 1              ; generate_amdgcn.py:142:14
	v_lshlrev_b32_e32 v1, 16, v39
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	v_lshlrev_b32_e32 v22, 16, v49
	v_lshlrev_b32_e32 v4, 16, v41
	v_lshlrev_b32_e32 v33, 16, v51
	v_lshlrev_b32_e32 v39, 16, v52
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_3) | instid1(VALU_DEP_4)
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_dual_mul_f32 v103, v1, v4 :: v_dual_lshlrev_b32 v2, 16, v47
	v_mul_f32_e32 v49, v0, v18
	v_mul_f32_e32 v51, v0, v22
	v_dual_mul_f32 v44, v0, v4 :: v_dual_lshlrev_b32 v41, 16, v54
	v_mul_f32_e32 v42, v0, v2
	v_mul_f32_e32 v47, v0, v7
	v_mul_f32_e32 v52, v0, v24
	v_mul_f32_e32 v53, v0, v33
	v_mul_f32_e32 v46, v0, v6
	v_mul_f32_e32 v54, v0, v39
	v_mul_f32_e32 v99, v0, v40
	v_mul_f32_e32 v48, v0, v8
	v_mul_f32_e32 v101, v0, v34
	.loc	1 144 9 is_stmt 0               ; generate_amdgcn.py:144:9
	v_mul_f32_e32 v4, v44, v11
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v100, v0, v41
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_dual_mul_f32 v0, v42, v9 :: v_dual_lshlrev_b32 v35, 2, v36
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v102, v1, v2
	v_mul_f32_e32 v109, v1, v20
	v_mul_f32_e32 v105, v1, v6
	v_mul_f32_e32 v107, v1, v8
	v_mul_f32_e32 v108, v1, v18
	v_mul_f32_e32 v106, v1, v7
	v_mul_f32_e32 v3, v1, v3
	v_mul_f32_e32 v110, v1, v22
	v_mul_f32_e32 v111, v1, v24
	v_mul_f32_e32 v33, v1, v33
	v_mul_f32_e32 v40, v1, v40
	v_mul_f32_e32 v104, v1, v5
	v_mul_f32_e32 v41, v1, v41
	v_mul_f32_e32 v34, v1, v34
	v_dual_mul_f32 v39, v1, v39 :: v_dual_mul_f32 v2, v43, v10
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_dual_mul_f32 v6, v45, v12 :: v_dual_mul_f32 v1, v102, v17
	v_mul_f32_e32 v10, v47, v14
	v_dual_mul_f32 v14, v49, v16 :: v_dual_mul_f32 v7, v104, v58
	v_mul_f32_e32 v16, v50, v25
	v_dual_mul_f32 v20, v52, v27 :: v_dual_mul_f32 v17, v109, v61
	v_dual_mul_f32 v8, v46, v13 :: v_dual_mul_f32 v5, v103, v19
	v_dual_mul_f32 v12, v48, v15 :: v_dual_mul_f32 v3, v3, v57
	v_dual_mul_f32 v18, v51, v26 :: v_dual_mul_f32 v9, v105, v21
	v_dual_mul_f32 v22, v53, v28 :: v_dual_mul_f32 v11, v106, v59
	v_dual_mul_f32 v28, v100, v31 :: v_dual_mul_f32 v19, v110, v62
	v_mul_f32_e32 v26, v99, v30
	v_dual_mul_f32 v30, v101, v32 :: v_dual_mul_f32 v21, v111, v63
	v_mul_f32_e32 v13, v107, v23
	v_dual_mul_f32 v23, v33, v64 :: v_dual_mul_f32 v24, v54, v29
	v_mul_f32_e32 v15, v108, v60
	v_mul_f32_e32 v25, v39, v65
	v_mul_f32_e32 v27, v40, v66
	v_mul_f32_e32 v29, v41, v67
	v_mul_f32_e32 v31, v34, v68
	ds_store_2addr_stride64_b64 v83, v[0:1], v[16:17] offset1:2
	ds_store_2addr_stride64_b64 v84, v[2:3], v[18:19] offset1:2
	ds_store_2addr_stride64_b64 v85, v[4:5], v[20:21] offset1:2
	ds_store_2addr_stride64_b64 v86, v[6:7], v[22:23] offset1:2
	ds_store_2addr_stride64_b64 v87, v[8:9], v[24:25] offset1:2
	ds_store_2addr_stride64_b64 v88, v[10:11], v[26:27] offset1:2
	ds_store_2addr_stride64_b64 v89, v[12:13], v[28:29] offset1:2
	ds_store_2addr_stride64_b64 v90, v[14:15], v[30:31] offset1:2
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_2addr_b64 v[0:3], v91 offset1:16
	ds_load_2addr_b64 v[4:7], v92 offset1:16
	ds_load_2addr_b64 v[8:11], v93 offset1:16
	ds_load_2addr_b64 v[12:15], v94 offset1:16
	ds_load_2addr_b64 v[16:19], v95 offset1:16
	ds_load_2addr_b64 v[20:23], v96 offset1:16
	ds_load_2addr_b64 v[24:27], v97 offset1:16
	ds_load_2addr_b64 v[28:31], v98 offset1:16
	.loc	1 218 9 is_stmt 1               ; generate_amdgcn.py:218:9
	v_add_lshl_u32 v32, v36, s26, 2
	v_add_lshl_u32 v33, v36, s27, 2
	v_add_lshl_u32 v34, v36, s28, 2
	v_add_lshl_u32 v39, v36, s29, 2
	v_add_lshl_u32 v40, v36, s30, 2
	v_add_lshl_u32 v41, v36, s31, 2
	v_add_lshl_u32 v42, v36, s33, 2
	v_add_lshl_u32 v43, v36, s34, 2
	s_waitcnt lgkmcnt(7)
	buffer_atomic_add_f32 v0, v35, s[0:3], 0 offen
	s_waitcnt lgkmcnt(6)
	buffer_atomic_add_f32 v4, v37, s[0:3], 0 offen
	s_waitcnt lgkmcnt(5)
	buffer_atomic_add_f32 v8, v38, s[0:3], 0 offen
	s_waitcnt lgkmcnt(4)
	buffer_atomic_add_f32 v12, v55, s[0:3], 0 offen
	s_waitcnt lgkmcnt(3)
	buffer_atomic_add_f32 v16, v56, s[0:3], 0 offen
	s_waitcnt lgkmcnt(2)
	buffer_atomic_add_f32 v20, v69, s[0:3], 0 offen
	s_waitcnt lgkmcnt(1)
	buffer_atomic_add_f32 v24, v70, s[0:3], 0 offen
	s_waitcnt lgkmcnt(0)
	s_clause 0x13
	buffer_atomic_add_f32 v28, v71, s[0:3], 0 offen
	buffer_atomic_add_f32 v2, v72, s[0:3], 0 offen
	buffer_atomic_add_f32 v6, v73, s[0:3], 0 offen
	buffer_atomic_add_f32 v10, v74, s[0:3], 0 offen
	buffer_atomic_add_f32 v14, v75, s[0:3], 0 offen
	buffer_atomic_add_f32 v18, v76, s[0:3], 0 offen
	buffer_atomic_add_f32 v22, v77, s[0:3], 0 offen
	buffer_atomic_add_f32 v26, v78, s[0:3], 0 offen
	buffer_atomic_add_f32 v30, v79, s[0:3], 0 offen
	buffer_atomic_add_f32 v1, v80, s[0:3], 0 offen
	buffer_atomic_add_f32 v5, v81, s[0:3], 0 offen
	buffer_atomic_add_f32 v9, v82, s[0:3], 0 offen
	buffer_atomic_add_f32 v13, v32, s[0:3], 0 offen
	buffer_atomic_add_f32 v17, v33, s[0:3], 0 offen
	buffer_atomic_add_f32 v21, v34, s[0:3], 0 offen
	buffer_atomic_add_f32 v25, v39, s[0:3], 0 offen
	buffer_atomic_add_f32 v29, v40, s[0:3], 0 offen
	buffer_atomic_add_f32 v3, v41, s[0:3], 0 offen
	buffer_atomic_add_f32 v7, v42, s[0:3], 0 offen
	buffer_atomic_add_f32 v11, v43, s[0:3], 0 offen
	v_add_lshl_u32 v44, v36, s35, 2
	v_add_lshl_u32 v45, v36, s36, 2
	v_add_lshl_u32 v46, v36, s37, 2
	v_add_lshl_u32 v47, v36, s38, 2
	v_add_lshl_u32 v0, v36, s4, 2
	s_clause 0x4
	buffer_atomic_add_f32 v15, v44, s[0:3], 0 offen
	buffer_atomic_add_f32 v19, v45, s[0:3], 0 offen
	buffer_atomic_add_f32 v23, v46, s[0:3], 0 offen
	buffer_atomic_add_f32 v27, v47, s[0:3], 0 offen
	buffer_atomic_add_f32 v31, v0, s[0:3], 0 offen
	.loc	1 80 1                          ; generate_amdgcn.py:80:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp14:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 39
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 36
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk, .Lfunc_end0-amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_vgpr, 112
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.numbered_sgpr, 39
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 4492
; TotalNumSgprs: 41
; NumVgprs: 112
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 41
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	11                              ; DW_FORM_data1
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	5                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	11                              ; DW_FORM_data1
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x64 DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x3e DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	88                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4d:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	87                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	5                               ; Abbrev [5] 0x59:0x14 DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.quad	.Ltmp12                         ; DW_AT_low_pc
	.long	.Ltmp13-.Ltmp12                 ; DW_AT_high_pc
	.byte	1                               ; DW_AT_call_file
	.byte	106                             ; DW_AT_call_line
	.byte	32                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk"             ; string offset=75 ; amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
    .private_segment_fixed_size: 0
    .sgpr_count:     41
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm64_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     112
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
