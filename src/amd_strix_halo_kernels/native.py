...
def _load_dispatch_library_cached(library_path_value: str) -> ctypes.CDLL:
    ...
    library.amd_strix_halo_kernels_launch_attention_bwd_hsaco.argtypes = [
        ctypes.c_char_p,
        ctypes.c_char_p,
        ctypes.c_int,
        ctypes.c_uint32,
    ...
    library.amd_strix_halo_kernels_launch_attention_bwd_hsaco.restype = ctypes.c_int
...
def launch_attention_bwd_hsaco(
    *,
    hsaco_path: str | Path,
    symbol: str,
    device_index: int,
    ...
    if len(buffer_ptrs) != 12:
        raise ValueError("attention backward requires exactly 12 buffer pointers")
    library = load_dispatch_library(library_path)
    rc = library.amd_strix_halo_kernels_launch_attention_bwd_hsaco(
        str(hsaco_path).encode(),
        symbol.encode(),
        int(device_index),
        int(grid[0]),
        ...
        int(shared_memory_bytes),
        int(stream_handle),
        *(ctypes.c_void_p(int(pointer)) for pointer in buffer_ptrs),
        int(batch),
        int(query_heads),
        int(kv_heads),
        int(query_length),
