	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk               ; -- Begin function amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk,@function
amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk:                      ; @amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 80 0                          ; generate_amdgcn.py:80:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[34:35], s[0:1], 0x30
	s_load_b32 s4, s[0:1], 0x38
.Ltmp0:
	.loc	1 90 16 prologue_end            ; generate_amdgcn.py:90:16
	s_abs_i32 s8, s2
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshlrev_b32_e32 v10, 4, v0
	.loc	1 158 24                        ; generate_amdgcn.py:158:24
	v_and_b32_e32 v34, 31, v0
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v3, 5, v0
	.loc	1 158 24                        ; generate_amdgcn.py:158:24
	v_lshrrev_b32_e32 v4, 3, v0
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_lshlrev_b32_e32 v35, 1, v0
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_and_b32_e32 v2, 0x70, v10
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:88:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s5, s35, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s9, s34, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s6, s5, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s11, s9, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_lshr_b32 s6, s6, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_lshr_b32 s11, s11, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_add_i32 s5, s5, s6
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s9, s9, s11
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s5, s5, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s9, s9, 4
.Ltmp11:
	.loc	1 90 16 is_stmt 1               ; generate_amdgcn.py:90:16
	s_abs_i32 s6, s5
	.loc	1 193 34                        ; generate_amdgcn.py:193:34
	v_mad_u64_u32 v[5:6], null, s35, v4, v[2:3]
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_cvt_f32_u32 s7, s6
	s_sub_i32 s10, 0, s6
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s7
	v_readfirstlane_b32 s7, v1
	s_mul_f32 s7, s7, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s7, s7
	s_mul_i32 s10, s10, s7
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s10, s7, s10
	s_add_i32 s7, s7, s10
	s_xor_b32 s10, s2, s5
	s_mul_hi_u32 s7, s8, s7
	s_ashr_i32 s10, s10, 31
	s_mul_i32 s11, s7, s6
	s_delay_alu instid0(SALU_CYCLE_1)
	s_sub_i32 s8, s8, s11
	s_add_i32 s11, s7, 1
	s_sub_i32 s12, s8, s6
	s_cmp_ge_u32 s8, s6
	s_cselect_b32 s7, s11, s7
	s_cselect_b32 s8, s12, s8
	s_add_i32 s11, s7, 1
	s_cmp_ge_u32 s8, s6
	s_cselect_b32 s6, s11, s7
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s6, s6, s10
	s_sub_i32 s6, s6, s10
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 92 31                         ; generate_amdgcn.py:92:31
	s_sub_i32 s7, s9, s6
	s_load_b256 s[8:15], s[0:1], 0x0
	.loc	1 92 20 is_stmt 0               ; generate_amdgcn.py:92:20
	s_min_i32 s7, s7, 1
	.loc	1 93 29 is_stmt 1               ; generate_amdgcn.py:93:29
	s_mul_i32 s5, s6, s5
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_abs_i32 s16, s7
	.loc	1 93 29                         ; generate_amdgcn.py:93:29
	s_sub_i32 s2, s2, s5
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_cvt_f32_u32 s17, s16
	s_abs_i32 s18, s2
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s17
	v_readfirstlane_b32 s17, v1
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[28:29], s[10:11]
	.loc	1 158 24                        ; generate_amdgcn.py:158:24
	v_lshlrev_b32_e32 v1, 1, v34
	s_mov_b32 s11, 0x31027000
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_mul_f32 s17, s17, 0x4f7ffffe
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	s_mov_b32 s31, s11
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(SKIP_1) | instid1(SALU_CYCLE_2)
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_cvt_u32_f32 s10, s17
	s_sub_i32 s17, 0, s16
	s_mul_i32 s17, s17, s10
	s_delay_alu instid0(SALU_CYCLE_1)
	s_mul_hi_u32 s5, s10, s17
	s_xor_b32 s17, s2, s7
	s_add_i32 s5, s10, s5
	s_ashr_i32 s17, s17, 31
	s_mul_hi_u32 s5, s18, s5
	s_mov_b32 s10, 0x7ffffffe
	s_mul_i32 s19, s5, s16
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	s_mov_b32 s30, s10
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_sub_i32 s18, s18, s19
	s_add_i32 s19, s5, 1
	s_sub_i32 s20, s18, s16
	s_cmp_ge_u32 s18, s16
	s_cselect_b32 s5, s19, s5
	s_cselect_b32 s18, s20, s18
	s_add_i32 s19, s5, 1
	s_cmp_ge_u32 s18, s16
	s_cselect_b32 s5, s19, s5
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_lshr_b32 s16, s4, 31
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_xor_b32 s19, s5, s17
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:154:22 ]
	s_add_i32 s21, s4, 0xff
.Ltmp13:
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_add_i32 s4, s4, s16
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_sub_i32 s5, s19, s17
	.loc	1 170 39                        ; generate_amdgcn.py:170:39
	s_lshl_b32 s20, s3, 6
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_ashr_i32 s18, s4, 1
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_mul_i32 s4, s5, s7
	.loc	1 170 22                        ; generate_amdgcn.py:170:22
	v_or_b32_e32 v8, s20, v1
.Ltmp14:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:154:22 ]
	s_ashr_i32 s3, s21, 31
.Ltmp15:
	.loc	1 189 34                        ; generate_amdgcn.py:189:34
	v_mad_u64_u32 v[6:7], null, s18, v3, v[1:2]
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_sub_i32 s2, s2, s4
	.loc	1 170 22                        ; generate_amdgcn.py:170:22
	v_or_b32_e32 v9, s20, v4
.Ltmp16:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:154:22 ]
	s_lshr_b32 s3, s3, 24
.Ltmp17:
	.loc	1 93 13                         ; generate_amdgcn.py:93:13
	s_add_i32 s2, s2, s6
.Ltmp18:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:154:22 ]
	s_add_i32 s7, s21, s3
.Ltmp19:
	.loc	1 171 27                        ; generate_amdgcn.py:171:27
	v_cmp_gt_i32_e32 vcc_lo, s18, v8
	.loc	1 171 51 is_stmt 0              ; generate_amdgcn.py:171:51
	v_cmp_gt_i32_e64 s3, 0x80, v8
	.loc	1 99 14 is_stmt 1               ; generate_amdgcn.py:99:14
	s_lshl_b32 s34, s2, 4
	.loc	1 100 14                        ; generate_amdgcn.py:100:14
	s_lshl_b32 s33, s5, 7
	.loc	1 171 27                        ; generate_amdgcn.py:171:27
	v_cmp_gt_i32_e64 s5, s18, v9
	.loc	1 171 51 is_stmt 0              ; generate_amdgcn.py:171:51
	v_cmp_gt_i32_e64 s4, 0x80, v9
.Ltmp20:
	.loc	2 43 12 is_stmt 1               ; standard.py:43:12 @[ generate_amdgcn.py:154:22 ]
	s_ashr_i32 s6, s7, 8
.Ltmp21:
	.loc	1 189 34                        ; generate_amdgcn.py:189:34
	s_mul_i32 s2, s34, s18
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_cmpk_gt_i32 s21, 0xff
	.loc	1 193 34                        ; generate_amdgcn.py:193:34
	s_mul_i32 s16, s35, s20
	.loc	1 189 34                        ; generate_amdgcn.py:189:34
	v_add3_u32 v6, s20, s2, v6
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_cselect_b32 s2, -1, 0
	.loc	1 171 26                        ; generate_amdgcn.py:171:26
	s_and_b32 s7, s3, vcc_lo
	.loc	1 193 34                        ; generate_amdgcn.py:193:34
	v_add3_u32 v5, s16, s33, v5
	.loc	1 171 26                        ; generate_amdgcn.py:171:26
	s_and_b32 s5, s4, s5
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_and_b32 vcc_lo, s2, s7
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	s_and_b32 s9, s9, 0xffff
	v_cndmask_b32_e32 v6, 0x80000000, v6, vcc_lo
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_and_b32 vcc_lo, s2, s5
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	s_and_b32 s29, s29, 0xffff
	v_cndmask_b32_e32 v5, 0x80000000, v5, vcc_lo
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	buffer_load_u16 v11, v6, s[8:11], 0 offen
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	buffer_load_b128 v[6:9], v5, s[28:31], 0 offen
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_and_b32_e32 v5, 56, v4
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v115, 0, v10
	s_mov_b32 s5, -1
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_cmpk_gt_i32 s21, 0x1ff
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xor_b32_e32 v5, v35, v5
	v_add_nc_u32_e32 v114, 0, v5
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshrrev_b32_e32 v5, 4, v0
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	s_waitcnt vmcnt(1)
	ds_store_b16 v114, v11 offset:8192
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	s_waitcnt vmcnt(0)
	ds_store_b128 v115, v[6:9]
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_cbranch_scc1 .LBB0_2
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 168 18                        ; generate_amdgcn.py:168:18
	s_and_b32 s25, s13, 0xffff
	s_mov_b32 s24, s12
	s_mov_b64 s[26:27], s[10:11]
	s_mov_b32 s5, 0
	s_branch .LBB0_3
.LBB0_2:
                                        ; implicit-def: $sgpr24_sgpr25
.LBB0_3:                                ; %Flow605
	.loc	1 0 18 is_stmt 0                ; generate_amdgcn.py:0:18
	s_load_b64 s[0:1], s[0:1], 0x20
	v_dual_mov_b32 v37, 0 :: v_dual_and_b32 v36, 15, v0
	v_bfe_u32 v103, v0, 4, 1
	v_or_b32_e32 v102, 62, v5
	v_or_b32_e32 v101, 0x7e, v5
	v_dual_mov_b32 v38, 0 :: v_dual_lshlrev_b32 v113, 2, v0
	v_dual_mov_b32 v39, 0 :: v_dual_add_nc_u32 v104, 0, v36
	v_or_b32_e32 v112, 0x3f0, v0
	v_or_b32_e32 v111, 0x7f0, v0
	v_or_b32_e32 v110, 0xbf0, v0
	v_or_b32_e32 v109, 0xff0, v0
	v_or_b32_e32 v108, 0x13f0, v0
	v_or_b32_e32 v107, 0x17f0, v0
	v_or_b32_e32 v106, 0x1bf0, v0
	v_or_b32_e32 v105, 0x1ff0, v0
	v_dual_mov_b32 v40, 0 :: v_dual_mov_b32 v41, 0
	v_dual_mov_b32 v42, 0 :: v_dual_mov_b32 v43, 0
	v_dual_mov_b32 v44, 0 :: v_dual_mov_b32 v45, 0
	v_dual_mov_b32 v46, 0 :: v_dual_mov_b32 v47, 0
	v_dual_mov_b32 v48, 0 :: v_dual_mov_b32 v49, 0
	v_dual_mov_b32 v50, 0 :: v_dual_mov_b32 v51, 0
	v_dual_mov_b32 v53, 0 :: v_dual_mov_b32 v54, 0
	v_dual_mov_b32 v55, 0 :: v_dual_mov_b32 v56, 0
	v_dual_mov_b32 v57, 0 :: v_dual_mov_b32 v58, 0
	v_dual_mov_b32 v59, 0 :: v_dual_mov_b32 v60, 0
	v_dual_mov_b32 v61, 0 :: v_dual_mov_b32 v62, 0
	v_dual_mov_b32 v63, 0 :: v_dual_mov_b32 v64, 0
	v_dual_mov_b32 v65, 0 :: v_dual_mov_b32 v66, 0
	v_dual_mov_b32 v67, 0 :: v_dual_mov_b32 v68, 0
	v_dual_mov_b32 v69, 0 :: v_dual_mov_b32 v70, 0
	v_dual_mov_b32 v71, 0 :: v_dual_mov_b32 v72, 0
	v_dual_mov_b32 v73, 0 :: v_dual_mov_b32 v74, 0
	v_dual_mov_b32 v75, 0 :: v_dual_mov_b32 v76, 0
	v_dual_mov_b32 v77, 0 :: v_dual_mov_b32 v78, 0
	v_dual_mov_b32 v79, 0 :: v_dual_mov_b32 v80, 0
	v_dual_mov_b32 v81, 0 :: v_dual_mov_b32 v82, 0
	v_dual_mov_b32 v83, 0 :: v_dual_mov_b32 v84, 0
	v_dual_mov_b32 v85, 0 :: v_dual_mov_b32 v86, 0
	v_dual_mov_b32 v87, 0 :: v_dual_mov_b32 v88, 0
	v_dual_mov_b32 v89, 0 :: v_dual_mov_b32 v90, 0
	v_dual_mov_b32 v91, 0 :: v_dual_mov_b32 v92, 0
	v_dual_mov_b32 v93, 0 :: v_dual_mov_b32 v94, 0
	v_dual_mov_b32 v95, 0 :: v_dual_mov_b32 v96, 0
	v_dual_mov_b32 v97, 0 :: v_dual_mov_b32 v98, 0
	v_dual_mov_b32 v99, 0 :: v_dual_mov_b32 v100, 0
	v_mov_b32_e32 v52, 0
	s_and_not1_b32 vcc_lo, exec_lo, s5
	s_add_i32 s5, s6, -1
	s_cbranch_vccnz .LBB0_6
; %bb.4:                                ; %.lr.ph
	.loc	1 161 9 is_stmt 1               ; generate_amdgcn.py:161:9
	s_add_i32 s44, s20, 0x80
	v_sub_nc_u32_e32 v117, s18, v4
	v_dual_mov_b32 v99, 0 :: v_dual_add_nc_u32 v4, s44, v4
	v_dual_mov_b32 v52, 0 :: v_dual_and_b32 v5, 56, v113
	v_dual_mov_b32 v97, 0 :: v_dual_add_nc_u32 v14, s34, v36
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_mul_lo_u32 v4, s35, v4
	v_dual_mov_b32 v100, 0 :: v_dual_lshlrev_b32 v13, 1, v101
	v_lshl_or_b32 v5, v36, 6, v5
	s_lshl_b32 s45, s19, 8
	s_lshl_b32 s19, s19, 7
	s_lshl_b32 s20, s17, 8
	v_mul_lo_u32 v14, s6, v14
	s_delay_alu instid0(VALU_DEP_4)
	v_add3_u32 v2, v4, s19, v2
	v_dual_mov_b32 v94, 0 :: v_dual_add_nc_u32 v3, s34, v3
	v_subrev_nc_u32_e32 v118, s20, v13
	v_dual_mov_b32 v98, 0 :: v_dual_lshlrev_b32 v13, 1, v103
	v_dual_mov_b32 v96, 0 :: v_dual_lshlrev_b32 v15, 1, v102
	v_xor_b32_e32 v6, 8, v5
	v_xor_b32_e32 v7, 16, v5
	v_xor_b32_e32 v8, 24, v5
	v_xor_b32_e32 v9, 32, v5
	v_xor_b32_e32 v10, 40, v5
	v_xor_b32_e32 v11, 48, v5
	v_xor_b32_e32 v12, 56, v5
	s_mov_b32 s16, 0
	s_lshl_b32 s17, s17, 7
	v_mad_u64_u32 v[17:18], null, s18, v3, v[1:2]
	v_sub_nc_u32_e32 v116, s18, v1
	v_subrev_nc_u32_e32 v119, s20, v13
	v_subrev_nc_u32_e32 v120, s20, v15
	v_subrev_nc_u32_e32 v122, s17, v2
	s_mov_b32 s17, s16
	s_mov_b32 s18, s16
	s_mov_b32 s19, s16
	s_mov_b32 s20, s16
	s_mov_b32 s21, s16
	s_mov_b32 s22, s16
	s_mov_b32 s23, s16
	v_dual_mov_b32 v90, 0 :: v_dual_add_nc_u32 v123, 0, v5
	v_dual_mov_b32 v95, 0 :: v_dual_add_nc_u32 v124, 0, v6
	v_dual_mov_b32 v88, 0 :: v_dual_add_nc_u32 v125, 0, v7
	v_dual_mov_b32 v93, 0 :: v_dual_add_nc_u32 v126, 0, v8
	v_mov_b32_e32 v1, s16
	v_dual_mov_b32 v92, 0 :: v_dual_lshlrev_b32 v121, 1, v14
	v_dual_mov_b32 v86, 0 :: v_dual_add_nc_u32 v127, 0, v9
	v_dual_mov_b32 v91, 0 :: v_dual_add_nc_u32 v128, 0, v10
	v_dual_mov_b32 v84, 0 :: v_dual_add_nc_u32 v129, 0, v11
	v_dual_mov_b32 v89, 0 :: v_dual_add_nc_u32 v130, 0, v12
	v_dual_mov_b32 v82, 0 :: v_dual_add_nc_u32 v131, 0, v112
	v_dual_mov_b32 v87, 0 :: v_dual_add_nc_u32 v132, 0, v111
	v_dual_mov_b32 v80, 0 :: v_dual_add_nc_u32 v133, 0, v110
	v_dual_mov_b32 v85, 0 :: v_dual_add_nc_u32 v134, 0, v109
	v_dual_mov_b32 v78, 0 :: v_dual_add_nc_u32 v135, 0, v108
	v_dual_mov_b32 v83, 0 :: v_dual_add_nc_u32 v136, 0, v107
	v_dual_mov_b32 v76, 0 :: v_dual_add_nc_u32 v137, 0, v106
	v_dual_mov_b32 v81, 0 :: v_dual_add_nc_u32 v138, 0, v105
	v_dual_mov_b32 v79, 0 :: v_dual_mov_b32 v74, 0
	v_dual_mov_b32 v77, 0 :: v_dual_mov_b32 v72, 0
	v_dual_mov_b32 v75, 0 :: v_dual_mov_b32 v70, 0
	v_dual_mov_b32 v73, 0 :: v_dual_mov_b32 v68, 0
	v_dual_mov_b32 v71, 0 :: v_dual_mov_b32 v66, 0
	v_dual_mov_b32 v69, 0 :: v_dual_mov_b32 v64, 0
	v_dual_mov_b32 v67, 0 :: v_dual_mov_b32 v62, 0
	v_dual_mov_b32 v65, 0 :: v_dual_mov_b32 v60, 0
	v_dual_mov_b32 v63, 0 :: v_dual_mov_b32 v58, 0
	v_dual_mov_b32 v61, 0 :: v_dual_mov_b32 v56, 0
	v_dual_mov_b32 v59, 0 :: v_dual_mov_b32 v54, 0
	v_dual_mov_b32 v57, 0 :: v_dual_mov_b32 v50, 0
	v_dual_mov_b32 v55, 0 :: v_dual_mov_b32 v48, 0
	v_dual_mov_b32 v53, 0 :: v_dual_mov_b32 v46, 0
	v_dual_mov_b32 v51, 0 :: v_dual_mov_b32 v44, 0
	v_dual_mov_b32 v49, 0 :: v_dual_mov_b32 v42, 0
	v_dual_mov_b32 v47, 0 :: v_dual_mov_b32 v40, 0
	v_dual_mov_b32 v45, 0 :: v_dual_mov_b32 v38, 0
	v_dual_mov_b32 v43, 0 :: v_dual_mov_b32 v2, s17
	v_dual_mov_b32 v41, 0 :: v_dual_mov_b32 v4, s19
	v_dual_mov_b32 v39, 0 :: v_dual_mov_b32 v6, s21
	v_dual_mov_b32 v37, 0 :: v_dual_mov_b32 v8, s23
	v_mov_b32_e32 v3, s18
	v_mov_b32_e32 v5, s20
	v_mov_b32_e32 v7, s22
	s_and_b32 s13, s13, 0xffff
	s_mov_b32 s27, 0x31027000
	s_mov_b32 s26, 0x7ffffffe
	s_mov_b64 s[24:25], s[12:13]
	s_and_b32 s37, s15, 0xffff
	s_mov_b32 s36, s14
	s_max_i32 s7, s5, 1
	s_lshl_b32 s46, s35, 1
	s_lshl_b32 s47, s35, 7
	s_mov_b32 s40, s12
	s_mov_b32 s41, s13
	s_mov_b32 s42, s26
	s_mov_b32 s43, s27
	s_mov_b32 s30, s10
	s_mov_b32 s31, s11
	s_mov_b32 s38, s26
	s_mov_b32 s39, s27
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 171 27                        ; generate_amdgcn.py:171:27
	v_cmp_lt_i32_e32 vcc_lo, s44, v116
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_add_nc_u32_e32 v9, s44, v17
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_add_i32 s7, s7, -1
	.loc	1 171 26                        ; generate_amdgcn.py:171:26
	s_and_b32 vcc_lo, vcc_lo, s3
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_cndmask_b32_e32 v9, 0x80000000, v9, vcc_lo
	.loc	1 171 27                        ; generate_amdgcn.py:171:27
	v_cmp_lt_i32_e32 vcc_lo, s44, v117
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_addk_i32 s44, 0x80
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	buffer_load_u16 v139, v9, s[8:11], 0 offen
	.loc	1 168 18                        ; generate_amdgcn.py:168:18
	buffer_load_u16 v140, v121, s[40:43], 0 offen
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:640
	ds_load_u8 v10, v104 offset:512
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[30:31], v123 offset:8192
	ds_load_b64 v[32:33], v124 offset:8192
	ds_load_b64 v[28:29], v125 offset:8192
	ds_load_b64 v[26:27], v126 offset:8192
	.loc	1 171 26                        ; generate_amdgcn.py:171:26
	s_and_b32 vcc_lo, vcc_lo, s4
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_add_nc_u32_e32 v121, 2, v121
	s_cmp_lg_u32 s7, 0
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:896
	ds_load_u8 v11, v104 offset:768
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:128
	ds_load_u8 v12, v104
	v_lshl_or_b32 v19, v10, 16, v9
	ds_load_u8 v9, v104 offset:1664
	ds_load_u8 v10, v104 offset:1536
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:384
	ds_load_u8 v13, v104 offset:256
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v18, v12, 16, v11
	ds_load_u8 v10, v104 offset:1920
	ds_load_u8 v11, v104 offset:1792
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1152
	ds_load_u8 v12, v104 offset:1024
	v_lshl_or_b32 v21, v10, 16, v9
	ds_load_u8 v9, v104 offset:2688
	ds_load_u8 v10, v104 offset:2560
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1408
	ds_load_u8 v13, v104 offset:1280
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v20, v12, 16, v11
	ds_load_u8 v10, v104 offset:2944
	ds_load_u8 v11, v104 offset:2816
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2176
	ds_load_u8 v12, v104 offset:2048
	v_lshl_or_b32 v23, v10, 16, v9
	ds_load_u8 v9, v104 offset:3712
	ds_load_u8 v10, v104 offset:3584
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2432
	ds_load_u8 v13, v104 offset:2304
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v22, v12, 16, v11
	ds_load_u8 v10, v104 offset:3968
	ds_load_u8 v11, v104 offset:3840
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3200
	ds_load_u8 v12, v104 offset:3072
	v_lshl_or_b32 v25, v10, 16, v9
	ds_load_u8 v9, v104 offset:4736
	ds_load_u8 v10, v104 offset:4608
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3456
	ds_load_u8 v13, v104 offset:3328
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v24, v12, 16, v11
	ds_load_u8 v10, v104 offset:4992
	ds_load_u8 v11, v104 offset:4864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4224
	ds_load_u8 v12, v104 offset:4096
	.loc	1 168 18                        ; generate_amdgcn.py:168:18
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v140, 16, v140
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_lshl_or_b32 v142, v10, 16, v9
	ds_load_u8 v9, v104 offset:5760
	ds_load_u8 v10, v104 offset:5632
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4480
	ds_load_u8 v13, v104 offset:4352
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v141, v12, 16, v11
	ds_load_u8 v10, v104 offset:6016
	ds_load_u8 v11, v104 offset:5888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5248
	ds_load_u8 v12, v104 offset:5120
	v_lshl_or_b32 v144, v10, 16, v9
	ds_load_u8 v9, v104 offset:6784
	ds_load_u8 v10, v104 offset:6656
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5504
	ds_load_u8 v13, v104 offset:5376
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v143, v12, 16, v11
	ds_load_u8 v10, v104 offset:7040
	ds_load_u8 v11, v104 offset:6912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6272
	ds_load_u8 v12, v104 offset:6144
	v_lshl_or_b32 v146, v10, 16, v9
	ds_load_u8 v9, v104 offset:7808
	ds_load_u8 v10, v104 offset:7680
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6528
	ds_load_u8 v13, v104 offset:6400
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v145, v12, 16, v11
	ds_load_u8 v10, v104 offset:8064
	ds_load_u8 v11, v104 offset:7936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7296
	ds_load_u8 v12, v104 offset:7168
	v_lshl_or_b32 v148, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7552
	ds_load_u8 v13, v104 offset:7424
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v147, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[18:19], v[30:31], v[1:8] neg_lo:[1,1,0]
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[18:19], v130 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_wmma_i32_16x16x16_iu4 v[9:16], v[20:21], v[32:33], v[9:16] neg_lo:[1,1,0]
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[20:21], v129 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_wmma_i32_16x16x16_iu4 v[9:16], v[22:23], v[28:29], v[9:16] neg_lo:[1,1,0]
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[22:23], v128 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_wmma_i32_16x16x16_iu4 v[9:16], v[24:25], v[26:27], v[9:16] neg_lo:[1,1,0]
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[24:25], v127 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[141:142], v[24:25], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[143:144], v[22:23], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[145:146], v[20:21], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[147:148], v[18:19], v[9:16] neg_lo:[1,1,0]
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v148, v9
	s_delay_alu instid0(VALU_DEP_2)
	v_cvt_f32_i32_e32 v147, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:656
	ds_load_u8 v10, v104 offset:528
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v146, v11
	v_cvt_f32_i32_e32 v145, v12
	v_cvt_f32_i32_e32 v144, v13
	v_cvt_f32_i32_e32 v143, v14
	v_cvt_f32_i32_e32 v142, v15
	v_cvt_f32_i32_e32 v141, v16
	v_dual_mul_f32 v147, v140, v147 :: v_dual_add_nc_u32 v244, s45, v118
	v_mul_f32_e32 v148, v140, v148
	v_mul_f32_e32 v146, v140, v146
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_add_nc_u32_e32 v118, s46, v118
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	buffer_load_u16 v244, v244, s[36:39], 0 offen
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:912
	ds_load_u8 v11, v104 offset:784
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:16
	ds_load_u8 v12, v104 offset:144
	v_lshl_or_b32 v150, v10, 16, v9
	ds_load_u8 v9, v104 offset:1680
	ds_load_u8 v10, v104 offset:1552
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v11, v12, 0xc0c0004
	ds_load_u8 v12, v104 offset:400
	ds_load_u8 v13, v104 offset:272
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v149, v12, 16, v11
	ds_load_u8 v10, v104 offset:1936
	ds_load_u8 v11, v104 offset:1808
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1168
	ds_load_u8 v12, v104 offset:1040
	v_lshl_or_b32 v152, v10, 16, v9
	ds_load_u8 v9, v104 offset:2704
	ds_load_u8 v10, v104 offset:2576
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1424
	ds_load_u8 v13, v104 offset:1296
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v151, v12, 16, v11
	ds_load_u8 v10, v104 offset:2960
	ds_load_u8 v11, v104 offset:2832
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2192
	ds_load_u8 v12, v104 offset:2064
	v_lshl_or_b32 v154, v10, 16, v9
	ds_load_u8 v9, v104 offset:3728
	ds_load_u8 v10, v104 offset:3600
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2448
	ds_load_u8 v13, v104 offset:2320
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v153, v12, 16, v11
	ds_load_u8 v10, v104 offset:3984
	ds_load_u8 v11, v104 offset:3856
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3216
	ds_load_u8 v12, v104 offset:3088
	v_lshl_or_b32 v156, v10, 16, v9
	ds_load_u8 v9, v104 offset:4752
	ds_load_u8 v10, v104 offset:4624
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3472
	ds_load_u8 v13, v104 offset:3344
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v155, v12, 16, v11
	ds_load_u8 v10, v104 offset:5008
	ds_load_u8 v11, v104 offset:4880
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4240
	ds_load_u8 v12, v104 offset:4112
	v_lshl_or_b32 v158, v10, 16, v9
	ds_load_u8 v9, v104 offset:5776
	ds_load_u8 v10, v104 offset:5648
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4496
	ds_load_u8 v13, v104 offset:4368
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v157, v12, 16, v11
	ds_load_u8 v10, v104 offset:6032
	ds_load_u8 v11, v104 offset:5904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5264
	ds_load_u8 v12, v104 offset:5136
	v_lshl_or_b32 v160, v10, 16, v9
	ds_load_u8 v9, v104 offset:6800
	ds_load_u8 v10, v104 offset:6672
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5520
	ds_load_u8 v13, v104 offset:5392
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v159, v12, 16, v11
	ds_load_u8 v10, v104 offset:7056
	ds_load_u8 v11, v104 offset:6928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6288
	ds_load_u8 v12, v104 offset:6160
	v_lshl_or_b32 v162, v10, 16, v9
	ds_load_u8 v9, v104 offset:7824
	ds_load_u8 v10, v104 offset:7696
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6544
	ds_load_u8 v13, v104 offset:6416
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v161, v12, 16, v11
	ds_load_u8 v10, v104 offset:8080
	ds_load_u8 v11, v104 offset:7952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7312
	ds_load_u8 v12, v104 offset:7184
	v_lshl_or_b32 v164, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7568
	ds_load_u8 v13, v104 offset:7440
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[149:150], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[151:152], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[153:154], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[155:156], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[157:158], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[159:160], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[161:162], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[163:164], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v156, v9
	v_cvt_f32_i32_e32 v155, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:672
	ds_load_u8 v10, v104 offset:544
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v154, v11
	v_cvt_f32_i32_e32 v153, v12
	v_cvt_f32_i32_e32 v152, v13
	v_cvt_f32_i32_e32 v151, v14
	v_cvt_f32_i32_e32 v150, v15
	v_cvt_f32_i32_e32 v149, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:928
	ds_load_u8 v11, v104 offset:800
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:32
	ds_load_u8 v12, v104 offset:160
	v_lshl_or_b32 v158, v10, 16, v9
	ds_load_u8 v9, v104 offset:1696
	ds_load_u8 v10, v104 offset:1568
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v11, v12, 0xc0c0004
	ds_load_u8 v12, v104 offset:416
	ds_load_u8 v13, v104 offset:288
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v157, v12, 16, v11
	ds_load_u8 v10, v104 offset:1952
	ds_load_u8 v11, v104 offset:1824
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1184
	ds_load_u8 v12, v104 offset:1056
	v_lshl_or_b32 v160, v10, 16, v9
	ds_load_u8 v9, v104 offset:2720
	ds_load_u8 v10, v104 offset:2592
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1440
	ds_load_u8 v13, v104 offset:1312
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v159, v12, 16, v11
	ds_load_u8 v10, v104 offset:2976
	ds_load_u8 v11, v104 offset:2848
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2208
	ds_load_u8 v12, v104 offset:2080
	v_lshl_or_b32 v162, v10, 16, v9
	ds_load_u8 v9, v104 offset:3744
	ds_load_u8 v10, v104 offset:3616
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2464
	ds_load_u8 v13, v104 offset:2336
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v161, v12, 16, v11
	ds_load_u8 v10, v104 offset:4000
	ds_load_u8 v11, v104 offset:3872
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3232
	ds_load_u8 v12, v104 offset:3104
	v_lshl_or_b32 v164, v10, 16, v9
	ds_load_u8 v9, v104 offset:4768
	ds_load_u8 v10, v104 offset:4640
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3488
	ds_load_u8 v13, v104 offset:3360
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v163, v12, 16, v11
	ds_load_u8 v10, v104 offset:5024
	ds_load_u8 v11, v104 offset:4896
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4256
	ds_load_u8 v12, v104 offset:4128
	v_lshl_or_b32 v166, v10, 16, v9
	ds_load_u8 v9, v104 offset:5792
	ds_load_u8 v10, v104 offset:5664
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4512
	ds_load_u8 v13, v104 offset:4384
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v165, v12, 16, v11
	ds_load_u8 v10, v104 offset:6048
	ds_load_u8 v11, v104 offset:5920
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5280
	ds_load_u8 v12, v104 offset:5152
	v_lshl_or_b32 v168, v10, 16, v9
	ds_load_u8 v9, v104 offset:6816
	ds_load_u8 v10, v104 offset:6688
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5536
	ds_load_u8 v13, v104 offset:5408
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v167, v12, 16, v11
	ds_load_u8 v10, v104 offset:7072
	ds_load_u8 v11, v104 offset:6944
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6304
	ds_load_u8 v12, v104 offset:6176
	v_lshl_or_b32 v170, v10, 16, v9
	ds_load_u8 v9, v104 offset:7840
	ds_load_u8 v10, v104 offset:7712
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6560
	ds_load_u8 v13, v104 offset:6432
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v169, v12, 16, v11
	ds_load_u8 v10, v104 offset:8096
	ds_load_u8 v11, v104 offset:7968
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7328
	ds_load_u8 v12, v104 offset:7200
	v_lshl_or_b32 v172, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7584
	ds_load_u8 v13, v104 offset:7456
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v171, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[157:158], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[159:160], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[161:162], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[163:164], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[165:166], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[167:168], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[169:170], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[171:172], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v164, v9
	v_cvt_f32_i32_e32 v163, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:688
	ds_load_u8 v10, v104 offset:560
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v162, v11
	v_cvt_f32_i32_e32 v161, v12
	v_cvt_f32_i32_e32 v160, v13
	v_cvt_f32_i32_e32 v159, v14
	v_cvt_f32_i32_e32 v158, v15
	v_cvt_f32_i32_e32 v157, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:944
	ds_load_u8 v11, v104 offset:816
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:48
	ds_load_u8 v12, v104 offset:176
	v_lshl_or_b32 v166, v10, 16, v9
	ds_load_u8 v9, v104 offset:1712
	ds_load_u8 v10, v104 offset:1584
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v11, v12, 0xc0c0004
	ds_load_u8 v12, v104 offset:432
	ds_load_u8 v13, v104 offset:304
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v165, v12, 16, v11
	ds_load_u8 v10, v104 offset:1968
	ds_load_u8 v11, v104 offset:1840
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1200
	ds_load_u8 v12, v104 offset:1072
	v_lshl_or_b32 v168, v10, 16, v9
	ds_load_u8 v9, v104 offset:2736
	ds_load_u8 v10, v104 offset:2608
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1456
	ds_load_u8 v13, v104 offset:1328
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v167, v12, 16, v11
	ds_load_u8 v10, v104 offset:2992
	ds_load_u8 v11, v104 offset:2864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2224
	ds_load_u8 v12, v104 offset:2096
	v_lshl_or_b32 v170, v10, 16, v9
	ds_load_u8 v9, v104 offset:3760
	ds_load_u8 v10, v104 offset:3632
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2480
	ds_load_u8 v13, v104 offset:2352
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v169, v12, 16, v11
	ds_load_u8 v10, v104 offset:4016
	ds_load_u8 v11, v104 offset:3888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3248
	ds_load_u8 v12, v104 offset:3120
	v_lshl_or_b32 v172, v10, 16, v9
	ds_load_u8 v9, v104 offset:4784
	ds_load_u8 v10, v104 offset:4656
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3504
	ds_load_u8 v13, v104 offset:3376
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v171, v12, 16, v11
	ds_load_u8 v10, v104 offset:5040
	ds_load_u8 v11, v104 offset:4912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4272
	ds_load_u8 v12, v104 offset:4144
	v_lshl_or_b32 v174, v10, 16, v9
	ds_load_u8 v9, v104 offset:5808
	ds_load_u8 v10, v104 offset:5680
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4528
	ds_load_u8 v13, v104 offset:4400
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v173, v12, 16, v11
	ds_load_u8 v10, v104 offset:6064
	ds_load_u8 v11, v104 offset:5936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5296
	ds_load_u8 v12, v104 offset:5168
	v_lshl_or_b32 v176, v10, 16, v9
	ds_load_u8 v9, v104 offset:6832
	ds_load_u8 v10, v104 offset:6704
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5552
	ds_load_u8 v13, v104 offset:5424
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v175, v12, 16, v11
	ds_load_u8 v10, v104 offset:7088
	ds_load_u8 v11, v104 offset:6960
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6320
	ds_load_u8 v12, v104 offset:6192
	v_lshl_or_b32 v178, v10, 16, v9
	ds_load_u8 v9, v104 offset:7856
	ds_load_u8 v10, v104 offset:7728
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6576
	ds_load_u8 v13, v104 offset:6448
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v177, v12, 16, v11
	ds_load_u8 v10, v104 offset:8112
	ds_load_u8 v11, v104 offset:7984
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7344
	ds_load_u8 v12, v104 offset:7216
	v_lshl_or_b32 v180, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7600
	ds_load_u8 v13, v104 offset:7472
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v179, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[165:166], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[167:168], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[169:170], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[171:172], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[173:174], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[175:176], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[177:178], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[179:180], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v172, v9
	v_cvt_f32_i32_e32 v171, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:704
	ds_load_u8 v10, v104 offset:576
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v170, v11
	v_cvt_f32_i32_e32 v169, v12
	v_cvt_f32_i32_e32 v168, v13
	v_cvt_f32_i32_e32 v167, v14
	v_cvt_f32_i32_e32 v166, v15
	v_cvt_f32_i32_e32 v165, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:960
	ds_load_u8 v11, v104 offset:832
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:192
	ds_load_u8 v12, v104 offset:64
	v_lshl_or_b32 v174, v10, 16, v9
	ds_load_u8 v9, v104 offset:1728
	ds_load_u8 v10, v104 offset:1600
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:448
	ds_load_u8 v13, v104 offset:320
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v173, v12, 16, v11
	ds_load_u8 v10, v104 offset:1984
	ds_load_u8 v11, v104 offset:1856
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1216
	ds_load_u8 v12, v104 offset:1088
	v_lshl_or_b32 v176, v10, 16, v9
	ds_load_u8 v9, v104 offset:2752
	ds_load_u8 v10, v104 offset:2624
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1472
	ds_load_u8 v13, v104 offset:1344
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v175, v12, 16, v11
	ds_load_u8 v10, v104 offset:3008
	ds_load_u8 v11, v104 offset:2880
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2240
	ds_load_u8 v12, v104 offset:2112
	v_lshl_or_b32 v178, v10, 16, v9
	ds_load_u8 v9, v104 offset:3776
	ds_load_u8 v10, v104 offset:3648
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2496
	ds_load_u8 v13, v104 offset:2368
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v177, v12, 16, v11
	ds_load_u8 v10, v104 offset:4032
	ds_load_u8 v11, v104 offset:3904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3264
	ds_load_u8 v12, v104 offset:3136
	v_lshl_or_b32 v180, v10, 16, v9
	ds_load_u8 v9, v104 offset:4800
	ds_load_u8 v10, v104 offset:4672
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3520
	ds_load_u8 v13, v104 offset:3392
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v179, v12, 16, v11
	ds_load_u8 v10, v104 offset:5056
	ds_load_u8 v11, v104 offset:4928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4288
	ds_load_u8 v12, v104 offset:4160
	v_lshl_or_b32 v182, v10, 16, v9
	ds_load_u8 v9, v104 offset:5824
	ds_load_u8 v10, v104 offset:5696
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4544
	ds_load_u8 v13, v104 offset:4416
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v181, v12, 16, v11
	ds_load_u8 v10, v104 offset:6080
	ds_load_u8 v11, v104 offset:5952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5312
	ds_load_u8 v12, v104 offset:5184
	v_lshl_or_b32 v184, v10, 16, v9
	ds_load_u8 v9, v104 offset:6848
	ds_load_u8 v10, v104 offset:6720
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5568
	ds_load_u8 v13, v104 offset:5440
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v183, v12, 16, v11
	ds_load_u8 v10, v104 offset:7104
	ds_load_u8 v11, v104 offset:6976
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6336
	ds_load_u8 v12, v104 offset:6208
	v_lshl_or_b32 v186, v10, 16, v9
	ds_load_u8 v9, v104 offset:7872
	ds_load_u8 v10, v104 offset:7744
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6592
	ds_load_u8 v13, v104 offset:6464
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v185, v12, 16, v11
	ds_load_u8 v10, v104 offset:8128
	ds_load_u8 v11, v104 offset:8000
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7360
	ds_load_u8 v12, v104 offset:7232
	v_lshl_or_b32 v188, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7616
	ds_load_u8 v13, v104 offset:7488
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v187, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[173:174], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[175:176], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[177:178], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[179:180], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[181:182], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[183:184], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[185:186], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[187:188], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v180, v9
	v_cvt_f32_i32_e32 v179, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:720
	ds_load_u8 v10, v104 offset:592
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v178, v11
	v_cvt_f32_i32_e32 v177, v12
	v_cvt_f32_i32_e32 v176, v13
	v_cvt_f32_i32_e32 v175, v14
	v_cvt_f32_i32_e32 v174, v15
	v_cvt_f32_i32_e32 v173, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:976
	ds_load_u8 v11, v104 offset:848
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:208
	ds_load_u8 v12, v104 offset:80
	v_lshl_or_b32 v182, v10, 16, v9
	ds_load_u8 v9, v104 offset:1744
	ds_load_u8 v10, v104 offset:1616
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:464
	ds_load_u8 v13, v104 offset:336
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v181, v12, 16, v11
	ds_load_u8 v10, v104 offset:2000
	ds_load_u8 v11, v104 offset:1872
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1232
	ds_load_u8 v12, v104 offset:1104
	v_lshl_or_b32 v184, v10, 16, v9
	ds_load_u8 v9, v104 offset:2768
	ds_load_u8 v10, v104 offset:2640
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1488
	ds_load_u8 v13, v104 offset:1360
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v183, v12, 16, v11
	ds_load_u8 v10, v104 offset:3024
	ds_load_u8 v11, v104 offset:2896
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2256
	ds_load_u8 v12, v104 offset:2128
	v_lshl_or_b32 v186, v10, 16, v9
	ds_load_u8 v9, v104 offset:3792
	ds_load_u8 v10, v104 offset:3664
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2512
	ds_load_u8 v13, v104 offset:2384
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v185, v12, 16, v11
	ds_load_u8 v10, v104 offset:4048
	ds_load_u8 v11, v104 offset:3920
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3280
	ds_load_u8 v12, v104 offset:3152
	v_lshl_or_b32 v188, v10, 16, v9
	ds_load_u8 v9, v104 offset:4816
	ds_load_u8 v10, v104 offset:4688
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3536
	ds_load_u8 v13, v104 offset:3408
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v187, v12, 16, v11
	ds_load_u8 v10, v104 offset:5072
	ds_load_u8 v11, v104 offset:4944
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4304
	ds_load_u8 v12, v104 offset:4176
	v_lshl_or_b32 v190, v10, 16, v9
	ds_load_u8 v9, v104 offset:5840
	ds_load_u8 v10, v104 offset:5712
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4560
	ds_load_u8 v13, v104 offset:4432
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v189, v12, 16, v11
	ds_load_u8 v10, v104 offset:6096
	ds_load_u8 v11, v104 offset:5968
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5328
	ds_load_u8 v12, v104 offset:5200
	v_lshl_or_b32 v192, v10, 16, v9
	ds_load_u8 v9, v104 offset:6864
	ds_load_u8 v10, v104 offset:6736
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5584
	ds_load_u8 v13, v104 offset:5456
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v191, v12, 16, v11
	ds_load_u8 v10, v104 offset:7120
	ds_load_u8 v11, v104 offset:6992
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6352
	ds_load_u8 v12, v104 offset:6224
	v_lshl_or_b32 v194, v10, 16, v9
	ds_load_u8 v9, v104 offset:7888
	ds_load_u8 v10, v104 offset:7760
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6608
	ds_load_u8 v13, v104 offset:6480
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v193, v12, 16, v11
	ds_load_u8 v10, v104 offset:8144
	ds_load_u8 v11, v104 offset:8016
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7376
	ds_load_u8 v12, v104 offset:7248
	v_lshl_or_b32 v196, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7632
	ds_load_u8 v13, v104 offset:7504
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v195, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[181:182], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[183:184], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[185:186], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[187:188], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[189:190], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[191:192], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[193:194], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[195:196], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v188, v9
	v_cvt_f32_i32_e32 v187, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:736
	ds_load_u8 v10, v104 offset:608
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v186, v11
	v_cvt_f32_i32_e32 v185, v12
	v_cvt_f32_i32_e32 v184, v13
	v_cvt_f32_i32_e32 v183, v14
	v_cvt_f32_i32_e32 v182, v15
	v_cvt_f32_i32_e32 v181, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:992
	ds_load_u8 v11, v104 offset:864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:224
	ds_load_u8 v12, v104 offset:96
	v_lshl_or_b32 v190, v10, 16, v9
	ds_load_u8 v9, v104 offset:1760
	ds_load_u8 v10, v104 offset:1632
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:480
	ds_load_u8 v13, v104 offset:352
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v189, v12, 16, v11
	ds_load_u8 v10, v104 offset:2016
	ds_load_u8 v11, v104 offset:1888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1248
	ds_load_u8 v12, v104 offset:1120
	v_lshl_or_b32 v192, v10, 16, v9
	ds_load_u8 v9, v104 offset:2784
	ds_load_u8 v10, v104 offset:2656
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:1504
	ds_load_u8 v13, v104 offset:1376
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v191, v12, 16, v11
	ds_load_u8 v10, v104 offset:3040
	ds_load_u8 v11, v104 offset:2912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:2272
	ds_load_u8 v12, v104 offset:2144
	v_lshl_or_b32 v194, v10, 16, v9
	ds_load_u8 v9, v104 offset:3808
	ds_load_u8 v10, v104 offset:3680
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:2528
	ds_load_u8 v13, v104 offset:2400
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v193, v12, 16, v11
	ds_load_u8 v10, v104 offset:4064
	ds_load_u8 v11, v104 offset:3936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:3296
	ds_load_u8 v12, v104 offset:3168
	v_lshl_or_b32 v196, v10, 16, v9
	ds_load_u8 v9, v104 offset:4832
	ds_load_u8 v10, v104 offset:4704
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3552
	ds_load_u8 v13, v104 offset:3424
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v195, v12, 16, v11
	ds_load_u8 v10, v104 offset:5088
	ds_load_u8 v11, v104 offset:4960
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4320
	ds_load_u8 v12, v104 offset:4192
	v_lshl_or_b32 v198, v10, 16, v9
	ds_load_u8 v9, v104 offset:5856
	ds_load_u8 v10, v104 offset:5728
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4576
	ds_load_u8 v13, v104 offset:4448
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v197, v12, 16, v11
	ds_load_u8 v10, v104 offset:6112
	ds_load_u8 v11, v104 offset:5984
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5344
	ds_load_u8 v12, v104 offset:5216
	v_lshl_or_b32 v200, v10, 16, v9
	ds_load_u8 v9, v104 offset:6880
	ds_load_u8 v10, v104 offset:6752
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5600
	ds_load_u8 v13, v104 offset:5472
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v199, v12, 16, v11
	ds_load_u8 v10, v104 offset:7136
	ds_load_u8 v11, v104 offset:7008
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6368
	ds_load_u8 v12, v104 offset:6240
	v_lshl_or_b32 v202, v10, 16, v9
	ds_load_u8 v9, v104 offset:7904
	ds_load_u8 v10, v104 offset:7776
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6624
	ds_load_u8 v13, v104 offset:6496
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v201, v12, 16, v11
	ds_load_u8 v10, v104 offset:8160
	ds_load_u8 v11, v104 offset:8032
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:7392
	ds_load_u8 v12, v104 offset:7264
	v_lshl_or_b32 v204, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7648
	ds_load_u8 v13, v104 offset:7520
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v203, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[189:190], v[30:31], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[191:192], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[193:194], v[28:29], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[195:196], v[26:27], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[197:198], v[24:25], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[199:200], v[22:23], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[9:16], v[201:202], v[20:21], v[9:16] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu4 v[9:16], v[203:204], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v196, v9
	v_cvt_f32_i32_e32 v195, v10
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:240
	ds_load_u8 v10, v104 offset:112
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v194, v11
	v_cvt_f32_i32_e32 v193, v12
	v_cvt_f32_i32_e32 v192, v13
	v_cvt_f32_i32_e32 v191, v14
	v_cvt_f32_i32_e32 v190, v15
	v_cvt_f32_i32_e32 v189, v16
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:496
	ds_load_u8 v11, v104 offset:368
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:752
	ds_load_u8 v12, v104 offset:624
	v_lshl_or_b32 v197, v10, 16, v9
	ds_load_u8 v9, v104 offset:1264
	ds_load_u8 v10, v104 offset:1136
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v131
	ds_load_u8 v13, v104 offset:880
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v198, v12, 16, v11
	ds_load_u8 v10, v104 offset:1520
	ds_load_u8 v11, v104 offset:1392
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:1776
	ds_load_u8 v12, v104 offset:1648
	v_lshl_or_b32 v199, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v132
	ds_load_u8 v13, v104 offset:1904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v200, v12, 16, v11
	v_wmma_i32_16x16x16_iu4 v[9:16], v[197:198], v[30:31], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v30, v104 offset:2288
	ds_load_u8 v31, v104 offset:2160
	v_wmma_i32_16x16x16_iu4 v[9:16], v[199:200], v[32:33], v[9:16] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v30, v31, v30, 0xc0c0004
	ds_load_u8 v31, v104 offset:2544
	ds_load_u8 v32, v104 offset:2416
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v31, v32, v31, 0xc0c0004
	ds_load_u8 v32, v104 offset:2800
	ds_load_u8 v33, v104 offset:2672
	v_lshl_or_b32 v30, v31, 16, v30
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v32, v33, v32, 0xc0c0004
	ds_load_u8 v33, v133
	ds_load_u8 v197, v104 offset:2928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v33, v197, v33, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v31, v33, 16, v32
	v_wmma_i32_16x16x16_iu4 v[9:16], v[30:31], v[28:29], v[9:16] neg_lo:[1,1,0]
	ds_load_u8 v28, v104 offset:3312
	ds_load_u8 v29, v104 offset:3184
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v28, v29, v28, 0xc0c0004
	ds_load_u8 v29, v104 offset:3568
	ds_load_u8 v30, v104 offset:3440
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v29, v30, v29, 0xc0c0004
	ds_load_u8 v30, v104 offset:3824
	ds_load_u8 v31, v104 offset:3696
	v_lshl_or_b32 v28, v29, 16, v28
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v30, v31, v30, 0xc0c0004
	ds_load_u8 v31, v134
	ds_load_u8 v32, v104 offset:3952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v31, v32, v31, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v29, v31, 16, v30
	v_wmma_i32_16x16x16_iu4 v[9:16], v[28:29], v[26:27], v[9:16] neg_lo:[1,1,0]
	ds_load_u8 v26, v104 offset:4336
	ds_load_u8 v27, v104 offset:4208
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v26, v27, v26, 0xc0c0004
	ds_load_u8 v27, v104 offset:4592
	ds_load_u8 v28, v104 offset:4464
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v27, v28, v27, 0xc0c0004
	ds_load_u8 v28, v104 offset:4848
	ds_load_u8 v29, v104 offset:4720
	v_lshl_or_b32 v26, v27, 16, v26
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v28, v29, v28, 0xc0c0004
	ds_load_u8 v29, v135
	ds_load_u8 v30, v104 offset:4976
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v29, v30, v29, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v27, v29, 16, v28
	v_wmma_i32_16x16x16_iu4 v[9:16], v[26:27], v[24:25], v[9:16] neg_lo:[1,1,0]
	ds_load_u8 v24, v104 offset:5360
	ds_load_u8 v25, v104 offset:5232
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v24, v25, v24, 0xc0c0004
	ds_load_u8 v25, v104 offset:5616
	ds_load_u8 v26, v104 offset:5488
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v25, v26, v25, 0xc0c0004
	ds_load_u8 v26, v104 offset:5872
	ds_load_u8 v27, v104 offset:5744
	v_lshl_or_b32 v24, v25, 16, v24
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v26, v27, v26, 0xc0c0004
	ds_load_u8 v27, v136
	ds_load_u8 v28, v104 offset:6000
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v27, v28, v27, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v25, v27, 16, v26
	v_wmma_i32_16x16x16_iu4 v[9:16], v[24:25], v[22:23], v[9:16] neg_lo:[1,1,0]
	ds_load_u8 v22, v104 offset:6384
	ds_load_u8 v23, v104 offset:6256
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v22, v23, v22, 0xc0c0004
	ds_load_u8 v23, v104 offset:6640
	ds_load_u8 v24, v104 offset:6512
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v23, v24, v23, 0xc0c0004
	ds_load_u8 v24, v104 offset:6896
	ds_load_u8 v25, v104 offset:6768
	v_lshl_or_b32 v22, v23, 16, v22
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v24, v25, v24, 0xc0c0004
	ds_load_u8 v25, v137
	ds_load_u8 v26, v104 offset:7024
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v25, v26, v25, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v23, v25, 16, v24
	v_wmma_i32_16x16x16_iu4 v[9:16], v[22:23], v[20:21], v[9:16] neg_lo:[1,1,0]
	ds_load_u8 v20, v104 offset:7408
	ds_load_u8 v21, v104 offset:7280
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v20, v21, v20, 0xc0c0004
	ds_load_u8 v21, v104 offset:7664
	ds_load_u8 v22, v104 offset:7536
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v21, v22, v21, 0xc0c0004
	ds_load_u8 v22, v104 offset:7920
	ds_load_u8 v23, v104 offset:7792
	v_lshl_or_b32 v20, v21, 16, v20
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v22, v23, v22, 0xc0c0004
	ds_load_u8 v23, v138
	ds_load_u8 v24, v104 offset:8048
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v23, v24, v23, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v21, v23, 16, v22
	v_wmma_i32_16x16x16_iu4 v[9:16], v[20:21], v[18:19], v[9:16] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v18, v12
	v_cvt_f32_i32_e32 v12, v13
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_add_nc_u32_e32 v13, s45, v120
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v21, v9
	v_cvt_f32_i32_e32 v20, v10
	v_cvt_f32_i32_e32 v19, v11
	v_cvt_f32_i32_e32 v11, v14
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	buffer_load_u16 v33, v13, s[36:39], 0 offen
	v_add_nc_u32_e32 v13, s45, v119
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v10, v15
	v_cvt_f32_i32_e32 v9, v16
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_clause 0x1f
	buffer_load_u16 v243, v13, s[36:39], 0 offen
	buffer_load_u16 v242, v13, s[36:39], 0 offen offset:4
	buffer_load_u16 v241, v13, s[36:39], 0 offen offset:8
	buffer_load_u16 v240, v13, s[36:39], 0 offen offset:12
	buffer_load_u16 v239, v13, s[36:39], 0 offen offset:16
	buffer_load_u16 v238, v13, s[36:39], 0 offen offset:20
	buffer_load_u16 v237, v13, s[36:39], 0 offen offset:24
	buffer_load_u16 v236, v13, s[36:39], 0 offen offset:28
	buffer_load_u16 v235, v13, s[36:39], 0 offen offset:32
	buffer_load_u16 v234, v13, s[36:39], 0 offen offset:36
	buffer_load_u16 v233, v13, s[36:39], 0 offen offset:40
	buffer_load_u16 v232, v13, s[36:39], 0 offen offset:44
	buffer_load_u16 v231, v13, s[36:39], 0 offen offset:48
	buffer_load_u16 v230, v13, s[36:39], 0 offen offset:52
	buffer_load_u16 v229, v13, s[36:39], 0 offen offset:56
	buffer_load_u16 v228, v13, s[36:39], 0 offen offset:60
	buffer_load_u16 v227, v13, s[36:39], 0 offen offset:64
	buffer_load_u16 v226, v13, s[36:39], 0 offen offset:68
	buffer_load_u16 v225, v13, s[36:39], 0 offen offset:72
	buffer_load_u16 v224, v13, s[36:39], 0 offen offset:76
	buffer_load_u16 v223, v13, s[36:39], 0 offen offset:80
	buffer_load_u16 v222, v13, s[36:39], 0 offen offset:84
	buffer_load_u16 v221, v13, s[36:39], 0 offen offset:88
	buffer_load_u16 v220, v13, s[36:39], 0 offen offset:92
	buffer_load_u16 v219, v13, s[36:39], 0 offen offset:96
	buffer_load_u16 v218, v13, s[36:39], 0 offen offset:100
	buffer_load_u16 v217, v13, s[36:39], 0 offen offset:104
	buffer_load_u16 v216, v13, s[36:39], 0 offen offset:108
	buffer_load_u16 v215, v13, s[36:39], 0 offen offset:112
	buffer_load_u16 v214, v13, s[36:39], 0 offen offset:116
	buffer_load_u16 v213, v13, s[36:39], 0 offen offset:120
	buffer_load_u16 v212, v13, s[36:39], 0 offen offset:128
	s_clause 0x1d
	buffer_load_u16 v211, v13, s[36:39], 0 offen offset:132
	buffer_load_u16 v210, v13, s[36:39], 0 offen offset:136
	buffer_load_u16 v209, v13, s[36:39], 0 offen offset:140
	buffer_load_u16 v208, v13, s[36:39], 0 offen offset:144
	buffer_load_u16 v207, v13, s[36:39], 0 offen offset:148
	buffer_load_u16 v206, v13, s[36:39], 0 offen offset:152
	buffer_load_u16 v205, v13, s[36:39], 0 offen offset:156
	buffer_load_u16 v204, v13, s[36:39], 0 offen offset:160
	buffer_load_u16 v203, v13, s[36:39], 0 offen offset:164
	buffer_load_u16 v202, v13, s[36:39], 0 offen offset:168
	buffer_load_u16 v201, v13, s[36:39], 0 offen offset:172
	buffer_load_u16 v200, v13, s[36:39], 0 offen offset:176
	buffer_load_u16 v199, v13, s[36:39], 0 offen offset:180
	buffer_load_u16 v198, v13, s[36:39], 0 offen offset:184
	buffer_load_u16 v197, v13, s[36:39], 0 offen offset:188
	buffer_load_u16 v32, v13, s[36:39], 0 offen offset:192
	buffer_load_u16 v31, v13, s[36:39], 0 offen offset:196
	buffer_load_u16 v30, v13, s[36:39], 0 offen offset:200
	buffer_load_u16 v29, v13, s[36:39], 0 offen offset:204
	buffer_load_u16 v28, v13, s[36:39], 0 offen offset:208
	buffer_load_u16 v27, v13, s[36:39], 0 offen offset:212
	buffer_load_u16 v26, v13, s[36:39], 0 offen offset:216
	buffer_load_u16 v25, v13, s[36:39], 0 offen offset:220
	buffer_load_u16 v24, v13, s[36:39], 0 offen offset:224
	buffer_load_u16 v23, v13, s[36:39], 0 offen offset:228
	buffer_load_u16 v22, v13, s[36:39], 0 offen offset:232
	buffer_load_u16 v16, v13, s[36:39], 0 offen offset:236
	buffer_load_u16 v15, v13, s[36:39], 0 offen offset:240
	buffer_load_u16 v14, v13, s[36:39], 0 offen offset:244
	buffer_load_u16 v13, v13, s[36:39], 0 offen offset:248
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_cndmask_b32_e32 v245, 0x80000000, v122, vcc_lo
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v145, v140, v145
	v_mul_f32_e32 v144, v140, v144
	v_mul_f32_e32 v143, v140, v143
	v_mul_f32_e32 v142, v140, v142
	v_mul_f32_e32 v141, v140, v141
	v_mul_f32_e32 v21, v140, v21
	v_mul_f32_e32 v20, v140, v20
	v_mul_f32_e32 v19, v140, v19
	v_mul_f32_e32 v18, v140, v18
	v_mul_f32_e32 v12, v140, v12
	v_mul_f32_e32 v11, v140, v11
	v_mul_f32_e32 v10, v140, v10
	v_mul_f32_e32 v9, v140, v9
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_add_nc_u32_e32 v119, s46, v119
	v_add_nc_u32_e32 v122, s47, v122
	v_add_nc_u32_e32 v120, s46, v120
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(61)
	v_lshlrev_b32_e32 v243, 16, v243
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_3) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v52, v148, v243
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(60)
	v_lshlrev_b32_e32 v148, 16, v242
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	s_waitcnt vmcnt(59)
	v_dual_fmac_f32 v100, v147, v148 :: v_dual_lshlrev_b32 v147, 16, v241
	s_waitcnt vmcnt(58)
	s_delay_alu instid0(VALU_DEP_1)
	v_dual_fmac_f32 v99, v146, v147 :: v_dual_lshlrev_b32 v146, 16, v240
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	buffer_load_b128 v[245:248], v245, s[28:31], 0 offen
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_store_b16 v114, v139 offset:8192
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	s_waitcnt vmcnt(58)
	v_dual_fmac_f32 v98, v145, v146 :: v_dual_lshlrev_b32 v145, 16, v239
	s_waitcnt vmcnt(57)
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_dual_fmac_f32 v97, v144, v145 :: v_dual_lshlrev_b32 v144, 16, v238
	s_waitcnt vmcnt(56)
	v_dual_fmac_f32 v96, v143, v144 :: v_dual_lshlrev_b32 v143, 16, v237
	s_waitcnt vmcnt(55)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_dual_fmac_f32 v95, v142, v143 :: v_dual_lshlrev_b32 v142, 16, v236
	v_dual_fmac_f32 v94, v141, v142 :: v_dual_mul_f32 v141, v140, v156
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(54)
	v_lshlrev_b32_e32 v142, 16, v235
	s_waitcnt vmcnt(13)
	v_lshlrev_b32_e32 v30, 16, v30
	s_waitcnt vmcnt(11)
	v_lshlrev_b32_e32 v28, 16, v28
	v_lshlrev_b32_e32 v32, 16, v32
	s_waitcnt vmcnt(9)
	v_lshlrev_b32_e32 v26, 16, v26
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v93, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v155 :: v_dual_lshlrev_b32 v142, 16, v234
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v33, 16, v33
	s_waitcnt vmcnt(7)
	v_lshlrev_b32_e32 v24, 16, v24
	s_waitcnt vmcnt(3)
	v_lshlrev_b32_e32 v15, 16, v15
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v92, v141, v142 :: v_dual_lshlrev_b32 v31, 16, v31
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v154 :: v_dual_lshlrev_b32 v142, 16, v233
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v44, v21, v24 :: v_dual_lshlrev_b32 v21, 16, v23
	s_delay_alu instid0(VALU_DEP_4)
	v_fmac_f32_e32 v40, v12, v15
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v12, 16, v14
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v91, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v153 :: v_dual_lshlrev_b32 v142, 16, v232
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v43, v20, v21 :: v_dual_lshlrev_b32 v20, 16, v22
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	v_fmac_f32_e32 v39, v11, v12
	s_waitcnt vmcnt(1)
	v_dual_fmac_f32 v90, v141, v142 :: v_dual_lshlrev_b32 v11, 16, v13
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v152 :: v_dual_lshlrev_b32 v142, 16, v231
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v42, v19, v20
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_dual_fmac_f32 v38, v10, v11 :: v_dual_lshlrev_b32 v29, 16, v29
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v27, 16, v27
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v89, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v151 :: v_dual_lshlrev_b32 v142, 16, v230
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v25, 16, v25
	v_lshlrev_b32_e32 v16, 16, v16
	v_lshlrev_b32_e32 v10, 16, v244
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v88, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v150 :: v_dual_lshlrev_b32 v142, 16, v229
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v41, v18, v16
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_fmac_f32_e32 v37, v9, v10
	v_fmac_f32_e32 v87, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v149 :: v_dual_lshlrev_b32 v142, 16, v228
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v86, v141, v142 :: v_dual_mul_f32 v141, v140, v164
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v142, 16, v227
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v85, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v163 :: v_dual_lshlrev_b32 v142, 16, v226
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v84, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v162 :: v_dual_lshlrev_b32 v142, 16, v225
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v83, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v161 :: v_dual_lshlrev_b32 v142, 16, v224
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v82, v141, v142 :: v_dual_mul_f32 v141, v140, v160
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v142, 16, v223
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v81, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v159 :: v_dual_lshlrev_b32 v142, 16, v222
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v80, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v158 :: v_dual_lshlrev_b32 v142, 16, v221
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v79, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v157 :: v_dual_lshlrev_b32 v142, 16, v220
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v78, v141, v142 :: v_dual_mul_f32 v141, v140, v172
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v142, 16, v219
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v77, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v171 :: v_dual_lshlrev_b32 v142, 16, v218
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v76, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v170 :: v_dual_lshlrev_b32 v142, 16, v217
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v75, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v169 :: v_dual_lshlrev_b32 v142, 16, v216
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v74, v141, v142 :: v_dual_mul_f32 v141, v140, v168
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v142, 16, v215
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v73, v141, v142
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v167 :: v_dual_lshlrev_b32 v142, 16, v214
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v72, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v141, v140, v166 :: v_dual_lshlrev_b32 v142, 16, v213
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v71, v141, v142
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v141, v140, v165
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v70, v141, v33 :: v_dual_mul_f32 v33, v140, v180
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v212
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v69, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v179
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v211
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v68, v33, v141 :: v_dual_mul_f32 v33, v140, v178
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v210
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v67, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v177
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v209
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v66, v33, v141 :: v_dual_mul_f32 v33, v140, v176
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v208
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v65, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v175
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v207
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v64, v33, v141 :: v_dual_mul_f32 v33, v140, v174
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v206
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v63, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v173
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v205
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v62, v33, v141 :: v_dual_mul_f32 v33, v140, v188
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v204
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v61, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v187
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v203
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v60, v33, v141 :: v_dual_mul_f32 v33, v140, v186
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v202
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v59, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v185
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v201
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v58, v33, v141 :: v_dual_mul_f32 v33, v140, v184
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v200
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v57, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v183
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v199
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v56, v33, v141 :: v_dual_mul_f32 v33, v140, v182
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v198
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v55, v33, v141
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v33, v140, v181
	.loc	1 205 18 is_stmt 1              ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v141, 16, v197
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_dual_fmac_f32 v54, v33, v141 :: v_dual_mul_f32 v33, v140, v196
	v_dual_fmac_f32 v53, v33, v32 :: v_dual_mul_f32 v32, v140, v195
	s_delay_alu instid0(VALU_DEP_1)
	v_fmac_f32_e32 v51, v32, v31
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v31, v140, v194
	.loc	1 193 26 is_stmt 1              ; generate_amdgcn.py:193:26
	s_waitcnt vmcnt(0)
	ds_store_b128 v115, v[245:248]
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v50, v31, v30
	.loc	1 206 20 is_stmt 0              ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v30, v140, v193
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v49, v30, v29
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v29, v140, v192
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v48, v29, v28
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v28, v140, v191
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v47, v28, v27
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v27, v140, v190
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v46, v27, v26
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v26, v140, v189
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fmac_f32_e32 v45, v26, v25
	.loc	1 161 9 is_stmt 1               ; generate_amdgcn.py:161:9
	s_cbranch_scc1 .LBB0_5
.LBB0_6:                                ; %Flow606
	.loc	1 168 26                        ; generate_amdgcn.py:168:26
	v_mul_i32_i24_e32 v1, s6, v36
	s_mul_i32 s4, s34, s6
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	s_max_i32 s3, s5, 0
	v_dual_mov_b32 v28, 0 :: v_dual_and_b32 v25, 0x1e0, v0
	.loc	1 168 26                        ; generate_amdgcn.py:168:26
	s_add_i32 s4, s4, s3
	v_dual_mov_b32 v27, 0 :: v_dual_mov_b32 v30, 0
	.loc	1 168 18 is_stmt 0              ; generate_amdgcn.py:168:18
	v_add_lshl_u32 v1, s4, v1, 1
	v_dual_mov_b32 v29, 0 :: v_dual_mov_b32 v32, 0
	v_dual_mov_b32 v31, 0 :: v_dual_mov_b32 v114, 0
	s_delay_alu instid0(VALU_DEP_3)
	v_cndmask_b32_e64 v1, 0x80000000, v1, s2
	v_dual_mov_b32 v33, 0 :: v_dual_mov_b32 v116, 0
	v_dual_mov_b32 v115, 0 :: v_dual_mov_b32 v118, 0
	buffer_load_u16 v26, v1, s[24:27], 0 offen
	v_dual_mov_b32 v117, 0 :: v_dual_mov_b32 v120, 0
	v_dual_mov_b32 v119, 0 :: v_dual_mov_b32 v122, 0
	v_dual_mov_b32 v121, 0 :: v_dual_mov_b32 v124, 0
	v_dual_mov_b32 v123, 0 :: v_dual_mov_b32 v126, 0
	v_dual_mov_b32 v125, 0 :: v_dual_mov_b32 v128, 0
	v_dual_mov_b32 v127, 0 :: v_dual_mov_b32 v130, 0
	v_dual_mov_b32 v129, 0 :: v_dual_mov_b32 v132, 0
	v_dual_mov_b32 v131, 0 :: v_dual_mov_b32 v134, 0
	v_dual_mov_b32 v133, 0 :: v_dual_mov_b32 v136, 0
	v_dual_mov_b32 v135, 0 :: v_dual_mov_b32 v138, 0
	v_dual_mov_b32 v137, 0 :: v_dual_mov_b32 v140, 0
	v_dual_mov_b32 v139, 0 :: v_dual_mov_b32 v142, 0
	v_dual_mov_b32 v141, 0 :: v_dual_mov_b32 v144, 0
	v_dual_mov_b32 v143, 0 :: v_dual_mov_b32 v146, 0
	v_dual_mov_b32 v145, 0 :: v_dual_mov_b32 v148, 0
	v_dual_mov_b32 v147, 0 :: v_dual_mov_b32 v150, 0
	v_dual_mov_b32 v149, 0 :: v_dual_mov_b32 v152, 0
	v_dual_mov_b32 v151, 0 :: v_dual_mov_b32 v154, 0
	v_dual_mov_b32 v153, 0 :: v_dual_mov_b32 v156, 0
	v_dual_mov_b32 v155, 0 :: v_dual_mov_b32 v158, 0
	v_dual_mov_b32 v157, 0 :: v_dual_mov_b32 v160, 0
	v_dual_mov_b32 v159, 0 :: v_dual_mov_b32 v162, 0
	v_dual_mov_b32 v161, 0 :: v_dual_mov_b32 v2, 0
	v_dual_mov_b32 v1, 0 :: v_dual_mov_b32 v4, 0
	v_dual_mov_b32 v3, 0 :: v_dual_mov_b32 v6, 0
	v_dual_mov_b32 v5, 0 :: v_dual_mov_b32 v8, 0
	v_mov_b32_e32 v7, 0
	s_mov_b32 s4, 0
	.loc	1 194 27 is_stmt 1              ; generate_amdgcn.py:194:27
	s_and_not1_b32 vcc_lo, exec_lo, s2
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_cbranch_vccnz .LBB0_8
; %bb.7:
	ds_load_u8 v1, v104 offset:640
	ds_load_u8 v2, v104 offset:512
	ds_load_u8 v3, v104 offset:896
	ds_load_u8 v4, v104 offset:768
	ds_load_u8 v5, v104 offset:128
	ds_load_u8 v6, v104
	ds_load_u8 v7, v104 offset:384
	ds_load_u8 v8, v104 offset:256
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
	s_mov_b32 s11, s4
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_and_b32_e32 v9, 56, v113
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v11, v104 offset:1664
	ds_load_u8 v12, v104 offset:1536
	ds_load_u8 v13, v104 offset:1920
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v111, 0, v111
	v_add_nc_u32_e32 v112, 0, v112
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_lshl_or_b32 v29, v36, 6, v9
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v9, v104 offset:1792
	ds_load_u8 v14, v104 offset:1152
	ds_load_u8 v15, v104 offset:1024
	ds_load_u8 v16, v104 offset:1408
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v1, v2, v1, 0xc0c0004
	s_waitcnt lgkmcnt(11)
	v_perm_b32 v2, v4, v3, 0xc0c0004
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v10, v29, 8, 0
	v_add_nc_u32_e32 v17, 0, v29
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v3, v104 offset:1280
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[21:22], v10 offset:8192
	ds_load_b64 v[23:24], v17 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v4, v6, v5, 0xc0c0004
	v_lshl_or_b32 v10, v2, 16, v1
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v1, v8, v7, 0xc0c0004
	ds_load_u8 v17, v104 offset:2688
	ds_load_u8 v18, v104 offset:2560
	ds_load_u8 v19, v104 offset:2944
	ds_load_u8 v20, v104 offset:2816
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v12, v9, v13, 0xc0c0004
	v_lshl_or_b32 v9, v1, 16, v4
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v13, v15, v14, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_3)
	v_lshl_or_b32 v12, v12, 16, v11
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v14, v3, v16, 0xc0c0004
	v_dual_mov_b32 v1, s4 :: v_dual_mov_b32 v6, s9
	v_dual_mov_b32 v2, s5 :: v_dual_mov_b32 v3, s6
	v_mov_b32_e32 v8, s11
	v_dual_mov_b32 v4, s7 :: v_dual_mov_b32 v5, s8
	v_mov_b32_e32 v7, s10
	v_lshl_or_b32 v11, v14, 16, v13
	ds_load_u8 v13, v104 offset:2432
	ds_load_u8 v14, v104 offset:2304
	s_waitcnt lgkmcnt(6)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:2176
	ds_load_u8 v10, v104 offset:2048
	v_wmma_i32_16x16x16_iu4 v[113:120], v[11:12], v[21:22], v[113:120] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v12, v20, v19, 0xc0c0004
	v_perm_b32 v11, v18, v17, 0xc0c0004
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v13, v14, v13, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v10, v29, 16, 0
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_3) | instid1(VALU_DEP_1)
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_lshl_or_b32 v9, v13, 16, v9
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[19:20], v10 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_lshl_or_b32 v10, v12, 16, v11
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[19:20], v[113:120] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:3712
	ds_load_u8 v10, v104 offset:3584
	ds_load_u8 v11, v104 offset:3968
	ds_load_u8 v12, v104 offset:3840
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v12, v11, 0xc0c0004
	ds_load_u8 v11, v104 offset:3200
	ds_load_u8 v12, v104 offset:3072
	v_lshl_or_b32 v10, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:3456
	ds_load_u8 v13, v104 offset:3328
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v9, v12, 16, v11
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v11, v29, 24, 0
	ds_load_b64 v[17:18], v11 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[17:18], v[113:120] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:4736
	ds_load_u8 v10, v104 offset:4608
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:4992
	ds_load_u8 v11, v104 offset:4864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:4224
	ds_load_u8 v12, v104 offset:4096
	v_lshl_or_b32 v10, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:4480
	ds_load_u8 v13, v104 offset:4352
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v9, v12, 16, v11
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v11, v29, 32, 0
	ds_load_b64 v[15:16], v11 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[15:16], v[113:120] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:5760
	ds_load_u8 v10, v104 offset:5632
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:6016
	ds_load_u8 v11, v104 offset:5888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:5248
	ds_load_u8 v12, v104 offset:5120
	v_lshl_or_b32 v10, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:5504
	ds_load_u8 v13, v104 offset:5376
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v9, v12, 16, v11
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v11, v29, 40, 0
	ds_load_b64 v[13:14], v11 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[13:14], v[113:120] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:6784
	ds_load_u8 v10, v104 offset:6656
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:7040
	ds_load_u8 v11, v104 offset:6912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v11, v10, 0xc0c0004
	ds_load_u8 v11, v104 offset:6272
	ds_load_u8 v12, v104 offset:6144
	v_lshl_or_b32 v10, v10, 16, v9
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:6528
	ds_load_u8 v27, v104 offset:6400
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v27, v12, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v9, v12, 16, v11
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v11, v29, 48, 0
	ds_load_b64 v[11:12], v11 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[9:10], v[11:12], v[113:120] neg_lo:[1,1,0]
	ds_load_u8 v9, v104 offset:7808
	ds_load_u8 v10, v104 offset:7680
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v10, v9, 0xc0c0004
	ds_load_u8 v10, v104 offset:8064
	ds_load_u8 v27, v104 offset:7936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v10, v27, v10, 0xc0c0004
	ds_load_u8 v27, v104 offset:7296
	ds_load_u8 v28, v104 offset:7168
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v27, v28, v27, 0xc0c0004
	ds_load_u8 v28, v104 offset:7552
	ds_load_u8 v30, v104 offset:7424
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v30, v30, v28, 0xc0c0004
	v_lshl_or_b32 v28, v10, 16, v9
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	v_xad_u32 v9, v29, 56, 0
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_3) | instid1(VALU_DEP_1)
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	v_lshl_or_b32 v27, v30, 16, v27
	.loc	1 189 26                        ; generate_amdgcn.py:189:26
	ds_load_b64 v[9:10], v9 offset:8192
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[113:120], v[27:28], v[9:10], v[113:120] neg_lo:[1,1,0]
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v27, v113
	s_delay_alu instid0(VALU_DEP_2)
	v_cvt_f32_i32_e32 v29, v115
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v113, v104 offset:656
	ds_load_u8 v115, v104 offset:528
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v30, v116
	v_cvt_f32_i32_e32 v31, v117
	v_cvt_f32_i32_e32 v32, v118
	v_cvt_f32_i32_e32 v28, v114
	v_cvt_f32_i32_e32 v33, v119
	v_cvt_f32_i32_e32 v114, v120
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v115, v113, 0xc0c0004
	ds_load_u8 v115, v104 offset:912
	ds_load_u8 v116, v104 offset:784
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v115, v116, v115, 0xc0c0004
	ds_load_u8 v116, v104 offset:16
	ds_load_u8 v117, v104 offset:144
	v_lshl_or_b32 v124, v115, 16, v113
	ds_load_u8 v113, v104 offset:1680
	ds_load_u8 v115, v104 offset:1552
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v116, v116, v117, 0xc0c0004
	ds_load_u8 v117, v104 offset:400
	ds_load_u8 v118, v104 offset:272
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v115, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v117, v118, v117, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v123, v117, 16, v116
	ds_load_u8 v115, v104 offset:1936
	ds_load_u8 v116, v104 offset:1808
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v115, v116, v115, 0xc0c0004
	ds_load_u8 v116, v104 offset:1168
	ds_load_u8 v117, v104 offset:1040
	v_lshl_or_b32 v126, v115, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v116, v117, v116, 0xc0c0004
	ds_load_u8 v117, v104 offset:1424
	ds_load_u8 v118, v104 offset:1296
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v117, v118, v117, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v125, v117, 16, v116
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2704
	ds_load_u8 v123, v104 offset:2576
	v_wmma_i32_16x16x16_iu4 v[115:122], v[125:126], v[21:22], v[115:122] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:2960
	ds_load_u8 v124, v104 offset:2832
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:2192
	ds_load_u8 v125, v104 offset:2064
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:2448
	ds_load_u8 v126, v104 offset:2320
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[19:20], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3728
	ds_load_u8 v123, v104 offset:3600
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:3984
	ds_load_u8 v124, v104 offset:3856
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:3216
	ds_load_u8 v125, v104 offset:3088
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:3472
	ds_load_u8 v126, v104 offset:3344
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[17:18], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4752
	ds_load_u8 v123, v104 offset:4624
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:5008
	ds_load_u8 v124, v104 offset:4880
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:4240
	ds_load_u8 v125, v104 offset:4112
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:4496
	ds_load_u8 v126, v104 offset:4368
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[15:16], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5776
	ds_load_u8 v123, v104 offset:5648
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:6032
	ds_load_u8 v124, v104 offset:5904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:5264
	ds_load_u8 v125, v104 offset:5136
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:5520
	ds_load_u8 v126, v104 offset:5392
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[13:14], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6800
	ds_load_u8 v123, v104 offset:6672
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:7056
	ds_load_u8 v124, v104 offset:6928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:6288
	ds_load_u8 v125, v104 offset:6160
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:6544
	ds_load_u8 v126, v104 offset:6416
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[11:12], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7824
	ds_load_u8 v123, v104 offset:7696
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:8080
	ds_load_u8 v124, v104 offset:7952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:7312
	ds_load_u8 v125, v104 offset:7184
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v125, v124, 0xc0c0004
	ds_load_u8 v124, v104 offset:7568
	ds_load_u8 v126, v104 offset:7440
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v126, v126, v124, 0xc0c0004
	v_lshl_or_b32 v124, v123, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v123, v126, 16, v125
	v_wmma_i32_16x16x16_iu4 v[115:122], v[123:124], v[9:10], v[115:122] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:672
	ds_load_u8 v123, v104 offset:544
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v115, v115
	v_cvt_f32_i32_e32 v116, v116
	v_cvt_f32_i32_e32 v117, v117
	v_cvt_f32_i32_e32 v118, v118
	v_cvt_f32_i32_e32 v119, v119
	v_cvt_f32_i32_e32 v120, v120
	v_cvt_f32_i32_e32 v121, v121
	v_cvt_f32_i32_e32 v122, v122
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	ds_load_u8 v123, v104 offset:928
	ds_load_u8 v124, v104 offset:800
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:32
	ds_load_u8 v125, v104 offset:160
	v_lshl_or_b32 v132, v123, 16, v113
	ds_load_u8 v113, v104 offset:1696
	ds_load_u8 v123, v104 offset:1568
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v124, v124, v125, 0xc0c0004
	ds_load_u8 v125, v104 offset:416
	ds_load_u8 v126, v104 offset:288
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v123, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v126, v125, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v131, v125, 16, v124
	ds_load_u8 v123, v104 offset:1952
	ds_load_u8 v124, v104 offset:1824
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v123, v124, v123, 0xc0c0004
	ds_load_u8 v124, v104 offset:1184
	ds_load_u8 v125, v104 offset:1056
	v_lshl_or_b32 v134, v123, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v124, v125, v124, 0xc0c0004
	ds_load_u8 v125, v104 offset:1440
	ds_load_u8 v126, v104 offset:1312
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v125, v126, v125, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v133, v125, 16, v124
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2720
	ds_load_u8 v131, v104 offset:2592
	v_wmma_i32_16x16x16_iu4 v[123:130], v[133:134], v[21:22], v[123:130] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:2976
	ds_load_u8 v132, v104 offset:2848
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:2208
	ds_load_u8 v133, v104 offset:2080
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:2464
	ds_load_u8 v134, v104 offset:2336
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[19:20], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3744
	ds_load_u8 v131, v104 offset:3616
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:4000
	ds_load_u8 v132, v104 offset:3872
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:3232
	ds_load_u8 v133, v104 offset:3104
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:3488
	ds_load_u8 v134, v104 offset:3360
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[17:18], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4768
	ds_load_u8 v131, v104 offset:4640
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:5024
	ds_load_u8 v132, v104 offset:4896
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:4256
	ds_load_u8 v133, v104 offset:4128
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:4512
	ds_load_u8 v134, v104 offset:4384
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[15:16], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5792
	ds_load_u8 v131, v104 offset:5664
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:6048
	ds_load_u8 v132, v104 offset:5920
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:5280
	ds_load_u8 v133, v104 offset:5152
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:5536
	ds_load_u8 v134, v104 offset:5408
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[13:14], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6816
	ds_load_u8 v131, v104 offset:6688
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:7072
	ds_load_u8 v132, v104 offset:6944
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:6304
	ds_load_u8 v133, v104 offset:6176
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:6560
	ds_load_u8 v134, v104 offset:6432
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[11:12], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7840
	ds_load_u8 v131, v104 offset:7712
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:8096
	ds_load_u8 v132, v104 offset:7968
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:7328
	ds_load_u8 v133, v104 offset:7200
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v133, v132, 0xc0c0004
	ds_load_u8 v132, v104 offset:7584
	ds_load_u8 v134, v104 offset:7456
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v134, v134, v132, 0xc0c0004
	v_lshl_or_b32 v132, v131, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v131, v134, 16, v133
	v_wmma_i32_16x16x16_iu4 v[123:130], v[131:132], v[9:10], v[123:130] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:688
	ds_load_u8 v131, v104 offset:560
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v123, v123
	v_cvt_f32_i32_e32 v124, v124
	v_cvt_f32_i32_e32 v125, v125
	v_cvt_f32_i32_e32 v126, v126
	v_cvt_f32_i32_e32 v127, v127
	v_cvt_f32_i32_e32 v128, v128
	v_cvt_f32_i32_e32 v129, v129
	v_cvt_f32_i32_e32 v130, v130
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	ds_load_u8 v131, v104 offset:944
	ds_load_u8 v132, v104 offset:816
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:48
	ds_load_u8 v133, v104 offset:176
	v_lshl_or_b32 v140, v131, 16, v113
	ds_load_u8 v113, v104 offset:1712
	ds_load_u8 v131, v104 offset:1584
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v132, v132, v133, 0xc0c0004
	ds_load_u8 v133, v104 offset:432
	ds_load_u8 v134, v104 offset:304
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v131, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v134, v133, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v139, v133, 16, v132
	ds_load_u8 v131, v104 offset:1968
	ds_load_u8 v132, v104 offset:1840
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v131, v132, v131, 0xc0c0004
	ds_load_u8 v132, v104 offset:1200
	ds_load_u8 v133, v104 offset:1072
	v_lshl_or_b32 v142, v131, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v132, v133, v132, 0xc0c0004
	ds_load_u8 v133, v104 offset:1456
	ds_load_u8 v134, v104 offset:1328
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v133, v134, v133, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v141, v133, 16, v132
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2736
	ds_load_u8 v139, v104 offset:2608
	v_wmma_i32_16x16x16_iu4 v[131:138], v[141:142], v[21:22], v[131:138] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:2992
	ds_load_u8 v140, v104 offset:2864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:2224
	ds_load_u8 v141, v104 offset:2096
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:2480
	ds_load_u8 v142, v104 offset:2352
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[19:20], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3760
	ds_load_u8 v139, v104 offset:3632
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:4016
	ds_load_u8 v140, v104 offset:3888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:3248
	ds_load_u8 v141, v104 offset:3120
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:3504
	ds_load_u8 v142, v104 offset:3376
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[17:18], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4784
	ds_load_u8 v139, v104 offset:4656
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:5040
	ds_load_u8 v140, v104 offset:4912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:4272
	ds_load_u8 v141, v104 offset:4144
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:4528
	ds_load_u8 v142, v104 offset:4400
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[15:16], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5808
	ds_load_u8 v139, v104 offset:5680
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:6064
	ds_load_u8 v140, v104 offset:5936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:5296
	ds_load_u8 v141, v104 offset:5168
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:5552
	ds_load_u8 v142, v104 offset:5424
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[13:14], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6832
	ds_load_u8 v139, v104 offset:6704
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:7088
	ds_load_u8 v140, v104 offset:6960
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:6320
	ds_load_u8 v141, v104 offset:6192
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:6576
	ds_load_u8 v142, v104 offset:6448
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[11:12], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7856
	ds_load_u8 v139, v104 offset:7728
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:8112
	ds_load_u8 v140, v104 offset:7984
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:7344
	ds_load_u8 v141, v104 offset:7216
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v141, v140, 0xc0c0004
	ds_load_u8 v140, v104 offset:7600
	ds_load_u8 v142, v104 offset:7472
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v142, v142, v140, 0xc0c0004
	v_lshl_or_b32 v140, v139, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v139, v142, 16, v141
	v_wmma_i32_16x16x16_iu4 v[131:138], v[139:140], v[9:10], v[131:138] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:704
	ds_load_u8 v139, v104 offset:576
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v131, v131
	v_cvt_f32_i32_e32 v132, v132
	v_cvt_f32_i32_e32 v133, v133
	v_cvt_f32_i32_e32 v134, v134
	v_cvt_f32_i32_e32 v135, v135
	v_cvt_f32_i32_e32 v136, v136
	v_cvt_f32_i32_e32 v137, v137
	v_cvt_f32_i32_e32 v138, v138
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	ds_load_u8 v139, v104 offset:960
	ds_load_u8 v140, v104 offset:832
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:192
	ds_load_u8 v141, v104 offset:64
	v_lshl_or_b32 v148, v139, 16, v113
	ds_load_u8 v113, v104 offset:1728
	ds_load_u8 v139, v104 offset:1600
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v140, v141, v140, 0xc0c0004
	ds_load_u8 v141, v104 offset:448
	ds_load_u8 v142, v104 offset:320
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v139, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v142, v141, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v147, v141, 16, v140
	ds_load_u8 v139, v104 offset:1984
	ds_load_u8 v140, v104 offset:1856
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v139, v140, v139, 0xc0c0004
	ds_load_u8 v140, v104 offset:1216
	ds_load_u8 v141, v104 offset:1088
	v_lshl_or_b32 v150, v139, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v140, v141, v140, 0xc0c0004
	ds_load_u8 v141, v104 offset:1472
	ds_load_u8 v142, v104 offset:1344
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v141, v142, v141, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v149, v141, 16, v140
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2752
	ds_load_u8 v147, v104 offset:2624
	v_wmma_i32_16x16x16_iu4 v[139:146], v[149:150], v[21:22], v[139:146] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:3008
	ds_load_u8 v148, v104 offset:2880
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:2240
	ds_load_u8 v149, v104 offset:2112
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:2496
	ds_load_u8 v150, v104 offset:2368
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[19:20], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3776
	ds_load_u8 v147, v104 offset:3648
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:4032
	ds_load_u8 v148, v104 offset:3904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:3264
	ds_load_u8 v149, v104 offset:3136
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:3520
	ds_load_u8 v150, v104 offset:3392
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[17:18], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4800
	ds_load_u8 v147, v104 offset:4672
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:5056
	ds_load_u8 v148, v104 offset:4928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:4288
	ds_load_u8 v149, v104 offset:4160
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:4544
	ds_load_u8 v150, v104 offset:4416
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[15:16], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5824
	ds_load_u8 v147, v104 offset:5696
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:6080
	ds_load_u8 v148, v104 offset:5952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:5312
	ds_load_u8 v149, v104 offset:5184
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:5568
	ds_load_u8 v150, v104 offset:5440
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[13:14], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6848
	ds_load_u8 v147, v104 offset:6720
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:7104
	ds_load_u8 v148, v104 offset:6976
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:6336
	ds_load_u8 v149, v104 offset:6208
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:6592
	ds_load_u8 v150, v104 offset:6464
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[11:12], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7872
	ds_load_u8 v147, v104 offset:7744
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:8128
	ds_load_u8 v148, v104 offset:8000
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:7360
	ds_load_u8 v149, v104 offset:7232
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v149, v148, 0xc0c0004
	ds_load_u8 v148, v104 offset:7616
	ds_load_u8 v150, v104 offset:7488
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v150, v150, v148, 0xc0c0004
	v_lshl_or_b32 v148, v147, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v147, v150, 16, v149
	v_wmma_i32_16x16x16_iu4 v[139:146], v[147:148], v[9:10], v[139:146] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:720
	ds_load_u8 v147, v104 offset:592
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v139, v139
	v_cvt_f32_i32_e32 v140, v140
	v_cvt_f32_i32_e32 v141, v141
	v_cvt_f32_i32_e32 v142, v142
	v_cvt_f32_i32_e32 v143, v143
	v_cvt_f32_i32_e32 v144, v144
	v_cvt_f32_i32_e32 v145, v145
	v_cvt_f32_i32_e32 v146, v146
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	ds_load_u8 v147, v104 offset:976
	ds_load_u8 v148, v104 offset:848
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:208
	ds_load_u8 v149, v104 offset:80
	v_lshl_or_b32 v156, v147, 16, v113
	ds_load_u8 v113, v104 offset:1744
	ds_load_u8 v147, v104 offset:1616
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v148, v149, v148, 0xc0c0004
	ds_load_u8 v149, v104 offset:464
	ds_load_u8 v150, v104 offset:336
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v147, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v150, v149, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v155, v149, 16, v148
	ds_load_u8 v147, v104 offset:2000
	ds_load_u8 v148, v104 offset:1872
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v147, v148, v147, 0xc0c0004
	ds_load_u8 v148, v104 offset:1232
	ds_load_u8 v149, v104 offset:1104
	v_lshl_or_b32 v158, v147, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v148, v149, v148, 0xc0c0004
	ds_load_u8 v149, v104 offset:1488
	ds_load_u8 v150, v104 offset:1360
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v149, v150, v149, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v157, v149, 16, v148
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2768
	ds_load_u8 v155, v104 offset:2640
	v_wmma_i32_16x16x16_iu4 v[147:154], v[157:158], v[21:22], v[147:154] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:3024
	ds_load_u8 v156, v104 offset:2896
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:2256
	ds_load_u8 v157, v104 offset:2128
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:2512
	ds_load_u8 v158, v104 offset:2384
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[19:20], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3792
	ds_load_u8 v155, v104 offset:3664
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:4048
	ds_load_u8 v156, v104 offset:3920
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:3280
	ds_load_u8 v157, v104 offset:3152
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:3536
	ds_load_u8 v158, v104 offset:3408
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[17:18], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4816
	ds_load_u8 v155, v104 offset:4688
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:5072
	ds_load_u8 v156, v104 offset:4944
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:4304
	ds_load_u8 v157, v104 offset:4176
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:4560
	ds_load_u8 v158, v104 offset:4432
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[15:16], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5840
	ds_load_u8 v155, v104 offset:5712
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:6096
	ds_load_u8 v156, v104 offset:5968
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:5328
	ds_load_u8 v157, v104 offset:5200
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:5584
	ds_load_u8 v158, v104 offset:5456
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[13:14], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6864
	ds_load_u8 v155, v104 offset:6736
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:7120
	ds_load_u8 v156, v104 offset:6992
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:6352
	ds_load_u8 v157, v104 offset:6224
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:6608
	ds_load_u8 v158, v104 offset:6480
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[11:12], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7888
	ds_load_u8 v155, v104 offset:7760
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:8144
	ds_load_u8 v156, v104 offset:8016
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:7376
	ds_load_u8 v157, v104 offset:7248
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v157, v156, 0xc0c0004
	ds_load_u8 v156, v104 offset:7632
	ds_load_u8 v158, v104 offset:7504
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v158, v158, v156, 0xc0c0004
	v_lshl_or_b32 v156, v155, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v155, v158, 16, v157
	v_wmma_i32_16x16x16_iu4 v[147:154], v[155:156], v[9:10], v[147:154] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:736
	ds_load_u8 v155, v104 offset:608
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v147, v147
	v_cvt_f32_i32_e32 v148, v148
	v_cvt_f32_i32_e32 v149, v149
	v_cvt_f32_i32_e32 v150, v150
	v_cvt_f32_i32_e32 v151, v151
	v_cvt_f32_i32_e32 v152, v152
	v_cvt_f32_i32_e32 v153, v153
	v_cvt_f32_i32_e32 v154, v154
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	ds_load_u8 v155, v104 offset:992
	ds_load_u8 v156, v104 offset:864
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:224
	ds_load_u8 v157, v104 offset:96
	v_lshl_or_b32 v164, v155, 16, v113
	ds_load_u8 v113, v104 offset:1760
	ds_load_u8 v155, v104 offset:1632
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v156, v157, v156, 0xc0c0004
	ds_load_u8 v157, v104 offset:480
	ds_load_u8 v158, v104 offset:352
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v113, v155, v113, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v158, v157, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v163, v157, 16, v156
	ds_load_u8 v155, v104 offset:2016
	ds_load_u8 v156, v104 offset:1888
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v155, v156, v155, 0xc0c0004
	ds_load_u8 v156, v104 offset:1248
	ds_load_u8 v157, v104 offset:1120
	v_lshl_or_b32 v166, v155, 16, v113
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v156, v157, v156, 0xc0c0004
	ds_load_u8 v157, v104 offset:1504
	ds_load_u8 v158, v104 offset:1376
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v157, v158, v157, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v165, v157, 16, v156
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:2784
	ds_load_u8 v163, v104 offset:2656
	v_wmma_i32_16x16x16_iu4 v[155:162], v[165:166], v[21:22], v[155:162] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:3040
	ds_load_u8 v164, v104 offset:2912
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:2272
	ds_load_u8 v165, v104 offset:2144
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:2528
	ds_load_u8 v166, v104 offset:2400
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[19:20], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:3808
	ds_load_u8 v163, v104 offset:3680
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:4064
	ds_load_u8 v164, v104 offset:3936
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:3296
	ds_load_u8 v165, v104 offset:3168
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:3552
	ds_load_u8 v166, v104 offset:3424
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[17:18], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:4832
	ds_load_u8 v163, v104 offset:4704
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:5088
	ds_load_u8 v164, v104 offset:4960
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:4320
	ds_load_u8 v165, v104 offset:4192
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:4576
	ds_load_u8 v166, v104 offset:4448
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[15:16], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:5856
	ds_load_u8 v163, v104 offset:5728
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:6112
	ds_load_u8 v164, v104 offset:5984
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:5344
	ds_load_u8 v165, v104 offset:5216
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:5600
	ds_load_u8 v166, v104 offset:5472
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[13:14], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:6880
	ds_load_u8 v163, v104 offset:6752
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:7136
	ds_load_u8 v164, v104 offset:7008
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:6368
	ds_load_u8 v165, v104 offset:6240
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:6624
	ds_load_u8 v166, v104 offset:6496
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[11:12], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:7904
	ds_load_u8 v163, v104 offset:7776
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:8160
	ds_load_u8 v164, v104 offset:8032
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:7392
	ds_load_u8 v165, v104 offset:7264
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v164, 0xc0c0004
	ds_load_u8 v164, v104 offset:7648
	ds_load_u8 v166, v104 offset:7520
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v166, v166, v164, 0xc0c0004
	v_lshl_or_b32 v164, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v163, v166, 16, v165
	v_wmma_i32_16x16x16_iu4 v[155:162], v[163:164], v[9:10], v[155:162] neg_lo:[1,1,0]
	ds_load_u8 v113, v104 offset:240
	ds_load_u8 v163, v104 offset:112
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v155, v155
	v_cvt_f32_i32_e32 v156, v156
	v_cvt_f32_i32_e32 v157, v157
	v_cvt_f32_i32_e32 v158, v158
	v_cvt_f32_i32_e32 v159, v159
	v_cvt_f32_i32_e32 v160, v160
	v_cvt_f32_i32_e32 v161, v161
	v_cvt_f32_i32_e32 v162, v162
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v113, v163, v113, 0xc0c0004
	ds_load_u8 v163, v104 offset:496
	ds_load_u8 v164, v104 offset:368
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v163, v164, v163, 0xc0c0004
	ds_load_u8 v164, v104 offset:752
	ds_load_u8 v165, v104 offset:624
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v164, v165, v164, 0xc0c0004
	ds_load_u8 v112, v112
	ds_load_u8 v165, v104 offset:880
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v165, v165, v112, 0xc0c0004
	v_lshl_or_b32 v112, v163, 16, v113
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v113, v165, 16, v164
	v_wmma_i32_16x16x16_iu4 v[1:8], v[112:113], v[23:24], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v23, v104 offset:1264
	ds_load_u8 v24, v104 offset:1136
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v23, v24, v23, 0xc0c0004
	ds_load_u8 v24, v104 offset:1520
	ds_load_u8 v112, v104 offset:1392
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v24, v112, v24, 0xc0c0004
	ds_load_u8 v112, v104 offset:1776
	ds_load_u8 v113, v104 offset:1648
	v_lshl_or_b32 v23, v24, 16, v23
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v112, v113, v112, 0xc0c0004
	ds_load_u8 v111, v111
	ds_load_u8 v113, v104 offset:1904
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v111, v113, v111, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v24, v111, 16, v112
	v_wmma_i32_16x16x16_iu4 v[1:8], v[23:24], v[21:22], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v21, v104 offset:2288
	ds_load_u8 v22, v104 offset:2160
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v21, v22, v21, 0xc0c0004
	ds_load_u8 v22, v104 offset:2544
	ds_load_u8 v23, v104 offset:2416
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v22, v23, v22, 0xc0c0004
	ds_load_u8 v23, v104 offset:2800
	ds_load_u8 v24, v104 offset:2672
	v_lshl_or_b32 v21, v22, 16, v21
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v23, v24, v23, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v24, 0, v110
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v24, v24
	ds_load_u8 v110, v104 offset:2928
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v24, v110, v24, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v22, v24, 16, v23
	v_wmma_i32_16x16x16_iu4 v[1:8], v[21:22], v[19:20], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v19, v104 offset:3312
	ds_load_u8 v20, v104 offset:3184
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v19, v20, v19, 0xc0c0004
	ds_load_u8 v20, v104 offset:3568
	ds_load_u8 v21, v104 offset:3440
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v20, v21, v20, 0xc0c0004
	ds_load_u8 v21, v104 offset:3824
	ds_load_u8 v22, v104 offset:3696
	v_lshl_or_b32 v19, v20, 16, v19
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v21, v22, v21, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v22, 0, v109
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v22, v22
	ds_load_u8 v23, v104 offset:3952
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v22, v23, v22, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v20, v22, 16, v21
	v_wmma_i32_16x16x16_iu4 v[1:8], v[19:20], v[17:18], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v17, v104 offset:4336
	ds_load_u8 v18, v104 offset:4208
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v17, v18, v17, 0xc0c0004
	ds_load_u8 v18, v104 offset:4592
	ds_load_u8 v19, v104 offset:4464
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v18, v19, v18, 0xc0c0004
	ds_load_u8 v19, v104 offset:4848
	ds_load_u8 v20, v104 offset:4720
	v_lshl_or_b32 v17, v18, 16, v17
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v19, v20, v19, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v20, 0, v108
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v20, v20
	ds_load_u8 v21, v104 offset:4976
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v20, v21, v20, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v18, v20, 16, v19
	v_wmma_i32_16x16x16_iu4 v[1:8], v[17:18], v[15:16], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v15, v104 offset:5360
	ds_load_u8 v16, v104 offset:5232
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v15, v16, v15, 0xc0c0004
	ds_load_u8 v16, v104 offset:5616
	ds_load_u8 v17, v104 offset:5488
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v16, v17, v16, 0xc0c0004
	ds_load_u8 v17, v104 offset:5872
	ds_load_u8 v18, v104 offset:5744
	v_lshl_or_b32 v15, v16, 16, v15
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v17, v18, v17, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v18, 0, v107
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v18, v18
	ds_load_u8 v19, v104 offset:6000
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v18, v19, v18, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v16, v18, 16, v17
	v_wmma_i32_16x16x16_iu4 v[1:8], v[15:16], v[13:14], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v13, v104 offset:6384
	ds_load_u8 v14, v104 offset:6256
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v13, v14, v13, 0xc0c0004
	ds_load_u8 v14, v104 offset:6640
	ds_load_u8 v15, v104 offset:6512
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v14, v15, v14, 0xc0c0004
	ds_load_u8 v15, v104 offset:6896
	ds_load_u8 v16, v104 offset:6768
	v_lshl_or_b32 v13, v14, 16, v13
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v15, v16, v15, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v16, 0, v106
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v16, v16
	ds_load_u8 v17, v104 offset:7024
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v16, v17, v16, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v14, v16, 16, v15
	v_wmma_i32_16x16x16_iu4 v[1:8], v[13:14], v[11:12], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v11, v104 offset:7408
	ds_load_u8 v12, v104 offset:7280
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v11, v12, v11, 0xc0c0004
	ds_load_u8 v12, v104 offset:7664
	ds_load_u8 v13, v104 offset:7536
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v12, v13, v12, 0xc0c0004
	ds_load_u8 v13, v104 offset:7920
	ds_load_u8 v14, v104 offset:7792
	v_lshl_or_b32 v11, v12, 16, v11
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v13, v14, v13, 0xc0c0004
	.loc	1 193 26                        ; generate_amdgcn.py:193:26
	v_add_nc_u32_e32 v14, 0, v105
	.loc	1 194 27                        ; generate_amdgcn.py:194:27
	ds_load_u8 v15, v104 offset:8048
	ds_load_u8 v14, v14
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v14, v15, v14, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v12, v14, 16, v13
	v_wmma_i32_16x16x16_iu4 v[1:8], v[11:12], v[9:10], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_cvt_f32_i32_e32 v1, v1
	v_cvt_f32_i32_e32 v2, v2
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_cvt_f32_i32_e32 v3, v3
	v_cvt_f32_i32_e32 v4, v4
	v_cvt_f32_i32_e32 v5, v5
	v_cvt_f32_i32_e32 v6, v6
	v_cvt_f32_i32_e32 v7, v7
	v_cvt_f32_i32_e32 v8, v8
.LBB0_8:
	.loc	1 205 36                        ; generate_amdgcn.py:205:36
	s_mul_i32 s3, s3, s35
	v_mov_b16_e64 v198.l, 0
	.loc	1 205 26 is_stmt 0              ; generate_amdgcn.py:205:26
	s_add_i32 s3, s33, s3
	.loc	1 168 18 is_stmt 1              ; generate_amdgcn.py:168:18
	s_waitcnt vmcnt(0)
	v_mov_b16_e64 v198.h, v26.l
	.loc	1 205 26                        ; generate_amdgcn.py:205:26
	v_add_lshl_u32 v9, s3, v103, 1
	.loc	1 205 18 is_stmt 0              ; generate_amdgcn.py:205:18
	s_and_b32 s5, s15, 0xffff
	s_mov_b32 s7, 0x31027000
	s_mov_b32 s6, 0x7ffffffe
	s_mov_b32 s4, s14
	.loc	1 206 20 is_stmt 1              ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v33, v33, v198 :: v_dual_add_nc_u32 v18, 32, v9
	v_dual_mul_f32 v28, v28, v198 :: v_dual_add_nc_u32 v11, 4, v9
	v_dual_mul_f32 v116, v116, v198 :: v_dual_add_nc_u32 v19, 36, v9
	v_dual_mul_f32 v27, v27, v198 :: v_dual_add_nc_u32 v12, 8, v9
	v_dual_mul_f32 v115, v115, v198 :: v_dual_add_nc_u32 v20, 40, v9
	v_dual_mul_f32 v30, v30, v198 :: v_dual_add_nc_u32 v13, 12, v9
	v_dual_mul_f32 v118, v118, v198 :: v_dual_add_nc_u32 v21, 44, v9
	v_dual_mul_f32 v29, v29, v198 :: v_dual_add_nc_u32 v14, 16, v9
	v_dual_mul_f32 v32, v32, v198 :: v_dual_add_nc_u32 v15, 20, v9
	v_dual_mul_f32 v31, v31, v198 :: v_dual_add_nc_u32 v16, 24, v9
	v_dual_mul_f32 v114, v114, v198 :: v_dual_add_nc_u32 v17, 28, v9
	v_dual_mul_f32 v117, v117, v198 :: v_dual_add_nc_u32 v22, 48, v9
	v_dual_mul_f32 v120, v120, v198 :: v_dual_add_nc_u32 v23, 52, v9
	v_dual_mul_f32 v119, v119, v198 :: v_dual_add_nc_u32 v24, 56, v9
	v_dual_mul_f32 v122, v122, v198 :: v_dual_add_nc_u32 v103, 60, v9
	v_dual_mul_f32 v159, v159, v198 :: v_dual_add_nc_u32 v192, 0xe0, v9
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_cndmask_b32_e64 v10, 0x80000000, v9, s2
	v_cndmask_b32_e64 v18, 0x80000000, v18, s2
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v121, v121, v198 :: v_dual_add_nc_u32 v104, 64, v9
	v_dual_mul_f32 v2, v2, v198 :: v_dual_add_nc_u32 v193, 0xe4, v9
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_cndmask_b32_e64 v11, 0x80000000, v11, s2
	v_cndmask_b32_e64 v19, 0x80000000, v19, s2
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v124, v124, v198 :: v_dual_add_nc_u32 v105, 0x44, v9
	v_dual_mul_f32 v161, v161, v198 :: v_dual_add_nc_u32 v194, 0xe8, v9
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_cndmask_b32_e64 v12, 0x80000000, v12, s2
	v_cndmask_b32_e64 v20, 0x80000000, v20, s2
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v123, v123, v198 :: v_dual_add_nc_u32 v106, 0x48, v9
	v_dual_mul_f32 v4, v4, v198 :: v_dual_add_nc_u32 v195, 0xec, v9
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_cndmask_b32_e64 v13, 0x80000000, v13, s2
	v_cndmask_b32_e64 v21, 0x80000000, v21, s2
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v126, v126, v198 :: v_dual_add_nc_u32 v107, 0x4c, v9
	v_dual_mul_f32 v125, v125, v198 :: v_dual_add_nc_u32 v108, 0x50, v9
	v_dual_mul_f32 v128, v128, v198 :: v_dual_add_nc_u32 v109, 0x54, v9
	v_dual_mul_f32 v127, v127, v198 :: v_dual_add_nc_u32 v110, 0x58, v9
	v_dual_mul_f32 v130, v130, v198 :: v_dual_add_nc_u32 v111, 0x5c, v9
	v_dual_mul_f32 v129, v129, v198 :: v_dual_add_nc_u32 v112, 0x60, v9
	v_dual_mul_f32 v132, v132, v198 :: v_dual_add_nc_u32 v113, 0x64, v9
	v_dual_mul_f32 v134, v134, v198 :: v_dual_add_nc_u32 v163, 0x68, v9
	v_dual_mul_f32 v131, v131, v198 :: v_dual_add_nc_u32 v164, 0x6c, v9
	v_dual_mul_f32 v136, v136, v198 :: v_dual_add_nc_u32 v165, 0x70, v9
	v_dual_mul_f32 v133, v133, v198 :: v_dual_add_nc_u32 v166, 0x74, v9
	v_dual_mul_f32 v138, v138, v198 :: v_dual_add_nc_u32 v167, 0x78, v9
	v_dual_mul_f32 v135, v135, v198 :: v_dual_add_nc_u32 v168, 0x80, v9
	v_dual_mul_f32 v140, v140, v198 :: v_dual_add_nc_u32 v169, 0x84, v9
	v_dual_mul_f32 v137, v137, v198 :: v_dual_add_nc_u32 v170, 0x88, v9
	v_dual_mul_f32 v142, v142, v198 :: v_dual_add_nc_u32 v171, 0x8c, v9
	v_dual_mul_f32 v139, v139, v198 :: v_dual_add_nc_u32 v172, 0x90, v9
	v_dual_mul_f32 v144, v144, v198 :: v_dual_add_nc_u32 v173, 0x94, v9
	v_dual_mul_f32 v141, v141, v198 :: v_dual_add_nc_u32 v174, 0x98, v9
	v_dual_mul_f32 v146, v146, v198 :: v_dual_add_nc_u32 v175, 0x9c, v9
	v_dual_mul_f32 v143, v143, v198 :: v_dual_add_nc_u32 v176, 0xa0, v9
	v_dual_mul_f32 v148, v148, v198 :: v_dual_add_nc_u32 v177, 0xa4, v9
	v_dual_mul_f32 v145, v145, v198 :: v_dual_add_nc_u32 v178, 0xa8, v9
	v_dual_mul_f32 v150, v150, v198 :: v_dual_add_nc_u32 v179, 0xac, v9
	v_dual_mul_f32 v147, v147, v198 :: v_dual_add_nc_u32 v180, 0xb0, v9
	v_dual_mul_f32 v152, v152, v198 :: v_dual_add_nc_u32 v181, 0xb4, v9
	v_dual_mul_f32 v149, v149, v198 :: v_dual_add_nc_u32 v182, 0xb8, v9
	v_dual_mul_f32 v154, v154, v198 :: v_dual_add_nc_u32 v183, 0xbc, v9
	v_dual_mul_f32 v151, v151, v198 :: v_dual_add_nc_u32 v184, 0xc0, v9
	v_dual_mul_f32 v156, v156, v198 :: v_dual_add_nc_u32 v185, 0xc4, v9
	v_dual_mul_f32 v153, v153, v198 :: v_dual_add_nc_u32 v186, 0xc8, v9
	v_dual_mul_f32 v158, v158, v198 :: v_dual_add_nc_u32 v187, 0xcc, v9
	v_dual_mul_f32 v155, v155, v198 :: v_dual_add_nc_u32 v188, 0xd0, v9
	v_dual_mul_f32 v160, v160, v198 :: v_dual_add_nc_u32 v189, 0xd4, v9
	v_dual_mul_f32 v157, v157, v198 :: v_dual_add_nc_u32 v190, 0xd8, v9
	v_dual_mul_f32 v162, v162, v198 :: v_dual_add_nc_u32 v191, 0xdc, v9
	v_dual_mul_f32 v1, v1, v198 :: v_dual_add_nc_u32 v196, 0xf0, v9
	v_dual_mul_f32 v6, v6, v198 :: v_dual_add_nc_u32 v197, 0xf4, v9
	v_dual_mul_f32 v8, v8, v198 :: v_dual_add_nc_u32 v9, 0xf8, v9
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_add_lshl_u32 v101, s3, v101, 1
	v_cndmask_b32_e64 v14, 0x80000000, v14, s2
	v_cndmask_b32_e64 v15, 0x80000000, v15, s2
	v_cndmask_b32_e64 v16, 0x80000000, v16, s2
	v_cndmask_b32_e64 v17, 0x80000000, v17, s2
	v_cndmask_b32_e64 v22, 0x80000000, v22, s2
	v_cndmask_b32_e64 v23, 0x80000000, v23, s2
	v_cndmask_b32_e64 v24, 0x80000000, v24, s2
	v_cndmask_b32_e64 v103, 0x80000000, v103, s2
	v_cndmask_b32_e64 v192, 0x80000000, v192, s2
	s_clause 0xf
	buffer_load_u16 v10, v10, s[4:7], 0 offen
	buffer_load_u16 v11, v11, s[4:7], 0 offen
	buffer_load_u16 v12, v12, s[4:7], 0 offen
	buffer_load_u16 v13, v13, s[4:7], 0 offen
	buffer_load_u16 v14, v14, s[4:7], 0 offen
	buffer_load_u16 v15, v15, s[4:7], 0 offen
	buffer_load_u16 v16, v16, s[4:7], 0 offen
	buffer_load_u16 v17, v17, s[4:7], 0 offen
	buffer_load_u16 v18, v18, s[4:7], 0 offen
	buffer_load_u16 v19, v19, s[4:7], 0 offen
	buffer_load_u16 v20, v20, s[4:7], 0 offen
	buffer_load_u16 v21, v21, s[4:7], 0 offen
	buffer_load_u16 v22, v22, s[4:7], 0 offen
	buffer_load_u16 v23, v23, s[4:7], 0 offen
	buffer_load_u16 v24, v24, s[4:7], 0 offen
	buffer_load_u16 v103, v103, s[4:7], 0 offen
	v_cndmask_b32_e64 v104, 0x80000000, v104, s2
	v_cndmask_b32_e64 v193, 0x80000000, v193, s2
	v_cndmask_b32_e64 v105, 0x80000000, v105, s2
	v_cndmask_b32_e64 v194, 0x80000000, v194, s2
	v_cndmask_b32_e64 v106, 0x80000000, v106, s2
	v_cndmask_b32_e64 v195, 0x80000000, v195, s2
	v_cndmask_b32_e64 v107, 0x80000000, v107, s2
	v_cndmask_b32_e64 v196, 0x80000000, v196, s2
	v_cndmask_b32_e64 v197, 0x80000000, v197, s2
	v_cndmask_b32_e64 v9, 0x80000000, v9, s2
	v_cndmask_b32_e64 v101, 0x80000000, v101, s2
	v_cndmask_b32_e64 v108, 0x80000000, v108, s2
	v_cndmask_b32_e64 v109, 0x80000000, v109, s2
	v_cndmask_b32_e64 v110, 0x80000000, v110, s2
	v_cndmask_b32_e64 v111, 0x80000000, v111, s2
	v_add_lshl_u32 v102, s3, v102, 1
	s_clause 0xf
	buffer_load_u16 v192, v192, s[4:7], 0 offen
	buffer_load_u16 v193, v193, s[4:7], 0 offen
	buffer_load_u16 v194, v194, s[4:7], 0 offen
	buffer_load_u16 v195, v195, s[4:7], 0 offen
	buffer_load_u16 v196, v196, s[4:7], 0 offen
	buffer_load_u16 v197, v197, s[4:7], 0 offen
	buffer_load_u16 v9, v9, s[4:7], 0 offen
	buffer_load_u16 v101, v101, s[4:7], 0 offen
	buffer_load_u16 v104, v104, s[4:7], 0 offen
	buffer_load_u16 v105, v105, s[4:7], 0 offen
	buffer_load_u16 v106, v106, s[4:7], 0 offen
	buffer_load_u16 v107, v107, s[4:7], 0 offen
	buffer_load_u16 v108, v108, s[4:7], 0 offen
	buffer_load_u16 v109, v109, s[4:7], 0 offen
	buffer_load_u16 v110, v110, s[4:7], 0 offen
	buffer_load_u16 v111, v111, s[4:7], 0 offen
	v_cndmask_b32_e64 v112, 0x80000000, v112, s2
	v_cndmask_b32_e64 v168, 0x80000000, v168, s2
	v_cndmask_b32_e64 v184, 0x80000000, v184, s2
	v_cndmask_b32_e64 v113, 0x80000000, v113, s2
	v_cndmask_b32_e64 v169, 0x80000000, v169, s2
	v_cndmask_b32_e64 v185, 0x80000000, v185, s2
	v_cndmask_b32_e64 v163, 0x80000000, v163, s2
	v_cndmask_b32_e64 v170, 0x80000000, v170, s2
	v_cndmask_b32_e64 v186, 0x80000000, v186, s2
	v_cndmask_b32_e64 v164, 0x80000000, v164, s2
	v_cndmask_b32_e64 v171, 0x80000000, v171, s2
	v_cndmask_b32_e64 v187, 0x80000000, v187, s2
	v_cndmask_b32_e64 v165, 0x80000000, v165, s2
	v_cndmask_b32_e64 v166, 0x80000000, v166, s2
	v_cndmask_b32_e64 v167, 0x80000000, v167, s2
	v_cndmask_b32_e64 v102, 0x80000000, v102, s2
	v_cndmask_b32_e64 v172, 0x80000000, v172, s2
	v_cndmask_b32_e64 v173, 0x80000000, v173, s2
	v_cndmask_b32_e64 v174, 0x80000000, v174, s2
	v_cndmask_b32_e64 v175, 0x80000000, v175, s2
	v_cndmask_b32_e64 v188, 0x80000000, v188, s2
	v_cndmask_b32_e64 v189, 0x80000000, v189, s2
	v_cndmask_b32_e64 v190, 0x80000000, v190, s2
	v_cndmask_b32_e64 v191, 0x80000000, v191, s2
	s_clause 0x17
	buffer_load_u16 v112, v112, s[4:7], 0 offen
	buffer_load_u16 v113, v113, s[4:7], 0 offen
	buffer_load_u16 v163, v163, s[4:7], 0 offen
	buffer_load_u16 v164, v164, s[4:7], 0 offen
	buffer_load_u16 v165, v165, s[4:7], 0 offen
	buffer_load_u16 v166, v166, s[4:7], 0 offen
	buffer_load_u16 v167, v167, s[4:7], 0 offen
	buffer_load_u16 v102, v102, s[4:7], 0 offen
	buffer_load_u16 v168, v168, s[4:7], 0 offen
	buffer_load_u16 v169, v169, s[4:7], 0 offen
	buffer_load_u16 v170, v170, s[4:7], 0 offen
	buffer_load_u16 v171, v171, s[4:7], 0 offen
	buffer_load_u16 v172, v172, s[4:7], 0 offen
	buffer_load_u16 v173, v173, s[4:7], 0 offen
	buffer_load_u16 v174, v174, s[4:7], 0 offen
	buffer_load_u16 v175, v175, s[4:7], 0 offen
	buffer_load_u16 v184, v184, s[4:7], 0 offen
	buffer_load_u16 v185, v185, s[4:7], 0 offen
	buffer_load_u16 v186, v186, s[4:7], 0 offen
	buffer_load_u16 v187, v187, s[4:7], 0 offen
	buffer_load_u16 v188, v188, s[4:7], 0 offen
	buffer_load_u16 v189, v189, s[4:7], 0 offen
	buffer_load_u16 v190, v190, s[4:7], 0 offen
	buffer_load_u16 v191, v191, s[4:7], 0 offen
	v_cndmask_b32_e64 v176, 0x80000000, v176, s2
	v_cndmask_b32_e64 v177, 0x80000000, v177, s2
	v_cndmask_b32_e64 v178, 0x80000000, v178, s2
	v_cndmask_b32_e64 v179, 0x80000000, v179, s2
	v_cndmask_b32_e64 v180, 0x80000000, v180, s2
	v_cndmask_b32_e64 v181, 0x80000000, v181, s2
	v_cndmask_b32_e64 v182, 0x80000000, v182, s2
	v_cndmask_b32_e64 v183, 0x80000000, v183, s2
	s_clause 0x7
	buffer_load_u16 v176, v176, s[4:7], 0 offen
	buffer_load_u16 v177, v177, s[4:7], 0 offen
	buffer_load_u16 v178, v178, s[4:7], 0 offen
	buffer_load_u16 v179, v179, s[4:7], 0 offen
	buffer_load_u16 v180, v180, s[4:7], 0 offen
	buffer_load_u16 v181, v181, s[4:7], 0 offen
	buffer_load_u16 v182, v182, s[4:7], 0 offen
	buffer_load_u16 v183, v183, s[4:7], 0 offen
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_mul_f32_e32 v5, v5, v198
	v_mul_f32_e32 v7, v7, v198
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_lshlrev_b32_e32 v34, 2, v34
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 206 20                        ; generate_amdgcn.py:206:20
	v_dual_mul_f32 v3, v3, v198 :: v_dual_and_b32 v26, 0x180, v0
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_lshl_or_b32 v34, v36, 9, v34
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v199, 7, v0
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_lshl_b32 s4, s35, 2
	s_lshl_b32 s5, s35, 3
	s_mul_i32 s8, s35, 12
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_add_nc_u32_e32 v36, 0, v34
	.loc	1 218 9                         ; generate_amdgcn.py:218:9
	s_and_b32 s1, s1, 0xffff
	s_mov_b32 s3, s7
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v10, 16, v10
	s_waitcnt vmcnt(62)
	v_lshlrev_b32_e32 v11, 16, v11
	s_waitcnt vmcnt(61)
	v_lshlrev_b32_e32 v12, 16, v12
	s_waitcnt vmcnt(60)
	v_lshlrev_b32_e32 v13, 16, v13
	s_waitcnt vmcnt(59)
	v_lshlrev_b32_e32 v14, 16, v14
	s_waitcnt vmcnt(58)
	v_lshlrev_b32_e32 v15, 16, v15
	s_waitcnt vmcnt(57)
	v_lshlrev_b32_e32 v16, 16, v16
	s_waitcnt vmcnt(56)
	v_lshlrev_b32_e32 v17, 16, v17
	s_waitcnt vmcnt(55)
	v_lshlrev_b32_e32 v18, 16, v18
	s_waitcnt vmcnt(54)
	v_lshlrev_b32_e32 v19, 16, v19
	s_waitcnt vmcnt(53)
	v_lshlrev_b32_e32 v20, 16, v20
	s_waitcnt vmcnt(52)
	v_lshlrev_b32_e32 v21, 16, v21
	s_waitcnt vmcnt(51)
	v_lshlrev_b32_e32 v22, 16, v22
	s_waitcnt vmcnt(50)
	v_lshlrev_b32_e32 v23, 16, v23
	s_waitcnt vmcnt(49)
	v_lshlrev_b32_e32 v24, 16, v24
	s_waitcnt vmcnt(48)
	v_lshlrev_b32_e32 v103, 16, v103
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v10, v27, v10, v52
	v_fma_f32 v11, v28, v11, v100
	v_fma_f32 v12, v29, v12, v99
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(39)
	v_lshlrev_b32_e32 v104, 16, v104
	s_waitcnt vmcnt(38)
	v_lshlrev_b32_e32 v105, 16, v105
	s_waitcnt vmcnt(37)
	v_lshlrev_b32_e32 v106, 16, v106
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v13, v30, v13, v98
	v_fma_f32 v14, v31, v14, v97
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(34)
	v_lshlrev_b32_e32 v109, 16, v109
	s_waitcnt vmcnt(33)
	v_lshlrev_b32_e32 v110, 16, v110
	v_lshlrev_b32_e32 v101, 16, v101
	v_lshlrev_b32_e32 v9, 16, v9
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v15, v32, v15, v96
	v_fma_f32 v16, v33, v16, v95
	v_fma_f32 v17, v114, v17, v94
	v_fma_f32 v18, v115, v18, v93
	v_fma_f32 v19, v116, v19, v92
	v_fma_f32 v20, v117, v20, v91
	v_fma_f32 v21, v118, v21, v90
	v_fma_f32 v22, v119, v22, v89
	v_fma_f32 v23, v120, v23, v88
	v_fma_f32 v24, v121, v24, v87
	v_fma_f32 v27, v122, v103, v86
	v_fma_f32 v28, v123, v104, v85
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(32)
	v_lshlrev_b32_e32 v111, 16, v111
	v_lshlrev_b32_e32 v107, 16, v107
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v29, v124, v105, v84
	v_fma_f32 v33, v128, v109, v80
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(31)
	v_lshlrev_b32_e32 v112, 16, v112
	s_waitcnt vmcnt(23)
	v_lshlrev_b32_e32 v168, 16, v168
	s_waitcnt vmcnt(15)
	v_lshlrev_b32_e32 v184, 16, v184
	v_lshlrev_b32_e32 v113, 16, v113
	v_lshlrev_b32_e32 v169, 16, v169
	s_waitcnt vmcnt(14)
	v_lshlrev_b32_e32 v185, 16, v185
	v_lshlrev_b32_e32 v170, 16, v170
	s_waitcnt vmcnt(13)
	v_lshlrev_b32_e32 v186, 16, v186
	v_lshlrev_b32_e32 v171, 16, v171
	s_waitcnt vmcnt(12)
	v_lshlrev_b32_e32 v187, 16, v187
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v105, v131, v112, v77
	v_fma_f32 v112, v139, v168, v69
	v_fma_f32 v128, v155, v184, v53
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v108, 16, v108
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v30, v125, v106, v83
	v_fma_f32 v103, v129, v110, v79
	v_fma_f32 v106, v132, v113, v76
	v_fma_f32 v113, v140, v169, v68
	v_fma_f32 v129, v156, v185, v51
	v_fma_f32 v8, v8, v101, v37
	v_fma_f32 v7, v7, v9, v38
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v9, v52, v10, s2
	v_cndmask_b32_e64 v10, v100, v11, s2
	v_cndmask_b32_e64 v11, v99, v12, s2
	v_cndmask_b32_e64 v12, v98, v13, s2
	v_cndmask_b32_e64 v13, v97, v14, s2
	v_cndmask_b32_e64 v14, v96, v15, s2
	v_cndmask_b32_e64 v15, v95, v16, s2
	v_cndmask_b32_e64 v16, v94, v17, s2
	v_cndmask_b32_e64 v17, v93, v18, s2
	v_cndmask_b32_e64 v18, v92, v19, s2
	v_cndmask_b32_e64 v19, v91, v20, s2
	v_cndmask_b32_e64 v20, v90, v21, s2
	v_cndmask_b32_e64 v21, v89, v22, s2
	v_cndmask_b32_e64 v22, v88, v23, s2
	v_cndmask_b32_e64 v23, v87, v24, s2
	v_cndmask_b32_e64 v24, v86, v27, s2
	v_cndmask_b32_e64 v27, v85, v28, s2
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v172, 16, v172
	s_waitcnt vmcnt(11)
	v_lshlrev_b32_e32 v188, 16, v188
	v_lshlrev_b32_e32 v163, 16, v163
	v_lshlrev_b32_e32 v173, 16, v173
	s_waitcnt vmcnt(10)
	v_lshlrev_b32_e32 v189, 16, v189
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v104, v130, v111, v78
	v_fma_f32 v114, v141, v170, v67
	v_fma_f32 v130, v157, v186, v50
	v_fma_f32 v31, v126, v107, v82
	v_fma_f32 v115, v142, v171, v66
	v_fma_f32 v131, v158, v187, v49
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v69, v69, v112, s2
	v_cndmask_b32_e64 v53, v53, v128, s2
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v32, v127, v108, v81
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v28, v84, v29, s2
	v_cndmask_b32_e64 v68, v68, v113, s2
	v_cndmask_b32_e64 v51, v51, v129, s2
	v_cndmask_b32_e64 v8, v37, v8, s2
	v_xad_u32 v37, v34, 4, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v164, 16, v164
	v_lshlrev_b32_e32 v174, 16, v174
	s_waitcnt vmcnt(9)
	v_lshlrev_b32_e32 v190, 16, v190
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v29, v83, v30, s2
	ds_store_2addr_b32 v36, v9, v27 offset1:32
	v_xad_u32 v9, v34, 8, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v165, 16, v165
	v_lshlrev_b32_e32 v175, 16, v175
	s_waitcnt vmcnt(8)
	v_lshlrev_b32_e32 v191, 16, v191
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v116, v143, v172, v65
	v_fma_f32 v132, v159, v188, v48
	v_fma_f32 v107, v133, v163, v75
	v_fma_f32 v117, v144, v173, v64
	v_fma_f32 v133, v160, v189, v47
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v67, v67, v114, s2
	v_cndmask_b32_e64 v50, v50, v130, s2
	v_cndmask_b32_e64 v30, v82, v31, s2
	v_cndmask_b32_e64 v66, v66, v115, s2
	v_cndmask_b32_e64 v49, v49, v131, s2
	ds_store_2addr_b32 v36, v69, v53 offset0:64 offset1:96
	ds_store_2addr_b32 v37, v10, v28 offset1:32
	v_xad_u32 v10, v34, 12, 0
	v_cndmask_b32_e64 v31, v81, v32, s2
	ds_store_2addr_b32 v37, v68, v51 offset0:64 offset1:96
	ds_store_2addr_b32 v9, v11, v29 offset1:32
	v_xad_u32 v11, v34, 16, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(7)
	v_lshlrev_b32_e32 v176, 16, v176
	s_waitcnt vmcnt(6)
	v_lshlrev_b32_e32 v177, 16, v177
	v_lshlrev_b32_e32 v192, 16, v192
	v_lshlrev_b32_e32 v193, 16, v193
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v108, v134, v164, v74
	v_fma_f32 v118, v145, v174, v63
	v_fma_f32 v134, v161, v190, v46
	v_fma_f32 v109, v135, v165, v73
	v_fma_f32 v119, v146, v175, v62
	v_fma_f32 v135, v162, v191, v45
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v65, v65, v116, s2
	v_cndmask_b32_e64 v48, v48, v132, s2
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(5)
	v_lshlrev_b32_e32 v178, 16, v178
	v_lshlrev_b32_e32 v194, 16, v194
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v32, v80, v33, s2
	v_cndmask_b32_e64 v64, v64, v117, s2
	v_cndmask_b32_e64 v47, v47, v133, s2
	ds_store_2addr_b32 v9, v67, v50 offset0:64 offset1:96
	ds_store_2addr_b32 v10, v12, v30 offset1:32
	v_xad_u32 v9, v34, 20, 0
	v_cndmask_b32_e64 v33, v79, v103, s2
	ds_store_2addr_b32 v10, v66, v49 offset0:64 offset1:96
	ds_store_2addr_b32 v11, v13, v31 offset1:32
	v_xad_u32 v10, v34, 24, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(4)
	v_lshlrev_b32_e32 v179, 16, v179
	v_lshlrev_b32_e32 v195, 16, v195
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v120, v147, v176, v61
	v_fma_f32 v121, v148, v177, v60
	v_fma_f32 v1, v1, v192, v44
	v_fma_f32 v2, v2, v193, v43
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v63, v63, v118, s2
	v_cndmask_b32_e64 v46, v46, v134, s2
	v_cndmask_b32_e64 v52, v78, v104, s2
	v_cndmask_b32_e64 v62, v62, v119, s2
	v_cndmask_b32_e64 v45, v45, v135, s2
	ds_store_2addr_b32 v11, v65, v48 offset0:64 offset1:96
	ds_store_2addr_b32 v9, v14, v32 offset1:32
	v_xad_u32 v11, v34, 28, 0
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v122, v149, v178, v59
	v_fma_f32 v3, v3, v194, v42
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v77, v77, v105, s2
	ds_store_2addr_b32 v9, v64, v47 offset0:64 offset1:96
	ds_store_2addr_b32 v10, v15, v33 offset1:32
	v_xad_u32 v9, v34, 32, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(3)
	v_lshlrev_b32_e32 v180, 16, v180
	v_lshlrev_b32_e32 v196, 16, v196
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v123, v150, v179, v58
	v_fma_f32 v4, v4, v195, v41
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v61, v61, v120, s2
	v_cndmask_b32_e64 v60, v60, v121, s2
	v_cndmask_b32_e64 v1, v44, v1, s2
	v_cndmask_b32_e64 v2, v43, v2, s2
	ds_store_2addr_b32 v10, v63, v46 offset0:64 offset1:96
	ds_store_2addr_b32 v11, v16, v52 offset1:32
	v_xad_u32 v10, v34, 36, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v166, 16, v166
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v76, v76, v106, s2
	v_cndmask_b32_e64 v75, v75, v107, s2
	ds_store_2addr_b32 v11, v62, v45 offset0:64 offset1:96
	ds_store_2addr_b32 v9, v17, v77 offset1:32
	v_xad_u32 v11, v34, 40, 0
	v_cndmask_b32_e64 v59, v59, v122, s2
	v_cndmask_b32_e64 v3, v42, v3, s2
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v181, 16, v181
	v_lshlrev_b32_e32 v197, 16, v197
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v124, v151, v180, v57
	v_fma_f32 v5, v5, v196, v40
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v58, v58, v123, s2
	v_cndmask_b32_e64 v4, v41, v4, s2
	ds_store_2addr_b32 v9, v61, v1 offset0:64 offset1:96
	ds_store_2addr_b32 v10, v18, v76 offset1:32
	v_xad_u32 v1, v34, 44, 0
	ds_store_2addr_b32 v10, v60, v2 offset0:64 offset1:96
	ds_store_2addr_b32 v11, v19, v75 offset1:32
	ds_store_2addr_b32 v11, v59, v3 offset0:64 offset1:96
	v_and_b32_e32 v3, 60, v35
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v167, 16, v167
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v110, v136, v166, v72
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v74, v74, v108, s2
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v182, 16, v182
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v125, v152, v181, v56
	v_fma_f32 v6, v6, v197, v39
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v73, v73, v109, s2
	v_cndmask_b32_e64 v57, v57, v124, s2
	v_cndmask_b32_e64 v5, v40, v5, s2
	v_xad_u32 v2, v34, 48, 0
	ds_store_2addr_b32 v1, v58, v4 offset0:64 offset1:96
	v_lshrrev_b32_e32 v4, 5, v26
	v_lshl_or_b32 v3, v25, 2, v3
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v111, v137, v167, v71
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v72, v72, v110, s2
	ds_store_2addr_b32 v1, v20, v74 offset1:32
	v_xad_u32 v1, v34, 52, 0
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	v_lshlrev_b32_e32 v102, 16, v102
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v126, v153, v182, v55
	.loc	1 205 18                        ; generate_amdgcn.py:205:18
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v183, 16, v183
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v56, v56, v125, s2
	v_cndmask_b32_e64 v6, v39, v6, s2
	ds_store_2addr_b32 v2, v21, v73 offset1:32
	ds_store_2addr_b32 v2, v57, v5 offset0:64 offset1:96
	ds_store_2addr_b32 v1, v22, v72 offset1:32
	v_lshlrev_b32_e32 v5, 6, v0
	v_xor_b32_e32 v3, v3, v4
	v_cndmask_b32_e64 v71, v71, v111, s2
	v_xad_u32 v2, v34, 56, 0
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v102, v138, v102, v70
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	v_cndmask_b32_e64 v55, v55, v126, s2
	v_cndmask_b32_e64 v7, v38, v7, s2
	.loc	1 206 13                        ; generate_amdgcn.py:206:13
	v_fma_f32 v127, v154, v183, v54
	.loc	1 161 9                         ; generate_amdgcn.py:161:9
	ds_store_2addr_b32 v1, v56, v6 offset0:64 offset1:96
	ds_store_2addr_b32 v2, v23, v71 offset1:32
	ds_store_2addr_b32 v2, v55, v7 offset0:64 offset1:96
	v_and_or_b32 v1, v5, 64, v3
	v_cndmask_b32_e64 v70, v70, v102, s2
	v_xad_u32 v4, v34, 60, 0
	v_cndmask_b32_e64 v54, v54, v127, s2
	ds_store_2addr_b32 v4, v24, v70 offset1:32
	ds_store_2addr_b32 v4, v54, v8 offset0:64 offset1:96
	v_add_nc_u32_e32 v2, 0, v1
	v_xad_u32 v3, 0x810, v1, 0
	v_xad_u32 v4, 0x1020, v1, 0
	v_xad_u32 v1, 0x1830, v1, 0
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b32 v2, v2
	ds_load_b32 v3, v3
	ds_load_b32 v4, v4
	ds_load_b32 v1, v1
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	v_mul_lo_u32 v5, s35, v199
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_and_b32_e32 v0, 0x7f, v0
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_mul_i32 s2, s34, s35
	s_delay_alu instid0(SALU_CYCLE_1)
	s_add_i32 s2, s2, s33
	s_delay_alu instid0(VALU_DEP_1) | instid1(SALU_CYCLE_1)
	v_add3_u32 v0, s2, v0, v5
	.loc	1 218 9 is_stmt 0               ; generate_amdgcn.py:218:9
	s_mov_b32 s2, s6
	s_delay_alu instid0(VALU_DEP_1)
	v_lshlrev_b32_e32 v5, 2, v0
	v_add_lshl_u32 v6, v0, s4, 2
	v_add_lshl_u32 v7, v0, s5, 2
	v_add_lshl_u32 v0, v0, s8, 2
	s_waitcnt lgkmcnt(3)
	buffer_atomic_add_f32 v2, v5, s[0:3], 0 offen
	s_waitcnt lgkmcnt(2)
	buffer_atomic_add_f32 v3, v6, s[0:3], 0 offen
	s_waitcnt lgkmcnt(1)
	buffer_atomic_add_f32 v4, v7, s[0:3], 0 offen
	s_waitcnt lgkmcnt(0)
	buffer_atomic_add_f32 v1, v0, s[0:3], 0 offen
	.loc	1 80 1 is_stmt 1                ; generate_amdgcn.py:80:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp22:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 249
		.amdhsa_next_free_sgpr 48
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 63
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk, .Lfunc_end0-amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.num_vgpr, 249
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.numbered_sgpr, 48
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 28768
; TotalNumSgprs: 50
; NumVgprs: 249
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 31
; NumSGPRsForWavesPerEU: 50
; NumVGPRsForWavesPerEU: 249
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	11                              ; DW_FORM_data1
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x5c DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x36 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	88                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4d:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	87                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x59:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	154                             ; DW_AT_call_line
	.byte	22                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	.Ltmp16-.Lfunc_begin0
	.quad	.Ltmp17-.Lfunc_begin0
	.quad	.Ltmp18-.Lfunc_begin0
	.quad	.Ltmp19-.Lfunc_begin0
	.quad	.Ltmp20-.Lfunc_begin0
	.quad	.Ltmp21-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk"             ; string offset=75 ; amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 512
    .name:           amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk
    .private_segment_fixed_size: 0
    .sgpr_count:     50
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int4xint4_nn_sc256_none_bm16_bn128_bk128_gm1_w16_s2_weu2_sk8_maskk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     249
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
