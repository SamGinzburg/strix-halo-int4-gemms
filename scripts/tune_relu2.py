...
def default_tiles(dtype: OperandDType) -> tuple[TileConfig, ...]:
    ...
    seeded = seed_tile_configs(seed_dtype, epilogue=Epilogue.RELU2)[0]
...
def make_kernel(dtype: OperandDType, scale: ScaleSpec, tile: TileConfig, shape: BenchmarkShape) -> KernelMetadata:
    ...
    kernel_id = (
        make_mixed_kernel_id(dtype, b_dtype, scale, Epilogue.RELU2, even_tile)
        if dtype is OperandDType.BF16
        else make_kernel_id(dtype, scale, Epilogue.RELU2, even_tile)
    )
