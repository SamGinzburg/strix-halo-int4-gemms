	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk               ; -- Begin function amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk,@function
amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk:                      ; @amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 309 0                         ; generate_amdgcn.py:309:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[22:23], s[0:1], 0x30
	s_load_b32 s12, s[0:1], 0x38
.Ltmp0:
	.loc	1 319 16 prologue_end           ; generate_amdgcn.py:319:16
	s_abs_i32 s6, s2
	.loc	1 325 32                        ; generate_amdgcn.py:325:32
	v_and_b32_e32 v17, 15, v0
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_lshlrev_b32_e32 v44, 4, v0
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 327 14                        ; generate_amdgcn.py:327:14
	v_lshlrev_b32_e32 v2, 2, v17
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:317:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s23, 0xff
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s7, s22, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s9, s7, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_lshr_b32 s5, s5, 24
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_lshr_b32 s9, s9, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s7, s7, s9
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s4, s4, 8
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s7, s7, 4
.Ltmp11:
	.loc	1 318 24 is_stmt 1              ; generate_amdgcn.py:318:24
	s_lshl_b32 s13, s4, 2
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	.loc	1 319 16                        ; generate_amdgcn.py:319:16
	s_abs_i32 s4, s13
	s_cvt_f32_u32 s5, s4
	s_sub_i32 s8, 0, s4
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s5
	v_readfirstlane_b32 s5, v1
	s_mul_f32 s5, s5, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s5, s5
	s_mul_i32 s8, s8, s5
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s8, s5, s8
	s_add_i32 s5, s5, s8
	s_xor_b32 s8, s2, s13
	s_mul_hi_u32 s5, s6, s5
	s_ashr_i32 s8, s8, 31
	s_mul_i32 s9, s5, s4
	s_delay_alu instid0(SALU_CYCLE_1)
	s_sub_i32 s6, s6, s9
	s_add_i32 s9, s5, 1
	s_sub_i32 s10, s6, s4
	s_cmp_ge_u32 s6, s4
	s_cselect_b32 s5, s9, s5
	s_cselect_b32 s6, s10, s6
	s_add_i32 s9, s5, 1
	s_cmp_ge_u32 s6, s4
	s_cselect_b32 s4, s9, s5
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s4, s4, s8
	s_sub_i32 s14, s4, s8
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 320 19                        ; generate_amdgcn.py:320:19
	s_lshl_b32 s15, s14, 2
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_mul_i32 s14, s14, s13
	.loc	1 321 31                        ; generate_amdgcn.py:321:31
	s_sub_i32 s4, s7, s15
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_sub_i32 s2, s2, s14
	.loc	1 321 20                        ; generate_amdgcn.py:321:20
	s_min_i32 s16, s4, 4
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_abs_i32 s14, s2
	s_abs_i32 s17, s16
	s_load_b256 s[4:11], s[0:1], 0x0
	s_cvt_f32_u32 s18, s17
	s_sub_i32 s19, 0, s17
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s18
	v_readfirstlane_b32 s18, v1
	.loc	1 325 32                        ; generate_amdgcn.py:325:32
	v_lshrrev_b32_e32 v1, 4, v0
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[28:29], s[6:7]
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_mul_f32 s18, s18, 0x4f7ffffe
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_mul_lo_u32 v3, s12, v1
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_mul_lo_u32 v1, s23, v1
	s_mov_b32 s7, 0x31027000
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_cvt_u32_f32 s18, s18
	s_mov_b32 s6, 0x7ffffffe
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_mov_b32 s31, s7
	s_mov_b32 s30, s6
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_mul_i32 s19, s19, s18
	s_mov_b64 s[24:25], s[10:11]
	s_mul_hi_u32 s13, s18, s19
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_lshl_add_u32 v9, v17, 4, v1
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_add_i32 s18, s18, s13
	s_delay_alu instid0(SALU_CYCLE_1)
	s_mul_hi_u32 s13, s14, s18
	s_xor_b32 s18, s2, s16
	s_mul_i32 s19, s13, s17
	s_ashr_i32 s18, s18, 31
	s_sub_i32 s14, s14, s19
	s_add_i32 s19, s13, 1
	s_sub_i32 s20, s14, s17
	s_cmp_ge_u32 s14, s17
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_mad_u64_u32 v[10:11], null, s23, 48, v[9:10]
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_cselect_b32 s13, s19, s13
	s_cselect_b32 s14, s20, s14
	s_add_i32 s19, s13, 1
	s_cmp_ge_u32 s14, s17
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_lshl_add_u32 v15, s23, 4, v9
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_cselect_b32 s13, s19, s13
	.loc	1 368 22                        ; generate_amdgcn.py:368:22
	s_lshl_b32 s26, s3, 6
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_xor_b32 s13, s13, s18
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:387:30 ]
	s_add_i32 s3, s12, 0xff
.Ltmp13:
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_sub_i32 s14, s13, s18
.Ltmp14:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:333:28 ]
	s_add_i32 s13, s12, 63
.Ltmp15:
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_mul_i32 s16, s14, s16
	.loc	1 326 14                        ; generate_amdgcn.py:326:14
	s_lshl_b32 s33, s14, 8
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_sub_i32 s2, s2, s16
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_lshl_add_u32 v16, s23, 5, v9
	.loc	1 322 13                        ; generate_amdgcn.py:322:13
	s_add_i32 s2, s2, s15
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_mul_i32 s15, s23, s26
	.loc	1 325 14                        ; generate_amdgcn.py:325:14
	s_lshl_b32 s22, s2, 4
.Ltmp16:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:387:30 ]
	s_ashr_i32 s2, s3, 31
.Ltmp17:
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	s_mul_i32 s12, s22, s12
.Ltmp18:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:387:30 ]
	s_lshr_b32 s2, s2, 24
.Ltmp19:
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add3_u32 v41, v3, v2, s12
.Ltmp20:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:387:30 ]
	s_add_i32 s14, s3, s2
.Ltmp21:
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_gt_i32 s13, 63
	s_cselect_b32 s3, -1, 0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_and_b32 s5, s5, 0xffff
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_add_i32 s12, s33, s15
	.loc	1 378 22 is_stmt 0              ; generate_amdgcn.py:378:22
	s_and_b32 s29, s29, 0xffff
	.loc	1 374 30 is_stmt 1              ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v1, s26, v41
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmpk_gt_i32 s13, 0x7f
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v2, s12, v9
	v_add_nc_u32_e32 v3, s12, v15
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s2, -1, 0
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_lshl_b32 s15, s23, 6
	v_add_nc_u32_e32 v4, s12, v16
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s34, s26, 64
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v5, s12, v10
	s_add_i32 s12, s12, s15
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_cndmask_b32_e64 v1, 0x80000000, v1, s3
	.loc	1 374 30 is_stmt 0              ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v6, s34, v41
	.loc	1 378 22 is_stmt 1              ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v2, 0x80000000, v2, s3
	.loc	1 378 30 is_stmt 0              ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v26, s12, v9
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v7, 0x80000000, v3, s3
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v27, s12, v15
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v8, 0x80000000, v4, s3
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v28, s12, v16
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v22, 0x80000000, v5, s3
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v29, s12, v10
	.loc	1 374 22 is_stmt 1              ; generate_amdgcn.py:374:22
	buffer_load_b32 v38, v1, s[4:7], 0 offen
	v_cndmask_b32_e64 v1, 0x80000000, v6, s2
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_clause 0x3
	buffer_load_b128 v[3:6], v2, s[28:31], 0 offen
	buffer_load_b128 v[11:14], v7, s[28:31], 0 offen
	buffer_load_b128 v[18:21], v8, s[28:31], 0 offen
	buffer_load_b128 v[22:25], v22, s[28:31], 0 offen
	v_cndmask_b32_e64 v2, 0x80000000, v26, s2
	v_cndmask_b32_e64 v7, 0x80000000, v27, s2
	v_cndmask_b32_e64 v8, 0x80000000, v28, s2
	v_cndmask_b32_e64 v39, 0x80000000, v29, s2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v40, v1, s[4:7], 0 offen
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_clause 0x3
	buffer_load_b128 v[26:29], v2, s[28:31], 0 offen
	buffer_load_b128 v[30:33], v7, s[28:31], 0 offen
	buffer_load_b128 v[34:37], v8, s[28:31], 0 offen
	buffer_load_b128 v[45:48], v39, s[28:31], 0 offen
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_lshrrev_b32_e32 v1, 1, v0
	v_lshlrev_b32_e32 v2, 2, v0
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v8, 0, v44
	s_mov_b32 s12, 0
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmpk_gt_i32 s13, 0xbf
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_and_b32_e32 v7, 48, v1
	v_and_b32_e32 v42, 0x70, v1
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	v_xor_b32_e32 v43, v2, v7
	v_lshlrev_b32_e32 v2, 3, v0
	v_add_nc_u32_e32 v7, 0, v43
	s_waitcnt vmcnt(9)
	ds_store_b32 v7, v38 offset:32768
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(8)
	ds_store_b128 v8, v[3:6]
	s_waitcnt vmcnt(7)
	ds_store_b128 v8, v[11:14] offset:4096
	s_waitcnt vmcnt(6)
	ds_store_b128 v8, v[18:21] offset:8192
	s_waitcnt vmcnt(5)
	ds_store_b128 v8, v[22:25] offset:12288
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(4)
	ds_store_b32 v7, v40 offset:33792
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(3)
	ds_store_b128 v8, v[26:29] offset:16384
	s_waitcnt vmcnt(2)
	ds_store_b128 v8, v[30:33] offset:20480
	s_waitcnt vmcnt(1)
	ds_store_b128 v8, v[34:37] offset:24576
	s_waitcnt vmcnt(0)
	ds_store_b128 v8, v[45:48] offset:28672
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_2
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_and_b32_e32 v3, 48, v2
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_and_b32_e32 v1, 0x70, v1
	s_mov_b32 s10, 0
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_lshl_or_b32 v38, v17, 6, v3
	v_xor_b32_e32 v39, 16, v38
	v_xor_b32_e32 v40, 32, v38
	v_xor_b32_e32 v37, 48, v38
	s_branch .LBB0_3
.LBB0_2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	s_mov_b32 s10, -1
                                        ; implicit-def: $vgpr38
                                        ; implicit-def: $vgpr39
                                        ; implicit-def: $vgpr40
                                        ; implicit-def: $vgpr37
                                        ; implicit-def: $vgpr1
.LBB0_3:                                ; %Flow46
	s_load_b64 s[20:21], s[0:1], 0x20
	v_or_b32_e32 v3, s22, v17
	v_and_b32_e32 v34, 0xf0, v0
	s_ashr_i32 s0, s14, 8
	v_or_b32_e32 v35, s33, v0
	v_lshlrev_b32_e32 v13, 1, v0
	v_mul_lo_u32 v36, v3, s0
	v_lshlrev_b32_e32 v12, 2, v34
	v_lshlrev_b32_e32 v14, 5, v0
	v_lshlrev_b32_e32 v11, 1, v34
	s_and_not1_b32 vcc_lo, exec_lo, s10
	s_cbranch_vccnz .LBB0_7
; %bb.4:                                ; %.lr.ph
	v_dual_mov_b32 v18, 0 :: v_dual_and_b32 v1, 48, v2
.Ltmp22:
	.loc	2 43 12 is_stmt 1               ; standard.py:43:12 @[ generate_amdgcn.py:333:28 ]
	s_lshr_b32 s44, s13, 6
	s_mov_b32 s13, s12
	s_mov_b32 s14, s12
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v38, v17, 6, v1
	s_mov_b32 s15, s12
	s_mov_b32 s16, s12
	s_mov_b32 s17, s12
	s_mov_b32 s18, s12
	s_mov_b32 s19, s12
	v_and_b32_e32 v1, 32, v14
	v_and_b32_e32 v19, 28, v13
	v_xor_b32_e32 v39, 16, v38
	v_xor_b32_e32 v40, 32, v38
	v_xor_b32_e32 v37, 48, v38
	v_add3_u32 v20, 0, v12, v1
	v_dual_mov_b32 v1, s12 :: v_dual_mov_b32 v6, s17
	v_dual_mov_b32 v2, s13 :: v_dual_mov_b32 v3, s14
	v_mov_b32_e32 v8, s19
	v_dual_mov_b32 v4, s15 :: v_dual_mov_b32 v5, s16
	v_dual_mov_b32 v7, s18 :: v_dual_add_nc_u32 v46, 0, v11
	v_dual_mov_b32 v22, 0 :: v_dual_add_nc_u32 v45, v20, v19
	v_dual_mov_b32 v19, 0 :: v_dual_mov_b32 v24, 0
	v_dual_mov_b32 v20, 0 :: v_dual_mov_b32 v21, 0
	v_dual_mov_b32 v26, 0 :: v_dual_mov_b32 v23, 0
	v_dual_mov_b32 v28, 0 :: v_dual_mov_b32 v25, 0
	v_dual_mov_b32 v30, 0 :: v_dual_mov_b32 v27, 0
	v_dual_mov_b32 v32, 0 :: v_dual_mov_b32 v29, 0
	v_mov_b32_e32 v31, 0
	v_mov_b32_e32 v33, 0
	s_mov_b32 s39, 0x31027000
	s_mov_b32 s38, 0x7ffffffe
	s_add_i32 s11, s34, 64
	s_and_b32 s37, s9, 0xffff
	s_mov_b32 s36, s8
	s_and_b32 s41, s25, 0xffff
	s_mov_b32 s40, s24
	s_mov_b32 s27, 1
	s_add_i32 s10, 0, 0x8000
	s_add_i32 s1, 0, 0x8400
	s_add_i32 s35, 0, 0x4000
	s_add_i32 s44, s44, -3
	s_mov_b32 s30, s6
	s_mov_b32 s31, s7
	s_mov_b32 s42, s38
	s_mov_b32 s43, s39
.Ltmp23:
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	2 0 12 is_stmt 0                ; standard.py:0:12
	s_mov_b32 s14, s10
	s_mov_b32 s10, s1
	s_mov_b32 s1, s26
	.loc	1 374 22 is_stmt 1              ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v47, s14, v38
	v_add_nc_u32_e32 v48, s14, v39
	v_add_nc_u32_e32 v49, s14, v40
	v_add_nc_u32_e32 v50, s14, v37
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s14, s1, 31
	.loc	1 334 14                        ; generate_amdgcn.py:334:14
	s_lshl_b32 s16, s13, 6
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_lshr_b32 s14, s14, 24
	s_mov_b32 s15, s12
	s_add_i32 s1, s1, s14
	s_mov_b32 s26, s34
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s34, s11, s16
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s1, s1, 8
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add3_u32 v51, s15, v42, v17
	.loc	1 378 30 is_stmt 0              ; generate_amdgcn.py:378:30
	s_mul_i32 s15, s34, s23
	.loc	1 392 40 is_stmt 1              ; generate_amdgcn.py:392:40
	s_mul_i32 s14, s1, s23
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_add_i32 s15, s15, s33
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v79, v36, s1, 1
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v80, v35, s14, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s1, s0
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v52, s34, v41
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 vcc_lo, -1, 0
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v53, s15, v9
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_dual_cndmask_b32 v79, 0x80000000, v79 :: v_dual_cndmask_b32 v80, 0x80000000, v80
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v123, v52, s[4:7], 0 offen
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	buffer_load_u16 v124, v80, s[40:43], 0 offen
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	buffer_load_u16 v125, v79, s[36:39], 0 offen
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v54, s15, v15
	v_add_nc_u32_e32 v55, s15, v16
	v_add_nc_u32_e32 v56, s15, v10
	.loc	1 378 22 is_stmt 0              ; generate_amdgcn.py:378:22
	s_clause 0x3
	buffer_load_b128 v[79:82], v53, s[28:31], 0 offen
	buffer_load_b128 v[83:86], v54, s[28:31], 0 offen
	buffer_load_b128 v[87:90], v55, s[28:31], 0 offen
	buffer_load_b128 v[91:94], v56, s[28:31], 0 offen
	.loc	1 379 19 is_stmt 1              ; generate_amdgcn.py:379:19
	ds_load_u8 v95, v51 offset:2560
	ds_load_u8 v96, v51 offset:2944
	ds_load_u8 v97, v51 offset:2688
	ds_load_u8 v98, v51 offset:2432
	ds_load_u8 v99, v51 offset:2176
	ds_load_u8 v100, v51 offset:1280
	ds_load_u8 v101, v51 offset:1024
	ds_load_u8 v102, v51 offset:1792
	ds_load_u8 v103, v51 offset:1536
	ds_load_u8 v104, v51 offset:1920
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[63:66], v47
	ds_load_b128 v[67:70], v48
	ds_load_b128 v[71:74], v49
	ds_load_b128 v[75:78], v50
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v47, v51 offset:2304
	ds_load_u8 v48, v51 offset:2048
	ds_load_u8 v49, v51 offset:2816
	ds_load_u8 v50, v51 offset:3328
	ds_load_u8 v52, v51 offset:3072
	ds_load_u8 v57, v51 offset:3840
	ds_load_u8 v58, v51 offset:3584
	ds_load_u8 v59, v51 offset:3968
	ds_load_u8 v60, v51 offset:3712
	ds_load_u8 v61, v51 offset:3456
	ds_load_u8 v62, v51 offset:3200
	ds_load_u8 v53, v51 offset:1664
	ds_load_u8 v54, v51 offset:1408
	ds_load_u8 v55, v51 offset:1152
	ds_load_u8 v56, v51 offset:256
	ds_load_u8 v105, v51
	ds_load_u8 v106, v51 offset:768
	ds_load_u8 v107, v51 offset:512
	ds_load_u8 v108, v51 offset:896
	ds_load_u8 v109, v51 offset:640
	ds_load_u8 v110, v51 offset:384
	ds_load_u8 v111, v51 offset:128
	ds_load_u8 v112, v51 offset:7424
	ds_load_u8 v113, v51 offset:7168
	ds_load_u8 v114, v51 offset:7936
	ds_load_u8 v115, v51 offset:7680
	ds_load_u8 v116, v51 offset:8064
	ds_load_u8 v117, v51 offset:7808
	ds_load_u8 v118, v51 offset:7552
	ds_load_u8 v119, v51 offset:7296
	ds_load_u8 v120, v51 offset:6400
	ds_load_u8 v121, v51 offset:6144
	ds_load_u8 v122, v51 offset:6912
	ds_load_u8 v126, v51 offset:6656
	ds_load_u8 v127, v51 offset:7040
	ds_load_u8 v128, v51 offset:6784
	ds_load_u8 v129, v51 offset:6528
	ds_load_u8 v130, v51 offset:6272
	ds_load_u8 v131, v51 offset:5376
	ds_load_u8 v132, v51 offset:5120
	ds_load_u8 v133, v51 offset:5888
	ds_load_u8 v134, v51 offset:5632
	ds_load_u8 v135, v51 offset:6016
	ds_load_u8 v136, v51 offset:5760
	ds_load_u8 v137, v51 offset:5504
	ds_load_u8 v138, v51 offset:5248
	ds_load_u8 v139, v51 offset:4352
	ds_load_u8 v140, v51 offset:4096
	ds_load_u8 v141, v51 offset:4864
	ds_load_u8 v142, v51 offset:4608
	ds_load_u8 v143, v51 offset:4992
	ds_load_u8 v144, v51 offset:4736
	ds_load_u8 v145, v51 offset:4480
	ds_load_u8 v146, v51 offset:4224
	ds_load_u8 v147, v51 offset:11520
	ds_load_u8 v148, v51 offset:11264
	ds_load_u8 v149, v51 offset:12032
	ds_load_u8 v150, v51 offset:11776
	ds_load_u8 v151, v51 offset:12160
	ds_load_u8 v152, v51 offset:11904
	ds_load_u8 v153, v51 offset:11648
	ds_load_u8 v154, v51 offset:11392
	ds_load_u8 v155, v51 offset:10496
	ds_load_u8 v156, v51 offset:10240
	ds_load_u8 v157, v51 offset:11008
	ds_load_u8 v158, v51 offset:10752
	ds_load_u8 v159, v51 offset:11136
	ds_load_u8 v160, v51 offset:10880
	ds_load_u8 v161, v51 offset:10624
	ds_load_u8 v162, v51 offset:10368
	ds_load_u8 v163, v51 offset:9472
	ds_load_u8 v164, v51 offset:9216
	ds_load_u8 v165, v51 offset:9984
	ds_load_u8 v166, v51 offset:9728
	ds_load_u8 v167, v51 offset:10112
	ds_load_u8 v168, v51 offset:9856
	ds_load_u8 v169, v51 offset:9600
	ds_load_u8 v170, v51 offset:9344
	ds_load_u8 v171, v51 offset:8448
	ds_load_u8 v172, v51 offset:8192
	ds_load_u8 v173, v51 offset:8960
	ds_load_u8 v174, v51 offset:8704
	ds_load_u8 v175, v51 offset:9088
	ds_load_u8 v176, v51 offset:8832
	ds_load_u8 v177, v51 offset:8576
	ds_load_u8 v178, v51 offset:8320
	ds_load_u8 v179, v51 offset:15616
	ds_load_u8 v180, v51 offset:15360
	ds_load_u8 v181, v51 offset:16128
	ds_load_u8 v182, v51 offset:15872
	ds_load_u8 v183, v51 offset:16256
	ds_load_u8 v184, v51 offset:16000
	ds_load_u8 v185, v51 offset:15744
	ds_load_u8 v186, v51 offset:15488
	ds_load_u8 v187, v51 offset:14592
	ds_load_u8 v188, v51 offset:14336
	ds_load_u8 v189, v51 offset:15104
	ds_load_u8 v190, v51 offset:14848
	ds_load_u8 v191, v51 offset:15232
	ds_load_u8 v192, v51 offset:14976
	ds_load_u8 v193, v51 offset:14720
	ds_load_u8 v194, v51 offset:14464
	ds_load_u8 v195, v51 offset:13568
	ds_load_u8 v196, v51 offset:13312
	ds_load_u8 v197, v51 offset:14080
	ds_load_u8 v198, v51 offset:13824
	ds_load_u8 v199, v51 offset:14208
	ds_load_u8 v200, v51 offset:13952
	ds_load_u8 v201, v51 offset:13696
	ds_load_u8 v202, v51 offset:13440
	ds_load_u8 v203, v51 offset:12544
	ds_load_u8 v204, v51 offset:12288
	ds_load_u8 v205, v51 offset:13056
	ds_load_u8 v206, v51 offset:12800
	ds_load_u8 v207, v51 offset:13184
	ds_load_u8 v208, v51 offset:12928
	ds_load_u8 v209, v51 offset:12672
	ds_load_u8 v51, v51 offset:12416
	s_waitcnt lgkmcnt(62)
	v_perm_b32 v54, v55, v54, 0xc0c0004
	v_perm_b32 v111, v111, v110, 0xc0c0004
	v_perm_b32 v118, v119, v118, 0xc0c0004
	v_perm_b32 v116, v117, v116, 0xc0c0004
	v_perm_b32 v117, v130, v129, 0xc0c0004
	v_perm_b32 v119, v128, v127, 0xc0c0004
	v_perm_b32 v127, v138, v137, 0xc0c0004
	v_perm_b32 v128, v136, v135, 0xc0c0004
	v_perm_b32 v129, v146, v145, 0xc0c0004
	v_perm_b32 v47, v48, v47, 0xc0c0004
	v_perm_b32 v48, v95, v49, 0xc0c0004
	v_perm_b32 v130, v144, v143, 0xc0c0004
	v_perm_b32 v50, v52, v50, 0xc0c0004
	s_waitcnt lgkmcnt(56)
	v_perm_b32 v135, v154, v153, 0xc0c0004
	v_perm_b32 v52, v58, v57, 0xc0c0004
	v_perm_b32 v49, v101, v100, 0xc0c0004
	v_perm_b32 v95, v103, v102, 0xc0c0004
	v_perm_b32 v100, v105, v56, 0xc0c0004
	v_perm_b32 v101, v107, v106, 0xc0c0004
	v_perm_b32 v102, v113, v112, 0xc0c0004
	v_perm_b32 v103, v115, v114, 0xc0c0004
	v_perm_b32 v105, v121, v120, 0xc0c0004
	v_perm_b32 v106, v126, v122, 0xc0c0004
	v_perm_b32 v107, v132, v131, 0xc0c0004
	v_perm_b32 v112, v134, v133, 0xc0c0004
	v_perm_b32 v113, v140, v139, 0xc0c0004
	v_perm_b32 v114, v142, v141, 0xc0c0004
	v_perm_b32 v120, v150, v149, 0xc0c0004
	s_waitcnt lgkmcnt(52)
	v_perm_b32 v122, v158, v157, 0xc0c0004
	v_perm_b32 v61, v62, v61, 0xc0c0004
	v_perm_b32 v59, v60, v59, 0xc0c0004
	v_perm_b32 v60, v99, v98, 0xc0c0004
	v_perm_b32 v62, v97, v96, 0xc0c0004
	v_perm_b32 v53, v53, v104, 0xc0c0004
	v_perm_b32 v150, v109, v108, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v51, v51, v209, 0xc0c0004
	v_perm_b32 v157, v208, v207, 0xc0c0004
	v_lshl_or_b32 v58, v52, 16, v50
	v_lshl_or_b32 v57, v48, 16, v47
	v_lshl_or_b32 v56, v95, 16, v49
	v_lshl_or_b32 v55, v101, 16, v100
	v_perm_b32 v115, v148, v147, 0xc0c0004
	v_perm_b32 v121, v156, v155, 0xc0c0004
	v_perm_b32 v126, v164, v163, 0xc0c0004
	v_perm_b32 v131, v166, v165, 0xc0c0004
	v_perm_b32 v132, v172, v171, 0xc0c0004
	v_perm_b32 v133, v174, v173, 0xc0c0004
	v_lshl_or_b32 v98, v103, 16, v102
	v_lshl_or_b32 v97, v106, 16, v105
	v_lshl_or_b32 v96, v112, 16, v107
	v_lshl_or_b32 v95, v114, 16, v113
	v_lshl_or_b32 v110, v59, 16, v61
	v_lshl_or_b32 v109, v62, 16, v60
	v_lshl_or_b32 v108, v53, 16, v54
	v_lshl_or_b32 v107, v150, 16, v111
	v_lshl_or_b32 v113, v119, 16, v117
	v_lshl_or_b32 v119, v157, 16, v51
	v_wmma_i32_16x16x16_iu8 v[47:54], v[55:58], v[63:66], v[1:8] neg_lo:[1,1,0]
	v_perm_b32 v134, v180, v179, 0xc0c0004
	v_perm_b32 v139, v182, v181, 0xc0c0004
	v_perm_b32 v140, v188, v187, 0xc0c0004
	v_perm_b32 v141, v190, v189, 0xc0c0004
	v_perm_b32 v142, v196, v195, 0xc0c0004
	v_perm_b32 v147, v198, v197, 0xc0c0004
	v_perm_b32 v148, v204, v203, 0xc0c0004
	v_perm_b32 v149, v206, v205, 0xc0c0004
	v_perm_b32 v136, v152, v151, 0xc0c0004
	v_perm_b32 v137, v162, v161, 0xc0c0004
	v_perm_b32 v138, v160, v159, 0xc0c0004
	v_perm_b32 v143, v170, v169, 0xc0c0004
	v_perm_b32 v144, v168, v167, 0xc0c0004
	v_perm_b32 v145, v178, v177, 0xc0c0004
	v_perm_b32 v146, v176, v175, 0xc0c0004
	v_lshl_or_b32 v102, v120, 16, v115
	v_lshl_or_b32 v101, v122, 16, v121
	v_lshl_or_b32 v100, v131, 16, v126
	v_lshl_or_b32 v99, v133, 16, v132
	v_lshl_or_b32 v114, v116, 16, v118
	v_lshl_or_b32 v112, v128, 16, v127
	v_lshl_or_b32 v111, v130, 16, v129
	v_wmma_i32_16x16x16_iu8 v[55:62], v[107:110], v[63:66], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[47:54], v[95:98], v[67:70], v[47:54] neg_lo:[1,1,0]
	v_perm_b32 v151, v186, v185, 0xc0c0004
	v_perm_b32 v152, v184, v183, 0xc0c0004
	v_perm_b32 v153, v194, v193, 0xc0c0004
	v_perm_b32 v154, v192, v191, 0xc0c0004
	v_perm_b32 v155, v202, v201, 0xc0c0004
	v_perm_b32 v156, v200, v199, 0xc0c0004
	v_lshl_or_b32 v106, v139, 16, v134
	v_lshl_or_b32 v105, v141, 16, v140
	v_lshl_or_b32 v104, v147, 16, v142
	v_lshl_or_b32 v103, v149, 16, v148
	v_lshl_or_b32 v118, v136, 16, v135
	v_lshl_or_b32 v117, v138, 16, v137
	v_lshl_or_b32 v116, v144, 16, v143
	v_lshl_or_b32 v115, v146, 16, v145
	v_wmma_i32_16x16x16_iu8 v[55:62], v[111:114], v[67:70], v[55:62] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[47:54], v[99:102], v[71:74], v[47:54] neg_lo:[1,1,0]
	v_lshl_or_b32 v122, v152, 16, v151
	v_lshl_or_b32 v121, v154, 16, v153
	v_lshl_or_b32 v120, v156, 16, v155
	v_wmma_i32_16x16x16_iu8 v[55:62], v[115:118], v[71:74], v[55:62] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[47:54], v[103:106], v[75:78], v[47:54] neg_lo:[1,1,0]
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_add_i32 s1, s27, 1
	s_mov_b32 s12, s35
	s_cmp_lt_i32 s1, 2
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_wmma_i32_16x16x16_iu8 v[55:62], v[119:122], v[75:78], v[55:62] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v63, v47
	v_cvt_f32_i32_e32 v64, v48
	v_cvt_f32_i32_e32 v65, v49
	v_cvt_f32_i32_e32 v66, v50
	v_cvt_f32_i32_e32 v67, v51
	v_cvt_f32_i32_e32 v68, v52
	v_cvt_f32_i32_e32 v69, v53
	v_cvt_f32_i32_e32 v70, v54
	v_cvt_f32_i32_e32 v71, v55
	v_cvt_f32_i32_e32 v72, v56
	v_cvt_f32_i32_e32 v73, v57
	v_cvt_f32_i32_e32 v74, v58
	v_cvt_f32_i32_e32 v75, v59
	v_cvt_f32_i32_e32 v76, v60
	v_cvt_f32_i32_e32 v77, v61
	v_cvt_f32_i32_e32 v78, v62
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s27, s1, 0
	s_add_i32 s14, s13, 1
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_lshl_b32 s15, s27, 14
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_lshl_b32 s1, s27, 10
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_add_i32 s35, s15, 0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_add_i32 s16, s1, 0
	s_delay_alu instid0(SALU_CYCLE_1)
	s_add_i32 s1, s16, 0x8000
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_lg_u32 s13, s44
	s_mov_b32 s13, s14
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_waitcnt vmcnt(4)
	v_lshlrev_b32_e32 v95, 16, v125
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v47, 16, v124
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	ds_store_b32 v45, v47 offset:34816
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b128 v[47:50], v46 offset:34816
	ds_load_b128 v[51:54], v46 offset:34832
	ds_load_b128 v[55:58], v46 offset:35328
	ds_load_b128 v[59:62], v46 offset:35344
	s_waitcnt lgkmcnt(3)
	v_mul_f32_e32 v47, v47, v95
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_fmac_f32 v18, v47, v63 :: v_dual_add_nc_u32 v211, s35, v44
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(1)
	v_mul_f32_e32 v56, v56, v95
	s_waitcnt lgkmcnt(0)
	v_mul_f32_e32 v62, v62, v95
	v_mul_f32_e32 v48, v48, v95
	v_mul_f32_e32 v49, v49, v95
	v_mul_f32_e32 v50, v50, v95
	v_mul_f32_e32 v51, v51, v95
	v_mul_f32_e32 v52, v52, v95
	v_mul_f32_e32 v54, v54, v95
	v_dual_mul_f32 v55, v55, v95 :: v_dual_fmac_f32 v20, v49, v65
	v_mul_f32_e32 v57, v57, v95
	v_mul_f32_e32 v58, v58, v95
	s_delay_alu instid0(VALU_DEP_4)
	v_dual_mul_f32 v60, v60, v95 :: v_dual_fmac_f32 v25, v54, v70
	v_mul_f32_e32 v61, v61, v95
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fmac_f32_e32 v27, v56, v72
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_dual_mul_f32 v53, v53, v95 :: v_dual_fmac_f32 v32, v62, v78
	v_mul_f32_e32 v59, v59, v95
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_fmac_f32 v23, v52, v68 :: v_dual_add_nc_u32 v210, s16, v43
	v_fmac_f32_e32 v19, v48, v64
	v_dual_fmac_f32 v21, v50, v66 :: v_dual_fmac_f32 v22, v51, v67
	v_dual_fmac_f32 v29, v58, v74 :: v_dual_fmac_f32 v24, v53, v69
	v_dual_fmac_f32 v26, v55, v71 :: v_dual_fmac_f32 v31, v60, v76
	v_fmac_f32_e32 v28, v57, v73
	v_dual_fmac_f32 v30, v59, v75 :: v_dual_fmac_f32 v33, v61, v77
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_store_b32 v210, v123 offset:32768
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(3)
	ds_store_b128 v211, v[79:82]
	s_waitcnt vmcnt(2)
	ds_store_b128 v211, v[83:86] offset:4096
	s_waitcnt vmcnt(1)
	ds_store_b128 v211, v[87:90] offset:8192
	s_waitcnt vmcnt(0)
	ds_store_b128 v211, v[91:94] offset:12288
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_5
; %bb.6:                                ; %Flow
	.loc	1 0 5 is_stmt 0                 ; generate_amdgcn.py:0:5
	v_mov_b32_e32 v1, v42
	s_branch .LBB0_8
.LBB0_7:
	v_dual_mov_b32 v32, 0 :: v_dual_mov_b32 v33, 0
	v_dual_mov_b32 v31, 0 :: v_dual_mov_b32 v30, 0
	v_dual_mov_b32 v29, 0 :: v_dual_mov_b32 v28, 0
	v_dual_mov_b32 v27, 0 :: v_dual_mov_b32 v26, 0
	v_dual_mov_b32 v25, 0 :: v_dual_mov_b32 v24, 0
	v_dual_mov_b32 v23, 0 :: v_dual_mov_b32 v22, 0
	v_dual_mov_b32 v21, 0 :: v_dual_mov_b32 v20, 0
	v_dual_mov_b32 v19, 0 :: v_dual_mov_b32 v18, 0
	s_add_i32 s10, 0, 0x8000
	s_add_i32 s1, 0, 0x8400
	s_add_i32 s35, 0, 0x4000
.LBB0_8:                                ; %._crit_edge
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 378 22 is_stmt 1              ; generate_amdgcn.py:378:22
	v_or_b32_e32 v61, v1, v17
	v_dual_mov_b32 v41, 0 :: v_dual_mov_b32 v46, 0
	v_dual_mov_b32 v42, 0 :: v_dual_mov_b32 v43, 0
	v_dual_mov_b32 v44, 0 :: v_dual_mov_b32 v45, 0
	v_dual_mov_b32 v47, 0 :: v_dual_mov_b32 v48, 0
	v_dual_mov_b32 v49, 0 :: v_dual_mov_b32 v50, 0
	v_dual_mov_b32 v51, 0 :: v_dual_mov_b32 v52, 0
	v_dual_mov_b32 v53, 0 :: v_dual_mov_b32 v54, 0
	v_dual_mov_b32 v55, 0 :: v_dual_mov_b32 v56, 0
	v_mov_b32_e32 v57, 0
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s3
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_cbranch_vccnz .LBB0_10
; %bb.9:
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v9, s12, v61
	s_mov_b32 s12, 0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v85, s10, v37
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_mov_b32 s13, s12
	s_mov_b32 s14, s12
	s_mov_b32 s15, s12
	s_mov_b32 s16, s12
	s_mov_b32 s17, s12
	s_mov_b32 s18, s12
	s_mov_b32 s19, s12
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v1, s10, v39
	v_add_nc_u32_e32 v2, s10, v38
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v3, v9 offset:3328
	ds_load_u8 v4, v9 offset:3072
	ds_load_u8 v5, v9 offset:3840
	ds_load_u8 v6, v9 offset:3584
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[42:45], v1
	ds_load_b128 v[54:57], v2
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v1, v9 offset:2304
	ds_load_u8 v2, v9 offset:2048
	ds_load_u8 v7, v9 offset:2816
	ds_load_u8 v8, v9 offset:2560
	ds_load_u8 v15, v9 offset:1280
	ds_load_u8 v16, v9 offset:1024
	ds_load_u8 v46, v9 offset:1792
	ds_load_u8 v47, v9 offset:1536
	ds_load_u8 v48, v9 offset:256
	ds_load_u8 v49, v9
	ds_load_u8 v50, v9 offset:768
	ds_load_u8 v51, v9 offset:512
	ds_load_u8 v58, v9 offset:3968
	ds_load_u8 v59, v9 offset:3712
	ds_load_u8 v60, v9 offset:3456
	ds_load_u8 v74, v9 offset:3200
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v3, v4, v3, 0xc0c0004
	s_waitcnt lgkmcnt(18)
	v_perm_b32 v4, v6, v5, 0xc0c0004
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v10, s10, v40
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v1, v2, v1, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_3)
	v_lshl_or_b32 v65, v4, 16, v3
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v2, v8, v7, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v5, v16, v15, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v6, v47, v46, 0xc0c0004
	v_lshl_or_b32 v64, v2, 16, v1
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v7, v49, v48, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v8, v51, v50, 0xc0c0004
	v_lshl_or_b32 v63, v6, 16, v5
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v58, v59, v58, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v60, v74, v60, 0xc0c0004
	v_lshl_or_b32 v62, v8, 16, v7
	v_dual_mov_b32 v1, s12 :: v_dual_mov_b32 v2, s13
	ds_load_u8 v70, v9 offset:7424
	ds_load_u8 v71, v9 offset:7168
	ds_load_u8 v72, v9 offset:7936
	ds_load_u8 v73, v9 offset:7680
	v_dual_mov_b32 v3, s14 :: v_dual_mov_b32 v4, s15
	v_dual_mov_b32 v5, s16 :: v_dual_mov_b32 v6, s17
	v_dual_mov_b32 v7, s18 :: v_dual_mov_b32 v8, s19
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[66:69], v10
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v75, v9 offset:2944
	ds_load_u8 v76, v9 offset:2688
	ds_load_u8 v77, v9 offset:2432
	ds_load_u8 v78, v9 offset:2176
	ds_load_u8 v79, v9 offset:1920
	ds_load_u8 v80, v9 offset:1664
	ds_load_u8 v81, v9 offset:1408
	ds_load_u8 v82, v9 offset:1152
	ds_load_u8 v15, v9 offset:896
	ds_load_u8 v16, v9 offset:640
	ds_load_u8 v83, v9 offset:384
	ds_load_u8 v84, v9 offset:128
	v_wmma_i32_16x16x16_iu8 v[46:53], v[62:65], v[54:57], v[1:8] neg_lo:[1,1,0]
	ds_load_u8 v10, v9 offset:6400
	ds_load_u8 v62, v9 offset:6144
	ds_load_u8 v63, v9 offset:6912
	ds_load_u8 v64, v9 offset:6656
	ds_load_u8 v65, v9 offset:5376
	ds_load_u8 v86, v9 offset:5120
	ds_load_u8 v87, v9 offset:5888
	ds_load_u8 v88, v9 offset:5632
	ds_load_u8 v89, v9 offset:8064
	ds_load_u8 v90, v9 offset:7808
	ds_load_u8 v91, v9 offset:7552
	ds_load_u8 v92, v9 offset:7296
	ds_load_u8 v99, v9 offset:6016
	ds_load_u8 v100, v9 offset:5760
	ds_load_u8 v101, v9 offset:5504
	ds_load_u8 v102, v9 offset:5248
	ds_load_u8 v119, v9 offset:10112
	ds_load_u8 v120, v9 offset:9856
	ds_load_u8 v121, v9 offset:9600
	ds_load_u8 v122, v9 offset:9344
	ds_load_u8 v136, v9 offset:14208
	ds_load_u8 v137, v9 offset:13952
	ds_load_u8 v138, v9 offset:13696
	ds_load_u8 v139, v9 offset:13440
	s_waitcnt lgkmcnt(39)
	v_perm_b32 v70, v71, v70, 0xc0c0004
	ds_load_u8 v71, v9 offset:4352
	ds_load_u8 v93, v9 offset:4096
	s_waitcnt lgkmcnt(39)
	v_perm_b32 v72, v73, v72, 0xc0c0004
	ds_load_u8 v73, v9 offset:4864
	ds_load_u8 v94, v9 offset:4608
	ds_load_u8 v95, v9 offset:7040
	ds_load_u8 v96, v9 offset:6784
	ds_load_u8 v97, v9 offset:6528
	ds_load_u8 v98, v9 offset:6272
	s_waitcnt lgkmcnt(30)
	v_perm_b32 v10, v62, v10, 0xc0c0004
	s_waitcnt lgkmcnt(28)
	v_perm_b32 v62, v64, v63, 0xc0c0004
	v_perm_b32 v74, v76, v75, 0xc0c0004
	s_waitcnt lgkmcnt(26)
	v_perm_b32 v63, v86, v65, 0xc0c0004
	v_lshl_or_b32 v65, v72, 16, v70
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v86, v88, v87, 0xc0c0004
	v_lshl_or_b32 v64, v62, 16, v10
	ds_load_u8 v87, v9 offset:4992
	ds_load_u8 v88, v9 offset:4736
	ds_load_u8 v103, v9 offset:4480
	ds_load_u8 v104, v9 offset:4224
	v_perm_b32 v59, v78, v77, 0xc0c0004
	v_perm_b32 v75, v82, v81, 0xc0c0004
	v_lshl_or_b32 v63, v86, 16, v63
	v_perm_b32 v78, v80, v79, 0xc0c0004
	v_perm_b32 v79, v84, v83, 0xc0c0004
	v_perm_b32 v15, v16, v15, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v73, v94, v73, 0xc0c0004
	ds_load_u8 v94, v9 offset:12032
	ds_load_u8 v106, v9 offset:11776
	v_perm_b32 v71, v93, v71, 0xc0c0004
	ds_load_u8 v93, v9 offset:11520
	ds_load_u8 v105, v9 offset:11264
	v_lshl_or_b32 v77, v58, 16, v60
	v_lshl_or_b32 v76, v74, 16, v59
	v_lshl_or_b32 v75, v78, 16, v75
	v_lshl_or_b32 v62, v73, 16, v71
	ds_load_u8 v10, v9 offset:10496
	ds_load_u8 v70, v9 offset:10240
	ds_load_u8 v71, v9 offset:11008
	ds_load_u8 v72, v9 offset:10752
	ds_load_u8 v73, v9 offset:9472
	ds_load_u8 v86, v9 offset:9216
	ds_load_u8 v107, v9 offset:9984
	ds_load_u8 v108, v9 offset:9728
	ds_load_u8 v109, v9 offset:12160
	ds_load_u8 v110, v9 offset:11904
	ds_load_u8 v111, v9 offset:11648
	ds_load_u8 v112, v9 offset:11392
	v_lshl_or_b32 v74, v15, 16, v79
	v_perm_b32 v15, v92, v91, 0xc0c0004
	v_perm_b32 v16, v90, v89, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[46:53], v[62:65], v[42:45], v[46:53] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v58, v98, v97, 0xc0c0004
	v_perm_b32 v59, v96, v95, 0xc0c0004
	v_perm_b32 v60, v102, v101, 0xc0c0004
	v_perm_b32 v78, v100, v99, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v79, v104, v103, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v10, v70, v10, 0xc0c0004
	v_perm_b32 v80, v88, v87, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v70, v72, v71, 0xc0c0004
	v_perm_b32 v94, v106, v94, 0xc0c0004
	ds_load_u8 v106, v9 offset:8960
	ds_load_u8 v114, v9 offset:8704
	ds_load_u8 v115, v9 offset:11136
	ds_load_u8 v116, v9 offset:10880
	ds_load_u8 v117, v9 offset:10624
	ds_load_u8 v118, v9 offset:10368
	v_perm_b32 v93, v105, v93, 0xc0c0004
	ds_load_u8 v105, v9 offset:8448
	ds_load_u8 v113, v9 offset:8192
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v71, v86, v73, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v72, v108, v107, 0xc0c0004
	v_lshl_or_b32 v64, v70, 16, v10
	v_lshl_or_b32 v65, v94, 16, v93
	ds_load_u8 v86, v9 offset:9088
	ds_load_u8 v107, v9 offset:8832
	ds_load_u8 v108, v9 offset:8576
	ds_load_u8 v123, v9 offset:8320
	v_wmma_i32_16x16x16_iu8 v[1:8], v[74:77], v[54:57], v[1:8] neg_lo:[1,1,0]
	v_lshl_or_b32 v63, v72, 16, v71
	v_lshl_or_b32 v57, v16, 16, v15
	v_lshl_or_b32 v56, v59, 16, v58
	v_lshl_or_b32 v55, v78, 16, v60
	v_lshl_or_b32 v54, v80, 16, v79
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v15, v112, v111, 0xc0c0004
	v_perm_b32 v16, v110, v109, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v106, v114, v106, 0xc0c0004
	v_perm_b32 v60, v122, v121, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v59, v116, v115, 0xc0c0004
	v_perm_b32 v74, v120, v119, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v58, v118, v117, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[1:8], v[54:57], v[42:45], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v73, v113, v105, 0xc0c0004
	ds_load_u8 v105, v9 offset:15616
	ds_load_u8 v113, v9 offset:15360
	ds_load_u8 v124, v9 offset:16128
	ds_load_u8 v125, v9 offset:15872
	v_lshl_or_b32 v45, v16, 16, v15
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v76, v107, v86, 0xc0c0004
	v_lshl_or_b32 v44, v59, 16, v58
	v_lshl_or_b32 v62, v106, 16, v73
	ds_load_u8 v10, v9 offset:14592
	ds_load_u8 v70, v9 offset:14336
	ds_load_u8 v71, v9 offset:15104
	ds_load_u8 v72, v9 offset:14848
	ds_load_u8 v73, v9 offset:13568
	ds_load_u8 v93, v9 offset:13312
	ds_load_u8 v94, v9 offset:14080
	ds_load_u8 v106, v9 offset:13824
	ds_load_u8 v114, v9 offset:16256
	ds_load_u8 v126, v9 offset:16000
	ds_load_u8 v127, v9 offset:15744
	ds_load_u8 v128, v9 offset:15488
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v75, v123, v108, 0xc0c0004
	v_lshl_or_b32 v43, v74, 16, v60
	v_perm_b32 v56, v139, v138, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[46:53], v[62:65], v[66:69], v[46:53] neg_lo:[1,1,0]
	v_perm_b32 v57, v137, v136, 0xc0c0004
	v_lshl_or_b32 v42, v76, 16, v75
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v10, v70, v10, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_2)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[42:45], v[66:69], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v135, v72, v71, 0xc0c0004
	v_perm_b32 v105, v113, v105, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v93, v93, v73, 0xc0c0004
	v_perm_b32 v113, v125, v124, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v94, v106, v94, 0xc0c0004
	ds_load_u8 v124, v9 offset:12544
	ds_load_u8 v125, v9 offset:12288
	ds_load_u8 v129, v9 offset:13056
	ds_load_u8 v130, v9 offset:12800
	ds_load_u8 v131, v9 offset:15232
	ds_load_u8 v132, v9 offset:14976
	ds_load_u8 v133, v9 offset:14720
	ds_load_u8 v134, v9 offset:14464
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[70:73], v85
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_lshl_or_b32 v64, v135, 16, v10
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v15, v128, v127, 0xc0c0004
	v_lshl_or_b32 v63, v94, 16, v93
	ds_load_u8 v10, v9 offset:13184
	ds_load_u8 v85, v9 offset:12928
	ds_load_u8 v93, v9 offset:12672
	ds_load_u8 v9, v9 offset:12416
	v_perm_b32 v16, v126, v114, 0xc0c0004
	v_lshl_or_b32 v65, v113, 16, v105
	v_lshl_or_b32 v43, v57, 16, v56
	s_delay_alu instid0(VALU_DEP_3)
	v_lshl_or_b32 v45, v16, 16, v15
	s_waitcnt lgkmcnt(11)
	v_perm_b32 v62, v125, v124, 0xc0c0004
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v94, v130, v129, 0xc0c0004
	s_waitcnt lgkmcnt(7)
	v_perm_b32 v55, v132, v131, 0xc0c0004
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v54, v134, v133, 0xc0c0004
	v_lshl_or_b32 v62, v94, 16, v62
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v10, v85, v10, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v9, v9, v93, 0xc0c0004
	v_lshl_or_b32 v44, v55, 16, v54
	v_wmma_i32_16x16x16_iu8 v[46:53], v[62:65], v[70:73], v[46:53] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_lshl_or_b32 v42, v10, 16, v9
	v_cvt_f32_i32_e32 v46, v46
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[42:45], v[70:73], v[1:8] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v42, v47
	v_cvt_f32_i32_e32 v43, v48
	v_cvt_f32_i32_e32 v44, v49
	v_cvt_f32_i32_e32 v45, v50
	v_cvt_f32_i32_e32 v47, v51
	v_cvt_f32_i32_e32 v48, v52
	v_cvt_f32_i32_e32 v49, v53
	v_cvt_f32_i32_e32 v50, v1
	v_cvt_f32_i32_e32 v51, v2
	v_cvt_f32_i32_e32 v52, v3
	v_cvt_f32_i32_e32 v53, v4
	v_cvt_f32_i32_e32 v54, v5
	v_cvt_f32_i32_e32 v55, v6
	v_cvt_f32_i32_e32 v56, v7
	v_cvt_f32_i32_e32 v57, v8
.LBB0_10:
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s4, s26, 31
	s_mov_b32 s11, 0x31027000
	s_lshr_b32 s4, s4, 24
	s_mov_b32 s10, 0x7ffffffe
	s_add_i32 s4, s26, s4
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s26, s10
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s4, s4, 8
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s27, s11
	.loc	1 392 40 is_stmt 0              ; generate_amdgcn.py:392:40
	s_mul_i32 s5, s4, s23
	.loc	1 389 34 is_stmt 1              ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s4, s0
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v1, v36, s4, 1
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v2, v35, s5, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 s4, -1, 0
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_and_b32 s25, s25, 0xffff
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s3, s4
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_and_b32 s9, s9, 0xffff
	v_dual_cndmask_b32 v1, 0x80000000, v1 :: v_dual_cndmask_b32 v2, 0x80000000, v2
	v_dual_mov_b32 v64, 0 :: v_dual_and_b32 v3, 28, v13
	v_mov_b32_e32 v62, 0
	buffer_load_u16 v60, v1, s[8:11], 0 offen
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	buffer_load_u16 v2, v2, s[24:27], 0 offen
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_and_b32_e32 v1, 32, v14
	v_dual_mov_b32 v63, 0 :: v_dual_add_nc_u32 v58, 0, v11
	v_dual_mov_b32 v65, 0 :: v_dual_mov_b32 v70, 0
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_2) | instid1(VALU_DEP_3)
	v_add3_u32 v1, 0, v12, v1
	v_dual_mov_b32 v66, 0 :: v_dual_mov_b32 v67, 0
	v_dual_mov_b32 v72, 0 :: v_dual_mov_b32 v69, 0
	v_dual_mov_b32 v74, 0 :: v_dual_add_nc_u32 v59, v1, v3
	v_dual_mov_b32 v71, 0 :: v_dual_mov_b32 v76, 0
	v_mov_b32_e32 v73, 0
	v_mov_b32_e32 v75, 0
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_mov_b32 s12, 0
	s_waitcnt vmcnt(0)
	v_dual_mov_b32 v68, 0 :: v_dual_lshlrev_b32 v1, 16, v2
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	ds_store_b32 v59, v1 offset:34816
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b128 v[1:4], v58 offset:34816
	ds_load_b128 v[5:8], v58 offset:34832
	ds_load_b128 v[9:12], v58 offset:35328
	ds_load_b128 v[13:16], v58 offset:35344
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_cbranch_vccnz .LBB0_12
; %bb.11:
	s_mov_b32 s13, s12
	s_mov_b32 s14, s12
	s_mov_b32 s15, s12
	s_mov_b32 s16, s12
	s_mov_b32 s17, s12
	s_mov_b32 s18, s12
	s_mov_b32 s19, s12
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v93, s35, v61
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v61, s1, v40
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v40, v93 offset:3328
	ds_load_u8 v41, v93 offset:3072
	ds_load_u8 v62, v93 offset:3840
	ds_load_u8 v63, v93 offset:3584
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v64, s1, v38
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v38, v93 offset:2304
	ds_load_u8 v65, v93 offset:2048
	ds_load_u8 v66, v93 offset:2816
	ds_load_u8 v67, v93 offset:2560
	ds_load_u8 v68, v93 offset:1280
	ds_load_u8 v69, v93 offset:1024
	ds_load_u8 v70, v93 offset:1792
	ds_load_u8 v71, v93 offset:1536
	ds_load_u8 v72, v93 offset:256
	ds_load_u8 v73, v93
	ds_load_u8 v74, v93 offset:768
	ds_load_u8 v75, v93 offset:512
	ds_load_u8 v94, v93 offset:3968
	ds_load_u8 v95, v93 offset:3712
	ds_load_u8 v96, v93 offset:3456
	ds_load_u8 v97, v93 offset:3200
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v39, s1, v39
	v_add_nc_u32_e32 v37, s1, v37
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_waitcnt lgkmcnt(18)
	v_perm_b32 v76, v41, v40, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v62, v63, v62, 0xc0c0004
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v63, v65, v38, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v65, v67, v66, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v66, v69, v68, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v68, v73, v72, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v69, v75, v74, 0xc0c0004
	v_perm_b32 v67, v71, v70, 0xc0c0004
	v_lshl_or_b32 v84, v62, 16, v76
	v_lshl_or_b32 v83, v65, 16, v63
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v94, v95, v94, 0xc0c0004
	v_lshl_or_b32 v81, v69, 16, v68
	v_mov_b32_e32 v76, s19
	ds_load_u8 v89, v93 offset:7424
	ds_load_u8 v90, v93 offset:7168
	ds_load_u8 v91, v93 offset:7936
	ds_load_u8 v92, v93 offset:7680
	v_dual_mov_b32 v75, s18 :: v_dual_mov_b32 v72, s15
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[38:41], v39
	ds_load_b128 v[77:80], v64
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_lshl_or_b32 v82, v67, 16, v66
	v_dual_mov_b32 v74, s17 :: v_dual_mov_b32 v73, s16
	v_dual_mov_b32 v70, s13 :: v_dual_mov_b32 v71, s14
	v_mov_b32_e32 v69, s12
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[85:88], v61
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v98, v93 offset:2944
	ds_load_u8 v99, v93 offset:2688
	ds_load_u8 v100, v93 offset:2432
	ds_load_u8 v101, v93 offset:2176
	ds_load_u8 v102, v93 offset:1920
	ds_load_u8 v103, v93 offset:1664
	ds_load_u8 v104, v93 offset:1408
	ds_load_u8 v105, v93 offset:1152
	ds_load_u8 v106, v93 offset:896
	ds_load_u8 v107, v93 offset:640
	ds_load_u8 v108, v93 offset:384
	ds_load_u8 v109, v93 offset:128
	ds_load_u8 v124, v93 offset:6016
	ds_load_u8 v125, v93 offset:5760
	ds_load_u8 v126, v93 offset:5504
	ds_load_u8 v127, v93 offset:5248
	ds_load_u8 v144, v93 offset:10112
	ds_load_u8 v145, v93 offset:9856
	ds_load_u8 v146, v93 offset:9600
	ds_load_u8 v147, v93 offset:9344
	ds_load_u8 v162, v93 offset:14208
	ds_load_u8 v163, v93 offset:13952
	ds_load_u8 v164, v93 offset:13696
	ds_load_u8 v165, v93 offset:13440
	s_waitcnt lgkmcnt(29)
	v_perm_b32 v89, v90, v89, 0xc0c0004
	ds_load_u8 v90, v93 offset:4352
	ds_load_u8 v118, v93 offset:4096
	s_waitcnt lgkmcnt(29)
	v_perm_b32 v91, v92, v91, 0xc0c0004
	ds_load_u8 v92, v93 offset:4864
	ds_load_u8 v119, v93 offset:4608
	ds_load_u8 v120, v93 offset:7040
	ds_load_u8 v121, v93 offset:6784
	ds_load_u8 v122, v93 offset:6528
	ds_load_u8 v123, v93 offset:6272
	s_waitcnt lgkmcnt(33)
	v_wmma_i32_16x16x16_iu8 v[61:68], v[81:84], v[77:80], v[69:76] neg_lo:[1,1,0]
	ds_load_u8 v81, v93 offset:6400
	ds_load_u8 v82, v93 offset:6144
	ds_load_u8 v83, v93 offset:6912
	ds_load_u8 v84, v93 offset:6656
	ds_load_u8 v110, v93 offset:5376
	ds_load_u8 v111, v93 offset:5120
	ds_load_u8 v112, v93 offset:5888
	ds_load_u8 v113, v93 offset:5632
	ds_load_u8 v114, v93 offset:8064
	ds_load_u8 v115, v93 offset:7808
	ds_load_u8 v116, v93 offset:7552
	ds_load_u8 v117, v93 offset:7296
	s_waitcnt lgkmcnt(40)
	v_perm_b32 v95, v101, v100, 0xc0c0004
	s_waitcnt lgkmcnt(34)
	v_perm_b32 v101, v107, v106, 0xc0c0004
	s_waitcnt lgkmcnt(32)
	v_perm_b32 v100, v109, v108, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v81, v82, v81, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v82, v84, v83, 0xc0c0004
	v_perm_b32 v92, v119, v92, 0xc0c0004
	ds_load_u8 v119, v93 offset:12032
	ds_load_u8 v131, v93 offset:11776
	v_perm_b32 v90, v118, v90, 0xc0c0004
	ds_load_u8 v118, v93 offset:11520
	ds_load_u8 v130, v93 offset:11264
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v110, v111, v110, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v111, v113, v112, 0xc0c0004
	v_lshl_or_b32 v84, v91, 16, v89
	v_lshl_or_b32 v83, v82, 16, v81
	v_lshl_or_b32 v81, v92, 16, v90
	ds_load_u8 v112, v93 offset:4992
	ds_load_u8 v113, v93 offset:4736
	ds_load_u8 v128, v93 offset:4480
	ds_load_u8 v129, v93 offset:4224
	v_lshl_or_b32 v82, v111, 16, v110
	ds_load_u8 v89, v93 offset:10496
	ds_load_u8 v90, v93 offset:10240
	ds_load_u8 v91, v93 offset:11008
	ds_load_u8 v92, v93 offset:10752
	ds_load_u8 v110, v93 offset:9472
	ds_load_u8 v111, v93 offset:9216
	ds_load_u8 v132, v93 offset:9984
	ds_load_u8 v133, v93 offset:9728
	ds_load_u8 v134, v93 offset:12160
	ds_load_u8 v135, v93 offset:11904
	ds_load_u8 v136, v93 offset:11648
	ds_load_u8 v137, v93 offset:11392
	v_wmma_i32_16x16x16_iu8 v[61:68], v[81:84], v[38:41], v[61:68] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(18)
	v_perm_b32 v119, v131, v119, 0xc0c0004
	ds_load_u8 v131, v93 offset:8960
	ds_load_u8 v139, v93 offset:8704
	ds_load_u8 v140, v93 offset:11136
	ds_load_u8 v141, v93 offset:10880
	ds_load_u8 v142, v93 offset:10624
	ds_load_u8 v143, v93 offset:10368
	s_waitcnt lgkmcnt(22)
	v_perm_b32 v118, v130, v118, 0xc0c0004
	ds_load_u8 v130, v93 offset:8448
	ds_load_u8 v138, v93 offset:8192
	s_waitcnt lgkmcnt(18)
	v_perm_b32 v89, v90, v89, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v90, v92, v91, 0xc0c0004
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v91, v111, v110, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v92, v133, v132, 0xc0c0004
	v_lshl_or_b32 v84, v119, 16, v118
	ds_load_u8 v110, v93 offset:9088
	ds_load_u8 v111, v93 offset:8832
	ds_load_u8 v132, v93 offset:8576
	ds_load_u8 v133, v93 offset:8320
	v_lshl_or_b32 v83, v90, 16, v89
	v_lshl_or_b32 v82, v92, 16, v91
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v131, v139, v131, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v130, v138, v130, 0xc0c0004
	ds_load_u8 v138, v93 offset:15616
	ds_load_u8 v148, v93 offset:15360
	ds_load_u8 v149, v93 offset:16128
	ds_load_u8 v150, v93 offset:15872
	v_lshl_or_b32 v81, v131, 16, v130
	ds_load_u8 v89, v93 offset:14592
	ds_load_u8 v90, v93 offset:14336
	ds_load_u8 v91, v93 offset:15104
	ds_load_u8 v92, v93 offset:14848
	ds_load_u8 v118, v93 offset:13568
	ds_load_u8 v119, v93 offset:13312
	ds_load_u8 v130, v93 offset:14080
	ds_load_u8 v131, v93 offset:13824
	ds_load_u8 v139, v93 offset:16256
	ds_load_u8 v151, v93 offset:16000
	ds_load_u8 v152, v93 offset:15744
	ds_load_u8 v153, v93 offset:15488
	v_wmma_i32_16x16x16_iu8 v[61:68], v[81:84], v[85:88], v[61:68] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v160, v90, v89, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v161, v92, v91, 0xc0c0004
	v_perm_b32 v138, v148, v138, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v118, v119, v118, 0xc0c0004
	v_perm_b32 v148, v150, v149, 0xc0c0004
	ds_load_u8 v149, v93 offset:12544
	ds_load_u8 v150, v93 offset:12288
	ds_load_u8 v154, v93 offset:13056
	ds_load_u8 v155, v93 offset:12800
	ds_load_u8 v156, v93 offset:15232
	ds_load_u8 v157, v93 offset:14976
	ds_load_u8 v158, v93 offset:14720
	ds_load_u8 v159, v93 offset:14464
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v119, v131, v130, 0xc0c0004
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[89:92], v37
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_lshl_or_b32 v83, v161, 16, v160
	v_lshl_or_b32 v84, v148, 16, v138
	v_lshl_or_b32 v82, v119, 16, v118
	ds_load_u8 v118, v93 offset:13184
	ds_load_u8 v119, v93 offset:12928
	ds_load_u8 v130, v93 offset:12672
	ds_load_u8 v131, v93 offset:12416
	v_perm_b32 v93, v97, v96, 0xc0c0004
	v_perm_b32 v97, v99, v98, 0xc0c0004
	v_perm_b32 v98, v105, v104, 0xc0c0004
	v_perm_b32 v99, v103, v102, 0xc0c0004
	v_perm_b32 v102, v129, v128, 0xc0c0004
	v_lshl_or_b32 v96, v94, 16, v93
	s_waitcnt lgkmcnt(11)
	v_perm_b32 v37, v150, v149, 0xc0c0004
	v_lshl_or_b32 v95, v97, 16, v95
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v81, v155, v154, 0xc0c0004
	v_lshl_or_b32 v94, v99, 16, v98
	v_lshl_or_b32 v93, v101, 16, v100
	v_perm_b32 v97, v115, v114, 0xc0c0004
	v_perm_b32 v98, v123, v122, 0xc0c0004
	v_lshl_or_b32 v81, v81, 16, v37
	v_perm_b32 v37, v117, v116, 0xc0c0004
	v_perm_b32 v99, v121, v120, 0xc0c0004
	v_perm_b32 v100, v127, v126, 0xc0c0004
	v_perm_b32 v101, v125, v124, 0xc0c0004
	v_perm_b32 v103, v113, v112, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[69:76], v[93:96], v[77:80], v[69:76] neg_lo:[1,1,0]
	v_lshl_or_b32 v80, v97, 16, v37
	v_lshl_or_b32 v79, v99, 16, v98
	v_lshl_or_b32 v78, v101, 16, v100
	v_lshl_or_b32 v77, v103, 16, v102
	v_perm_b32 v37, v137, v136, 0xc0c0004
	v_perm_b32 v93, v135, v134, 0xc0c0004
	v_perm_b32 v94, v143, v142, 0xc0c0004
	v_perm_b32 v95, v141, v140, 0xc0c0004
	v_perm_b32 v96, v147, v146, 0xc0c0004
	v_perm_b32 v97, v145, v144, 0xc0c0004
	v_perm_b32 v98, v133, v132, 0xc0c0004
	v_perm_b32 v99, v111, v110, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[69:76], v[77:80], v[38:41], v[69:76] neg_lo:[1,1,0]
	v_lshl_or_b32 v40, v93, 16, v37
	v_lshl_or_b32 v39, v95, 16, v94
	v_lshl_or_b32 v38, v97, 16, v96
	v_lshl_or_b32 v37, v99, 16, v98
	v_perm_b32 v41, v153, v152, 0xc0c0004
	v_perm_b32 v77, v151, v139, 0xc0c0004
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v78, v159, v158, 0xc0c0004
	v_perm_b32 v79, v157, v156, 0xc0c0004
	v_perm_b32 v80, v165, v164, 0xc0c0004
	v_perm_b32 v93, v163, v162, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v94, v131, v130, 0xc0c0004
	v_perm_b32 v95, v119, v118, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[69:76], v[37:40], v[85:88], v[69:76] neg_lo:[1,1,0]
	v_lshl_or_b32 v40, v77, 16, v41
	v_lshl_or_b32 v39, v79, 16, v78
	v_lshl_or_b32 v38, v93, 16, v80
	v_lshl_or_b32 v37, v95, 16, v94
	v_wmma_i32_16x16x16_iu8 v[61:68], v[81:84], v[89:92], v[61:68] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_wmma_i32_16x16x16_iu8 v[69:76], v[37:40], v[89:92], v[69:76] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v41, v61
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_cvt_f32_i32_e32 v62, v62
	v_cvt_f32_i32_e32 v63, v63
	v_cvt_f32_i32_e32 v64, v64
	v_cvt_f32_i32_e32 v65, v65
	v_cvt_f32_i32_e32 v66, v66
	v_cvt_f32_i32_e32 v67, v67
	v_cvt_f32_i32_e32 v68, v68
	v_cvt_f32_i32_e32 v69, v69
	v_cvt_f32_i32_e32 v70, v70
	v_cvt_f32_i32_e32 v71, v71
	v_cvt_f32_i32_e32 v72, v72
	v_cvt_f32_i32_e32 v73, v73
	v_cvt_f32_i32_e32 v74, v74
	v_cvt_f32_i32_e32 v75, v75
	v_cvt_f32_i32_e32 v76, v76
.LBB0_12:
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s1, s34, 31
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s26, s10
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_lshr_b32 s1, s1, 24
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s27, s11
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_add_i32 s1, s34, s1
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	v_mul_lo_u32 v17, s23, v17
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s1, s1, 8
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v37.l, 0
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s1, s0
	.loc	1 392 40                        ; generate_amdgcn.py:392:40
	s_mul_i32 s0, s1, s23
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v36, v36, s1, 1
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v35, v35, s0, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 s0, -1, 0
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	s_mul_i32 s1, s22, s23
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s2, s0
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_mov_b16_e32 v37.h, v60.l
	v_dual_cndmask_b32 v36, 0x80000000, v36 :: v_dual_cndmask_b32 v35, 0x80000000, v35
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v34, 1, v34
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	v_add3_u32 v17, s1, s33, v17
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	buffer_load_u16 v35, v35, s[24:27], 0 offen
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	buffer_load_u16 v36, v36, s[8:11], 0 offen
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_dual_mov_b32 v39, 0x5410 :: v_dual_and_b32 v38, 16, v0
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(0)
	v_mul_f32_e32 v0, v16, v37
	v_mul_f32_e32 v13, v13, v37
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	v_add_lshl_u32 v17, v17, v34, 1
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_dual_mov_b32 v40, 0x7632 :: v_dual_mul_f32 v15, v15, v37
	v_mov_b16_e32 v82.h, v37.l
	v_mov_b16_e32 v86.h, v37.l
	v_mov_b16_e32 v81.h, v37.l
	v_mov_b16_e32 v85.h, v37.l
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v15, v15, v56, v33
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_eq_u32_e32 vcc_lo, 0, v38
	v_mov_b16_e32 v78.h, v37.l
	v_mov_b16_e32 v84.h, v37.l
	v_mov_b16_e32 v77.h, v37.l
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v33, v33, v15, s3
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v83.h, v37.l
	v_mov_b16_e32 v16.h, v37.l
	v_mov_b16_e32 v80.h, v37.l
	v_mov_b16_e32 v79.h, v37.l
	v_mov_b16_e32 v60.h, v37.l
	v_mov_b16_e32 v61.h, v37.l
	v_mov_b16_e32 v87.h, v37.l
	v_mov_b16_e32 v88.h, v37.l
	s_mov_b32 s0, 0x76543210
	s_mov_b32 s22, s10
	s_mov_b32 s23, s11
	s_and_b32 s21, s21, 0xffff
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt vmcnt(0)
	v_dual_mul_f32 v1, v1, v37 :: v_dual_lshlrev_b32 v34, 16, v36
	v_mul_f32_e32 v7, v7, v37
	v_mul_f32_e32 v6, v6, v37
	v_mul_f32_e32 v5, v5, v37
	v_mul_f32_e32 v4, v4, v37
	v_mul_f32_e32 v3, v3, v37
	v_mul_f32_e32 v2, v2, v37
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v46, v1, v46, v18
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v1, 16, v35
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v0, v0, v57, v32
	v_fma_f32 v48, v7, v48, v24
	v_fma_f32 v47, v6, v47, v23
	v_fma_f32 v45, v5, v45, v22
	v_fma_f32 v44, v4, v44, v21
	v_fma_f32 v43, v3, v43, v20
	v_fma_f32 v42, v2, v42, v19
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v32, v32, v0, s3
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	ds_store_b32 v59, v1 offset:34816
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b128 v[0:3], v58 offset:34816
	ds_load_b128 v[4:7], v58 offset:34832
	v_mul_f32_e32 v14, v14, v37
	v_mul_f32_e32 v12, v12, v37
	v_mul_f32_e32 v11, v11, v37
	v_mul_f32_e32 v10, v10, v37
	v_mul_f32_e32 v9, v9, v37
	v_mul_f32_e32 v8, v8, v37
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v13, v13, v54, v30
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v19, v19, v42, s3
	v_cndmask_b32_e64 v18, v18, v46, s3
	v_cndmask_b32_e64 v24, v24, v48, s3
	v_cndmask_b32_e64 v22, v22, v45, s3
	v_cndmask_b32_e64 v30, v30, v13, s3
	v_cndmask_b32_e64 v23, v23, v47, s3
	v_cndmask_b32_e64 v20, v20, v43, s3
	v_cndmask_b32_e64 v21, v21, v44, s3
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v37.h, v37.l
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(1)
	v_mul_f32_e32 v0, v0, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v11, v11, v52, v28
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v1, v1, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v14, v14, v55, v31
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v3, v3, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v12, v12, v53, v29
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(0)
	v_mul_f32_e32 v5, v5, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v10, v10, v51, v27
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v31, v31, v14, s3
	v_cndmask_b32_e64 v28, v28, v11, s3
	v_cndmask_b32_e64 v29, v29, v12, s3
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	ds_load_b128 v[12:15], v58 offset:35344
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v27, v27, v10, s3
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v2, v2, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v50, v9, v50, v26
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v7, v7, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v49, v8, v49, v25
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	ds_load_b128 v[8:11], v58 offset:35328
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v1, v1, v62, v19
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v26, v26, v50, s3
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v0, v0, v41, v18
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v6, v6, v34
	v_mul_f32_e32 v4, v4, v34
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v1, v19, v1, s2
	v_cndmask_b32_e64 v25, v25, v49, s3
	v_cndmask_b32_e64 v0, v18, v0, s2
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v6, v6, v67, v24
	v_fma_f32 v4, v4, v65, v22
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_max_f32_e32 v1, v1, v1
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v2, v2, v63, v20
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	s_waitcnt lgkmcnt(1)
	v_dual_max_f32 v0, v0, v0 :: v_dual_mul_f32 v15, v15, v34
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v12, v12, v34
	v_mul_f32_e32 v13, v13, v34
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v6, v24, v6, s2
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v5, v5, v66, v23
	v_fma_f32 v15, v15, v76, v32
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt lgkmcnt(0)
	v_mul_f32_e32 v8, v8, v34
	v_mul_f32_e32 v9, v9, v34
	v_mul_f32_e32 v10, v10, v34
	v_mul_f32_e32 v11, v11, v34
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v12, v12, v73, v30
	v_fma_f32 v8, v8, v69, v26
	v_fma_f32 v9, v9, v70, v27
	v_fma_f32 v10, v10, v71, v28
	v_fma_f32 v13, v13, v74, v31
	v_fma_f32 v11, v11, v72, v29
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v8, v26, v8, s2
	v_cndmask_b32_e64 v12, v30, v12, s2
	v_cndmask_b32_e64 v9, v27, v9, s2
	v_cndmask_b32_e64 v10, v28, v10, s2
	v_cndmask_b32_e64 v13, v31, v13, s2
	v_cndmask_b32_e64 v11, v29, v11, s2
	v_cndmask_b32_e64 v15, v32, v15, s2
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_max_f32_e32 v8, v8, v8
	v_max_f32_e32 v12, v12, v12
	v_dual_max_f32 v6, v6, v6 :: v_dual_max_f32 v9, v9, v9
	v_dual_max_f32 v10, v10, v10 :: v_dual_max_f32 v13, v13, v13
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v3, v3, v64, v21
	v_fma_f32 v7, v7, v68, v25
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v4, v22, v4, s2
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_max_f32_e32 v11, v11, v11
	v_dual_max_f32 v15, v15, v15 :: v_dual_max_f32 v8, 0, v8
	v_max_f32_e32 v12, 0, v12
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v2, v20, v2, s2
	v_cndmask_b32_e64 v5, v23, v5, s2
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v6, 0, v6 :: v_dual_max_f32 v9, 0, v9
	v_dual_max_f32 v10, 0, v10 :: v_dual_max_f32 v13, 0, v13
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v3, v21, v3, s2
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v14, v14, v34
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v7, v25, v7, s2
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v4, v4, v4 :: v_dual_max_f32 v11, 0, v11
	v_dual_max_f32 v15, 0, v15 :: v_dual_mul_f32 v8, v8, v8
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_mul_f32_e32 v12, v12, v12
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v2, v2, v2 :: v_dual_max_f32 v5, v5, v5
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_dual_mul_f32 v6, v6, v6 :: v_dual_mul_f32 v9, v9, v9
	v_dual_mul_f32 v10, v10, v10 :: v_dual_mul_f32 v13, v13, v13
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_max_f32_e32 v3, v3, v3
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v14, v14, v75, v33
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v7, v7, v7 :: v_dual_max_f32 v4, 0, v4
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_mul_f32_e32 v11, v11, v11
	v_mul_f32_e32 v15, v15, v15
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v82.l, v8.h
	v_mov_b16_e32 v86.l, v12.h
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v2, 0, v2 :: v_dual_max_f32 v5, 0, v5
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v81.l, v9.h
	v_mov_b16_e32 v85.l, v13.h
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v14, v33, v14, s2
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v7, 0, v7 :: v_dual_mul_f32 v4, v4, v4
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v26, 1, v82
	v_and_b32_e32 v30, 1, v86
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_dual_mul_f32 v2, v2, v2 :: v_dual_mul_f32 v5, v5, v5
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v25, 1, v81
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v14, v14, v14 :: v_dual_and_b32 v29, 1, v85
	v_max_f32_e32 v1, 0, v1
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_mul_f32_e32 v7, v7, v7
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v78.l, v4.h
	v_add3_u32 v8, v8, v26, 0x7fff
	v_add3_u32 v12, v12, v30, 0x7fff
	v_mov_b16_e32 v84.l, v10.h
	v_mov_b16_e32 v77.l, v5.h
	v_add3_u32 v9, v9, v25, 0x7fff
	v_add3_u32 v13, v13, v29, 0x7fff
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v14, 0, v14 :: v_dual_mul_f32 v1, v1, v1
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v83.l, v11.h
	v_mov_b16_e32 v9.l, v8.h
	v_mov_b16_e32 v13.l, v12.h
	v_and_b32_e32 v22, 1, v78
	.loc	1 413 15                        ; generate_amdgcn.py:413:15
	v_dual_max_f32 v0, 0, v0 :: v_dual_max_f32 v3, 0, v3
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v28, 1, v84
	v_and_b32_e32 v21, 1, v77
	v_mov_b16_e32 v16.l, v1.h
	v_dual_cndmask_b32 v8, v9, v13 :: v_dual_and_b32 v27, 1, v83
	v_add3_u32 v4, v4, v22, 0x7fff
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_dual_mul_f32 v0, v0, v0 :: v_dual_mul_f32 v3, v3, v3
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v80.l, v6.h
	v_add3_u32 v10, v10, v28, 0x7fff
	v_add3_u32 v5, v5, v21, 0x7fff
	v_mov_b16_e32 v79.l, v7.h
	v_add3_u32 v11, v11, v27, 0x7fff
	v_mov_b16_e32 v5.l, v4.h
	v_and_b32_e32 v16, 1, v16
	v_mov_b16_e32 v60.l, v3.h
	v_mov_b16_e32 v11.l, v10.h
	v_and_b32_e32 v24, 1, v80
	.loc	1 414 15                        ; generate_amdgcn.py:414:15
	v_mul_f32_e32 v14, v14, v14
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_add3_u32 v1, v1, v16, 0x7fff
	v_and_b32_e32 v23, 1, v79
	v_mov_b16_e32 v61.l, v2.h
	v_and_b32_e32 v19, 1, v60
	v_add3_u32 v6, v6, v24, 0x7fff
	v_mov_b16_e32 v37.l, v0.h
	v_add3_u32 v7, v7, v23, 0x7fff
	v_mov_b16_e32 v88.l, v14.h
	v_add3_u32 v3, v3, v19, 0x7fff
	v_mov_b16_e32 v7.l, v6.h
	v_and_b32_e32 v20, 1, v61
	v_mov_b16_e32 v87.l, v15.h
	v_cndmask_b32_e32 v4, 0x1054, v39, vcc_lo
	v_cndmask_b32_e32 v6, 0x3276, v40, vcc_lo
	v_and_b32_e32 v32, 1, v88
	v_add3_u32 v2, v2, v20, 0x7fff
	v_and_b32_e32 v31, 1, v87
	v_lshl_or_b32 v4, v4, 8, v4
	v_lshl_or_b32 v6, v6, 8, v6
	v_add3_u32 v14, v14, v32, 0x7fff
	v_mov_b16_e32 v3.l, v2.h
	v_and_b32_e32 v18, 1, v37
	v_and_b32_e32 v4, 0x540054, v4
	v_and_b32_e32 v6, 0x760076, v6
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_add3_u32 v0, v0, v18, 0x7fff
	v_lshl_or_b32 v4, v4, 4, v4
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_lshl_or_b32 v6, v6, 4, v6
	v_mov_b16_e32 v1.l, v0.h
	v_add3_u32 v0, v15, v31, 0x7fff
	v_mov_b16_e32 v0.l, v14.h
	v_and_b32_e32 v10, 0x5040504, v4
	s_delay_alu instid0(VALU_DEP_4)
	v_cndmask_b32_e32 v2, v5, v1, vcc_lo
	v_cndmask_b32_e32 v1, v1, v5, vcc_lo
	v_cndmask_b32_e32 v5, v7, v3, vcc_lo
	v_cndmask_b32_e32 v3, v3, v7, vcc_lo
	v_cndmask_b32_e32 v7, v13, v9, vcc_lo
	v_dual_cndmask_b32 v9, v0, v11 :: v_dual_cndmask_b32 v0, v11, v0
	v_permlanex16_b32 v1, v1, s0, 0xfedcba98 op_sel:[1,0]
	v_and_b32_e32 v11, 0x7060706, v6
	v_permlanex16_b32 v3, v3, s0, 0xfedcba98 op_sel:[1,0]
	v_permlanex16_b32 v6, v8, s0, 0xfedcba98 op_sel:[1,0]
	v_permlanex16_b32 v8, v0, s0, 0xfedcba98 op_sel:[1,0]
	v_perm_b32 v0, v1, v2, v10
	v_perm_b32 v1, v1, v2, v11
	v_perm_b32 v2, v3, v5, v10
	v_perm_b32 v3, v3, v5, v11
	v_perm_b32 v4, v6, v7, v10
	v_perm_b32 v5, v6, v7, v11
	v_perm_b32 v6, v8, v9, v10
	v_perm_b32 v7, v8, v9, v11
	s_clause 0x1
	buffer_store_b128 v[0:3], v17, s[20:23], 0 offen
	buffer_store_b128 v[4:7], v17, s[20:23], 0 offen offset:256
	.loc	1 309 1                         ; generate_amdgcn.py:309:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp24:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 45
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 63
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk, .Lfunc_end0-amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_vgpr, 212
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.numbered_sgpr, 45
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 11284
; TotalNumSgprs: 47
; NumVgprs: 212
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 47
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	5                               ; DW_FORM_data2
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x6c DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x46 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	317                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4e:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	316                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x5b:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	387                             ; DW_AT_call_line
	.byte	30                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x68:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges3                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	333                             ; DW_AT_call_line
	.byte	28                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp16-.Lfunc_begin0
	.quad	.Ltmp17-.Lfunc_begin0
	.quad	.Ltmp18-.Lfunc_begin0
	.quad	.Ltmp19-.Lfunc_begin0
	.quad	.Ltmp20-.Lfunc_begin0
	.quad	.Ltmp21-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges3:
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	.Ltmp22-.Lfunc_begin0
	.quad	.Ltmp23-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk"             ; string offset=75 ; amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
    .private_segment_fixed_size: 0
    .sgpr_count:     47
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int8xint8_nn_sc256_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     212
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
