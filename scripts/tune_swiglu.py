...
def parse_tile(value: str) -> TileConfig:
    ...
    return TileConfig(block_m, block_n, block_k, group_m, warps, stages, waves, even_k=True, split_k=split[0] if split else 1)
...
def default_tiles(dtype: OperandDType) -> tuple[TileConfig, ...]:
    ...
    seeded = seed_tile_configs(seed_dtype, epilogue=Epilogue.SWIGLU)[0]
...
def make_kernel(dtype: OperandDType, scale: ScaleSpec, tile: TileConfig, shape: BenchmarkShape) -> KernelMetadata:
    ...
    kernel_id = (
        make_mixed_kernel_id(dtype, b_dtype, scale, Epilogue.SWIGLU, even_tile)
        if dtype is OperandDType.BF16
        else make_kernel_id(dtype, scale, Epilogue.SWIGLU, even_tile)
    )
...
def make_inputs(kernel: KernelMetadata, shape: BenchmarkShape) -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    ...
    b_bf16 = torch.randn((shape.k, shape.n * 2), device="cuda", dtype=torch.bfloat16) * 0.1
    ...
    if kernel.scale.mode is ScaleMode.PER_CHANNEL:
        ...
        b_scale = torch.linspace(1.10, 0.90, shape.n * 2, device="cuda", dtype=torch.bfloat16)
        ...
        b_scale = torch.linspace(1.15, 0.85, shape.n * 2 * scale_cols, device="cuda", dtype=torch.bfloat16).reshape(
            scale_cols, shape.n * 2
        )
        gemm = torch.zeros((shape.m, shape.n * 2), device="cuda", dtype=torch.float32)
    ...
    up, gate_values = gemm.split(shape.n, dim=1)
    expected = up * torch.nn.functional.silu(gate_values)
...
def benchmark_kernel(
    ...
    *,
    ...
    samples = tuple(float(sample) for sample in do_bench(lambda: launch(kernel, shape, args), warmup=warmup_ms, rep=rep_ms, return_mode="all"))
    summary = summarize_runtime_samples(samples)
    runtime_ms = float(summary["runtime_ms_median"])
    tops = tops_for_runtime(shape, runtime_ms) * 2.0
    return {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Tune direct Triton generated SwiGLU kernels on a real shape.")
    ...
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "benchmarks" / "swiglu_tuning.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not torch.cuda.is_available() or torch.version.hip is None:
        raise RuntimeError("SwiGLU tuning requires a ROCm torch CUDA/HIP device")
    ...
    for scale in scales:
        for tile in tiles:
            ...
            print(f"benchmark {kernel.kernel_id} shape={args.shape.m},{args.shape.n},{args.shape.k}", flush=True)
            try:
                records.append(benchmark_kernel(kernel, args.shape, warmup_ms=args.warmup_ms, rep_ms=args.rep_ms))
            except Exception as exc:
                failures.append({"kernel_id": kernel.kernel_id, "error": repr(exc)})
                print(f"FAILED {kernel.kernel_id}: {exc!r}", flush=True)

    if not records:
        raise RuntimeError("no successful tuning records")
    best = max(records, key=lambda record: record["tops"])
    best_kernel = next(
        make_kernel(args.dtype, scale, tile, args.shape)
        for scale in scales
        for tile in tiles
        if make_kernel(args.dtype, scale, tile, args.shape).kernel_id == best["kernel_id"]
    )
    artifacts = save_artifacts(best_kernel, args.shape, args.artifact_dir) if args.save_best_artifacts else {}
    payload = {
        "shape": args.shape.to_dict(),
        "dtype": args.dtype.value,
        "records": sorted(records, key=lambda record: record["tops"], reverse=True),
        "failures": failures,
        "best_kernel_id": best["kernel_id"],
        "best_tops": best["tops"],
    ...
    print(args.output)
    print(f"best {best['kernel_id']} {best['runtime_ms']:.6f} ms {best['tops']:.3f} TOPS")
    print(f"mean {mean(record['tops'] for record in records):.3f} TOPS over {len(records)} successful configs")
    if artifacts:
