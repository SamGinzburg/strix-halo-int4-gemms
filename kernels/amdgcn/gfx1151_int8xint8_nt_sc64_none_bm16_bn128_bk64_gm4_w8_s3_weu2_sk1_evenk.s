	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk               ; -- Begin function amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk,@function
amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk:                      ; @amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 309 0                         ; generate_amdgcn.py:309:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[14:15], s[0:1], 0x30
	s_load_b32 s12, s[0:1], 0x38
.Ltmp0:
	.loc	1 319 16 prologue_end           ; generate_amdgcn.py:319:16
	s_abs_i32 s6, s2
	.loc	1 327 14                        ; generate_amdgcn.py:327:14
	v_lshlrev_b32_e32 v5, 4, v0
	.loc	1 325 32                        ; generate_amdgcn.py:325:32
	v_lshrrev_b32_e32 v4, 4, v0
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v3, 2, v0
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 327 14                        ; generate_amdgcn.py:327:14
	v_and_b32_e32 v2, 48, v5
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:317:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s15, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s7, s14, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s9, s7, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_lshr_b32 s5, s5, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_lshr_b32 s9, s9, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s7, s7, s9
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s4, s4, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s7, s7, 4
.Ltmp11:
	.loc	1 318 24 is_stmt 1              ; generate_amdgcn.py:318:24
	s_lshl_b32 s13, s4, 2
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_mul_lo_u32 v7, s12, v4
	.loc	1 319 16                        ; generate_amdgcn.py:319:16
	s_abs_i32 s4, s13
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	v_mad_u64_u32 v[9:10], null, s12, v3, v[2:3]
	.loc	1 319 16                        ; generate_amdgcn.py:319:16
	s_cvt_f32_u32 s5, s4
	s_sub_i32 s8, 0, s4
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(SKIP_1) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s5
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	v_lshl_add_u32 v36, s12, 6, v9
	.loc	1 319 16                        ; generate_amdgcn.py:319:16
	v_readfirstlane_b32 s5, v1
	s_mul_f32 s5, s5, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s5, s5
	s_mul_i32 s8, s8, s5
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s8, s5, s8
	s_add_i32 s5, s5, s8
	s_xor_b32 s8, s2, s13
	s_mul_hi_u32 s5, s6, s5
	s_ashr_i32 s8, s8, 31
	s_mul_i32 s9, s5, s4
	s_delay_alu instid0(SALU_CYCLE_1)
	s_sub_i32 s6, s6, s9
	s_add_i32 s9, s5, 1
	s_sub_i32 s10, s6, s4
	s_cmp_ge_u32 s6, s4
	s_cselect_b32 s5, s9, s5
	s_cselect_b32 s6, s10, s6
	s_add_i32 s9, s5, 1
	s_cmp_ge_u32 s6, s4
	s_cselect_b32 s4, s9, s5
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s4, s4, s8
	s_sub_i32 s14, s4, s8
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 320 19                        ; generate_amdgcn.py:320:19
	s_lshl_b32 s16, s14, 2
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_mul_i32 s14, s14, s13
	.loc	1 321 31                        ; generate_amdgcn.py:321:31
	s_sub_i32 s4, s7, s16
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_sub_i32 s2, s2, s14
	.loc	1 321 20                        ; generate_amdgcn.py:321:20
	s_min_i32 s17, s4, 4
	s_load_b256 s[4:11], s[0:1], 0x0
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_abs_i32 s18, s17
	s_xor_b32 s13, s2, s17
	s_cvt_f32_u32 s19, s18
	s_ashr_i32 s13, s13, 31
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s19
	v_readfirstlane_b32 s19, v1
	.loc	1 325 32                        ; generate_amdgcn.py:325:32
	v_and_b32_e32 v1, 15, v0
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[20:21], s[6:7]
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_sub_i32 s6, 0, s18
	s_mul_f32 s19, s19, 0x4f7ffffe
	s_abs_i32 s7, s2
	.loc	1 327 14                        ; generate_amdgcn.py:327:14
	v_lshlrev_b32_e32 v6, 2, v1
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_cvt_u32_f32 s19, s19
	s_mul_i32 s6, s6, s19
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s6, s19, s6
	s_add_i32 s19, s19, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s6, s7, s19
	s_mul_i32 s14, s6, s18
	s_add_i32 s19, s6, 1
	s_sub_i32 s14, s7, s14
	s_mov_b32 s7, 0x31027000
	s_sub_i32 s22, s14, s18
	s_cmp_ge_u32 s14, s18
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_mov_b32 s23, s7
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_cselect_b32 s6, s19, s6
	s_cselect_b32 s14, s22, s14
	s_add_i32 s19, s6, 1
	s_cmp_ge_u32 s14, s18
	s_cselect_b32 s14, s19, s6
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:333:28 ]
	s_add_i32 s26, s12, 63
.Ltmp13:
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_xor_b32 s14, s14, s13
	.loc	1 368 22                        ; generate_amdgcn.py:368:22
	s_lshl_b32 s18, s3, 6
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_sub_i32 s13, s14, s13
	s_mov_b32 s6, 0x7ffffffe
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_mul_i32 s14, s13, s17
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_mov_b32 s22, s6
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_sub_i32 s2, s2, s14
	.loc	1 326 14                        ; generate_amdgcn.py:326:14
	s_lshl_b32 s14, s13, 7
	.loc	1 322 13                        ; generate_amdgcn.py:322:13
	s_add_i32 s2, s2, s16
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	s_mul_i32 s35, s14, s12
	.loc	1 325 14                        ; generate_amdgcn.py:325:14
	s_lshl_b32 s24, s2, 4
.Ltmp14:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:333:28 ]
	s_ashr_i32 s2, s26, 31
.Ltmp15:
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	s_mul_i32 s3, s24, s12
.Ltmp16:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:333:28 ]
	s_lshr_b32 s2, s2, 26
.Ltmp17:
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add3_u32 v37, v7, v6, s3
.Ltmp18:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:333:28 ]
	s_add_i32 s25, s26, s2
.Ltmp19:
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_gt_i32 s26, 63
	s_mov_b32 s19, 0
	s_cselect_b32 s3, -1, 0
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	s_add_i32 s2, s35, s18
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v2, s18, v37
	.loc	1 374 22 is_stmt 0              ; generate_amdgcn.py:374:22
	s_and_b32 s5, s5, 0xffff
	.loc	1 376 22 is_stmt 1              ; generate_amdgcn.py:376:22
	s_and_b32 s21, s21, 0xffff
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmpk_gt_i32 s26, 0x7f
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	v_add_nc_u32_e32 v3, s2, v9
	v_add_nc_u32_e32 v6, s2, v36
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s2, -1, 0
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s33, s18, 64
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_cndmask_b32_e64 v2, 0x80000000, v2, s3
	.loc	1 374 30 is_stmt 0              ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v7, s33, v37
	.loc	1 376 22 is_stmt 1              ; generate_amdgcn.py:376:22
	v_cndmask_b32_e64 v8, 0x80000000, v3, s3
	.loc	1 376 30 is_stmt 0              ; generate_amdgcn.py:376:30
	v_add_nc_u32_e32 v3, 64, v3
	v_add_nc_u32_e32 v10, 64, v6
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_cndmask_b32_e64 v6, 0x80000000, v6, s3
	.loc	1 374 22 is_stmt 1              ; generate_amdgcn.py:374:22
	buffer_load_b32 v29, v2, s[4:7], 0 offen
	v_cndmask_b32_e64 v2, 0x80000000, v7, s2
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_cndmask_b32_e64 v3, 0x80000000, v3, s2
	v_cndmask_b32_e64 v7, 0x80000000, v10, s2
	s_clause 0x1
	buffer_load_b128 v[13:16], v8, s[20:23], 0 offen
	buffer_load_b128 v[17:20], v6, s[20:23], 0 offen
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v30, v2, s[4:7], 0 offen
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_clause 0x1
	buffer_load_b128 v[21:24], v3, s[20:23], 0 offen
	buffer_load_b128 v[25:28], v7, s[20:23], 0 offen
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_and_b32_e32 v2, 0xe0, v0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_lshrrev_b32_e32 v3, 1, v0
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_lshlrev_b32_e32 v6, 1, v0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_lshlrev_b32_e32 v8, 2, v0
	s_mov_b64 s[16:17], s[10:11]
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v7, 1, v2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_and_b32_e32 v10, 48, v3
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_and_b32_e32 v6, 48, v6
	v_lshlrev_b32_e32 v3, 3, v0
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmpk_gt_i32 s26, 0xbf
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_and_or_b32 v4, v4, 1, v7
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_xor_b32_e32 v38, v8, v10
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_xor_b32_e32 v39, v5, v6
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_or_b32_e32 v8, 2, v4
	v_or_b32_e32 v10, 4, v4
	v_or_b32_e32 v11, 6, v4
	v_or_b32_e32 v12, 8, v4
	v_or_b32_e32 v5, 10, v4
	v_or_b32_e32 v6, 12, v4
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v31, 0, v38
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_or_b32_e32 v7, 14, v4
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v32, 0, v39
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt vmcnt(5)
	ds_store_b32 v31, v29 offset:16384
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_waitcnt vmcnt(4)
	ds_store_b128 v32, v[13:16]
	s_waitcnt vmcnt(3)
	ds_store_b128 v32, v[17:20] offset:4096
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(2)
	ds_store_b32 v31, v30 offset:17408
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v32, v[21:24] offset:8192
	s_waitcnt vmcnt(0)
	ds_store_b128 v32, v[25:28] offset:12288
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_2
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_and_b32_e32 v13, 48, v3
	s_mov_b32 s10, 0
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v28, v1, 6, v13
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_lshl_or_b32 v33, v2, 5, v28
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_xor_b32_e32 v31, 16, v28
	v_xor_b32_e32 v29, 32, v28
	v_xor_b32_e32 v30, 48, v28
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_xor_b32_e32 v35, 16, v33
	v_xor_b32_e32 v32, 32, v33
	v_xor_b32_e32 v34, 48, v33
	s_branch .LBB0_3
.LBB0_2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	s_mov_b32 s10, -1
                                        ; implicit-def: $vgpr28
                                        ; implicit-def: $vgpr31
                                        ; implicit-def: $vgpr29
                                        ; implicit-def: $vgpr30
                                        ; implicit-def: $vgpr33
                                        ; implicit-def: $vgpr35
                                        ; implicit-def: $vgpr32
                                        ; implicit-def: $vgpr34
.LBB0_3:                                ; %Flow73
	s_load_b64 s[12:13], s[0:1], 0x20
	v_or_b32_e32 v18, s24, v1
	s_ashr_i32 s0, s25, 6
	v_or_b32_e32 v26, s14, v4
	v_or_b32_e32 v25, s14, v8
	v_or_b32_e32 v24, s14, v10
	v_mul_lo_u32 v27, v18, s0
	v_or_b32_e32 v22, s14, v11
	v_or_b32_e32 v20, s14, v12
	v_or_b32_e32 v23, s14, v5
	v_or_b32_e32 v21, s14, v6
	v_or_b32_e32 v19, s14, v7
	s_and_not1_b32 vcc_lo, exec_lo, s10
	s_cbranch_vccnz .LBB0_6
; %bb.4:                                ; %.lr.ph
	v_dual_mov_b32 v10, 0 :: v_dual_and_b32 v3, 48, v3
	s_mov_b32 s27, 0x31027000
	s_mov_b32 s26, 0x7ffffffe
	.loc	1 333 5 is_stmt 1               ; generate_amdgcn.py:333:5
	s_max_i32 s36, s0, 3
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_3) | instid1(VALU_DEP_2)
	v_lshl_or_b32 v28, v1, 6, v3
	v_dual_mov_b32 v1, 0 :: v_dual_mov_b32 v14, 0
	s_add_i32 s11, s33, 64
	s_and_b32 s25, s9, 0xffff
	v_lshl_or_b32 v33, v2, 5, v28
	v_xor_b32_e32 v31, 16, v28
	v_xor_b32_e32 v29, 32, v28
	v_xor_b32_e32 v30, 48, v28
	v_dual_mov_b32 v2, v1 :: v_dual_mov_b32 v17, 0
	v_xor_b32_e32 v35, 16, v33
	v_xor_b32_e32 v32, 32, v33
	v_xor_b32_e32 v34, 48, v33
	v_dual_mov_b32 v3, v1 :: v_dual_mov_b32 v16, 0
	v_dual_mov_b32 v4, v1 :: v_dual_mov_b32 v13, 0
	v_dual_mov_b32 v5, v1 :: v_dual_mov_b32 v12, 0
	v_dual_mov_b32 v6, v1 :: v_dual_mov_b32 v15, 0
	v_mov_b32_e32 v7, v1
	v_dual_mov_b32 v8, v1 :: v_dual_mov_b32 v11, 0
	s_mov_b32 s24, s8
	s_and_b32 s29, s17, 0xffff
	s_mov_b32 s28, s16
	s_add_i32 s10, 0, 0x4000
	s_add_i32 s1, 0, 0x4400
	s_add_i32 s34, 0, 0x2000
	s_add_i32 s36, s36, -3
	s_mov_b32 s37, 1
	s_mov_b32 s22, s6
	s_mov_b32 s23, s7
	s_mov_b32 s30, s26
	s_mov_b32 s31, s27
	s_mov_b32 s38, 0
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 0 5 is_stmt 0                 ; generate_amdgcn.py:0:5
	s_mov_b32 s39, s18
	.loc	1 334 14 is_stmt 1              ; generate_amdgcn.py:334:14
	s_lshl_b32 s40, s38, 6
	s_mov_b32 s18, s33
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s33, s11, s40
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s40, s39, 31
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v40, s33, v37
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_lshr_b32 s40, s40, 26
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	s_add_i32 s41, s33, s35
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_add_i32 s39, s39, s40
	s_delay_alu instid0(SALU_CYCLE_1)
	s_ashr_i32 s39, s39, 6
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v88, v40, s[4:7], 0 offen
	.loc	1 392 40                        ; generate_amdgcn.py:392:40
	s_mul_i32 s40, s39, s15
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v40, v27, s39, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s39, s0
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v41, v26, s40, 1
	v_add_lshl_u32 v42, v25, s40, 1
	v_add_lshl_u32 v43, v24, s40, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 vcc_lo, -1, 0
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v44, v22, s40, 1
	v_add_lshl_u32 v45, v20, s40, 1
	v_add_lshl_u32 v46, v23, s40, 1
	v_add_lshl_u32 v47, v21, s40, 1
	v_add_lshl_u32 v48, v19, s40, 1
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_dual_cndmask_b32 v40, 0x80000000, v40 :: v_dual_cndmask_b32 v41, 0x80000000, v41
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_dual_cndmask_b32 v42, 0x80000000, v42 :: v_dual_cndmask_b32 v43, 0x80000000, v43
	v_dual_cndmask_b32 v44, 0x80000000, v44 :: v_dual_cndmask_b32 v45, 0x80000000, v45
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_dual_cndmask_b32 v46, 0x80000000, v46 :: v_dual_cndmask_b32 v47, 0x80000000, v47
	v_cndmask_b32_e32 v48, 0x80000000, v48, vcc_lo
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	buffer_load_u16 v89, v40, s[24:27], 0 offen
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_clause 0x7
	buffer_load_u16 v90, v41, s[28:31], 0 offen
	buffer_load_u16 v91, v42, s[28:31], 0 offen
	buffer_load_u16 v92, v43, s[28:31], 0 offen
	buffer_load_u16 v93, v44, s[28:31], 0 offen
	buffer_load_u16 v94, v45, s[28:31], 0 offen
	buffer_load_u16 v95, v46, s[28:31], 0 offen
	buffer_load_u16 v96, v47, s[28:31], 0 offen
	buffer_load_u16 v97, v48, s[28:31], 0 offen
	.loc	1 376 30                        ; generate_amdgcn.py:376:30
	v_add_nc_u32_e32 v40, s41, v9
	v_add_nc_u32_e32 v41, s41, v36
	.loc	1 376 22 is_stmt 0              ; generate_amdgcn.py:376:22
	s_clause 0x1
	buffer_load_b128 v[48:51], v40, s[20:23], 0 offen
	buffer_load_b128 v[52:55], v41, s[20:23], 0 offen
	s_mov_b32 s39, s10
	s_mov_b32 s10, s1
	.loc	1 374 22 is_stmt 1              ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v40, s39, v28
	v_add_nc_u32_e32 v41, s39, v31
	s_mov_b32 s1, s19
	v_add_nc_u32_e32 v42, s39, v29
	v_add_nc_u32_e32 v43, s39, v30
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v44, s1, v33
	v_add_nc_u32_e32 v45, s1, v35
	v_add_nc_u32_e32 v46, s1, v32
	v_add_nc_u32_e32 v47, s1, v34
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[56:59], v40
	ds_load_b128 v[60:63], v41
	ds_load_b128 v[64:67], v42
	ds_load_b128 v[68:71], v43
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	ds_load_b128 v[72:75], v44
	ds_load_b128 v[76:79], v45
	ds_load_b128 v[80:83], v46
	ds_load_b128 v[84:87], v47
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_add_i32 s1, s37, 1
	s_mov_b32 s19, s34
	s_cmp_lt_i32 s1, 2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s37, s1, 0
	s_add_i32 s39, s38, 1
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_lshl_b32 s1, s37, 10
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_lshl_b32 s34, s37, 13
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_add_i32 s40, s1, 0
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_add_i32 s34, s34, 0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_add_i32 s1, s40, 0x4000
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_lg_u32 s38, s36
	s_mov_b32 s38, s39
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_barrier
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_wmma_i32_16x16x16_iu8 v[40:47], v[72:75], v[56:59], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[40:47], v[76:79], v[60:63], v[40:47] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[40:47], v[80:83], v[64:67], v[40:47] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[40:47], v[84:87], v[68:71], v[40:47] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v46, v46
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cvt_f32_i32_e32 v42, v42
	v_cvt_f32_i32_e32 v45, v45
	s_delay_alu instid0(VALU_DEP_4)
	v_cvt_f32_i32_e32 v41, v41
	v_cvt_f32_i32_e32 v40, v40
	v_cvt_f32_i32_e32 v43, v43
	v_cvt_f32_i32_e32 v44, v44
	v_cvt_f32_i32_e32 v47, v47
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_waitcnt vmcnt(10)
	v_lshlrev_b32_e32 v56, 16, v89
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_waitcnt vmcnt(9)
	v_lshlrev_b32_e32 v57, 16, v90
	s_waitcnt vmcnt(7)
	v_lshlrev_b32_e32 v59, 16, v92
	s_waitcnt vmcnt(6)
	v_lshlrev_b32_e32 v60, 16, v93
	s_waitcnt vmcnt(5)
	v_lshlrev_b32_e32 v61, 16, v94
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	s_waitcnt vmcnt(4)
	v_dual_mul_f32 v57, v56, v57 :: v_dual_lshlrev_b32 v62, 16, v95
	s_waitcnt vmcnt(2)
	v_dual_mul_f32 v59, v56, v59 :: v_dual_lshlrev_b32 v64, 16, v97
	s_delay_alu instid0(VALU_DEP_2)
	v_dual_mul_f32 v62, v56, v62 :: v_dual_lshlrev_b32 v63, 16, v96
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v58, 16, v91
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v60, v56, v60
	v_mul_f32_e32 v61, v56, v61
	v_mul_f32_e32 v64, v56, v64
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_fmac_f32 v16, v59, v42 :: v_dual_fmac_f32 v13, v62, v45
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v58, v56, v58
	v_mul_f32_e32 v56, v56, v63
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v98, s40, v38
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_fmac_f32 v10, v57, v40 :: v_dual_add_nc_u32 v99, s34, v39
	s_delay_alu instid0(VALU_DEP_4)
	v_fmac_f32_e32 v17, v58, v41
	v_dual_fmac_f32 v15, v60, v43 :: v_dual_fmac_f32 v14, v61, v44
	v_fmac_f32_e32 v12, v56, v46
	v_fmac_f32_e32 v11, v64, v47
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_store_b32 v98, v88 offset:16384
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v99, v[48:51]
	s_waitcnt vmcnt(0)
	ds_store_b128 v99, v[52:55] offset:4096
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_5
	s_branch .LBB0_7
.LBB0_6:
	.loc	1 0 5 is_stmt 0                 ; generate_amdgcn.py:0:5
	v_dual_mov_b32 v11, 0 :: v_dual_mov_b32 v12, 0
	v_dual_mov_b32 v15, 0 :: v_dual_mov_b32 v16, 0
	v_dual_mov_b32 v13, 0 :: v_dual_mov_b32 v14, 0
	v_dual_mov_b32 v17, 0 :: v_dual_mov_b32 v10, 0
	s_add_i32 s10, 0, 0x4000
	s_add_i32 s1, 0, 0x4400
	s_add_i32 s34, 0, 0x2000
.LBB0_7:                                ; %Flow74
	v_dual_mov_b32 v1, 0 :: v_dual_and_b32 v36, 0xf0, v0
	v_dual_mov_b32 v6, 0 :: v_dual_mov_b32 v9, 0
	v_dual_mov_b32 v2, 0 :: v_dual_mov_b32 v3, 0
	v_dual_mov_b32 v4, 0 :: v_dual_mov_b32 v7, 0
	v_dual_mov_b32 v5, 0 :: v_dual_mov_b32 v8, 0
	.loc	1 379 19 is_stmt 1              ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s3
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_cbranch_vccnz .LBB0_9
; %bb.8:
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v2, s19, v35
	v_add_nc_u32_e32 v6, s19, v33
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v37, s10, v31
	v_add_nc_u32_e32 v38, s10, v28
	v_add_nc_u32_e32 v40, s10, v30
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	ds_load_b128 v[2:5], v2
	ds_load_b128 v[6:9], v6
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[45:48], v37
	ds_load_b128 v[49:52], v38
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v37, 0
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v39, s19, v34
	v_add_nc_u32_e32 v38, s19, v32
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v41, s10, v29
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v43, v37
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	ds_load_b128 v[53:56], v39
	ds_load_b128 v[57:60], v38
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[61:64], v40
	ds_load_b128 v[65:68], v41
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v38, v37
	v_mov_b32_e32 v39, v37
	v_mov_b32_e32 v40, v37
	v_mov_b32_e32 v41, v37
	v_mov_b32_e32 v42, v37
	v_mov_b32_e32 v44, v37
	s_waitcnt lgkmcnt(4)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[37:44], v[6:9], v[49:52], v[37:44] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[37:44], v[2:5], v[45:48], v[37:44] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[37:44], v[57:60], v[65:68], v[37:44] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[37:44], v[53:56], v[61:64], v[37:44] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_cvt_f32_i32_e32 v3, v39
	v_cvt_f32_i32_e32 v7, v40
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_cvt_f32_i32_e32 v2, v41
	v_cvt_f32_i32_e32 v4, v42
	v_cvt_f32_i32_e32 v5, v43
	v_cvt_f32_i32_e32 v8, v44
	v_cvt_f32_i32_e32 v6, v37
	v_cvt_f32_i32_e32 v9, v38
.LBB0_9:
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s4, s18, 31
	s_mov_b32 s11, 0x31027000
	s_lshr_b32 s4, s4, 26
	s_mov_b32 s10, 0x7ffffffe
	s_add_i32 s4, s18, s4
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s18, s10
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s4, s4, 6
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s19, s11
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s4, s0
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v37, v27, s4, 1
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 s5, -1, 0
	.loc	1 392 40                        ; generate_amdgcn.py:392:40
	s_mul_i32 s4, s4, s15
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s3, s5
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v38, v26, s4, 1
	v_add_lshl_u32 v39, v25, s4, 1
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_cndmask_b32_e32 v37, 0x80000000, v37, vcc_lo
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v40, v24, s4, 1
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_and_b32 s9, s9, 0xffff
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v42, v23, s4, 1
	v_add_lshl_u32 v41, v20, s4, 1
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	buffer_load_u16 v50, v37, s[8:11], 0 offen
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_dual_cndmask_b32 v37, 0x80000000, v38 :: v_dual_cndmask_b32 v38, 0x80000000, v39
	v_dual_cndmask_b32 v39, 0x80000000, v40 :: v_dual_cndmask_b32 v42, 0x80000000, v42
	v_add_lshl_u32 v40, v22, s4, 1
	v_add_lshl_u32 v44, v19, s4, 1
	v_add_lshl_u32 v43, v21, s4, 1
	s_and_b32 s17, s17, 0xffff
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_dual_cndmask_b32 v41, 0x80000000, v41 :: v_dual_cndmask_b32 v40, 0x80000000, v40
	v_cndmask_b32_e32 v45, 0x80000000, v44, vcc_lo
	s_delay_alu instid0(VALU_DEP_3)
	v_cndmask_b32_e32 v43, 0x80000000, v43, vcc_lo
	s_clause 0x7
	buffer_load_u16 v51, v37, s[16:19], 0 offen
	buffer_load_u16 v52, v38, s[16:19], 0 offen
	buffer_load_u16 v48, v39, s[16:19], 0 offen
	buffer_load_u16 v49, v40, s[16:19], 0 offen
	buffer_load_u16 v46, v41, s[16:19], 0 offen
	buffer_load_u16 v47, v42, s[16:19], 0 offen
	buffer_load_u16 v44, v43, s[16:19], 0 offen
	buffer_load_u16 v45, v45, s[16:19], 0 offen
	v_dual_mov_b32 v38, 0 :: v_dual_mov_b32 v37, 0
	v_dual_mov_b32 v40, 0 :: v_dual_mov_b32 v39, 0
	v_dual_mov_b32 v42, 0 :: v_dual_mov_b32 v41, 0
	v_mov_b32_e32 v43, 0
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_cbranch_vccnz .LBB0_11
; %bb.10:
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v1, s34, v35
	v_add_nc_u32_e32 v33, s34, v33
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v31, s1, v31
	v_add_nc_u32_e32 v28, s1, v28
	v_add_nc_u32_e32 v29, s1, v29
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	ds_load_b128 v[37:40], v1
	ds_load_b128 v[53:56], v33
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[57:60], v31
	ds_load_b128 v[61:64], v28
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v28, 0
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	v_add_nc_u32_e32 v34, s34, v34
	v_add_nc_u32_e32 v1, s34, v32
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_dual_mov_b32 v31, v28 :: v_dual_add_nc_u32 v30, s1, v30
	.loc	1 376 22                        ; generate_amdgcn.py:376:22
	ds_load_b128 v[65:68], v34
	ds_load_b128 v[69:72], v1
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[73:76], v30
	ds_load_b128 v[77:80], v29
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v29, v28
	v_mov_b32_e32 v30, v28
	v_mov_b32_e32 v32, v28
	v_mov_b32_e32 v33, v28
	v_mov_b32_e32 v34, v28
	v_mov_b32_e32 v35, v28
	s_waitcnt lgkmcnt(4)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[28:35], v[53:56], v[61:64], v[28:35] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[28:35], v[37:40], v[57:60], v[28:35] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[28:35], v[69:72], v[77:80], v[28:35] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[28:35], v[65:68], v[73:76], v[28:35] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_cvt_f32_i32_e32 v1, v28
	v_cvt_f32_i32_e32 v38, v29
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_cvt_f32_i32_e32 v39, v30
	v_cvt_f32_i32_e32 v42, v31
	v_cvt_f32_i32_e32 v37, v32
	v_cvt_f32_i32_e32 v40, v33
	v_cvt_f32_i32_e32 v41, v34
	v_cvt_f32_i32_e32 v43, v35
.LBB0_11:
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s1, s33, 31
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s18, s10
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_lshr_b32 s1, s1, 26
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_mov_b32 s19, s11
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_add_i32 s1, s33, s1
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v28.l, 0
	.loc	1 388 29                        ; generate_amdgcn.py:388:29
	s_ashr_i32 s1, s1, 6
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_waitcnt vmcnt(8)
	v_mov_b16_e32 v28.h, v50.l
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cmp_lt_i32 s1, s0
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_add_lshl_u32 v27, v27, s1, 1
	.loc	1 392 40                        ; generate_amdgcn.py:392:40
	s_mul_i32 s1, s1, s15
	.loc	1 389 34                        ; generate_amdgcn.py:389:34
	s_cselect_b32 s0, -1, 0
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v25, v25, s1, 1
	v_add_lshl_u32 v26, v26, s1, 1
	v_add_lshl_u32 v22, v22, s1, 1
	v_add_lshl_u32 v24, v24, s1, 1
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s2, s0
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_add_lshl_u32 v20, v20, s1, 1
	v_add_lshl_u32 v23, v23, s1, 1
	v_add_lshl_u32 v21, v21, s1, 1
	v_add_lshl_u32 v19, v19, s1, 1
	v_dual_cndmask_b32 v25, 0x80000000, v25 :: v_dual_cndmask_b32 v24, 0x80000000, v24
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	v_dual_cndmask_b32 v27, 0x80000000, v27 :: v_dual_cndmask_b32 v26, 0x80000000, v26
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_dual_cndmask_b32 v22, 0x80000000, v22 :: v_dual_cndmask_b32 v23, 0x80000000, v23
	v_dual_cndmask_b32 v20, 0x80000000, v20 :: v_dual_cndmask_b32 v21, 0x80000000, v21
	v_dual_cndmask_b32 v19, 0x80000000, v19 :: v_dual_and_b32 v0, 16, v0
	s_clause 0x7
	buffer_load_u16 v25, v25, s[16:19], 0 offen
	buffer_load_u16 v26, v26, s[16:19], 0 offen
	buffer_load_u16 v22, v22, s[16:19], 0 offen
	buffer_load_u16 v24, v24, s[16:19], 0 offen
	buffer_load_u16 v23, v23, s[16:19], 0 offen
	buffer_load_u16 v20, v20, s[16:19], 0 offen
	buffer_load_u16 v19, v19, s[16:19], 0 offen
	buffer_load_u16 v21, v21, s[16:19], 0 offen
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	buffer_load_u16 v27, v27, s[8:11], 0 offen
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_waitcnt vmcnt(15)
	v_mov_b16_e32 v30.h, v52.l
	v_mov_b16_e32 v30.l, v28.l
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v29, 1, v36
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_mov_b16_e32 v31.h, v51.l
	s_waitcnt vmcnt(13)
	v_mov_b16_e32 v32.h, v49.l
	v_mov_b16_e32 v33.h, v48.l
	s_waitcnt vmcnt(11)
	v_mov_b16_e32 v34.h, v47.l
	v_mov_b16_e32 v35.h, v46.l
	s_waitcnt vmcnt(9)
	v_mov_b16_e32 v36.h, v45.l
	v_mov_b16_e32 v44.h, v44.l
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_dual_mov_b32 v45, 0x5410 :: v_dual_mul_f32 v30, v28, v30
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_mov_b16_e32 v31.l, v28.l
	v_mov_b16_e32 v32.l, v28.l
	v_mov_b16_e32 v33.l, v28.l
	v_mov_b16_e32 v34.l, v28.l
	v_mov_b16_e32 v35.l, v28.l
	v_mov_b16_e32 v36.l, v28.l
	v_mov_b16_e32 v44.l, v28.l
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_dual_mov_b32 v46, 0x7632 :: v_dual_mul_f32 v31, v28, v31
	v_mov_b16_e32 v47.h, v28.l
	v_mov_b16_e32 v48.h, v28.l
	v_mov_b16_e32 v49.h, v28.l
	v_mov_b16_e32 v50.h, v28.l
	v_mov_b16_e32 v51.h, v28.l
	v_mov_b16_e32 v52.h, v28.l
	v_mov_b16_e32 v53.h, v28.l
	v_mov_b16_e32 v54.h, v28.l
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v33, v28, v33
	v_mul_f32_e32 v32, v28, v32
	v_mul_f32_e32 v35, v28, v35
	v_mul_f32_e32 v34, v28, v34
	v_mul_f32_e32 v44, v28, v44
	v_mul_f32_e32 v28, v28, v36
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v9, v30, v9, v17
	v_fma_f32 v6, v31, v6, v10
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	v_mul_lo_u32 v18, v18, s15
	.loc	1 416 9 is_stmt 0               ; generate_amdgcn.py:416:9
	s_and_b32 s13, s13, 0xffff
	s_mov_b32 s15, s11
	.loc	1 333 5 is_stmt 1               ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v9, v17, v9, s3
	v_cndmask_b32_e64 v6, v10, v6, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v17, 16, v19
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v8, v28, v8, v11
	.loc	1 390 22                        ; generate_amdgcn.py:390:22
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v10, 16, v27
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v3, v33, v3, v16
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_2) | instid1(VALU_DEP_4)
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v8, v11, v8, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v11, 16, v25
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v7, v32, v7, v15
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v3, v16, v3, s3
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v11, v10, v11
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v7, v15, v7, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v15, 16, v23
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v4, v34, v4, v13
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_fma_f32 v11, v11, v38, v9
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v4, v13, v4, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v13, 16, v22
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v5, v44, v5, v12
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_dual_mul_f32 v13, v10, v13 :: v_dual_lshlrev_b32 v16, 16, v20
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v5, v12, v5, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v12, 16, v26
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v2, v35, v2, v14
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_dual_mul_f32 v12, v10, v12 :: v_dual_lshlrev_b32 v19, 16, v21
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v2, v14, v2, s3
	.loc	1 392 22                        ; generate_amdgcn.py:392:22
	v_lshlrev_b32_e32 v14, 16, v24
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v1, v12, v1, v6
	v_fma_f32 v12, v13, v42, v7
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v1, v6, v1, s2
	v_cndmask_b32_e64 v6, v9, v11, s2
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cndmask_b32_e64 v7, v7, v12, s2
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v48.l, v1.h
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v15, v10, v15
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v47.l, v6.h
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v14, v10, v14
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e32 vcc_lo, v6, v6
	v_cmp_o_f32_e64 s0, v1, v1
	v_mov_b16_e32 v49.l, v7.h
	v_and_b32_e32 v9, 1, v47
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v13, v14, v39, v3
	v_fma_f32 v14, v15, v40, v4
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e64 s1, v7, v7
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_add3_u32 v6, v6, v9, 0x7fff
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v3, v3, v13, s2
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cndmask_b32_e64 v4, v4, v14, s2
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cndmask_b16 v6.h, 0x7fff, v6.h, vcc_lo
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_mov_b16_e32 v50.l, v3.h
	v_mov_b16_e32 v51.l, v4.h
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v16, v10, v16
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e64 s3, v4, v4
	v_cmp_eq_u32_e32 vcc_lo, 0, v0
	v_and_b32_e32 v9, 1, v50
	v_and_b32_e32 v11, 1, v51
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v15, v16, v37, v2
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_add3_u32 v4, v4, v11, 0x7fff
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v2, v2, v15, s2
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_2) | instid1(VALU_DEP_4)
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v52.l, v2.h
	.loc	1 394 24                        ; generate_amdgcn.py:394:24
	v_mul_f32_e32 v19, v10, v19
	v_mul_f32_e32 v10, v10, v17
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e64 s4, v2, v2
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_and_b32_e32 v12, 1, v52
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_fma_f32 v16, v19, v41, v5
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_fma_f32 v10, v10, v43, v8
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_add3_u32 v2, v2, v12, 0x7fff
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v5, v5, v16, s2
	v_cndmask_b32_e64 v8, v8, v10, s2
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v10, 1, v48
	v_cmp_o_f32_e64 s2, v3, v3
	v_add3_u32 v3, v3, v9, 0x7fff
	v_mov_b16_e32 v54.l, v5.h
	v_mov_b16_e32 v53.l, v8.h
	v_add3_u32 v1, v1, v10, 0x7fff
	v_cndmask_b16 v1.l, 0x7fff, v2.h, s4
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_dual_cndmask_b32 v9, 0x1054, v45 :: v_dual_and_b32 v2, 1, v54
	v_and_b32_e32 v0, 1, v53
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_4) | instid1(VALU_DEP_4)
	v_cndmask_b16 v6.l, 0x7fff, v1.h, s0
	v_cndmask_b16 v1.h, 0x7fff, v4.h, s3
	v_dual_cndmask_b32 v10, 0x3276, v46 :: v_dual_and_b32 v13, 1, v49
	v_cmp_o_f32_e64 s5, v8, v8
	v_cmp_o_f32_e64 s6, v5, v5
	v_cndmask_b32_e32 v4, v1, v6, vcc_lo
	s_delay_alu instid0(VALU_DEP_4)
	v_add3_u32 v7, v7, v13, 0x7fff
	v_add3_u32 v2, v5, v2, 0x7fff
	v_add3_u32 v0, v8, v0, 0x7fff
	v_lshl_or_b32 v5, v9, 8, v9
	v_cndmask_b32_e32 v1, v6, v1, vcc_lo
	v_lshl_or_b32 v8, v10, 8, v10
	v_cndmask_b16 v7.h, 0x7fff, v7.h, s1
	v_cndmask_b16 v7.l, 0x7fff, v3.h, s2
	v_cndmask_b16 v0.h, 0x7fff, v0.h, s5
	v_cndmask_b16 v0.l, 0x7fff, v2.h, s6
	v_and_b32_e32 v3, 0x540054, v5
	v_and_b32_e32 v5, 0x760076, v8
	s_mov_b32 s0, 0x76543210
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cndmask_b32_e32 v6, v0, v7, vcc_lo
	v_lshl_or_b32 v2, v3, 4, v3
	s_delay_alu instid0(VALU_DEP_3)
	v_lshl_or_b32 v3, v5, 4, v5
	v_cndmask_b32_e32 v5, v7, v0, vcc_lo
	v_permlanex16_b32 v1, v1, s0, 0xfedcba98 op_sel:[1,0]
	.loc	1 416 18 is_stmt 0              ; generate_amdgcn.py:416:18
	v_or_b32_e32 v7, s14, v29
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v2, 0x5040504, v2
	v_and_b32_e32 v3, 0x7060706, v3
	v_permlanex16_b32 v5, v5, s0, 0xfedcba98 op_sel:[1,0]
	s_mov_b32 s14, s10
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_perm_b32 v0, v1, v4, v2
	v_perm_b32 v1, v1, v4, v3
	s_delay_alu instid0(VALU_DEP_3)
	v_perm_b32 v2, v5, v6, v2
	v_perm_b32 v3, v5, v6, v3
	v_add_lshl_u32 v4, v7, v18, 1
	buffer_store_b128 v[0:3], v4, s[12:15], 0 offen
	.loc	1 309 1 is_stmt 1               ; generate_amdgcn.py:309:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp20:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 42
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 33
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk, .Lfunc_end0-amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.num_vgpr, 100
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.numbered_sgpr, 42
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 4216
; TotalNumSgprs: 44
; NumVgprs: 100
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 44
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	5                               ; DW_FORM_data2
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x5f DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x39 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	317                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4e:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	316                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x5b:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	333                             ; DW_AT_call_line
	.byte	28                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	.Ltmp16-.Lfunc_begin0
	.quad	.Ltmp17-.Lfunc_begin0
	.quad	.Ltmp18-.Lfunc_begin0
	.quad	.Ltmp19-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk"             ; string offset=75 ; amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk
    .private_segment_fixed_size: 0
    .sgpr_count:     44
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int8xint8_nt_sc64_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_evenk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     100
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
