	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk               ; -- Begin function amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk,@function
amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk:                      ; @amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 80 0                          ; generate_amdgcn.py:80:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_load_b64 s[6:7], s[0:1], 0x30
.Ltmp0:
	.loc	1 90 16 prologue_end            ; generate_amdgcn.py:90:16
	s_abs_i32 s9, s2
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_and_b32_e32 v9, 15, v0
	v_lshrrev_b32_e32 v11, 4, v0
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_lshlrev_b32_e32 v6, 3, v0
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_and_b32_e32 v12, 0x7f, v0
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_and_b32_e32 v13, 0x80, v0
	v_and_b32_e32 v15, 8, v0
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_add_nc_u32_e32 v17, 0, v6
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:88:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s7, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s6, s6, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s11, s6, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_lshr_b32 s5, s5, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_lshr_b32 s11, s11, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_add_i32 s6, s6, s11
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:88:17 ]
	s_ashr_i32 s4, s4, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:87:17 ]
	s_ashr_i32 s6, s6, 4
.Ltmp11:
	.loc	1 89 24 is_stmt 1               ; generate_amdgcn.py:89:24
	s_lshl_b32 s4, s4, 2
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	.loc	1 90 16                         ; generate_amdgcn.py:90:16
	s_abs_i32 s5, s4
	s_cvt_f32_u32 s8, s5
	s_sub_i32 s10, 0, s5
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	v_rcp_iflag_f32_e32 v1, s8
	v_readfirstlane_b32 s8, v1
	s_mul_f32 s8, s8, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s8, s8
	s_mul_i32 s10, s10, s8
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s10, s8, s10
	s_add_i32 s8, s8, s10
	s_xor_b32 s10, s2, s4
	s_mul_hi_u32 s8, s9, s8
	s_ashr_i32 s10, s10, 31
	s_mul_i32 s11, s8, s5
	s_delay_alu instid0(SALU_CYCLE_1)
	s_sub_i32 s9, s9, s11
	s_add_i32 s11, s8, 1
	s_sub_i32 s12, s9, s5
	s_cmp_ge_u32 s9, s5
	s_cselect_b32 s8, s11, s8
	s_cselect_b32 s9, s12, s9
	s_add_i32 s11, s8, 1
	s_cmp_ge_u32 s9, s5
	s_load_b128 s[12:15], s[0:1], 0x0
	s_cselect_b32 s5, s11, s8
	s_load_b32 s11, s[0:1], 0x38
	s_xor_b32 s5, s5, s10
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_sub_i32 s5, s5, s10
	.loc	1 91 19                         ; generate_amdgcn.py:91:19
	s_lshl_b32 s8, s5, 2
	.loc	1 93 29                         ; generate_amdgcn.py:93:29
	s_mul_i32 s5, s5, s4
	.loc	1 92 31                         ; generate_amdgcn.py:92:31
	s_sub_i32 s6, s6, s8
	.loc	1 93 29                         ; generate_amdgcn.py:93:29
	s_sub_i32 s2, s2, s5
	.loc	1 92 20                         ; generate_amdgcn.py:92:20
	s_min_i32 s6, s6, 4
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_abs_i32 s5, s2
	s_abs_i32 s9, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_f32_u32 s10, s9
	v_rcp_iflag_f32_e32 v1, s10
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[16:17], s[14:15]
	s_sub_i32 s14, 0, s9
	s_mov_b32 s15, 0x31027000
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(TRANS32_DEP_1)
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_mov_b32 s19, s15
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	v_readfirstlane_b32 s10, v1
	s_mul_f32 s10, s10, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s10, s10
	s_mul_i32 s14, s14, s10
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s4, s10, s14
	s_add_i32 s10, s10, s4
	s_delay_alu instid0(SALU_CYCLE_1)
	s_mul_hi_u32 s4, s5, s10
	s_xor_b32 s10, s2, s6
	s_mul_i32 s14, s4, s9
	s_ashr_i32 s21, s10, 31
	s_sub_i32 s5, s5, s14
	s_add_i32 s10, s4, 1
	s_sub_i32 s18, s5, s9
	s_cmp_ge_u32 s5, s9
	s_mov_b32 s14, 0x7ffffffe
	s_cselect_b32 s4, s10, s4
	s_cselect_b32 s5, s18, s5
	s_add_i32 s10, s4, 1
	s_cmp_ge_u32 s5, s9
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_mov_b32 s18, s14
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_cselect_b32 s4, s10, s4
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_lshr_b32 s5, s11, 31
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_xor_b32 s22, s4, s21
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_add_i32 s4, s11, s5
	.loc	1 94 13                         ; generate_amdgcn.py:94:13
	s_sub_i32 s5, s22, s21
	.loc	1 108 23                        ; generate_amdgcn.py:108:23
	s_lshl_b32 s24, s3, 4
	.loc	1 96 22                         ; generate_amdgcn.py:96:22
	s_ashr_i32 s23, s4, 1
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_mul_i32 s4, s5, s6
	.loc	1 108 18                        ; generate_amdgcn.py:108:18
	v_or_b32_e32 v4, s24, v9
	.loc	1 93 28                         ; generate_amdgcn.py:93:28
	s_sub_i32 s2, s2, s4
	.loc	1 100 14                        ; generate_amdgcn.py:100:14
	s_lshl_b32 s3, s5, 7
	.loc	1 93 13                         ; generate_amdgcn.py:93:13
	s_add_i32 s2, s2, s8
	.loc	1 108 18                        ; generate_amdgcn.py:108:18
	v_or_b32_e32 v3, s24, v11
	.loc	1 99 14                         ; generate_amdgcn.py:99:14
	s_lshl_b32 s6, s2, 4
	.loc	1 109 23                        ; generate_amdgcn.py:109:23
	v_cmp_gt_i32_e32 vcc_lo, s23, v4
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cmp_gt_i32 s11, 1
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	s_mul_i32 s2, s6, s23
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cselect_b32 s20, -1, 0
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	s_mul_i32 s4, s7, s24
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_and_b32 vcc_lo, s20, vcc_lo
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshlrev_b32_e32 v10, 3, v9
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_and_b32 s13, s13, 0xffff
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_and_b32 s17, s17, 0xffff
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	.loc	1 131 30 is_stmt 0              ; generate_amdgcn.py:131:30
	v_mad_u64_u32 v[1:2], null, s7, v11, v[10:11]
	.loc	1 127 30 is_stmt 1              ; generate_amdgcn.py:127:30
	v_mul_lo_u32 v2, s23, v11
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	v_add3_u32 v20, s4, s3, v1
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	v_add3_u32 v19, v2, v9, s2
	.loc	1 110 23                        ; generate_amdgcn.py:110:23
	v_cmp_gt_i32_e64 s2, s23, v3
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	v_add_nc_u32_e32 v2, s24, v19
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 127 22 is_stmt 0              ; generate_amdgcn.py:127:22
	v_cndmask_b32_e32 v1, 0x80000000, v2, vcc_lo
	.loc	1 106 9 is_stmt 1               ; generate_amdgcn.py:106:9
	s_and_b32 vcc_lo, s20, s2
	s_mov_b32 s2, 0
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_cndmask_b32_e32 v2, 0x80000000, v20, vcc_lo
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	buffer_load_u8 v4, v1, s[12:15], 0 offen
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	buffer_load_b64 v[2:3], v2, s[16:19], 0 offen
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_bfe_i32 v1, v0, 7, 1
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cmpk_gt_i32 s11, 0x101
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_and_b32_e32 v1, 0x88, v1
	v_xor_b32_e32 v5, v1, v12
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_lshrrev_b32_e32 v1, 1, v0
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v16, 0, v5
	s_waitcnt vmcnt(1)
	ds_store_b8 v16, v4 offset:2048
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_waitcnt vmcnt(0)
	ds_store_b64 v17, v[2:3]
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cbranch_scc1 .LBB0_2
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_and_b32_e32 v18, 8, v0
	s_branch .LBB0_3
.LBB0_2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	s_mov_b32 s2, -1
                                        ; implicit-def: $vgpr18
.LBB0_3:                                ; %Flow53
	s_clause 0x1
	s_load_b128 s[8:11], s[0:1], 0x10
	s_load_b64 s[4:5], s[0:1], 0x20
	v_cmp_eq_u32_e64 s0, 0, v13
	v_and_b32_e32 v14, 0x70, v1
	v_dual_mov_b32 v1, 0 :: v_dual_mov_b32 v2, 0
	v_dual_mov_b32 v3, 0 :: v_dual_mov_b32 v4, 0
	v_dual_mov_b32 v5, 0 :: v_dual_mov_b32 v6, 0
	v_dual_mov_b32 v7, 0 :: v_dual_mov_b32 v8, 0
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_cbranch_vccnz .LBB0_8
; %bb.4:                                ; %.lr.ph
	.loc	1 106 9 is_stmt 1               ; generate_amdgcn.py:106:9
	v_sub_nc_u32_e32 v18, s23, v9
	.loc	1 108 18                        ; generate_amdgcn.py:108:18
	s_add_i32 s2, s24, 0x80
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	s_lshl_b32 s1, s7, 7
	.loc	1 127 30                        ; generate_amdgcn.py:127:30
	v_add_nc_u32_e32 v1, s2, v19
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	v_sub_nc_u32_e32 v19, s23, v11
	.loc	1 109 23                        ; generate_amdgcn.py:109:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v18
	.loc	1 131 30                        ; generate_amdgcn.py:131:30
	v_add_nc_u32_e32 v2, s1, v20
	.loc	1 131 22 is_stmt 0              ; generate_amdgcn.py:131:22
	s_mov_b32 s18, s14
	s_mov_b32 s19, s15
	v_bfe_i32 v3, v0, 3, 1
	.loc	1 127 22 is_stmt 1              ; generate_amdgcn.py:127:22
	v_cndmask_b32_e32 v1, 0x80000000, v1, vcc_lo
	.loc	1 110 23                        ; generate_amdgcn.py:110:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v19
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:106:32 ]
	s_add_i32 s2, s23, 0x7f
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	2 43 12 is_stmt 0               ; standard.py:43:12 @[ generate_amdgcn.py:106:32 ]
	s_lshr_b32 s2, s2, 7
.Ltmp13:
	.loc	1 127 22 is_stmt 1              ; generate_amdgcn.py:127:22
	buffer_load_u8 v33, v1, s[12:15], 0 offen
	v_lshlrev_b32_e32 v1, 4, v0
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_max_u32 s25, s2, 2
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_2) | instid1(VALU_DEP_1)
	v_and_b32_e32 v4, 0x70, v1
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_dual_mov_b32 v1, 0 :: v_dual_cndmask_b32 v2, 0x80000000, v2
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cmp_eq_u32 s25, 2
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_mov_b32_e32 v6, v1
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	buffer_load_b64 v[23:24], v2, s[16:19], 0 offen
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_dual_mov_b32 v7, v1 :: v_dual_add_nc_u32 v2, 0, v14
	v_mov_b32_e32 v8, v1
	s_delay_alu instid0(VALU_DEP_2)
	v_add_nc_u32_e32 v20, v2, v9
	v_and_or_b32 v2, 0x88, v3, v4
	ds_load_u8 v29, v20 offset:640
	ds_load_u8 v30, v20 offset:896
	ds_load_u8 v31, v20 offset:768
	ds_load_u8 v32, v20 offset:512
	ds_load_u8 v34, v20 offset:128
	ds_load_u8 v35, v20 offset:384
	ds_load_u8 v36, v20 offset:256
	ds_load_u8 v37, v20
	ds_load_u8 v38, v20 offset:1664
	ds_load_u8 v39, v20 offset:1920
	ds_load_u8 v40, v20 offset:1792
	ds_load_u8 v41, v20 offset:1536
	ds_load_u8 v42, v20 offset:1152
	ds_load_u8 v43, v20 offset:1280
	ds_load_u8 v44, v20 offset:1024
	ds_load_u8 v45, v20 offset:1408
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v21, 0, v2
	v_xor_b32_e32 v5, 8, v2
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_mov_b32_e32 v2, v1
	v_mov_b32_e32 v3, v1
	v_mov_b32_e32 v4, v1
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_b64 v[25:26], v21 offset:2048
	v_add_nc_u32_e32 v22, 0, v5
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v29, v32, v29, 0xc0c0004
	v_perm_b32 v30, v31, v30, 0xc0c0004
	v_mov_b32_e32 v5, v1
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v32, v36, v35, 0xc0c0004
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_b64 v[27:28], v22 offset:2048
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v31, v37, v34, 0xc0c0004
	v_lshl_or_b32 v30, v30, 16, v29
	s_waitcnt lgkmcnt(7)
	v_perm_b32 v35, v40, v39, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v34, v41, v38, 0xc0c0004
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_waitcnt lgkmcnt(0)
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_lshl_or_b32 v29, v32, 16, v31
	v_perm_b32 v36, v44, v42, 0xc0c0004
	v_perm_b32 v37, v43, v45, 0xc0c0004
	v_lshl_or_b32 v32, v35, 16, v34
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_barrier
	s_waitcnt vmcnt(1)
	ds_store_b8 v16, v33 offset:2048
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_waitcnt vmcnt(0)
	ds_store_b64 v17, v[23:24]
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_lshl_or_b32 v31, v37, 16, v36
	v_wmma_i32_16x16x16_iu4 v[1:8], v[29:30], v[25:26], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[1:8], v[31:32], v[27:28], v[1:8] neg_lo:[1,1,0]
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cbranch_scc1 .LBB0_7
; %bb.5:                                ; %.peel.next
	s_add_i32 s2, s24, 0x100
	s_lshl_b32 s22, s22, 7
	v_add_nc_u32_e32 v23, s2, v11
	v_add_nc_u32_e32 v11, s6, v11
	s_lshl_b32 s21, s21, 7
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_mul_lo_u32 v23, s7, v23
	v_add3_u32 v23, v23, s22, v10
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_mad_u64_u32 v[10:11], null, s23, v11, v[9:10]
	v_subrev_nc_u32_e32 v11, s21, v23
	s_add_i32 s21, s25, -2
.LBB0_6:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 110 23                        ; generate_amdgcn.py:110:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v19
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_add_nc_u32_e32 v23, s2, v10
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_add_i32 s21, s21, -1
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_cndmask_b32_e32 v24, 0x80000000, v11, vcc_lo
	.loc	1 109 23                        ; generate_amdgcn.py:109:23
	v_cmp_lt_i32_e32 vcc_lo, s2, v18
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	v_add_nc_u32_e32 v11, s1, v11
	s_addk_i32 s2, 0x80
	s_cmp_lg_u32 s21, 0
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_cndmask_b32_e32 v23, 0x80000000, v23, vcc_lo
	buffer_load_u8 v33, v23, s[12:15], 0 offen
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	buffer_load_b64 v[23:24], v24, s[16:19], 0 offen
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	ds_load_u8 v29, v20 offset:640
	ds_load_u8 v30, v20 offset:896
	ds_load_u8 v31, v20 offset:768
	ds_load_u8 v32, v20 offset:512
	ds_load_u8 v34, v20 offset:128
	ds_load_u8 v35, v20 offset:384
	ds_load_u8 v36, v20 offset:256
	ds_load_u8 v37, v20
	ds_load_u8 v38, v20 offset:1664
	ds_load_u8 v39, v20 offset:1920
	ds_load_u8 v40, v20 offset:1792
	ds_load_u8 v41, v20 offset:1536
	ds_load_u8 v42, v20 offset:1152
	ds_load_u8 v43, v20 offset:1280
	ds_load_u8 v44, v20 offset:1024
	ds_load_u8 v45, v20 offset:1408
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_b64 v[25:26], v21 offset:2048
	ds_load_b64 v[27:28], v22 offset:2048
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(1)
	ds_store_b8 v16, v33 offset:2048
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	s_waitcnt vmcnt(0)
	ds_store_b64 v17, v[23:24]
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_perm_b32 v29, v32, v29, 0xc0c0004
	v_perm_b32 v30, v31, v30, 0xc0c0004
	v_perm_b32 v31, v37, v34, 0xc0c0004
	v_perm_b32 v32, v36, v35, 0xc0c0004
	v_perm_b32 v34, v41, v38, 0xc0c0004
	v_perm_b32 v35, v40, v39, 0xc0c0004
	v_perm_b32 v36, v44, v42, 0xc0c0004
	v_perm_b32 v37, v43, v45, 0xc0c0004
	v_lshl_or_b32 v30, v30, 16, v29
	v_lshl_or_b32 v29, v32, 16, v31
	v_lshl_or_b32 v32, v35, 16, v34
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_lshl_or_b32 v31, v37, 16, v36
	v_wmma_i32_16x16x16_iu4 v[1:8], v[29:30], v[25:26], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu4 v[1:8], v[31:32], v[27:28], v[1:8] neg_lo:[1,1,0]
	.loc	1 106 9                         ; generate_amdgcn.py:106:9
	s_cbranch_scc1 .LBB0_6
.LBB0_7:                                ; %Flow52
	.loc	1 0 9 is_stmt 0                 ; generate_amdgcn.py:0:9
	v_mov_b32_e32 v18, v15
.LBB0_8:                                ; %._crit_edge
	v_and_b32_e32 v11, 0xf0, v0
	.loc	1 127 22 is_stmt 1              ; generate_amdgcn.py:127:22
	v_and_b32_e32 v10, 7, v0
	s_delay_alu instid0(VALU_DEP_3)
	v_cmp_eq_u32_e64 s1, 0, v18
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_and_not1_b32 vcc_lo, exec_lo, s20
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_cbranch_vccnz .LBB0_10
; %bb.9:
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_cndmask_b32_e64 v15, 0x88, 0, s1
	.loc	1 131 22                        ; generate_amdgcn.py:131:22
	v_add3_u32 v16, 0, v14, v9
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	ds_load_u8 v19, v16 offset:640
	ds_load_u8 v20, v16 offset:896
	ds_load_u8 v21, v16 offset:768
	ds_load_u8 v22, v16 offset:512
	ds_load_u8 v23, v16 offset:128
	ds_load_u8 v24, v16 offset:384
	ds_load_u8 v25, v16 offset:256
	ds_load_u8 v26, v16
	ds_load_u8 v27, v16 offset:1664
	ds_load_u8 v28, v16 offset:1792
	ds_load_u8 v29, v16 offset:1536
	ds_load_u8 v30, v16 offset:1920
	ds_load_u8 v31, v16 offset:1152
	ds_load_u8 v32, v16 offset:1280
	ds_load_u8 v33, v16 offset:1024
	ds_load_u8 v34, v16 offset:1408
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	v_lshl_or_b32 v15, v10, 4, v15
	s_delay_alu instid0(VALU_DEP_1)
	v_add_nc_u32_e32 v17, 0, v15
	v_xad_u32 v18, v15, 8, 0
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v19, v22, v19, 0xc0c0004
	.loc	1 127 22                        ; generate_amdgcn.py:127:22
	ds_load_b64 v[15:16], v17 offset:2048
	ds_load_b64 v[17:18], v18 offset:2048
	.loc	1 132 23                        ; generate_amdgcn.py:132:23
	v_perm_b32 v20, v21, v20, 0xc0c0004
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v21, v26, v23, 0xc0c0004
	v_perm_b32 v22, v25, v24, 0xc0c0004
	s_waitcnt lgkmcnt(7)
	v_perm_b32 v23, v29, v27, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v24, v28, v30, 0xc0c0004
	v_lshl_or_b32 v20, v20, 16, v19
	v_lshl_or_b32 v19, v22, 16, v21
	s_waitcnt lgkmcnt(3)
	v_perm_b32 v22, v33, v31, 0xc0c0004
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v25, v32, v34, 0xc0c0004
	v_lshl_or_b32 v21, v24, 16, v23
	s_waitcnt lgkmcnt(1)
	v_wmma_i32_16x16x16_iu4 v[1:8], v[19:20], v[15:16], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v20, v25, 16, v22
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu4 v[1:8], v[20:21], v[17:18], v[1:8] neg_lo:[1,1,0]
.LBB0_10:                               ; %._crit_edge._crit_edge
	.loc	1 142 22                        ; generate_amdgcn.py:142:22
	v_or_b32_e32 v9, s6, v9
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v2, v2
	.loc	1 142 14                        ; generate_amdgcn.py:142:14
	s_and_b32 s13, s9, 0xffff
	s_mov_b32 s15, 0x31027000
	s_mov_b32 s14, 0x7ffffffe
	v_lshlrev_b32_e32 v9, 1, v9
	s_mov_b32 s12, s8
	.loc	1 100 32                        ; generate_amdgcn.py:100:32
	v_bfe_u32 v15, v0, 4, 1
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v4, v4
	v_cvt_f32_i32_e32 v8, v8
	.loc	1 142 14                        ; generate_amdgcn.py:142:14
	buffer_load_u16 v9, v9, s[12:15], 0 offen
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	s_and_b32 s13, s11, 0xffff
	.loc	1 143 22 is_stmt 0              ; generate_amdgcn.py:143:22
	v_or3_b32 v14, v15, v14, s3
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	s_mov_b32 s12, s10
	.loc	1 141 15 is_stmt 1              ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v3, v3
	v_cvt_f32_i32_e32 v5, v5
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_cndmask_b32_e64 v22, 0x1040, 0, s1
	.loc	1 99 32                         ; generate_amdgcn.py:99:32
	v_lshrrev_b32_e32 v13, 7, v13
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v7, v7
	v_cvt_f32_i32_e32 v1, v1
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_cndmask_b32_e64 v25, 0x204, 0, s0
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_mul_i32 s0, s6, s7
	v_mul_lo_u32 v13, s7, v13
	s_add_i32 s0, s0, s3
	.loc	1 141 15                        ; generate_amdgcn.py:141:15
	v_cvt_f32_i32_e32 v6, v6
	.loc	1 218 23                        ; generate_amdgcn.py:218:23
	s_lshl_b32 s1, s7, 1
	s_mul_i32 s8, s7, 6
	s_lshl_b32 s2, s7, 2
	s_lshl_b32 s9, s7, 3
	s_mul_i32 s10, s7, 10
	s_mul_i32 s11, s7, 12
	.loc	1 218 9 is_stmt 0               ; generate_amdgcn.py:218:9
	s_and_b32 s5, s5, 0xffff
	s_mov_b32 s6, s14
	.loc	1 142 14 is_stmt 1              ; generate_amdgcn.py:142:14
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v9, 16, v9
	.loc	1 143 22                        ; generate_amdgcn.py:143:22
	v_lshlrev_b32_e32 v14, 1, v14
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 143 14 is_stmt 0              ; generate_amdgcn.py:143:14
	v_or_b32_e32 v15, 4, v14
	v_or_b32_e32 v16, 8, v14
	v_or_b32_e32 v17, 12, v14
	v_or_b32_e32 v18, 16, v14
	v_or_b32_e32 v19, 20, v14
	v_or_b32_e32 v20, 24, v14
	v_or_b32_e32 v21, 28, v14
	s_clause 0x7
	buffer_load_u16 v14, v14, s[12:15], 0 offen
	buffer_load_u16 v15, v15, s[12:15], 0 offen
	buffer_load_u16 v16, v16, s[12:15], 0 offen
	buffer_load_u16 v17, v17, s[12:15], 0 offen
	buffer_load_u16 v18, v18, s[12:15], 0 offen
	buffer_load_u16 v19, v19, s[12:15], 0 offen
	buffer_load_u16 v20, v20, s[12:15], 0 offen
	buffer_load_u16 v21, v21, s[12:15], 0 offen
	.loc	1 144 9 is_stmt 1               ; generate_amdgcn.py:144:9
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	s_waitcnt vmcnt(7)
	v_lshlrev_b32_e32 v14, 16, v14
	s_waitcnt vmcnt(4)
	v_lshlrev_b32_e32 v17, 16, v17
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshlrev_b32_e32 v24, 1, v0
	.loc	1 143 14                        ; generate_amdgcn.py:143:14
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v19, 16, v19
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v20, 16, v20
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v21, 16, v21
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v17, v9, v17
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_3) | instid1(VALU_DEP_4)
	.loc	1 144 9 is_stmt 0               ; generate_amdgcn.py:144:9
	v_dual_mul_f32 v4, v17, v4 :: v_dual_lshlrev_b32 v11, 1, v11
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_dual_mul_f32 v14, v9, v14 :: v_dual_lshlrev_b32 v15, 16, v15
	v_mul_f32_e32 v20, v9, v20
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_and_b32_e32 v24, 28, v24
	v_lshl_or_b32 v11, v10, 2, v11
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_dual_mul_f32 v1, v14, v1 :: v_dual_lshlrev_b32 v16, 16, v16
	v_dual_mul_f32 v7, v20, v7 :: v_dual_lshlrev_b32 v18, 16, v18
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_3) | instid1(VALU_DEP_4)
	v_xor_b32_e32 v11, v11, v22
	v_lshlrev_b32_e32 v23, 2, v0
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_dual_mul_f32 v19, v9, v19 :: v_dual_lshlrev_b32 v0, 5, v0
	v_mul_f32_e32 v15, v9, v15
	.loc	1 144 9                         ; generate_amdgcn.py:144:9
	v_lshl_or_b32 v10, v10, 9, v11
	.loc	1 218 23 is_stmt 1              ; generate_amdgcn.py:218:23
	v_add3_u32 v11, s0, v12, v13
	.loc	1 144 16                        ; generate_amdgcn.py:144:16
	v_mul_f32_e32 v16, v9, v16
	v_mul_f32_e32 v18, v9, v18
	v_mul_f32_e32 v9, v9, v21
	.loc	1 144 9 is_stmt 0               ; generate_amdgcn.py:144:9
	v_and_or_b32 v22, 0x1c0, v23, v24
	v_add_nc_u32_e32 v13, 0, v10
	v_mul_f32_e32 v2, v15, v2
	v_xad_u32 v23, v10, 8, 0
	v_mul_f32_e32 v3, v16, v3
	v_xor_b32_e32 v12, v22, v25
	v_xad_u32 v22, v10, 4, 0
	v_xad_u32 v24, v10, 12, 0
	v_mul_f32_e32 v5, v18, v5
	v_xad_u32 v25, v10, 20, 0
	v_and_or_b32 v0, v0, 32, v12
	v_xad_u32 v12, v10, 16, 0
	v_xad_u32 v26, v10, 24, 0
	v_xad_u32 v10, v10, 28, 0
	s_delay_alu instid0(VALU_DEP_4)
	v_dual_mul_f32 v6, v19, v6 :: v_dual_add_nc_u32 v27, 0, v0
	v_xad_u32 v28, 0x408, v0, 0
	v_xad_u32 v30, 0xc18, v0, 0
	v_xad_u32 v29, 0x810, v0, 0
	v_xad_u32 v31, 0x1040, v0, 0
	v_xad_u32 v32, 0x1448, v0, 0
	v_xad_u32 v33, 0x1850, v0, 0
	v_mul_f32_e32 v8, v9, v8
	ds_store_b32 v13, v1
	ds_store_b32 v22, v2
	ds_store_b32 v23, v3
	ds_store_b32 v24, v4
	ds_store_b32 v12, v5
	ds_store_b32 v25, v6
	ds_store_b32 v26, v7
	ds_store_b32 v10, v8
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b32 v1, v27
	ds_load_b32 v2, v28
	ds_load_b32 v3, v29
	v_xad_u32 v0, 0x1c58, v0, 0
	ds_load_b32 v4, v30
	ds_load_b32 v5, v31
	ds_load_b32 v6, v32
	ds_load_b32 v7, v33
	ds_load_b32 v0, v0
	.loc	1 218 9 is_stmt 1               ; generate_amdgcn.py:218:9
	v_lshlrev_b32_e32 v8, 2, v11
	v_add_lshl_u32 v9, v11, s1, 2
	v_add_lshl_u32 v12, v11, s8, 2
	v_add_lshl_u32 v10, v11, s2, 2
	v_add_lshl_u32 v13, v11, s9, 2
	.loc	1 218 23 is_stmt 0              ; generate_amdgcn.py:218:23
	s_mul_i32 s0, s7, 14
	.loc	1 218 9                         ; generate_amdgcn.py:218:9
	s_mov_b32 s7, s15
	v_add_lshl_u32 v14, v11, s10, 2
	v_add_lshl_u32 v15, v11, s11, 2
	s_waitcnt lgkmcnt(7)
	buffer_atomic_add_f32 v1, v8, s[4:7], 0 offen
	s_waitcnt lgkmcnt(6)
	buffer_atomic_add_f32 v2, v9, s[4:7], 0 offen
	s_waitcnt lgkmcnt(5)
	buffer_atomic_add_f32 v3, v10, s[4:7], 0 offen
	v_add_lshl_u32 v1, v11, s0, 2
	s_waitcnt lgkmcnt(4)
	buffer_atomic_add_f32 v4, v12, s[4:7], 0 offen
	s_waitcnt lgkmcnt(3)
	buffer_atomic_add_f32 v5, v13, s[4:7], 0 offen
	s_waitcnt lgkmcnt(2)
	buffer_atomic_add_f32 v6, v14, s[4:7], 0 offen
	s_waitcnt lgkmcnt(1)
	buffer_atomic_add_f32 v7, v15, s[4:7], 0 offen
	s_waitcnt lgkmcnt(0)
	buffer_atomic_add_f32 v0, v1, s[4:7], 0 offen
	.loc	1 80 1 is_stmt 1                ; generate_amdgcn.py:80:1
	s_endpgm
.Ltmp14:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 26
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 25
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk, .Lfunc_end0-amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_vgpr, 46
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.numbered_sgpr, 26
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 3160
; TotalNumSgprs: 28
; NumVgprs: 46
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 28
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	11                              ; DW_FORM_data1
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	5                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	11                              ; DW_FORM_data1
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x64 DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x3e DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	88                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4d:0xc DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.byte	87                              ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	5                               ; Abbrev [5] 0x59:0x14 DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.quad	.Ltmp12                         ; DW_AT_low_pc
	.long	.Ltmp13-.Ltmp12                 ; DW_AT_high_pc
	.byte	1                               ; DW_AT_call_file
	.byte	106                             ; DW_AT_call_line
	.byte	32                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk"             ; string offset=75 ; amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk
    .private_segment_fixed_size: 0
    .sgpr_count:     28
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int4xint4_nn_pc_none_bm16_bn128_bk32_gm4_w8_s2_weu2_sk8_maskk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     46
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
