	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk               ; -- Begin function amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk,@function
amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk:                      ; @amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 338 0                         ; generate_amdgcn.py:338:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[28:29], s[0:1], 0x30
	s_load_b128 s[20:23], s[0:1], 0x0
.Ltmp0:
	.loc	1 349 16 prologue_end           ; generate_amdgcn.py:349:16
	s_abs_i32 s7, s2
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_lshlrev_b32_e32 v9, 4, v0
	v_lshrrev_b32_e32 v2, 2, v0
	v_and_b32_e32 v43, 15, v0
	.loc	1 356 32                        ; generate_amdgcn.py:356:32
	v_and_b32_e32 v10, 0x60, v0
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:347:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s29, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:346:17 ]
	s_add_i32 s8, s28, 63
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_ashr_i32 s10, s8, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_lshr_b32 s5, s5, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_lshr_b32 s10, s10, 26
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_add_i32 s8, s8, s10
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_ashr_i32 s4, s4, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_ashr_i32 s8, s8, 6
.Ltmp11:
	.loc	1 348 24 is_stmt 1              ; generate_amdgcn.py:348:24
	s_lshl_b32 s4, s4, 2
	s_mov_b64 s[24:25], s[22:23]
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	s_abs_i32 s5, s4
	s_mov_b32 s23, 0x31027000
	s_cvt_f32_u32 s6, s5
	s_sub_i32 s9, 0, s5
	s_mov_b32 s22, 0x7ffffffe
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_mov_b32 s27, s23
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	v_rcp_iflag_f32_e32 v1, s6
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_mov_b32 s26, s22
	s_delay_alu instid0(TRANS32_DEP_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_3)
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	v_readfirstlane_b32 s6, v1
	s_mul_f32 s6, s6, 0x4f7ffffe
	s_cvt_u32_f32 s6, s6
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_i32 s9, s9, s6
	s_mul_hi_u32 s9, s6, s9
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_4) | instid1(SALU_CYCLE_1)
	s_add_i32 s6, s6, s9
	s_xor_b32 s9, s2, s4
	s_mul_hi_u32 s6, s7, s6
	s_ashr_i32 s9, s9, 31
	s_mul_i32 s10, s6, s5
	s_sub_i32 s7, s7, s10
	s_add_i32 s10, s6, 1
	s_sub_i32 s11, s7, s5
	s_cmp_ge_u32 s7, s5
	s_cselect_b32 s6, s10, s6
	s_cselect_b32 s7, s11, s7
	s_add_i32 s10, s6, 1
	s_cmp_ge_u32 s7, s5
	s_cselect_b32 s5, s10, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s5, s5, s9
	s_sub_i32 s5, s5, s9
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 350 19                        ; generate_amdgcn.py:350:19
	s_lshl_b32 s6, s5, 2
	.loc	1 352 29                        ; generate_amdgcn.py:352:29
	s_mul_i32 s5, s5, s4
	.loc	1 351 31                        ; generate_amdgcn.py:351:31
	s_sub_i32 s7, s8, s6
	.loc	1 352 29                        ; generate_amdgcn.py:352:29
	s_sub_i32 s2, s2, s5
	.loc	1 351 20                        ; generate_amdgcn.py:351:20
	s_min_i32 s9, s7, 4
	s_load_b32 s5, s[0:1], 0x38
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_abs_i32 s8, s9
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_2)
	s_cvt_f32_u32 s7, s8
	s_sub_i32 s10, 0, s8
	v_rcp_iflag_f32_e32 v1, s7
	s_delay_alu instid0(TRANS32_DEP_1) | instskip(SKIP_3) | instid1(VALU_DEP_1)
	v_readfirstlane_b32 s7, v1
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_and_b32_e32 v1, 48, v9
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_mul_f32 s7, s7, 0x4f7ffffe
	.loc	1 404 30                        ; generate_amdgcn.py:404:30
	s_waitcnt lgkmcnt(0)
	v_mad_u64_u32 v[3:4], null, s5, v2, v[1:2]
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_cvt_u32_f32 s7, s7
	s_mul_i32 s10, s10, s7
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_2) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s4, s7, s10
	s_abs_i32 s10, s2
	s_add_i32 s7, s7, s4
	s_mul_hi_u32 s4, s10, s7
	s_xor_b32 s7, s2, s9
	s_mul_i32 s11, s4, s8
	s_ashr_i32 s7, s7, 31
	s_sub_i32 s10, s10, s11
	s_add_i32 s11, s4, 1
	s_sub_i32 s12, s10, s8
	s_cmp_ge_u32 s10, s8
	s_cselect_b32 s4, s11, s4
	s_cselect_b32 s10, s12, s10
	s_add_i32 s11, s4, 1
	s_cmp_ge_u32 s10, s8
	s_cselect_b32 s4, s11, s4
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s8, s4, s7
	s_sub_i32 s4, s8, s7
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 352 28                        ; generate_amdgcn.py:352:28
	s_mul_i32 s9, s4, s9
	.loc	1 356 14                        ; generate_amdgcn.py:356:14
	s_lshl_b32 s14, s4, 7
	.loc	1 352 28                        ; generate_amdgcn.py:352:28
	s_sub_i32 s2, s2, s9
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:363:28 ]
	s_add_i32 s9, s5, 0x1ff
.Ltmp13:
	.loc	1 352 13                        ; generate_amdgcn.py:352:13
	s_add_i32 s2, s2, s6
	.loc	1 398 22                        ; generate_amdgcn.py:398:22
	s_lshl_b32 s6, s3, 6
	.loc	1 355 14                        ; generate_amdgcn.py:355:14
	s_lshl_b32 s15, s2, 6
	.loc	1 406 30                        ; generate_amdgcn.py:406:30
	s_mul_i32 s2, s14, s5
	.loc	1 404 30                        ; generate_amdgcn.py:404:30
	v_mad_u64_u32 v[4:5], null, s15, s5, v[3:4]
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmpk_gt_i32 s9, 0x1ff
	s_cselect_b32 s3, -1, 0
	.loc	1 406 30                        ; generate_amdgcn.py:406:30
	s_add_i32 s2, s2, s6
	s_lshl_b32 s4, s5, 6
	v_add_nc_u32_e32 v6, s2, v3
	.loc	1 404 30                        ; generate_amdgcn.py:404:30
	v_add_nc_u32_e32 v5, s6, v4
	.loc	1 406 30                        ; generate_amdgcn.py:406:30
	v_add3_u32 v3, s2, s4, v3
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_and_b32 s21, s21, 0xffff
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_and_b32 s25, s25, 0xffff
	.loc	1 404 30                        ; generate_amdgcn.py:404:30
	v_add3_u32 v4, 0x200, s6, v4
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmpk_gt_i32 s9, 0x3ff
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_cndmask_b32_e64 v5, 0x80000000, v5, s3
	.loc	1 406 30                        ; generate_amdgcn.py:406:30
	v_add_nc_u32_e32 v7, 0x200, v6
	v_add_nc_u32_e32 v8, 0x200, v3
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cselect_b32 s2, -1, 0
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_cndmask_b32_e64 v6, 0x80000000, v6, s3
	v_cndmask_b32_e64 v3, 0x80000000, v3, s3
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_cndmask_b32_e64 v4, 0x80000000, v4, s2
	buffer_load_b128 v[11:14], v5, s[20:23], 0 offen
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_cndmask_b32_e64 v5, 0x80000000, v7, s2
	v_cndmask_b32_e64 v7, 0x80000000, v8, s2
	s_clause 0x1
	buffer_load_b128 v[15:18], v6, s[24:27], 0 offen
	buffer_load_b128 v[19:22], v3, s[24:27], 0 offen
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	buffer_load_b128 v[23:26], v4, s[20:23], 0 offen
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_clause 0x1
	buffer_load_b128 v[27:30], v5, s[24:27], 0 offen
	buffer_load_b128 v[31:34], v7, s[24:27], 0 offen
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_lshlrev_b32_e32 v3, 1, v0
	v_lshlrev_b32_e32 v5, 3, v0
	v_lshlrev_b32_e32 v4, 5, v10
	s_mov_b32 s4, 0
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmpk_gt_i32 s9, 0x5ff
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_and_b32_e32 v3, 48, v3
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	v_xor_b32_e32 v52, v9, v3
	v_lshlrev_b32_e32 v3, 6, v43
	v_add_nc_u32_e32 v6, 0, v52
	s_waitcnt vmcnt(5)
	ds_store_b128 v6, v[11:14] offset:16384
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_waitcnt vmcnt(4)
	ds_store_b128 v6, v[15:18]
	s_waitcnt vmcnt(3)
	ds_store_b128 v6, v[19:22] offset:4096
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(2)
	ds_store_b128 v6, v[23:26] offset:20480
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v6, v[27:30] offset:8192
	s_waitcnt vmcnt(0)
	ds_store_b128 v6, v[31:34] offset:12288
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cbranch_scc1 .LBB0_3
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_and_b32_e32 v6, 48, v5
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_and_or_b32 v44, 0x430, v5, v3
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_or3_b32 v48, v4, v6, v3
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_xor_b32_e32 v45, 16, v44
	v_xor_b32_e32 v47, 32, v44
	v_xor_b32_e32 v46, 48, v44
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_xor_b32_e32 v49, 16, v48
	v_xor_b32_e32 v51, 32, v48
	v_xor_b32_e32 v50, 48, v48
	s_clause 0x1
	s_load_b128 s[16:19], s[0:1], 0x10
	s_load_b64 s[12:13], s[0:1], 0x20
	s_cbranch_execz .LBB0_4
; %bb.2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	v_dual_mov_b32 v11, 0 :: v_dual_mov_b32 v12, 0
	v_dual_mov_b32 v13, 0 :: v_dual_mov_b32 v14, 0
	v_dual_mov_b32 v15, 0 :: v_dual_mov_b32 v16, 0
	v_dual_mov_b32 v17, 0 :: v_dual_mov_b32 v18, 0
	v_dual_mov_b32 v19, 0 :: v_dual_mov_b32 v20, 0
	v_dual_mov_b32 v21, 0 :: v_dual_mov_b32 v22, 0
	v_dual_mov_b32 v23, 0 :: v_dual_mov_b32 v24, 0
	v_dual_mov_b32 v25, 0 :: v_dual_mov_b32 v26, 0
	v_dual_mov_b32 v27, 0 :: v_dual_mov_b32 v28, 0
	v_dual_mov_b32 v29, 0 :: v_dual_mov_b32 v30, 0
	v_dual_mov_b32 v31, 0 :: v_dual_mov_b32 v34, 0
	v_dual_mov_b32 v33, 0 :: v_dual_mov_b32 v36, 0
	v_dual_mov_b32 v35, 0 :: v_dual_mov_b32 v38, 0
	v_dual_mov_b32 v37, 0 :: v_dual_mov_b32 v40, 0
	v_dual_mov_b32 v39, 0 :: v_dual_mov_b32 v42, 0
	v_dual_mov_b32 v41, 0 :: v_dual_mov_b32 v32, 0
	s_add_i32 s28, 0, 0x4000
	s_add_i32 s0, 0, 0x5000
	s_add_i32 s1, 0, 0x2000
	s_branch .LBB0_6
.LBB0_3:
                                        ; implicit-def: $vgpr44
                                        ; implicit-def: $vgpr45
                                        ; implicit-def: $vgpr47
                                        ; implicit-def: $vgpr46
                                        ; implicit-def: $vgpr48
                                        ; implicit-def: $vgpr49
                                        ; implicit-def: $vgpr51
                                        ; implicit-def: $vgpr50
	s_clause 0x1
	s_load_b128 s[16:19], s[0:1], 0x10
	s_load_b64 s[12:13], s[0:1], 0x20
.LBB0_4:                                ; %.lr.ph
	.loc	1 363 5 is_stmt 1               ; generate_amdgcn.py:363:5
	v_lshl_or_b32 v6, s8, 7, v2
	s_lshl_b32 s1, s7, 7
	v_and_or_b32 v44, 0x430, v5, v3
	v_dual_mov_b32 v32, 0 :: v_dual_and_b32 v5, 48, v5
	s_delay_alu instid0(VALU_DEP_3)
	v_subrev_nc_u32_e32 v6, s1, v6
.Ltmp14:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:363:28 ]
	s_lshr_b32 s0, s9, 9
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
.Ltmp15:
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_or_b32_e32 v8, 64, v6
	s_mov_b32 s11, s4
	v_add_nc_u32_e32 v7, s15, v2
	v_or3_b32 v48, v4, v5, v3
	v_mad_u64_u32 v[2:3], null, s5, v6, s[6:7]
	v_mad_u64_u32 v[3:4], null, s5, v8, s[6:7]
	s_delay_alu instid0(VALU_DEP_4)
	v_mad_u64_u32 v[4:5], null, s5, v7, s[6:7]
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	v_add3_u32 v53, v2, v1, 0x400
	v_add3_u32 v54, v3, v1, 0x400
	v_xor_b32_e32 v45, 16, v44
	v_add3_u32 v55, v4, v1, 0x400
	v_dual_mov_b32 v1, s4 :: v_dual_mov_b32 v4, s7
	v_xor_b32_e32 v47, 32, v44
	v_xor_b32_e32 v46, 48, v44
	v_xor_b32_e32 v49, 16, v48
	v_xor_b32_e32 v51, 32, v48
	v_xor_b32_e32 v50, 48, v48
	v_dual_mov_b32 v2, s5 :: v_dual_mov_b32 v3, s6
	v_dual_mov_b32 v6, s9 :: v_dual_mov_b32 v5, s8
	v_dual_mov_b32 v8, s11 :: v_dual_mov_b32 v7, s10
	v_dual_mov_b32 v42, 0 :: v_dual_mov_b32 v41, 0
	v_dual_mov_b32 v40, 0 :: v_dual_mov_b32 v39, 0
	v_dual_mov_b32 v38, 0 :: v_dual_mov_b32 v37, 0
	v_dual_mov_b32 v36, 0 :: v_dual_mov_b32 v35, 0
	v_dual_mov_b32 v34, 0 :: v_dual_mov_b32 v33, 0
	v_dual_mov_b32 v30, 0 :: v_dual_mov_b32 v31, 0
	v_dual_mov_b32 v28, 0 :: v_dual_mov_b32 v29, 0
	v_dual_mov_b32 v26, 0 :: v_dual_mov_b32 v27, 0
	v_dual_mov_b32 v24, 0 :: v_dual_mov_b32 v25, 0
	v_dual_mov_b32 v22, 0 :: v_dual_mov_b32 v23, 0
	v_dual_mov_b32 v20, 0 :: v_dual_mov_b32 v21, 0
	v_dual_mov_b32 v18, 0 :: v_dual_mov_b32 v19, 0
	v_dual_mov_b32 v16, 0 :: v_dual_mov_b32 v17, 0
	v_dual_mov_b32 v14, 0 :: v_dual_mov_b32 v15, 0
	v_dual_mov_b32 v12, 0 :: v_dual_mov_b32 v13, 0
	v_mov_b32_e32 v11, 0
	s_add_i32 s30, s0, -2
	s_add_i32 s28, 0, 0x4000
	s_add_i32 s0, 0, 0x5000
	s_add_i32 s1, 0, 0x2000
	s_mov_b32 s5, 1
	s_mov_b32 s26, s22
	s_mov_b32 s27, s23
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	buffer_load_b128 v[88:91], v55, s[20:23], 0 offen
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_clause 0x1
	buffer_load_b128 v[92:95], v53, s[24:27], 0 offen
	buffer_load_b128 v[96:99], v54, s[24:27], 0 offen
	s_mov_b32 s6, s28
	s_mov_b32 s28, s0
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v56, s6, v44
	s_mov_b32 s0, s4
	v_add_nc_u32_e32 v57, s6, v45
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_add_nc_u32_e32 v60, s0, v48
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v58, s6, v47
	v_add_nc_u32_e32 v59, s6, v46
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_add_nc_u32_e32 v61, s0, v49
	v_add_nc_u32_e32 v62, s0, v51
	v_add_nc_u32_e32 v63, s0, v50
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[72:75], v56
	ds_load_b128 v[100:103], v56 offset:2048
	ds_load_b128 v[104:107], v57
	ds_load_b128 v[108:111], v57 offset:2048
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[80:83], v60
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[112:115], v58
	ds_load_b128 v[116:119], v58 offset:2048
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[120:123], v60 offset:4096
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[124:127], v59
	ds_load_b128 v[128:131], v59 offset:2048
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[132:135], v61
	ds_load_b128 v[136:139], v61 offset:4096
	ds_load_b128 v[140:143], v62
	ds_load_b128 v[144:147], v62 offset:4096
	ds_load_b128 v[148:151], v63
	ds_load_b128 v[152:155], v63 offset:4096
	s_mov_b32 s4, s1
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_add_i32 s1, s5, 1
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_waitcnt lgkmcnt(0)
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmp_lt_i32 s1, 2
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_barrier
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cselect_b32 s5, s1, 0
	s_add_i32 s30, s30, -1
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_lshl_b32 s1, s5, 12
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[56:63], v[80:83], v[72:75], v[1:8] neg_lo:[1,1,0]
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_add_i32 s6, s1, 0
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[64:71], v[120:123], v[72:75], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[72:79], v[80:83], v[100:103], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[80:87], v[120:123], v[100:103], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[56:63], v[132:135], v[104:107], v[56:63] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_wmma_i32_16x16x16_iu8 v[64:71], v[136:139], v[104:107], v[64:71] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[72:79], v[132:135], v[108:111], v[72:79] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_4)
	v_wmma_i32_16x16x16_iu8 v[80:87], v[136:139], v[108:111], v[80:87] neg_lo:[1,1,0]
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_add_i32 s1, s6, s1
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[56:63], v[140:143], v[112:115], v[56:63] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[64:71], v[144:147], v[112:115], v[64:71] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[72:79], v[140:143], v[116:119], v[72:79] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[80:87], v[144:147], v[116:119], v[80:87] neg_lo:[1,1,0]
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_add_i32 s0, s6, 0x4000
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[56:63], v[148:151], v[124:127], v[56:63] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[64:71], v[152:155], v[124:127], v[64:71] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[72:79], v[148:151], v[128:131], v[72:79] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[80:87], v[152:155], v[128:131], v[80:87] neg_lo:[1,1,0]
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmp_lg_u32 s30, 0
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v56, v56
	v_cvt_f32_i32_e32 v57, v57
	v_cvt_f32_i32_e32 v58, v58
	v_cvt_f32_i32_e32 v59, v59
	v_cvt_f32_i32_e32 v60, v60
	v_cvt_f32_i32_e32 v61, v61
	v_cvt_f32_i32_e32 v62, v62
	v_cvt_f32_i32_e32 v63, v63
	v_cvt_f32_i32_e32 v64, v64
	v_cvt_f32_i32_e32 v65, v65
	v_cvt_f32_i32_e32 v66, v66
	v_cvt_f32_i32_e32 v67, v67
	v_cvt_f32_i32_e32 v68, v68
	v_cvt_f32_i32_e32 v69, v69
	v_cvt_f32_i32_e32 v70, v70
	v_cvt_f32_i32_e32 v71, v71
	v_cvt_f32_i32_e32 v72, v72
	v_cvt_f32_i32_e32 v73, v73
	v_cvt_f32_i32_e32 v74, v74
	v_cvt_f32_i32_e32 v75, v75
	v_cvt_f32_i32_e32 v76, v76
	v_cvt_f32_i32_e32 v77, v77
	v_cvt_f32_i32_e32 v78, v78
	v_cvt_f32_i32_e32 v79, v79
	v_cvt_f32_i32_e32 v80, v80
	v_cvt_f32_i32_e32 v81, v81
	v_cvt_f32_i32_e32 v82, v82
	v_cvt_f32_i32_e32 v83, v83
	v_cvt_f32_i32_e32 v84, v84
	v_cvt_f32_i32_e32 v85, v85
	v_cvt_f32_i32_e32 v86, v86
	v_cvt_f32_i32_e32 v87, v87
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v39, v39, v60 :: v_dual_add_nc_u32 v54, 0x200, v54
	v_dual_add_f32 v32, v32, v56 :: v_dual_add_nc_u32 v53, 0x200, v53
	v_dual_add_f32 v42, v42, v57 :: v_dual_add_nc_u32 v55, 0x200, v55
	v_dual_add_f32 v31, v31, v67 :: v_dual_add_nc_u32 v156, s6, v52
	v_dual_add_f32 v28, v28, v70 :: v_dual_add_nc_u32 v157, s1, v52
	v_dual_add_f32 v41, v41, v58 :: v_dual_add_f32 v40, v40, v59
	v_dual_add_f32 v38, v38, v61 :: v_dual_add_f32 v37, v37, v62
	v_dual_add_f32 v36, v36, v63 :: v_dual_add_f32 v35, v35, v64
	v_dual_add_f32 v34, v34, v65 :: v_dual_add_f32 v33, v33, v66
	v_dual_add_f32 v30, v30, v68 :: v_dual_add_f32 v29, v29, v69
	v_dual_add_f32 v27, v27, v71 :: v_dual_add_f32 v26, v26, v72
	v_dual_add_f32 v25, v25, v73 :: v_dual_add_f32 v24, v24, v74
	v_dual_add_f32 v23, v23, v75 :: v_dual_add_f32 v22, v22, v76
	v_dual_add_f32 v21, v21, v77 :: v_dual_add_f32 v20, v20, v78
	v_dual_add_f32 v19, v19, v79 :: v_dual_add_f32 v18, v18, v80
	v_dual_add_f32 v17, v17, v81 :: v_dual_add_f32 v16, v16, v82
	v_dual_add_f32 v15, v15, v83 :: v_dual_add_f32 v14, v14, v84
	v_dual_add_f32 v13, v13, v85 :: v_dual_add_f32 v12, v12, v86
	v_add_f32_e32 v11, v11, v87
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_waitcnt vmcnt(2)
	ds_store_b128 v156, v[88:91] offset:16384
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v157, v[92:95]
	s_waitcnt vmcnt(0)
	ds_store_b128 v157, v[96:99] offset:4096
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cbranch_scc1 .LBB0_5
.LBB0_6:                                ; %._crit_edge
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_and_not1_b32 vcc_lo, exec_lo, s3
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_cbranch_vccnz .LBB0_8
; %bb.7:
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_add_nc_u32_e32 v52, s4, v51
	v_add_nc_u32_e32 v1, s4, v49
	v_add_nc_u32_e32 v60, s4, v50
	v_add_nc_u32_e32 v2, s4, v48
	s_mov_b32 s4, 0
	ds_load_b128 v[76:79], v52
	ds_load_b128 v[80:83], v1 offset:4096
	ds_load_b128 v[84:87], v1
	ds_load_b128 v[88:91], v2 offset:4096
	ds_load_b128 v[92:95], v2
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
	s_mov_b32 s11, s4
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v3, s28, v44
	v_add_nc_u32_e32 v1, s28, v45
	ds_load_b128 v[96:99], v3 offset:2048
	ds_load_b128 v[68:71], v3
	v_add_nc_u32_e32 v53, s28, v47
	ds_load_b128 v[100:103], v1 offset:2048
	ds_load_b128 v[104:107], v1
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_dual_mov_b32 v1, s4 :: v_dual_mov_b32 v8, s11
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v61, s28, v46
	ds_load_b128 v[108:111], v53
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_dual_mov_b32 v2, s5 :: v_dual_mov_b32 v3, s6
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[112:115], v60
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_dual_mov_b32 v4, s7 :: v_dual_mov_b32 v5, s8
	v_dual_mov_b32 v6, s9 :: v_dual_mov_b32 v7, s10
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[116:119], v61
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[120:123], v52 offset:4096
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[124:127], v53 offset:2048
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(7)
	v_wmma_i32_16x16x16_iu8 v[52:59], v[92:95], v[68:71], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(5)
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[52:59], v[84:87], v[104:107], v[52:59] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(4)
	v_wmma_i32_16x16x16_iu8 v[52:59], v[76:79], v[108:111], v[52:59] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(2)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[52:59], v[112:115], v[116:119], v[52:59] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v53, v53
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_cvt_f32_i32_e32 v54, v54
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v42, v42, v53
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v53, v55
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[128:131], v60 offset:4096
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[132:135], v61 offset:2048
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v55, v57
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v41, v41, v54
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v54, v56
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v40, v40, v53
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[60:67], v[88:91], v[68:71], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[68:75], v[92:95], v[96:99], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[1:8], v[88:91], v[96:99], v[1:8] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_wmma_i32_16x16x16_iu8 v[60:67], v[80:83], v[104:107], v[60:67] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v56, v58
	v_wmma_i32_16x16x16_iu8 v[68:75], v[84:87], v[100:103], v[68:75] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v57, v59
	v_wmma_i32_16x16x16_iu8 v[1:8], v[80:83], v[100:103], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(3)
	v_wmma_i32_16x16x16_iu8 v[60:67], v[120:123], v[108:111], v[60:67] neg_lo:[1,1,0]
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v39, v39, v54
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(2)
	v_wmma_i32_16x16x16_iu8 v[68:75], v[76:79], v[124:127], v[68:75] neg_lo:[1,1,0]
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v38, v38, v55
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[1:8], v[120:123], v[124:127], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(1)
	v_wmma_i32_16x16x16_iu8 v[60:67], v[128:131], v[116:119], v[60:67] neg_lo:[1,1,0]
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v36, v36, v57
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu8 v[68:75], v[112:115], v[132:135], v[68:75] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[1:8], v[128:131], v[132:135], v[1:8] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v53, v60
	v_cvt_f32_i32_e32 v54, v61
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v37, v37, v56
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v56, v63
	v_cvt_f32_i32_e32 v55, v62
	v_cvt_f32_i32_e32 v57, v64
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v34, v34, v54
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v54, v66
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v35, v35, v53
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v53, v65
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v33, v33, v55 :: v_dual_add_f32 v30, v30, v57
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v55, v67
	v_cvt_f32_i32_e32 v57, v69
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v29, v29, v53
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v53, v70
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v31, v31, v56
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v56, v68
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v28, v28, v54
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v54, v71
	v_cvt_f32_i32_e32 v1, v1
	v_cvt_f32_i32_e32 v2, v2
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v26, v26, v56
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v56, v73
	v_cvt_f32_i32_e32 v3, v3
	v_cvt_f32_i32_e32 v4, v4
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v25, v25, v57
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v57, v74
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v27, v27, v55
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v55, v72
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v24, v24, v53 :: v_dual_add_f32 v23, v23, v54
	v_dual_add_f32 v18, v18, v1 :: v_dual_add_f32 v21, v21, v56
	v_add_f32_e32 v16, v16, v3
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v53, v75
	v_cvt_f32_i32_e32 v1, v5
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v17, v17, v2
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v2, v6
	v_cvt_f32_i32_e32 v3, v7
	v_cvt_f32_i32_e32 v5, v52
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v15, v15, v4
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v4, v8
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v22, v22, v55
	v_add_f32_e32 v20, v20, v57
	v_dual_add_f32 v19, v19, v53 :: v_dual_add_f32 v12, v12, v3
	v_dual_add_f32 v14, v14, v1 :: v_dual_add_f32 v13, v13, v2
	v_dual_add_f32 v11, v11, v4 :: v_dual_add_f32 v32, v32, v5
.LBB0_8:
	.loc	1 0 13 is_stmt 0                ; generate_amdgcn.py:0:13
	v_dual_mov_b32 v1, 0 :: v_dual_mov_b32 v2, 0
	v_dual_mov_b32 v3, 0 :: v_dual_mov_b32 v4, 0
	v_dual_mov_b32 v5, 0 :: v_dual_mov_b32 v6, 0
	v_dual_mov_b32 v7, 0 :: v_dual_mov_b32 v8, 0
	v_dual_mov_b32 v52, 0 :: v_dual_mov_b32 v53, 0
	v_dual_mov_b32 v54, 0 :: v_dual_mov_b32 v55, 0
	v_dual_mov_b32 v56, 0 :: v_dual_mov_b32 v57, 0
	v_dual_mov_b32 v58, 0 :: v_dual_mov_b32 v59, 0
	v_dual_mov_b32 v60, 0 :: v_dual_mov_b32 v61, 0
	v_dual_mov_b32 v62, 0 :: v_dual_mov_b32 v63, 0
	v_dual_mov_b32 v64, 0 :: v_dual_mov_b32 v65, 0
	v_dual_mov_b32 v66, 0 :: v_dual_mov_b32 v67, 0
	v_dual_mov_b32 v68, 0 :: v_dual_mov_b32 v69, 0
	v_dual_mov_b32 v70, 0 :: v_dual_mov_b32 v71, 0
	v_dual_mov_b32 v72, 0 :: v_dual_mov_b32 v73, 0
	v_dual_mov_b32 v74, 0 :: v_dual_mov_b32 v75, 0
	.loc	1 409 19 is_stmt 1              ; generate_amdgcn.py:409:19
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_cbranch_vccnz .LBB0_10
; %bb.9:
	.loc	1 0 19 is_stmt 0                ; generate_amdgcn.py:0:19
	s_mov_b32 s4, 0
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
	s_mov_b32 s11, s4
	.loc	1 406 22 is_stmt 1              ; generate_amdgcn.py:406:22
	v_add_nc_u32_e32 v1, s1, v51
	v_add_nc_u32_e32 v2, s1, v49
	v_add_nc_u32_e32 v4, s1, v48
	ds_load_b128 v[68:71], v1 offset:4096
	ds_load_b128 v[72:75], v1
	ds_load_b128 v[76:79], v2 offset:4096
	ds_load_b128 v[80:83], v2
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v1, s0, v44
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[84:87], v4 offset:4096
	ds_load_b128 v[88:91], v4
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v4, s0, v45
	ds_load_b128 v[92:95], v1 offset:2048
	ds_load_b128 v[60:63], v1
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	v_add_nc_u32_e32 v3, s1, v50
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_add_nc_u32_e32 v2, s0, v47
	v_add_nc_u32_e32 v1, s0, v46
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_mov_b32_e32 v51, s11
	v_mov_b32_e32 v45, s5
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[104:107], v4 offset:2048
	ds_load_b128 v[108:111], v4
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_mov_b32_e32 v47, s7
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[96:99], v2 offset:2048
	ds_load_b128 v[100:103], v2
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_dual_mov_b32 v50, s10 :: v_dual_mov_b32 v49, s9
	v_mov_b32_e32 v48, s8
	v_mov_b32_e32 v46, s6
	v_mov_b32_e32 v44, s4
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[112:115], v1
	.loc	1 406 22                        ; generate_amdgcn.py:406:22
	ds_load_b128 v[116:119], v3
	ds_load_b128 v[120:123], v3 offset:4096
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	ds_load_b128 v[124:127], v1 offset:2048
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(8)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[88:91], v[60:63], v[44:51] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[52:59], v[84:87], v[60:63], v[44:51] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[60:67], v[88:91], v[92:95], v[44:51] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[44:51], v[84:87], v[92:95], v[44:51] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(6)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[80:83], v[108:111], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[52:59], v[76:79], v[108:111], v[52:59] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[60:67], v[80:83], v[104:107], v[60:67] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[44:51], v[76:79], v[104:107], v[44:51] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(4)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[72:75], v[100:103], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[52:59], v[68:71], v[100:103], v[52:59] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[60:67], v[72:75], v[96:99], v[60:67] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[44:51], v[68:71], v[96:99], v[44:51] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(2)
	v_wmma_i32_16x16x16_iu8 v[1:8], v[116:119], v[112:115], v[1:8] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(1)
	v_wmma_i32_16x16x16_iu8 v[52:59], v[120:123], v[112:115], v[52:59] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu8 v[60:67], v[116:119], v[124:127], v[60:67] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[44:51], v[120:123], v[124:127], v[44:51] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v1, v1
	v_cvt_f32_i32_e32 v2, v2
	v_cvt_f32_i32_e32 v3, v3
	v_cvt_f32_i32_e32 v4, v4
	v_cvt_f32_i32_e32 v5, v5
	v_cvt_f32_i32_e32 v6, v6
	v_cvt_f32_i32_e32 v7, v7
	v_cvt_f32_i32_e32 v8, v8
	v_cvt_f32_i32_e32 v52, v52
	v_cvt_f32_i32_e32 v53, v53
	v_cvt_f32_i32_e32 v54, v54
	v_cvt_f32_i32_e32 v55, v55
	v_cvt_f32_i32_e32 v56, v56
	v_cvt_f32_i32_e32 v57, v57
	v_cvt_f32_i32_e32 v58, v58
	v_cvt_f32_i32_e32 v59, v59
	v_cvt_f32_i32_e32 v60, v60
	v_cvt_f32_i32_e32 v61, v61
	v_cvt_f32_i32_e32 v62, v62
	v_cvt_f32_i32_e32 v63, v63
	v_cvt_f32_i32_e32 v64, v64
	v_cvt_f32_i32_e32 v65, v65
	v_cvt_f32_i32_e32 v66, v66
	v_cvt_f32_i32_e32 v67, v67
	v_cvt_f32_i32_e32 v68, v44
	v_cvt_f32_i32_e32 v69, v45
	v_cvt_f32_i32_e32 v70, v46
	v_cvt_f32_i32_e32 v71, v47
	v_cvt_f32_i32_e32 v72, v48
	v_cvt_f32_i32_e32 v73, v49
	v_cvt_f32_i32_e32 v74, v50
	v_cvt_f32_i32_e32 v75, v51
.LBB0_10:
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v1, v32, v1 :: v_dual_and_b32 v44, 0x80, v0
	.loc	1 356 32                        ; generate_amdgcn.py:356:32
	v_bfe_u32 v45, v0, 4, 1
	v_lshrrev_b32_e32 v46, 1, v10
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	s_and_b32 s5, s17, 0xffff
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_lshrrev_b32_e32 v47, 3, v44
	s_mov_b32 s7, 0x31027000
	s_mov_b32 s6, 0x7ffffffe
	.loc	1 435 22                        ; generate_amdgcn.py:435:22
	v_or3_b32 v45, v45, v46, s14
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	s_mov_b32 s4, s16
	.loc	1 434 22 is_stmt 0              ; generate_amdgcn.py:434:22
	v_or3_b32 v43, v47, v43, s15
	.loc	1 429 13 is_stmt 1              ; generate_amdgcn.py:429:13
	v_add_f32_e32 v2, v42, v2
	v_dual_add_f32 v3, v41, v3 :: v_dual_and_b32 v86, 0x7f, v0
	.loc	1 435 22                        ; generate_amdgcn.py:435:22
	v_lshlrev_b32_e32 v45, 1, v45
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v4, v40, v4 :: v_dual_lshlrev_b32 v43, 1, v43
	v_dual_add_f32 v5, v39, v5 :: v_dual_add_f32 v6, v38, v6
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_or_b32_e32 v46, 4, v45
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	v_or_b32_e32 v49, 64, v43
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_or_b32_e32 v79, 0x80, v45
	v_or_b32_e32 v47, 8, v45
	v_or_b32_e32 v80, 0x84, v45
	v_or_b32_e32 v48, 12, v45
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	s_clause 0x1
	buffer_load_u16 v43, v43, s[4:7], 0 offen
	buffer_load_u16 v49, v49, s[4:7], 0 offen
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	s_and_b32 s5, s19, 0xffff
	s_mov_b32 s4, s18
	v_or_b32_e32 v81, 0x88, v45
	v_or_b32_e32 v50, 16, v45
	v_or_b32_e32 v51, 20, v45
	v_or_b32_e32 v76, 24, v45
	v_or_b32_e32 v77, 28, v45
	v_or_b32_e32 v82, 0x8c, v45
	s_clause 0x7
	buffer_load_u16 v78, v45, s[4:7], 0 offen
	buffer_load_u16 v46, v46, s[4:7], 0 offen
	buffer_load_u16 v47, v47, s[4:7], 0 offen
	buffer_load_u16 v48, v48, s[4:7], 0 offen
	buffer_load_u16 v50, v50, s[4:7], 0 offen
	buffer_load_u16 v51, v51, s[4:7], 0 offen
	buffer_load_u16 v76, v76, s[4:7], 0 offen
	buffer_load_u16 v77, v77, s[4:7], 0 offen
	v_or_b32_e32 v83, 0x90, v45
	v_or_b32_e32 v84, 0x94, v45
	v_or_b32_e32 v85, 0x98, v45
	v_or_b32_e32 v45, 0x9c, v45
	s_clause 0x7
	buffer_load_u16 v79, v79, s[4:7], 0 offen
	buffer_load_u16 v80, v80, s[4:7], 0 offen
	buffer_load_u16 v81, v81, s[4:7], 0 offen
	buffer_load_u16 v82, v82, s[4:7], 0 offen
	buffer_load_u16 v83, v83, s[4:7], 0 offen
	buffer_load_u16 v84, v84, s[4:7], 0 offen
	buffer_load_u16 v85, v85, s[4:7], 0 offen
	buffer_load_u16 v45, v45, s[4:7], 0 offen
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v7, v37, v7 :: v_dual_add_f32 v8, v36, v8
	v_dual_add_f32 v58, v28, v58 :: v_dual_add_f32 v59, v27, v59
	v_dual_add_f32 v60, v26, v60 :: v_dual_add_f32 v61, v25, v61
	v_dual_add_f32 v62, v24, v62 :: v_dual_add_f32 v63, v23, v63
	v_dual_add_f32 v66, v20, v66 :: v_dual_add_f32 v67, v19, v67
	v_dual_add_f32 v74, v12, v74 :: v_dual_add_f32 v75, v11, v75
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_lshlrev_b32_e32 v90, 2, v0
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_lshrrev_b32_e32 v88, 7, v0
	v_bfe_i32 v87, v0, 7, 1
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_and_b32_e32 v89, 14, v0
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v3, v41, v3, s2
	v_cndmask_b32_e64 v5, v39, v5, s2
	v_cndmask_b32_e64 v38, v38, v6, s2
	v_cndmask_b32_e64 v7, v37, v7, s2
	v_cndmask_b32_e64 v27, v27, v59, s2
	v_cndmask_b32_e64 v37, v26, v60, s2
	v_cndmask_b32_e64 v39, v24, v62, s2
	v_cndmask_b32_e64 v41, v20, v66, s2
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_and_b32_e32 v6, 0x78, v90
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v59, v11, v75, s2
	v_cndmask_b32_e64 v1, v32, v1, s2
	v_cndmask_b32_e64 v32, v40, v4, s2
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	v_mul_lo_u32 v4, s29, v88
	.loc	1 450 9 is_stmt 0               ; generate_amdgcn.py:450:9
	v_lshl_or_b32 v6, v10, 3, v6
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	s_mul_i32 s15, s15, s29
	.loc	1 363 5 is_stmt 1               ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v28, v28, v58, s2
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	s_add_i32 s40, s15, s14
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v64, v22, v64 :: v_dual_add_f32 v65, v21, v65
	v_dual_add_f32 v68, v18, v68 :: v_dual_add_f32 v69, v17, v69
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	v_add3_u32 v58, s40, v86, v4
	.loc	1 450 9 is_stmt 0               ; generate_amdgcn.py:450:9
	v_lshlrev_b32_e32 v4, 11, v89
	.loc	1 429 13 is_stmt 1              ; generate_amdgcn.py:429:13
	v_dual_add_f32 v52, v35, v52 :: v_dual_add_f32 v53, v34, v53
	v_dual_add_f32 v56, v30, v56 :: v_dual_add_f32 v57, v29, v57
	v_dual_add_f32 v72, v14, v72 :: v_dual_add_f32 v73, v13, v73
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v2, v42, v2, s2
	v_cndmask_b32_e64 v40, v22, v64, s2
	v_cndmask_b32_e64 v42, v18, v68, s2
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v54, v33, v54 :: v_dual_add_f32 v55, v31, v55
	v_dual_add_f32 v70, v16, v70 :: v_dual_add_f32 v71, v15, v71
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v36, v36, v8, s2
	v_cndmask_b32_e64 v35, v35, v52, s2
	v_cndmask_b32_e64 v30, v30, v56, s2
	v_cndmask_b32_e64 v23, v23, v63, s2
	v_cndmask_b32_e64 v52, v17, v69, s2
	v_cndmask_b32_e64 v56, v13, v73, s2
	v_cndmask_b32_e64 v33, v33, v54, s2
	v_cndmask_b32_e64 v31, v31, v55, s2
	v_cndmask_b32_e64 v29, v29, v57, s2
	v_cndmask_b32_e64 v54, v15, v71, s2
	v_cndmask_b32_e64 v55, v14, v72, s2
	v_cndmask_b32_e64 v57, v12, v74, s2
	v_cndmask_b32_e64 v34, v34, v53, s2
	v_cndmask_b32_e64 v25, v25, v61, s2
	v_cndmask_b32_e64 v53, v16, v70, s2
	v_cndmask_b32_e64 v21, v21, v65, s2
	v_cndmask_b32_e64 v19, v19, v67, s2
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 450 23 is_stmt 0              ; generate_amdgcn.py:450:23
	s_lshl_b32 s20, s29, 1
	s_lshl_b32 s21, s29, 2
	s_mul_i32 s22, s29, 6
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_add_lshl_u32 v61, v58, s20, 2
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	s_lshl_b32 s23, s29, 3
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_add_lshl_u32 v62, v58, s21, 2
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	s_mul_i32 s24, s29, 10
	s_mul_i32 s25, s29, 12
	s_mul_i32 s26, s29, 14
	s_lshl_b32 s27, s29, 4
	s_mul_i32 s28, s29, 18
	s_mul_i32 s30, s29, 20
	s_mul_i32 s31, s29, 22
	s_mul_i32 s33, s29, 24
	s_mul_i32 s34, s29, 26
	s_mul_i32 s35, s29, 28
	s_mul_i32 s36, s29, 30
	s_lshl_b32 s37, s29, 5
	s_mul_i32 s38, s29, 34
	s_mul_i32 s39, s29, 36
	s_mul_i32 s19, s29, 38
	s_mul_i32 s18, s29, 40
	s_mul_i32 s17, s29, 42
	s_mul_i32 s16, s29, 44
	s_mul_i32 s11, s29, 46
	s_mul_i32 s10, s29, 48
	s_mul_i32 s9, s29, 50
	s_mul_i32 s8, s29, 52
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	s_and_b32 s13, s13, 0xffff
	s_mov_b32 s14, s6
	s_mov_b32 s15, s7
	v_add_lshl_u32 v63, v58, s22, 2
	v_add_lshl_u32 v64, v58, s23, 2
	v_add_lshl_u32 v65, v58, s24, 2
	v_add_lshl_u32 v66, v58, s25, 2
	v_add_lshl_u32 v67, v58, s26, 2
	v_add_lshl_u32 v68, v58, s27, 2
	v_add_lshl_u32 v69, v58, s28, 2
	v_add_lshl_u32 v70, v58, s30, 2
	v_add_lshl_u32 v71, v58, s31, 2
	v_add_lshl_u32 v72, v58, s33, 2
	v_add_lshl_u32 v73, v58, s34, 2
	v_add_lshl_u32 v74, v58, s35, 2
	v_add_lshl_u32 v75, v58, s36, 2
	v_add_lshl_u32 v86, v58, s37, 2
	v_add_lshl_u32 v88, v58, s39, 2
	.loc	1 450 23                        ; generate_amdgcn.py:450:23
	s_mul_i32 s5, s29, 54
	s_mul_i32 s4, s29, 56
	s_mul_i32 s3, s29, 58
	s_mul_i32 s1, s29, 60
	s_mul_i32 s0, s29, 62
	.loc	1 435 14 is_stmt 1              ; generate_amdgcn.py:435:14
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v22, 16, v84
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v24, 16, v85
	v_lshlrev_b32_e32 v20, 16, v83
	v_lshlrev_b32_e32 v11, 16, v50
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_and_b32_e32 v91, 1, v0
	v_bfe_i32 v0, v0, 0, 1
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v26, 16, v45
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_and_b32_e32 v0, 0x840, v0
	v_xor_b32_e32 v0, v6, v0
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_2) | instid1(VALU_DEP_3)
	v_or3_b32 v0, v0, v4, v44
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	v_lshlrev_b32_e32 v4, 16, v49
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v18, 16, v82
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_add_nc_u32_e32 v96, 0, v0
	v_xad_u32 v97, v0, 8, 0
	v_xad_u32 v98, v0, 16, 0
	v_xad_u32 v99, v0, 24, 0
	v_xad_u32 v100, v0, 32, 0
	v_xad_u32 v101, v0, 40, 0
	v_xad_u32 v102, v0, 48, 0
	v_xad_u32 v103, v0, 56, 0
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	v_lshlrev_b32_e32 v0, 16, v43
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v10, 16, v48
	v_lshlrev_b32_e32 v17, 16, v81
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_and_b32_e32 v13, 0x840, v87
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v15, 16, v79
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v48, v0, v11
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_lshlrev_b32_e32 v8, 6, v91
	v_and_b32_e32 v9, 0x700, v9
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v14, 16, v77
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_lshlrev_b32_e32 v12, 2, v89
	v_add_lshl_u32 v87, v58, s38, 2
	v_xor_b32_e32 v8, v13, v8
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v13, 16, v76
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v76, v0, v15
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_or3_b32 v6, v9, v12, v8
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v12, 16, v51
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_xad_u32 v89, 0x1008, v6, 0
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v105, v4, v12
	v_mul_f32_e32 v81, v0, v22
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v8, 16, v46
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v50, v0, v13 :: v_dual_lshlrev_b32 v9, 16, v47
	.loc	1 450 9                         ; generate_amdgcn.py:450:9
	v_xad_u32 v90, 0x2010, v6, 0
	v_xad_u32 v91, 0x3018, v6, 0
	v_xad_u32 v92, 0x4020, v6, 0
	v_xad_u32 v93, 0x5028, v6, 0
	v_xad_u32 v94, 0x6030, v6, 0
	v_xad_u32 v95, 0x7038, v6, 0
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v13, v4, v13 :: v_dual_add_nc_u32 v44, 0, v6
	v_mul_f32_e32 v85, v4, v8
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	v_lshlrev_b32_e32 v16, 16, v80
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v113, v4, v24
	v_mul_f32_e32 v80, v0, v20
	v_dual_mul_f32 v11, v4, v11 :: v_dual_lshlrev_b32 v6, 16, v78
	v_mul_f32_e32 v46, v0, v9
	v_mul_f32_e32 v83, v0, v26
	v_mul_f32_e32 v9, v4, v9
	v_mul_f32_e32 v104, v4, v10
	v_mul_f32_e32 v43, v0, v6
	v_mul_f32_e32 v45, v0, v8
	v_mul_f32_e32 v82, v0, v24
	v_mul_f32_e32 v84, v4, v6
	v_mul_f32_e32 v107, v4, v15
	v_mul_f32_e32 v106, v4, v14
	v_dual_mul_f32 v108, v4, v16 :: v_dual_mul_f32 v13, v41, v13
	v_mul_f32_e32 v109, v4, v17
	v_mul_f32_e32 v49, v0, v12
	v_mul_f32_e32 v111, v4, v20
	v_mul_f32_e32 v110, v4, v18
	.loc	1 436 9 is_stmt 0               ; generate_amdgcn.py:436:9
	v_mul_f32_e32 v12, v7, v50
	v_dual_mul_f32 v7, v23, v104 :: v_dual_mul_f32 v114, v4, v26
	v_dual_mul_f32 v8, v5, v48 :: v_dual_mul_f32 v5, v39, v9
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v112, v4, v22
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v4, v3, v46 :: v_dual_mul_f32 v3, v25, v85
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v77, v0, v16 :: v_dual_lshlrev_b32 v60, 2, v58
	v_mul_f32_e32 v78, v0, v17
	v_mul_f32_e32 v47, v0, v10
	v_mul_f32_e32 v79, v0, v18
	v_dual_mul_f32 v51, v0, v14 :: v_dual_mul_f32 v2, v2, v45
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v9, v40, v11 :: v_dual_mul_f32 v16, v35, v76
	v_mul_f32_e32 v11, v21, v105
	v_dual_mul_f32 v0, v1, v43 :: v_dual_mul_f32 v15, v19, v106
	v_mul_f32_e32 v1, v37, v84
	v_mul_f32_e32 v17, v42, v107
	v_dual_mul_f32 v18, v34, v77 :: v_dual_mul_f32 v19, v52, v108
	v_mul_f32_e32 v20, v33, v78
	v_dual_mul_f32 v21, v53, v109 :: v_dual_mul_f32 v6, v32, v47
	v_dual_mul_f32 v23, v54, v110 :: v_dual_mul_f32 v22, v31, v79
	v_mul_f32_e32 v10, v38, v49
	v_mul_f32_e32 v14, v36, v51
	v_mul_f32_e32 v24, v30, v80
	v_mul_f32_e32 v26, v29, v81
	v_mul_f32_e32 v28, v28, v82
	v_mul_f32_e32 v30, v27, v83
	v_mul_f32_e32 v25, v55, v111
	v_mul_f32_e32 v27, v56, v112
	v_mul_f32_e32 v29, v57, v113
	v_mul_f32_e32 v31, v59, v114
	.loc	1 450 9 is_stmt 1               ; generate_amdgcn.py:450:9
	ds_store_2addr_stride64_b64 v96, v[0:1], v[16:17] offset1:2
	ds_store_2addr_stride64_b64 v97, v[2:3], v[18:19] offset1:2
	ds_store_2addr_stride64_b64 v98, v[4:5], v[20:21] offset1:2
	ds_store_2addr_stride64_b64 v99, v[6:7], v[22:23] offset1:2
	ds_store_2addr_stride64_b64 v100, v[8:9], v[24:25] offset1:2
	ds_store_2addr_stride64_b64 v101, v[10:11], v[26:27] offset1:2
	ds_store_2addr_stride64_b64 v102, v[12:13], v[28:29] offset1:2
	ds_store_2addr_stride64_b64 v103, v[14:15], v[30:31] offset1:2
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_2addr_b64 v[0:3], v44 offset1:16
	ds_load_2addr_b64 v[4:7], v89 offset1:16
	ds_load_2addr_b64 v[8:11], v90 offset1:16
	ds_load_2addr_b64 v[12:15], v91 offset1:16
	ds_load_2addr_b64 v[16:19], v92 offset1:16
	ds_load_2addr_b64 v[20:23], v93 offset1:16
	ds_load_2addr_b64 v[24:27], v94 offset1:16
	ds_load_2addr_b64 v[28:31], v95 offset1:16
	v_add_lshl_u32 v32, v58, s19, 2
	v_add_lshl_u32 v33, v58, s18, 2
	v_add_lshl_u32 v34, v58, s17, 2
	v_add_lshl_u32 v35, v58, s16, 2
	v_add_lshl_u32 v36, v58, s11, 2
	v_add_lshl_u32 v37, v58, s10, 2
	v_add_lshl_u32 v38, v58, s9, 2
	v_add_lshl_u32 v39, v58, s8, 2
	s_waitcnt lgkmcnt(7)
	buffer_atomic_add_f32 v0, v60, s[12:15], 0 offen
	s_waitcnt lgkmcnt(6)
	buffer_atomic_add_f32 v4, v61, s[12:15], 0 offen
	s_waitcnt lgkmcnt(5)
	buffer_atomic_add_f32 v8, v62, s[12:15], 0 offen
	s_waitcnt lgkmcnt(4)
	buffer_atomic_add_f32 v12, v63, s[12:15], 0 offen
	s_waitcnt lgkmcnt(3)
	buffer_atomic_add_f32 v16, v64, s[12:15], 0 offen
	s_waitcnt lgkmcnt(2)
	buffer_atomic_add_f32 v20, v65, s[12:15], 0 offen
	s_waitcnt lgkmcnt(1)
	buffer_atomic_add_f32 v24, v66, s[12:15], 0 offen
	s_waitcnt lgkmcnt(0)
	s_clause 0x13
	buffer_atomic_add_f32 v28, v67, s[12:15], 0 offen
	buffer_atomic_add_f32 v2, v68, s[12:15], 0 offen
	buffer_atomic_add_f32 v6, v69, s[12:15], 0 offen
	buffer_atomic_add_f32 v10, v70, s[12:15], 0 offen
	buffer_atomic_add_f32 v14, v71, s[12:15], 0 offen
	buffer_atomic_add_f32 v18, v72, s[12:15], 0 offen
	buffer_atomic_add_f32 v22, v73, s[12:15], 0 offen
	buffer_atomic_add_f32 v26, v74, s[12:15], 0 offen
	buffer_atomic_add_f32 v30, v75, s[12:15], 0 offen
	buffer_atomic_add_f32 v1, v86, s[12:15], 0 offen
	buffer_atomic_add_f32 v5, v87, s[12:15], 0 offen
	buffer_atomic_add_f32 v9, v88, s[12:15], 0 offen
	buffer_atomic_add_f32 v13, v32, s[12:15], 0 offen
	buffer_atomic_add_f32 v17, v33, s[12:15], 0 offen
	buffer_atomic_add_f32 v21, v34, s[12:15], 0 offen
	buffer_atomic_add_f32 v25, v35, s[12:15], 0 offen
	buffer_atomic_add_f32 v29, v36, s[12:15], 0 offen
	buffer_atomic_add_f32 v3, v37, s[12:15], 0 offen
	buffer_atomic_add_f32 v7, v38, s[12:15], 0 offen
	buffer_atomic_add_f32 v11, v39, s[12:15], 0 offen
	v_add_lshl_u32 v40, v58, s5, 2
	v_add_lshl_u32 v41, v58, s4, 2
	v_add_lshl_u32 v42, v58, s3, 2
	v_add_lshl_u32 v43, v58, s1, 2
	v_add_lshl_u32 v0, v58, s0, 2
	s_clause 0x4
	buffer_atomic_add_f32 v15, v40, s[12:15], 0 offen
	buffer_atomic_add_f32 v19, v41, s[12:15], 0 offen
	buffer_atomic_add_f32 v23, v42, s[12:15], 0 offen
	buffer_atomic_add_f32 v27, v43, s[12:15], 0 offen
	buffer_atomic_add_f32 v31, v0, s[12:15], 0 offen
	.loc	1 338 1                         ; generate_amdgcn.py:338:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp16:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 41
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 45
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk, .Lfunc_end0-amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.num_vgpr, 158
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.numbered_sgpr, 41
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 5704
; TotalNumSgprs: 43
; NumVgprs: 158
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 43
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	5                               ; DW_FORM_data2
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x5f DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x39 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	347                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4e:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	346                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x5b:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	363                             ; DW_AT_call_line
	.byte	28                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk"             ; string offset=75 ; amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk
    .private_segment_fixed_size: 0
    .sgpr_count:     43
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int8xint8_nt_pc_none_bm64_bn128_bk64_gm4_w8_s3_weu2_sk8_evenk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     158
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
