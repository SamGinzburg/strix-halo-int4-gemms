	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk               ; -- Begin function amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk,@function
amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk:                      ; @amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 338 0                         ; generate_amdgcn.py:338:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_clause 0x1
	s_load_b64 s[24:25], s[0:1], 0x30
	s_load_b128 s[12:15], s[0:1], 0x0
.Ltmp0:
	.loc	1 349 16 prologue_end           ; generate_amdgcn.py:349:16
	s_abs_i32 s7, s2
	.loc	1 356 32                        ; generate_amdgcn.py:356:32
	v_and_b32_e32 v14, 0xf0, v0
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_and_b32_e32 v12, 15, v0
	v_lshlrev_b32_e32 v31, 2, v0
	v_or_b32_e32 v33, 0x3f0, v0
	v_and_b32_e32 v34, 8, v0
	.loc	1 357 14                        ; generate_amdgcn.py:357:14
	v_lshrrev_b32_e32 v3, 2, v14
	v_and_b32_e32 v35, 32, v0
	.loc	1 355 32                        ; generate_amdgcn.py:355:32
	v_and_b32_e32 v2, 12, v31
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v56, 0, v31
.Ltmp1:
	.file	2 "<repo>/.venv/lib/python3.12/site-packages/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:347:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s25, 0xff
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:346:17 ]
	s_add_i32 s8, s24, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_ashr_i32 s10, s8, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_lshr_b32 s5, s5, 24
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_lshr_b32 s10, s10, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_add_i32 s8, s8, s10
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:347:17 ]
	s_ashr_i32 s4, s4, 8
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:346:17 ]
	s_ashr_i32 s8, s8, 4
.Ltmp11:
	.loc	1 348 24 is_stmt 1              ; generate_amdgcn.py:348:24
	s_lshl_b32 s4, s4, 2
	s_mov_b64 s[20:21], s[14:15]
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	s_abs_i32 s5, s4
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_mul_lo_u32 v4, s25, v3
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	s_cvt_f32_u32 s6, s5
	s_sub_i32 s9, 0, s5
	s_mov_b32 s15, 0x31027000
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_mov_b32 s23, s15
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	v_rcp_iflag_f32_e32 v1, s6
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_lshl_add_u32 v9, v12, 4, v4
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(TRANS32_DEP_1)
	v_mad_u64_u32 v[10:11], null, s25, 3, v[9:10]
	v_lshl_add_u32 v32, s25, 1, v9
	.loc	1 349 16                        ; generate_amdgcn.py:349:16
	v_readfirstlane_b32 s6, v1
	s_mul_f32 s6, s6, 0x4f7ffffe
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	s_cvt_u32_f32 s6, s6
	s_mul_i32 s9, s9, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s9, s6, s9
	s_add_i32 s6, s6, s9
	s_xor_b32 s9, s2, s4
	s_mul_hi_u32 s6, s7, s6
	s_ashr_i32 s9, s9, 31
	s_mul_i32 s10, s6, s5
	s_delay_alu instid0(SALU_CYCLE_1)
	s_sub_i32 s7, s7, s10
	s_add_i32 s10, s6, 1
	s_sub_i32 s11, s7, s5
	s_cmp_ge_u32 s7, s5
	s_cselect_b32 s6, s10, s6
	s_cselect_b32 s7, s11, s7
	s_add_i32 s10, s6, 1
	s_cmp_ge_u32 s7, s5
	s_cselect_b32 s5, s10, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s5, s5, s9
	s_sub_i32 s5, s5, s9
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 350 19                        ; generate_amdgcn.py:350:19
	s_lshl_b32 s6, s5, 2
	.loc	1 352 29                        ; generate_amdgcn.py:352:29
	s_mul_i32 s5, s5, s4
	.loc	1 351 31                        ; generate_amdgcn.py:351:31
	s_sub_i32 s7, s8, s6
	.loc	1 352 29                        ; generate_amdgcn.py:352:29
	s_sub_i32 s2, s2, s5
	.loc	1 351 20                        ; generate_amdgcn.py:351:20
	s_min_i32 s7, s7, 4
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_abs_i32 s5, s2
	s_abs_i32 s8, s7
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_2)
	s_cvt_f32_u32 s9, s8
	s_sub_i32 s11, 0, s8
	v_rcp_iflag_f32_e32 v1, s9
	s_load_b32 s9, s[0:1], 0x38
	s_delay_alu instid0(TRANS32_DEP_1) | instskip(SKIP_2) | instid1(VALU_DEP_1)
	v_readfirstlane_b32 s10, v1
	.loc	1 357 14                        ; generate_amdgcn.py:357:14
	v_lshrrev_b32_e32 v1, 2, v0
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_mul_f32 s10, s10, 0x4f7ffffe
	.loc	1 402 30                        ; generate_amdgcn.py:402:30
	v_mul_lo_u32 v1, s24, v1
	s_delay_alu instid0(SALU_CYCLE_2) | instskip(NEXT) | instid1(SALU_CYCLE_3)
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_cvt_u32_f32 s10, s10
	s_mul_i32 s11, s11, s10
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_hi_u32 s4, s10, s11
	s_add_i32 s10, s10, s4
	s_delay_alu instid0(SALU_CYCLE_1)
	s_mul_hi_u32 s4, s5, s10
	s_xor_b32 s10, s2, s7
	s_mul_i32 s11, s4, s8
	s_ashr_i32 s10, s10, 31
	s_sub_i32 s5, s5, s11
	s_add_i32 s11, s4, 1
	s_sub_i32 s14, s5, s8
	s_cmp_ge_u32 s5, s8
	s_cselect_b32 s4, s11, s4
	s_cselect_b32 s5, s14, s5
	s_add_i32 s11, s4, 1
	s_cmp_ge_u32 s5, s8
	s_mov_b32 s14, 0x7ffffffe
	s_cselect_b32 s4, s11, s4
	.loc	1 398 22                        ; generate_amdgcn.py:398:22
	s_lshl_b32 s31, s3, 6
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_xor_b32 s4, s4, s10
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_mov_b32 s22, s14
	.loc	1 353 13                        ; generate_amdgcn.py:353:13
	s_sub_i32 s4, s4, s10
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 352 28                        ; generate_amdgcn.py:352:28
	s_mul_i32 s5, s4, s7
	.loc	1 356 14                        ; generate_amdgcn.py:356:14
	s_lshl_b32 s26, s4, 8
	.loc	1 352 28                        ; generate_amdgcn.py:352:28
	s_sub_i32 s2, s2, s5
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:363:28 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s5, s9, 63
.Ltmp13:
	.loc	1 352 13                        ; generate_amdgcn.py:352:13
	s_add_i32 s2, s2, s6
	.loc	1 402 30                        ; generate_amdgcn.py:402:30
	s_mul_i32 s6, s24, s31
	.loc	1 355 14                        ; generate_amdgcn.py:355:14
	s_lshl_b32 s27, s2, 4
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	s_mul_i32 s2, s25, s31
	.loc	1 402 30                        ; generate_amdgcn.py:402:30
	v_add3_u32 v11, v1, v2, s27
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmp_gt_i32 s5, 63
	s_cselect_b32 s3, -1, 0
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	s_add_i32 s4, s26, s2
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 402 30                        ; generate_amdgcn.py:402:30
	v_add_nc_u32_e32 v1, s6, v11
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v2, s4, v9
	v_add_nc_u32_e32 v4, s4, v32
	v_add_nc_u32_e32 v5, s4, v10
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_and_b32 s13, s13, 0xffff
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_and_b32 s21, s21, 0xffff
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmpk_gt_i32 s5, 0x7f
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_cndmask_b32_e64 v1, 0x80000000, v1, s3
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cselect_b32 s2, -1, 0
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v4, 0x80000000, v4, s3
	.loc	1 408 30 is_stmt 0              ; generate_amdgcn.py:408:30
	s_lshl_b32 s8, s25, 6
	v_add_nc_u32_e32 v19, s25, v2
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v13, 0x80000000, v5, s3
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	s_add_i32 s4, s4, s8
	.loc	1 402 30 is_stmt 1              ; generate_amdgcn.py:402:30
	s_lshl_b32 s7, s24, 6
	.loc	1 402 22 is_stmt 0              ; generate_amdgcn.py:402:22
	buffer_load_b32 v55, v1, s[12:15], 0 offen
	.loc	1 408 22 is_stmt 1              ; generate_amdgcn.py:408:22
	s_clause 0x1
	buffer_load_b128 v[5:8], v4, s[20:23], 0 offen
	buffer_load_b128 v[15:18], v13, s[20:23], 0 offen
	v_cndmask_b32_e64 v1, 0x80000000, v2, s3
	.loc	1 408 30 is_stmt 0              ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v13, s4, v32
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v2, 0x80000000, v19, s3
	.loc	1 402 30 is_stmt 1              ; generate_amdgcn.py:402:30
	v_add3_u32 v4, s6, s7, v11
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v27, s4, v10
	.loc	1 408 22 is_stmt 0              ; generate_amdgcn.py:408:22
	s_clause 0x1
	buffer_load_b128 v[19:22], v1, s[20:23], 0 offen
	buffer_load_b128 v[23:26], v2, s[20:23], 0 offen
	v_cndmask_b32_e64 v2, 0x80000000, v13, s2
	.loc	1 402 22 is_stmt 1              ; generate_amdgcn.py:402:22
	v_cndmask_b32_e64 v1, 0x80000000, v4, s2
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v4, 0x80000000, v27, s2
	s_clause 0x1
	buffer_load_b128 v[27:30], v2, s[20:23], 0 offen
	buffer_load_b128 v[43:46], v4, s[20:23], 0 offen
	.loc	1 408 30 is_stmt 0              ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v2, s4, v9
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_and_b32_e32 v13, 14, v0
	s_mov_b32 s4, 0
	.loc	1 363 5 is_stmt 1               ; generate_amdgcn.py:363:5
	s_cmpk_gt_i32 s5, 0xbf
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v4, s25, v2
	.loc	1 408 22 is_stmt 0              ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v2, 0x80000000, v2, s2
	v_cndmask_b32_e64 v4, 0x80000000, v4, s2
	s_clause 0x1
	buffer_load_b128 v[47:50], v2, s[20:23], 0 offen
	buffer_load_b128 v[51:54], v4, s[20:23], 0 offen
	.loc	1 402 22 is_stmt 1              ; generate_amdgcn.py:402:22
	buffer_load_b32 v2, v1, s[12:15], 0 offen
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_bfe_i32 v1, v0, 0, 1
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_and_b32_e32 v1, 0x420, v1
	v_xor_b32_e32 v4, v1, v3
	v_lshlrev_b32_e32 v1, 6, v0
	v_lshlrev_b32_e32 v3, 5, v0
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_2)
	v_lshl_or_b32 v36, v13, 10, v4
	v_lshlrev_b32_e32 v4, 3, v0
	v_xor_b32_e32 v37, 0x90, v36
	v_xor_b32_e32 v38, 0x120, v36
	v_xor_b32_e32 v39, 0x1b0, v36
	v_xor_b32_e32 v40, 0x210, v36
	v_add_nc_u32_e32 v57, 0, v36
	v_xor_b32_e32 v41, 0x330, v36
	v_xor_b32_e32 v42, 0x3a0, v36
	v_add_nc_u32_e32 v58, 0, v37
	v_add_nc_u32_e32 v59, 0, v38
	v_add_nc_u32_e32 v60, 0, v39
	v_add_nc_u32_e32 v61, 0, v40
	v_add_nc_u32_e32 v62, 0, v41
	v_add_nc_u32_e32 v63, 0, v42
	v_add_nc_u32_e32 v64, 0x4000, v57
	v_add_nc_u32_e32 v65, 0x4000, v58
	v_add_nc_u32_e32 v66, 0x4000, v59
	v_add_nc_u32_e32 v67, 0x4000, v60
	v_add_nc_u32_e32 v68, 0x4000, v61
	v_add_nc_u32_e32 v69, 0x4000, v62
	v_add_nc_u32_e32 v70, 0x4000, v63
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_waitcnt vmcnt(9)
	ds_store_b32 v56, v55 offset:32768
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_waitcnt vmcnt(7)
	v_perm_b32 v55, v15, v5, 0x4000501
	v_perm_b32 v5, v15, v5, 0x6020703
	v_perm_b32 v15, v16, v6, 0x4000501
	v_perm_b32 v6, v16, v6, 0x6020703
	v_perm_b32 v16, v17, v7, 0x4000501
	v_perm_b32 v7, v17, v7, 0x6020703
	v_perm_b32 v17, v18, v8, 0x4000501
	v_perm_b32 v8, v18, v8, 0x6020703
	s_waitcnt vmcnt(5)
	v_perm_b32 v18, v23, v19, 0x4000501
	v_perm_b32 v19, v23, v19, 0x6020703
	v_perm_b32 v23, v24, v20, 0x4000501
	v_perm_b32 v20, v24, v20, 0x6020703
	v_perm_b32 v24, v25, v21, 0x4000501
	v_perm_b32 v21, v25, v21, 0x6020703
	v_perm_b32 v25, v26, v22, 0x4000501
	v_perm_b32 v22, v26, v22, 0x6020703
	s_waitcnt vmcnt(3)
	v_perm_b32 v26, v43, v27, 0x4000501
	v_perm_b32 v27, v43, v27, 0x6020703
	v_perm_b32 v43, v44, v28, 0x4000501
	v_perm_b32 v28, v44, v28, 0x6020703
	v_perm_b32 v44, v45, v29, 0x4000501
	v_perm_b32 v29, v45, v29, 0x6020703
	v_perm_b32 v45, v46, v30, 0x4000501
	v_perm_b32 v30, v46, v30, 0x6020703
	v_perm_b32 v46, v55, v18, 0x7060302
	v_perm_b32 v18, v55, v18, 0x5040100
	v_perm_b32 v55, v5, v19, 0x7060302
	v_perm_b32 v5, v5, v19, 0x5040100
	v_perm_b32 v19, v15, v23, 0x7060302
	v_perm_b32 v15, v15, v23, 0x5040100
	v_perm_b32 v23, v6, v20, 0x7060302
	v_perm_b32 v6, v6, v20, 0x5040100
	v_perm_b32 v20, v16, v24, 0x7060302
	v_perm_b32 v16, v16, v24, 0x5040100
	v_perm_b32 v24, v7, v21, 0x7060302
	v_perm_b32 v7, v7, v21, 0x5040100
	v_perm_b32 v21, v17, v25, 0x7060302
	v_perm_b32 v17, v17, v25, 0x5040100
	v_perm_b32 v25, v8, v22, 0x7060302
	v_perm_b32 v8, v8, v22, 0x5040100
	s_waitcnt vmcnt(1)
	v_perm_b32 v22, v51, v47, 0x4000501
	v_perm_b32 v47, v51, v47, 0x6020703
	v_perm_b32 v51, v52, v48, 0x4000501
	v_perm_b32 v48, v52, v48, 0x6020703
	v_perm_b32 v52, v53, v49, 0x4000501
	v_perm_b32 v49, v53, v49, 0x6020703
	v_perm_b32 v53, v54, v50, 0x4000501
	v_perm_b32 v50, v54, v50, 0x6020703
	ds_store_2addr_b32 v57, v46, v18 offset1:16
	ds_store_2addr_b32 v58, v55, v5 offset1:16
	ds_store_2addr_b32 v59, v19, v15 offset1:16
	ds_store_2addr_b32 v60, v23, v6 offset1:16
	ds_store_2addr_b32 v61, v20, v16 offset1:16
	ds_store_2addr_b32 v57, v24, v7 offset0:160 offset1:176
	ds_store_2addr_b32 v62, v21, v17 offset1:16
	ds_store_2addr_b32 v63, v25, v8 offset1:16
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(0)
	ds_store_b32 v56, v2 offset:33792
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_perm_b32 v2, v26, v22, 0x7060302
	v_perm_b32 v5, v26, v22, 0x5040100
	v_perm_b32 v6, v27, v47, 0x7060302
	v_perm_b32 v7, v27, v47, 0x5040100
	v_perm_b32 v8, v43, v51, 0x7060302
	v_perm_b32 v15, v43, v51, 0x5040100
	v_perm_b32 v16, v28, v48, 0x7060302
	v_perm_b32 v17, v28, v48, 0x5040100
	v_perm_b32 v18, v44, v52, 0x7060302
	v_perm_b32 v19, v44, v52, 0x5040100
	v_perm_b32 v20, v29, v49, 0x7060302
	v_perm_b32 v21, v29, v49, 0x5040100
	v_perm_b32 v22, v45, v53, 0x7060302
	v_perm_b32 v23, v45, v53, 0x5040100
	v_perm_b32 v24, v30, v50, 0x7060302
	v_perm_b32 v25, v30, v50, 0x5040100
	ds_store_2addr_b32 v64, v2, v5 offset1:16
	ds_store_2addr_b32 v65, v6, v7 offset1:16
	ds_store_2addr_b32 v66, v8, v15 offset1:16
	ds_store_2addr_b32 v67, v16, v17 offset1:16
	ds_store_2addr_b32 v68, v18, v19 offset1:16
	ds_store_2addr_b32 v64, v20, v21 offset0:160 offset1:176
	ds_store_2addr_b32 v69, v22, v23 offset1:16
	ds_store_2addr_b32 v70, v24, v25 offset1:16
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cbranch_scc1 .LBB0_3
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_and_b32_e32 v7, 0x1800, v3
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_or_b32_e32 v2, 0x3f0, v0
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_and_b32_e32 v43, 48, v4
	v_and_b32_e32 v6, 8, v0
	v_and_b32_e32 v5, 32, v0
	v_and_or_b32 v44, 0x1c0, v1, v7
	s_clause 0x1
	s_load_b128 s[16:19], s[0:1], 0x10
	s_load_b64 s[0:1], s[0:1], 0x20
	s_cbranch_execz .LBB0_4
; %bb.2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	v_dual_mov_b32 v16, 0 :: v_dual_mov_b32 v17, 0
	v_dual_mov_b32 v18, 0 :: v_dual_mov_b32 v19, 0
	v_dual_mov_b32 v20, 0 :: v_dual_mov_b32 v21, 0
	v_dual_mov_b32 v22, 0 :: v_dual_mov_b32 v23, 0
	v_dual_mov_b32 v24, 0 :: v_dual_mov_b32 v25, 0
	v_dual_mov_b32 v26, 0 :: v_dual_mov_b32 v27, 0
	v_dual_mov_b32 v28, 0 :: v_dual_mov_b32 v29, 0
	v_dual_mov_b32 v30, 0 :: v_dual_mov_b32 v15, 0
	s_add_i32 s29, 0, 0x8000
	s_add_i32 s28, 0, 0x8400
	s_add_i32 s30, 0, 0x4000
	s_branch .LBB0_7
.LBB0_3:
                                        ; implicit-def: $vgpr2
                                        ; implicit-def: $vgpr43
                                        ; implicit-def: $vgpr6
                                        ; implicit-def: $vgpr5
                                        ; implicit-def: $vgpr44
	s_clause 0x1
	s_load_b128 s[16:19], s[0:1], 0x10
	s_load_b64 s[0:1], s[0:1], 0x20
.LBB0_4:                                ; %.lr.ph
	v_bfe_i32 v2, v0, 3, 1
.Ltmp14:
	.loc	2 43 12 is_stmt 1               ; standard.py:43:12 @[ generate_amdgcn.py:363:28 ]
	s_lshr_b32 s33, s5, 6
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	v_dual_mov_b32 v15, 0 :: v_dual_and_b32 v2, 0x210, v2
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
	s_mov_b32 s11, s4
	v_and_b32_e32 v3, 0x1800, v3
	v_and_b32_e32 v43, 48, v4
	v_bfe_i32 v4, v0, 5, 1
.Ltmp15:
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v45, s25, v9
	v_dual_mov_b32 v29, 0 :: v_dual_mov_b32 v26, 0
	v_and_or_b32 v44, 0x1c0, v1, v3
	v_xor_b32_e32 v1, v2, v43
	v_dual_mov_b32 v27, 0 :: v_dual_and_b32 v2, 0x420, v4
	v_mov_b32_e32 v24, 0
	v_mov_b32_e32 v30, 0
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_3) | instid1(VALU_DEP_4)
	v_or_b32_e32 v1, v44, v1
	v_dual_mov_b32 v28, 0 :: v_dual_mov_b32 v25, 0
	v_dual_mov_b32 v22, 0 :: v_dual_mov_b32 v23, 0
	v_mov_b32_e32 v20, 0
	v_xor_b32_e32 v46, v1, v2
	v_dual_mov_b32 v1, s4 :: v_dual_mov_b32 v8, s11
	v_dual_mov_b32 v2, s5 :: v_dual_mov_b32 v3, s6
	s_delay_alu instid0(VALU_DEP_3)
	v_xor_b32_e32 v47, 16, v46
	v_xor_b32_e32 v48, 32, v46
	v_xor_b32_e32 v49, 48, v46
	v_dual_mov_b32 v4, s7 :: v_dual_mov_b32 v5, s8
	v_dual_mov_b32 v6, s9 :: v_dual_mov_b32 v7, s10
	v_dual_mov_b32 v21, 0 :: v_dual_mov_b32 v18, 0
	v_dual_mov_b32 v19, 0 :: v_dual_mov_b32 v16, 0
	v_mov_b32_e32 v17, 0
	s_addk_i32 s31, 0x80
	s_add_i32 s29, 0, 0x8000
	s_add_i32 s28, 0, 0x8400
	s_add_i32 s30, 0, 0x4000
	s_add_i32 s33, s33, -3
	s_mov_b32 s34, 1
	s_mov_b32 s22, s14
	s_mov_b32 s23, s15
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	1 364 14                        ; generate_amdgcn.py:364:14
	s_lshl_b32 s6, s5, 6
	s_mov_b32 s7, s4
	.loc	1 398 17                        ; generate_amdgcn.py:398:17
	s_add_i32 s6, s31, s6
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v55, s7, v49
	.loc	1 402 30                        ; generate_amdgcn.py:402:30
	v_mad_u64_u32 v[50:51], null, s6, s24, v[11:12]
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	s_mul_i32 s6, s6, s25
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_add_i32 s8, s34, 1
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	s_add_i32 s6, s6, s26
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmp_lt_i32 s8, 2
	.loc	1 408 30                        ; generate_amdgcn.py:408:30
	v_add_nc_u32_e32 v51, s6, v9
	v_add_nc_u32_e32 v52, s6, v45
	v_add_nc_u32_e32 v53, s6, v32
	v_add_nc_u32_e32 v54, s6, v10
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	buffer_load_b32 v126, v50, s[12:15], 0 offen
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_clause 0x3
	buffer_load_b128 v[66:69], v51, s[20:23], 0 offen
	buffer_load_b128 v[70:73], v52, s[20:23], 0 offen
	buffer_load_b128 v[74:77], v53, s[20:23], 0 offen
	buffer_load_b128 v[78:81], v54, s[20:23], 0 offen
	v_add_nc_u32_e32 v52, s7, v46
	s_mov_b32 s6, s29
	v_add_nc_u32_e32 v53, s7, v47
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v50, s6, v12
	v_add_nc_u32_e32 v51, s6, v33
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v54, s7, v48
	ds_load_b128 v[58:61], v52
	ds_load_b128 v[82:85], v52 offset:8192
	ds_load_b128 v[86:89], v53
	ds_load_b128 v[90:93], v53 offset:8192
	ds_load_b128 v[94:97], v54
	ds_load_b128 v[98:101], v54 offset:8192
	ds_load_b128 v[102:105], v55
	ds_load_b128 v[106:109], v55 offset:8192
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v52, v50 offset:208
	ds_load_u8 v53, v50 offset:240
	ds_load_u8 v54, v50 offset:224
	ds_load_u8 v55, v50 offset:192
	ds_load_u8 v56, v50 offset:144
	ds_load_u8 v57, v50 offset:176
	ds_load_u8 v62, v50 offset:160
	ds_load_u8 v63, v50 offset:128
	ds_load_u8 v64, v50 offset:80
	ds_load_u8 v65, v50 offset:112
	ds_load_u8 v110, v50 offset:96
	ds_load_u8 v111, v50 offset:64
	ds_load_u8 v112, v50
	ds_load_u8 v113, v50 offset:16
	ds_load_u8 v114, v50 offset:32
	ds_load_u8 v115, v50 offset:48
	ds_load_u8 v116, v50 offset:464
	ds_load_u8 v117, v50 offset:496
	ds_load_u8 v118, v50 offset:480
	ds_load_u8 v119, v50 offset:448
	ds_load_u8 v120, v50 offset:400
	ds_load_u8 v121, v50 offset:432
	ds_load_u8 v122, v50 offset:416
	ds_load_u8 v123, v50 offset:384
	ds_load_u8 v124, v50 offset:336
	ds_load_u8 v125, v50 offset:368
	ds_load_u8 v127, v50 offset:352
	ds_load_u8 v128, v50 offset:320
	ds_load_u8 v129, v50 offset:272
	ds_load_u8 v130, v50 offset:304
	ds_load_u8 v131, v50 offset:288
	ds_load_u8 v132, v50 offset:256
	ds_load_u8 v133, v50 offset:720
	ds_load_u8 v134, v50 offset:752
	ds_load_u8 v135, v50 offset:736
	ds_load_u8 v136, v50 offset:704
	ds_load_u8 v137, v50 offset:656
	ds_load_u8 v138, v50 offset:688
	ds_load_u8 v139, v50 offset:672
	ds_load_u8 v140, v50 offset:640
	ds_load_u8 v141, v50 offset:592
	ds_load_u8 v142, v50 offset:624
	ds_load_u8 v143, v50 offset:608
	ds_load_u8 v144, v50 offset:576
	ds_load_u8 v145, v50 offset:528
	ds_load_u8 v146, v50 offset:560
	ds_load_u8 v147, v50 offset:544
	ds_load_u8 v148, v50 offset:512
	ds_load_u8 v149, v50 offset:976
	ds_load_u8 v51, v51
	ds_load_u8 v150, v50 offset:992
	ds_load_u8 v151, v50 offset:960
	ds_load_u8 v152, v50 offset:912
	ds_load_u8 v153, v50 offset:944
	ds_load_u8 v154, v50 offset:928
	ds_load_u8 v155, v50 offset:896
	ds_load_u8 v156, v50 offset:848
	ds_load_u8 v157, v50 offset:880
	ds_load_u8 v158, v50 offset:864
	ds_load_u8 v159, v50 offset:832
	ds_load_u8 v160, v50 offset:784
	ds_load_u8 v161, v50 offset:816
	ds_load_u8 v162, v50 offset:800
	ds_load_u8 v50, v50 offset:768
	s_waitcnt lgkmcnt(60)
	v_perm_b32 v52, v55, v52, 0xc0c0004
	v_perm_b32 v53, v54, v53, 0xc0c0004
	s_waitcnt lgkmcnt(56)
	v_perm_b32 v54, v63, v56, 0xc0c0004
	v_perm_b32 v55, v62, v57, 0xc0c0004
	s_waitcnt lgkmcnt(52)
	v_perm_b32 v56, v111, v64, 0xc0c0004
	v_perm_b32 v57, v110, v65, 0xc0c0004
	s_waitcnt lgkmcnt(50)
	v_perm_b32 v62, v112, v113, 0xc0c0004
	s_waitcnt lgkmcnt(48)
	v_perm_b32 v63, v114, v115, 0xc0c0004
	s_waitcnt lgkmcnt(44)
	v_perm_b32 v64, v119, v116, 0xc0c0004
	v_perm_b32 v65, v118, v117, 0xc0c0004
	s_waitcnt lgkmcnt(40)
	v_perm_b32 v114, v123, v120, 0xc0c0004
	v_perm_b32 v115, v122, v121, 0xc0c0004
	s_waitcnt lgkmcnt(36)
	v_perm_b32 v118, v128, v124, 0xc0c0004
	v_perm_b32 v119, v127, v125, 0xc0c0004
	s_waitcnt lgkmcnt(32)
	v_perm_b32 v120, v132, v129, 0xc0c0004
	v_perm_b32 v121, v131, v130, 0xc0c0004
	s_waitcnt lgkmcnt(28)
	v_perm_b32 v122, v136, v133, 0xc0c0004
	v_perm_b32 v123, v135, v134, 0xc0c0004
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v124, v140, v137, 0xc0c0004
	v_perm_b32 v125, v139, v138, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v131, v151, v149, 0xc0c0004
	v_perm_b32 v51, v150, v51, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v50, v50, v160, 0xc0c0004
	v_perm_b32 v136, v162, v161, 0xc0c0004
	v_lshl_or_b32 v113, v53, 16, v52
	v_lshl_or_b32 v112, v55, 16, v54
	v_lshl_or_b32 v111, v57, 16, v56
	v_lshl_or_b32 v110, v63, 16, v62
	v_perm_b32 v127, v144, v141, 0xc0c0004
	v_perm_b32 v128, v143, v142, 0xc0c0004
	v_perm_b32 v129, v148, v145, 0xc0c0004
	v_perm_b32 v130, v147, v146, 0xc0c0004
	v_lshl_or_b32 v117, v65, 16, v64
	v_lshl_or_b32 v116, v115, 16, v114
	v_lshl_or_b32 v115, v119, 16, v118
	v_lshl_or_b32 v114, v121, 16, v120
	v_lshl_or_b32 v121, v123, 16, v122
	v_lshl_or_b32 v120, v125, 16, v124
	v_lshl_or_b32 v125, v51, 16, v131
	v_lshl_or_b32 v122, v136, 16, v50
	v_wmma_i32_16x16x16_iu8 v[50:57], v[58:61], v[110:113], v[1:8] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[58:65], v[82:85], v[110:113], v[1:8] neg_lo:[1,1,0]
	v_perm_b32 v132, v155, v152, 0xc0c0004
	v_perm_b32 v133, v154, v153, 0xc0c0004
	v_perm_b32 v134, v159, v156, 0xc0c0004
	v_perm_b32 v135, v158, v157, 0xc0c0004
	v_lshl_or_b32 v119, v128, 16, v127
	v_lshl_or_b32 v118, v130, 16, v129
	v_wmma_i32_16x16x16_iu8 v[50:57], v[86:89], v[114:117], v[50:57] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[58:65], v[90:93], v[114:117], v[58:65] neg_lo:[1,1,0]
	v_lshl_or_b32 v124, v133, 16, v132
	v_lshl_or_b32 v123, v135, 16, v134
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cselect_b32 s34, s8, 0
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[50:57], v[94:97], v[118:121], v[50:57] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[58:65], v[98:101], v[118:121], v[58:65] neg_lo:[1,1,0]
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_lshl_b32 s7, s34, 10
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_lshl_b32 s8, s34, 14
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_add_i32 s7, s7, 0
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[50:57], v[102:105], v[122:125], v[50:57] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[58:65], v[106:109], v[122:125], v[58:65] neg_lo:[1,1,0]
	s_mov_b32 s4, s30
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_add_i32 s30, s8, 0
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_add_i32 s6, s5, 1
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v50, v50
	v_cvt_f32_i32_e32 v51, v51
	v_cvt_f32_i32_e32 v52, v52
	v_cvt_f32_i32_e32 v53, v53
	v_cvt_f32_i32_e32 v54, v54
	v_cvt_f32_i32_e32 v55, v55
	v_cvt_f32_i32_e32 v56, v56
	v_cvt_f32_i32_e32 v57, v57
	v_cvt_f32_i32_e32 v58, v58
	v_cvt_f32_i32_e32 v59, v59
	v_cvt_f32_i32_e32 v60, v60
	v_cvt_f32_i32_e32 v61, v61
	v_cvt_f32_i32_e32 v62, v62
	v_cvt_f32_i32_e32 v63, v63
	v_cvt_f32_i32_e32 v64, v64
	v_cvt_f32_i32_e32 v65, v65
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v15, v15, v50 :: v_dual_add_f32 v30, v30, v51
	v_dual_add_f32 v28, v28, v53 :: v_dual_add_nc_u32 v163, s7, v31
	v_add_f32_e32 v29, v29, v52
	v_dual_add_f32 v27, v27, v54 :: v_dual_add_f32 v26, v26, v55
	v_dual_add_f32 v25, v25, v56 :: v_dual_add_f32 v24, v24, v57
	v_dual_add_f32 v23, v23, v58 :: v_dual_add_f32 v22, v22, v59
	v_dual_add_f32 v19, v19, v62 :: v_dual_add_nc_u32 v164, s30, v36
	v_dual_add_f32 v21, v21, v60 :: v_dual_add_nc_u32 v166, s30, v38
	v_dual_add_f32 v20, v20, v61 :: v_dual_add_nc_u32 v167, s30, v39
	v_dual_add_f32 v17, v17, v64 :: v_dual_add_nc_u32 v170, s30, v42
	v_add_f32_e32 v18, v18, v63
	v_add_f32_e32 v16, v16, v65
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v165, s30, v37
	s_mov_b32 s29, s28
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_add_i32 s28, s7, 0x8000
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cmp_lg_u32 s5, s33
	s_mov_b32 s5, s6
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v168, s30, v40
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v169, s30, v41
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_waitcnt vmcnt(4)
	ds_store_b32 v163, v126 offset:32768
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	s_waitcnt vmcnt(2)
	v_perm_b32 v50, v70, v66, 0x4000501
	v_perm_b32 v52, v70, v66, 0x6020703
	s_waitcnt vmcnt(0)
	v_perm_b32 v51, v78, v74, 0x4000501
	v_perm_b32 v53, v78, v74, 0x6020703
	v_perm_b32 v54, v71, v67, 0x4000501
	v_perm_b32 v55, v79, v75, 0x4000501
	v_perm_b32 v56, v71, v67, 0x6020703
	v_perm_b32 v57, v79, v75, 0x6020703
	v_perm_b32 v58, v72, v68, 0x4000501
	v_perm_b32 v59, v80, v76, 0x4000501
	v_perm_b32 v60, v72, v68, 0x6020703
	v_perm_b32 v61, v80, v76, 0x6020703
	v_perm_b32 v62, v73, v69, 0x4000501
	v_perm_b32 v63, v81, v77, 0x4000501
	v_perm_b32 v64, v73, v69, 0x6020703
	v_perm_b32 v65, v81, v77, 0x6020703
	v_perm_b32 v66, v51, v50, 0x7060302
	v_perm_b32 v50, v51, v50, 0x5040100
	v_perm_b32 v51, v53, v52, 0x7060302
	v_perm_b32 v52, v53, v52, 0x5040100
	v_perm_b32 v53, v55, v54, 0x7060302
	v_perm_b32 v54, v55, v54, 0x5040100
	v_perm_b32 v55, v57, v56, 0x7060302
	v_perm_b32 v56, v57, v56, 0x5040100
	v_perm_b32 v57, v59, v58, 0x7060302
	v_perm_b32 v58, v59, v58, 0x5040100
	v_perm_b32 v59, v61, v60, 0x7060302
	v_perm_b32 v60, v61, v60, 0x5040100
	v_perm_b32 v61, v63, v62, 0x7060302
	v_perm_b32 v62, v63, v62, 0x5040100
	v_perm_b32 v63, v65, v64, 0x7060302
	v_perm_b32 v64, v65, v64, 0x5040100
	ds_store_2addr_b32 v164, v66, v50 offset1:16
	ds_store_2addr_b32 v165, v51, v52 offset1:16
	ds_store_2addr_b32 v166, v53, v54 offset1:16
	ds_store_2addr_b32 v167, v55, v56 offset1:16
	ds_store_2addr_b32 v168, v57, v58 offset1:16
	ds_store_2addr_b32 v164, v59, v60 offset0:160 offset1:176
	ds_store_2addr_b32 v169, v61, v62 offset1:16
	ds_store_2addr_b32 v170, v63, v64 offset1:16
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	s_cbranch_scc1 .LBB0_5
; %bb.6:                                ; %Flow
	.loc	1 0 5 is_stmt 0                 ; generate_amdgcn.py:0:5
	v_dual_mov_b32 v2, v33 :: v_dual_mov_b32 v5, v35
	v_mov_b32_e32 v6, v34
.LBB0_7:                                ; %Flow22
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 408 22 is_stmt 1              ; generate_amdgcn.py:408:22
	v_cmp_eq_u32_e32 vcc_lo, 0, v6
	v_and_b32_e32 v1, 1, v0
	s_mov_b32 s8, 0
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_cndmask_b32_e64 v3, 0x210, 0, vcc_lo
	v_cmp_eq_u32_e32 vcc_lo, 0, v5
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_2) | instid1(VALU_DEP_2)
	v_xor_b32_e32 v3, v3, v43
	v_cndmask_b32_e64 v4, 0x420, 0, vcc_lo
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_and_not1_b32 vcc_lo, exec_lo, s3
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_or_b32_e32 v3, v44, v3
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_xor_b32_e32 v3, v3, v4
	v_xor_b32_e32 v4, 16, v3
	v_xor_b32_e32 v5, 32, v3
	v_xor_b32_e32 v6, 48, v3
	s_cbranch_vccnz .LBB0_9
; %bb.8:
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_mov_b32 s9, s8
	s_mov_b32 s10, s8
	s_mov_b32 s11, s8
	s_mov_b32 s12, s8
	s_mov_b32 s13, s8
	s_mov_b32 s14, s8
	s_mov_b32 s15, s8
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v11, s29, v12
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v31, v11 offset:208
	ds_load_u8 v32, v11 offset:224
	ds_load_u8 v33, v11 offset:192
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v75, s4, v5
	ds_load_b128 v[7:10], v75
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v35, v11 offset:240
	ds_load_u8 v36, v11 offset:144
	ds_load_u8 v37, v11 offset:176
	ds_load_u8 v38, v11 offset:160
	ds_load_u8 v39, v11 offset:128
	ds_load_u8 v40, v11 offset:112
	ds_load_u8 v41, v11 offset:80
	ds_load_u8 v42, v11 offset:96
	ds_load_u8 v43, v11 offset:64
	ds_load_u8 v44, v11
	ds_load_u8 v45, v11 offset:16
	ds_load_u8 v46, v11 offset:32
	ds_load_u8 v55, v11 offset:48
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v34, s4, v3
	ds_load_b128 v[47:50], v34 offset:8192
	ds_load_b128 v[51:54], v34
	v_add_nc_u32_e32 v71, s4, v6
	v_add_nc_u32_e32 v70, s4, v4
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v32, v32, v35, 0xc0c0004
	v_perm_b32 v31, v33, v31, 0xc0c0004
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v33, s29, v2
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v34, v43, v41, 0xc0c0004
	v_perm_b32 v35, v42, v40, 0xc0c0004
	v_lshl_or_b32 v58, v32, 16, v31
	v_perm_b32 v31, v39, v36, 0xc0c0004
	v_perm_b32 v32, v38, v37, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_4)
	v_lshl_or_b32 v56, v35, 16, v34
	ds_load_u8 v79, v33
	v_lshl_or_b32 v57, v32, 16, v31
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v31, v44, v45, 0xc0c0004
	s_waitcnt lgkmcnt(3)
	v_perm_b32 v32, v46, v55, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_1)
	v_lshl_or_b32 v55, v32, 16, v31
	v_mov_b32_e32 v38, s15
	ds_load_u8 v39, v11 offset:464
	ds_load_u8 v40, v11 offset:480
	ds_load_u8 v41, v11 offset:448
	ds_load_u8 v42, v11 offset:496
	ds_load_u8 v43, v11 offset:400
	ds_load_u8 v44, v11 offset:432
	ds_load_u8 v45, v11 offset:416
	ds_load_u8 v46, v11 offset:384
	ds_load_u8 v63, v11 offset:336
	ds_load_u8 v64, v11 offset:352
	ds_load_u8 v65, v11 offset:320
	ds_load_u8 v66, v11 offset:272
	ds_load_u8 v67, v11 offset:304
	ds_load_u8 v68, v11 offset:288
	ds_load_u8 v69, v11 offset:256
	ds_load_u8 v73, v11 offset:368
	ds_load_u8 v74, v11 offset:720
	ds_load_u8 v76, v11 offset:736
	ds_load_u8 v77, v11 offset:704
	ds_load_u8 v78, v11 offset:592
	ds_load_u8 v80, v11 offset:608
	ds_load_u8 v81, v11 offset:576
	ds_load_u8 v82, v11 offset:528
	ds_load_u8 v83, v11 offset:560
	ds_load_u8 v84, v11 offset:544
	ds_load_u8 v85, v11 offset:512
	v_dual_mov_b32 v37, s14 :: v_dual_mov_b32 v36, s13
	v_dual_mov_b32 v35, s12 :: v_dual_mov_b32 v34, s11
	v_dual_mov_b32 v33, s10 :: v_dual_mov_b32 v32, s9
	v_mov_b32_e32 v31, s8
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	ds_load_b128 v[59:62], v70
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v93, v11 offset:976
	ds_load_u8 v94, v11 offset:992
	ds_load_u8 v95, v11 offset:960
	ds_load_u8 v96, v11 offset:912
	ds_load_u8 v97, v11 offset:944
	ds_load_u8 v98, v11 offset:928
	ds_load_u8 v99, v11 offset:896
	s_waitcnt lgkmcnt(30)
	v_perm_b32 v86, v40, v42, 0xc0c0004
	v_perm_b32 v72, v41, v39, 0xc0c0004
	s_waitcnt lgkmcnt(26)
	v_perm_b32 v87, v46, v43, 0xc0c0004
	v_perm_b32 v88, v45, v44, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[39:46], v[51:54], v[55:58], v[31:38] neg_lo:[1,1,0]
	v_lshl_or_b32 v54, v86, 16, v72
	ds_load_u8 v72, v11 offset:752
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v51, v65, v63, 0xc0c0004
	s_waitcnt lgkmcnt(19)
	v_perm_b32 v52, v64, v73, 0xc0c0004
	v_perm_b32 v63, v69, v66, 0xc0c0004
	v_perm_b32 v64, v68, v67, 0xc0c0004
	ds_load_u8 v65, v11 offset:656
	ds_load_u8 v66, v11 offset:688
	ds_load_u8 v67, v11 offset:672
	ds_load_u8 v68, v11 offset:640
	v_lshl_or_b32 v53, v88, 16, v87
	v_lshl_or_b32 v52, v52, 16, v51
	v_lshl_or_b32 v51, v64, 16, v63
	ds_load_u8 v64, v11 offset:624
	ds_load_u8 v86, v11 offset:848
	ds_load_u8 v87, v11 offset:864
	ds_load_u8 v88, v11 offset:832
	ds_load_u8 v89, v11 offset:784
	ds_load_u8 v90, v11 offset:816
	ds_load_u8 v91, v11 offset:800
	ds_load_u8 v92, v11 offset:768
	ds_load_u8 v11, v11 offset:880
	s_waitcnt lgkmcnt(29)
	v_perm_b32 v63, v77, v74, 0xc0c0004
	s_waitcnt lgkmcnt(21)
	v_wmma_i32_16x16x16_iu8 v[39:46], v[59:62], v[51:54], v[39:46] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v69, v76, v72, 0xc0c0004
	v_perm_b32 v72, v84, v83, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v64, v80, v64, 0xc0c0004
	v_perm_b32 v66, v67, v66, 0xc0c0004
	v_perm_b32 v65, v68, v65, 0xc0c0004
	v_perm_b32 v67, v81, v78, 0xc0c0004
	v_perm_b32 v68, v85, v82, 0xc0c0004
	v_lshl_or_b32 v62, v69, 16, v63
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v80, v88, v86, 0xc0c0004
	v_lshl_or_b32 v61, v66, 16, v65
	v_lshl_or_b32 v60, v64, 16, v67
	v_lshl_or_b32 v59, v72, 16, v68
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	ds_load_b128 v[63:66], v70 offset:8192
	ds_load_b128 v[67:70], v71 offset:8192
	ds_load_b128 v[71:74], v71
	ds_load_b128 v[75:78], v75 offset:8192
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v11, v87, v11, 0xc0c0004
	v_perm_b32 v81, v92, v89, 0xc0c0004
	v_perm_b32 v82, v91, v90, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[39:46], v[7:10], v[59:62], v[39:46] neg_lo:[1,1,0]
	v_perm_b32 v7, v95, v93, 0xc0c0004
	v_perm_b32 v8, v94, v79, 0xc0c0004
	v_perm_b32 v9, v99, v96, 0xc0c0004
	v_perm_b32 v79, v98, v97, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	v_lshl_or_b32 v10, v8, 16, v7
	v_lshl_or_b32 v8, v11, 16, v80
	v_lshl_or_b32 v9, v79, 16, v9
	v_lshl_or_b32 v7, v82, 16, v81
	s_waitcnt lgkmcnt(1)
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[39:46], v[71:74], v[7:10], v[39:46] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v11, v40
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v30, v30, v11
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_wmma_i32_16x16x16_iu8 v[31:38], v[47:50], v[55:58], v[31:38] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v11, v45
	s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_1) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[31:38], v[63:66], v[51:54], v[31:38] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	v_wmma_i32_16x16x16_iu8 v[31:38], v[75:78], v[59:62], v[31:38] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_4) | instid1(VALU_DEP_3)
	v_wmma_i32_16x16x16_iu8 v[31:38], v[67:70], v[7:10], v[31:38] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v7, v41
	v_cvt_f32_i32_e32 v8, v42
	v_cvt_f32_i32_e32 v9, v43
	v_cvt_f32_i32_e32 v10, v44
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v29, v29, v7 :: v_dual_add_f32 v28, v28, v8
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v7, v46
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v27, v27, v9 :: v_dual_add_f32 v26, v26, v10
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v8, v31
	v_cvt_f32_i32_e32 v9, v32
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v25, v25, v11
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v10, v33
	v_cvt_f32_i32_e32 v11, v34
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v24, v24, v7 :: v_dual_add_f32 v23, v23, v8
	v_add_f32_e32 v22, v22, v9
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v7, v35
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v20, v20, v11
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v8, v36
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v21, v21, v10
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_cvt_f32_i32_e32 v9, v37
	v_cvt_f32_i32_e32 v10, v38
	v_cvt_f32_i32_e32 v11, v39
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v19, v19, v7 :: v_dual_add_f32 v18, v18, v8
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_dual_add_f32 v17, v17, v9 :: v_dual_add_f32 v16, v16, v10
	v_add_f32_e32 v15, v15, v11
.LBB0_9:
	.loc	1 0 13 is_stmt 0                ; generate_amdgcn.py:0:13
	v_dual_mov_b32 v7, 0 :: v_dual_mov_b32 v8, 0
	v_dual_mov_b32 v9, 0 :: v_dual_mov_b32 v10, 0
	v_dual_mov_b32 v11, 0 :: v_dual_mov_b32 v32, 0
	v_dual_mov_b32 v31, 0 :: v_dual_mov_b32 v34, 0
	v_dual_mov_b32 v33, 0 :: v_dual_mov_b32 v36, 0
	v_dual_mov_b32 v35, 0 :: v_dual_mov_b32 v38, 0
	v_dual_mov_b32 v37, 0 :: v_dual_mov_b32 v40, 0
	v_mov_b32_e32 v39, 0
	v_mov_b32_e32 v41, 0
	.loc	1 409 19 is_stmt 1              ; generate_amdgcn.py:409:19
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_cbranch_vccnz .LBB0_11
; %bb.10:
	.loc	1 0 19 is_stmt 0                ; generate_amdgcn.py:0:19
	s_mov_b32 s4, 0
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_mov_b32 s5, s4
	s_mov_b32 s6, s4
	s_mov_b32 s7, s4
	s_mov_b32 s8, s4
	s_mov_b32 s9, s4
	s_mov_b32 s10, s4
	s_mov_b32 s11, s4
	.loc	1 402 22 is_stmt 1              ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v11, s28, v12
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v7, v11 offset:208
	ds_load_u8 v8, v11 offset:224
	ds_load_u8 v9, v11 offset:192
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	v_add_nc_u32_e32 v31, s30, v5
	v_add_nc_u32_e32 v32, s30, v6
	v_add_nc_u32_e32 v33, s30, v4
	v_add_nc_u32_e32 v34, s30, v3
	ds_load_b128 v[3:6], v31
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v10, v11 offset:240
	ds_load_u8 v35, v11 offset:144
	ds_load_u8 v36, v11 offset:176
	ds_load_u8 v37, v11 offset:160
	ds_load_u8 v38, v11 offset:128
	ds_load_u8 v39, v11 offset:112
	ds_load_u8 v40, v11 offset:80
	ds_load_u8 v41, v11 offset:96
	ds_load_u8 v42, v11 offset:64
	ds_load_u8 v43, v11
	ds_load_u8 v44, v11 offset:16
	ds_load_u8 v45, v11 offset:32
	ds_load_u8 v46, v11 offset:48
	.loc	1 402 22                        ; generate_amdgcn.py:402:22
	v_add_nc_u32_e32 v2, s28, v2
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v48, v8, v10, 0xc0c0004
	v_perm_b32 v47, v9, v7, 0xc0c0004
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	ds_load_b128 v[7:10], v34 offset:8192
	ds_load_b128 v[50:53], v34
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	s_waitcnt lgkmcnt(10)
	v_perm_b32 v34, v38, v35, 0xc0c0004
	v_perm_b32 v35, v37, v36, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v36, v42, v40, 0xc0c0004
	v_perm_b32 v37, v41, v39, 0xc0c0004
	v_lshl_or_b32 v57, v48, 16, v47
	v_lshl_or_b32 v56, v35, 16, v34
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v34, v43, v44, 0xc0c0004
	s_waitcnt lgkmcnt(2)
	v_perm_b32 v35, v45, v46, 0xc0c0004
	v_lshl_or_b32 v55, v37, 16, v36
	s_delay_alu instid0(VALU_DEP_2)
	v_lshl_or_b32 v54, v35, 16, v34
	v_dual_mov_b32 v41, s11 :: v_dual_mov_b32 v40, s10
	ds_load_u8 v66, v11 offset:336
	ds_load_u8 v67, v11 offset:352
	ds_load_u8 v68, v11 offset:320
	ds_load_u8 v69, v11 offset:272
	ds_load_u8 v70, v11 offset:304
	ds_load_u8 v71, v11 offset:288
	ds_load_u8 v72, v11 offset:256
	ds_load_u8 v42, v11 offset:464
	ds_load_u8 v47, v11 offset:480
	ds_load_u8 v48, v11 offset:448
	ds_load_u8 v43, v11 offset:496
	ds_load_u8 v44, v11 offset:400
	ds_load_u8 v45, v11 offset:432
	ds_load_u8 v46, v11 offset:416
	ds_load_u8 v49, v11 offset:384
	ds_load_u8 v74, v11 offset:368
	ds_load_u8 v75, v11 offset:592
	ds_load_u8 v76, v11 offset:608
	ds_load_u8 v77, v11 offset:576
	ds_load_u8 v78, v11 offset:528
	ds_load_u8 v79, v11 offset:560
	ds_load_u8 v80, v11 offset:544
	ds_load_u8 v81, v11 offset:512
	ds_load_u8 v83, v11 offset:720
	ds_load_u8 v84, v11 offset:736
	ds_load_u8 v85, v11 offset:704
	v_dual_mov_b32 v39, s9 :: v_dual_mov_b32 v38, s8
	v_dual_mov_b32 v37, s7 :: v_dual_mov_b32 v36, s6
	v_dual_mov_b32 v35, s5 :: v_dual_mov_b32 v34, s4
	ds_load_u8 v2, v2
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	ds_load_b128 v[58:61], v33 offset:8192
	ds_load_b128 v[62:65], v33
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	ds_load_u8 v33, v11 offset:752
	s_waitcnt lgkmcnt(19)
	v_perm_b32 v82, v47, v43, 0xc0c0004
	v_perm_b32 v73, v48, v42, 0xc0c0004
	s_waitcnt lgkmcnt(15)
	v_perm_b32 v86, v49, v44, 0xc0c0004
	v_perm_b32 v87, v46, v45, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[42:49], v[50:53], v[54:57], v[34:41] neg_lo:[1,1,0]
	v_perm_b32 v50, v68, v66, 0xc0c0004
	s_waitcnt lgkmcnt(14)
	v_perm_b32 v51, v67, v74, 0xc0c0004
	v_perm_b32 v66, v72, v69, 0xc0c0004
	v_perm_b32 v67, v71, v70, 0xc0c0004
	ds_load_u8 v68, v11 offset:656
	ds_load_u8 v69, v11 offset:688
	ds_load_u8 v70, v11 offset:672
	ds_load_u8 v71, v11 offset:640
	v_lshl_or_b32 v53, v82, 16, v73
	v_lshl_or_b32 v52, v87, 16, v86
	v_lshl_or_b32 v51, v51, 16, v50
	v_lshl_or_b32 v50, v67, 16, v66
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v66, v85, v83, 0xc0c0004
	ds_load_u8 v67, v11 offset:624
	ds_load_u8 v82, v11 offset:848
	ds_load_u8 v83, v11 offset:864
	ds_load_u8 v85, v11 offset:832
	ds_load_u8 v86, v11 offset:784
	ds_load_u8 v87, v11 offset:816
	ds_load_u8 v88, v11 offset:800
	ds_load_u8 v89, v11 offset:768
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v33, v84, v33, 0xc0c0004
	ds_load_u8 v84, v11 offset:976
	ds_load_u8 v90, v11 offset:992
	ds_load_u8 v91, v11 offset:960
	ds_load_u8 v92, v11 offset:912
	ds_load_u8 v93, v11 offset:944
	ds_load_u8 v94, v11 offset:928
	ds_load_u8 v95, v11 offset:896
	ds_load_u8 v11, v11 offset:880
	v_perm_b32 v72, v80, v79, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[42:49], v[62:65], v[50:53], v[42:49] neg_lo:[1,1,0]
	v_lshl_or_b32 v65, v33, 16, v66
	v_wmma_i32_16x16x16_iu8 v[34:41], v[7:10], v[54:57], v[34:41] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[34:41], v[58:61], v[50:53], v[34:41] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(17)
	v_perm_b32 v69, v70, v69, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v68, v71, v68, 0xc0c0004
	v_perm_b32 v70, v77, v75, 0xc0c0004
	s_waitcnt lgkmcnt(15)
	v_perm_b32 v67, v76, v67, 0xc0c0004
	v_perm_b32 v71, v81, v78, 0xc0c0004
	s_waitcnt lgkmcnt(6)
	v_perm_b32 v2, v90, v2, 0xc0c0004
	v_lshl_or_b32 v64, v69, 16, v68
	v_perm_b32 v7, v88, v87, 0xc0c0004
	v_lshl_or_b32 v63, v67, 16, v70
	v_lshl_or_b32 v62, v72, 16, v71
	.loc	1 408 22                        ; generate_amdgcn.py:408:22
	ds_load_b128 v[66:69], v31 offset:8192
	ds_load_b128 v[70:73], v32 offset:8192
	ds_load_b128 v[74:77], v32
	.loc	1 409 19                        ; generate_amdgcn.py:409:19
	v_perm_b32 v31, v85, v82, 0xc0c0004
	s_waitcnt lgkmcnt(3)
	v_perm_b32 v11, v83, v11, 0xc0c0004
	v_perm_b32 v32, v89, v86, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[42:49], v[3:6], v[62:65], v[42:49] neg_lo:[1,1,0]
	v_perm_b32 v3, v91, v84, 0xc0c0004
	v_perm_b32 v4, v95, v92, 0xc0c0004
	v_perm_b32 v6, v94, v93, 0xc0c0004
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	v_lshl_or_b32 v5, v2, 16, v3
	v_lshl_or_b32 v3, v11, 16, v31
	v_lshl_or_b32 v4, v6, 16, v4
	v_lshl_or_b32 v2, v7, 16, v32
	s_waitcnt lgkmcnt(2)
	v_wmma_i32_16x16x16_iu8 v[34:41], v[66:69], v[62:65], v[34:41] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(0)
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_wmma_i32_16x16x16_iu8 v[42:49], v[74:77], v[2:5], v[42:49] neg_lo:[1,1,0]
	v_wmma_i32_16x16x16_iu8 v[34:41], v[70:73], v[2:5], v[34:41] neg_lo:[1,1,0]
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cvt_f32_i32_e32 v7, v42
	v_cvt_f32_i32_e32 v8, v43
	s_delay_alu instid0(VALU_DEP_4)
	v_cvt_f32_i32_e32 v9, v44
	v_cvt_f32_i32_e32 v10, v45
	v_cvt_f32_i32_e32 v11, v46
	v_cvt_f32_i32_e32 v31, v47
	v_cvt_f32_i32_e32 v32, v48
	v_cvt_f32_i32_e32 v33, v49
	v_cvt_f32_i32_e32 v34, v34
	v_cvt_f32_i32_e32 v35, v35
	v_cvt_f32_i32_e32 v36, v36
	v_cvt_f32_i32_e32 v37, v37
	v_cvt_f32_i32_e32 v38, v38
	v_cvt_f32_i32_e32 v39, v39
	v_cvt_f32_i32_e32 v40, v40
	v_cvt_f32_i32_e32 v41, v41
.LBB0_11:
	.loc	1 434 22                        ; generate_amdgcn.py:434:22
	v_or_b32_e32 v2, s27, v12
	.loc	1 435 22                        ; generate_amdgcn.py:435:22
	v_or_b32_e32 v3, s26, v0
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	s_and_b32 s5, s17, 0xffff
	s_mov_b32 s7, 0x31027000
	s_mov_b32 s6, 0x7ffffffe
	v_lshlrev_b32_e32 v2, 1, v2
	s_mov_b32 s4, s16
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v4, v30, v8 :: v_dual_lshlrev_b32 v3, 1, v3
	v_dual_add_f32 v5, v29, v9 :: v_dual_add_f32 v6, v28, v10
	.loc	1 434 14                        ; generate_amdgcn.py:434:14
	buffer_load_u16 v42, v2, s[4:7], 0 offen
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	s_and_b32 s5, s19, 0xffff
	s_mov_b32 s4, s18
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_dual_add_f32 v8, v26, v31 :: v_dual_add_f32 v9, v25, v32
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	buffer_load_u16 v2, v3, s[4:7], 0 offen
	.loc	1 429 13                        ; generate_amdgcn.py:429:13
	v_add_f32_e32 v3, v15, v7
	v_dual_add_f32 v7, v27, v11 :: v_dual_add_f32 v10, v24, v33
	v_dual_add_f32 v11, v23, v34 :: v_dual_add_f32 v32, v21, v36
	v_dual_add_f32 v31, v22, v35 :: v_dual_add_f32 v34, v19, v38
	v_dual_add_f32 v33, v20, v37 :: v_dual_add_f32 v36, v17, v40
	v_dual_add_f32 v35, v18, v39 :: v_dual_lshlrev_b32 v38, 2, v14
	v_dual_add_f32 v37, v16, v41 :: v_dual_mov_b32 v44, 0x5410
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v46, v15, v3, s2
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_lshlrev_b32_e32 v1, 5, v1
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v27, v27, v7, s2
	s_delay_alu instid0(VALU_DEP_4)
	v_cndmask_b32_e64 v16, v16, v37, s2
	.loc	1 356 32                        ; generate_amdgcn.py:356:32
	v_lshrrev_b32_e32 v43, 1, v14
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_lshl_add_u32 v14, v14, 1, 0
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v30, v30, v4, s2
	v_cndmask_b32_e64 v29, v29, v5, s2
	v_cndmask_b32_e64 v28, v28, v6, s2
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v26, v26, v8, s2
	v_cndmask_b32_e64 v25, v25, v9, s2
	v_cndmask_b32_e64 v24, v24, v10, s2
	v_cndmask_b32_e64 v23, v23, v11, s2
	.loc	1 448 18                        ; generate_amdgcn.py:448:18
	v_mul_lo_u32 v39, s25, v12
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v21, v21, v32, s2
	v_cndmask_b32_e64 v19, v19, v34, s2
	v_cndmask_b32_e64 v20, v20, v33, s2
	v_cndmask_b32_e64 v17, v17, v36, s2
	v_cndmask_b32_e64 v18, v18, v35, s2
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v40.h, 0
	.loc	1 363 5                         ; generate_amdgcn.py:363:5
	v_cndmask_b32_e64 v22, v22, v31, s2
	.loc	1 448 18                        ; generate_amdgcn.py:448:18
	s_mul_i32 s27, s27, s25
	s_mov_b32 s3, 0x76543210
	v_add3_u32 v36, s27, s26, v39
	.loc	1 448 9 is_stmt 0               ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v31.h, v40.h
	v_mov_b16_e32 v32.h, v40.h
	v_mov_b16_e32 v34.h, v40.h
	v_mov_b16_e32 v33.h, v40.h
	v_mov_b16_e32 v35.h, v40.h
	v_mov_b16_e32 v39.h, v40.h
	.loc	1 448 18                        ; generate_amdgcn.py:448:18
	v_add_lshl_u32 v36, v36, v43, 1
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	s_and_b32 s1, s1, 0xffff
	s_mov_b32 s2, s6
	.loc	1 434 14 is_stmt 1              ; generate_amdgcn.py:434:14
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v37, 16, v42
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_and_b32_e32 v41, 16, v0
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_add3_u32 v0, 0, v38, v1
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v38.h, v40.h
	v_mov_b16_e32 v42.h, v40.h
	.loc	1 435 14                        ; generate_amdgcn.py:435:14
	s_waitcnt vmcnt(0)
	v_lshlrev_b32_e32 v1, 16, v2
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_lshl_add_u32 v0, v13, 1, v0
	ds_store_b32 v0, v1
	s_waitcnt lgkmcnt(0)
	s_barrier
	ds_load_b128 v[0:3], v14
	ds_load_b128 v[4:7], v14 offset:16
	ds_load_b128 v[8:11], v14 offset:512
	ds_load_b128 v[12:15], v14 offset:528
	s_waitcnt lgkmcnt(2)
	v_mul_f32_e32 v4, v4, v37
	s_delay_alu instid0(VALU_DEP_1)
	.loc	1 436 9 is_stmt 0               ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v4, v27, v4 :: v_dual_mov_b32 v45, 0x7632
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_mul_f32_e32 v2, v2, v37
	v_mul_f32_e32 v3, v3, v37
	v_mul_f32_e32 v0, v0, v37
	.loc	1 448 9 is_stmt 1               ; generate_amdgcn.py:448:9
	v_cmp_eq_u32_e32 vcc_lo, 0, v41
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_max_f32_e32 v4, 0, v4
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_mul_f32_e32 v2, v29, v2
	.loc	1 436 16 is_stmt 0              ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v6, v6, v37 :: v_dual_mul_f32 v3, v28, v3
	v_mul_f32_e32 v7, v7, v37
	s_waitcnt lgkmcnt(1)
	v_mul_f32_e32 v8, v8, v37
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_mul_f32_e32 v0, v46, v0
	v_mul_f32_e32 v6, v25, v6
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v10, v10, v37 :: v_dual_mul_f32 v7, v24, v7
	.loc	1 443 15 is_stmt 1              ; generate_amdgcn.py:443:15
	v_max_f32_e32 v3, 0, v3
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v1, v1, v37 :: v_dual_mul_f32 v8, v23, v8
	s_waitcnt lgkmcnt(0)
	s_delay_alu instid0(VALU_DEP_3)
	v_dual_mul_f32 v12, v12, v37 :: v_dual_max_f32 v7, 0, v7
	v_mul_f32_e32 v5, v5, v37
	v_dual_mul_f32 v11, v11, v37 :: v_dual_mul_f32 v10, v21, v10
	v_mul_f32_e32 v14, v14, v37
	.loc	1 436 9 is_stmt 0               ; generate_amdgcn.py:436:9
	v_mul_f32_e32 v1, v30, v1
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v9, v9, v37 :: v_dual_max_f32 v0, 0, v0
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v12, v19, v12 :: v_dual_mul_f32 v5, v26, v5
	.loc	1 443 15 is_stmt 1              ; generate_amdgcn.py:443:15
	v_dual_max_f32 v6, 0, v6 :: v_dual_mul_f32 v13, v13, v37
	v_dual_max_f32 v2, 0, v2 :: v_dual_mul_f32 v11, v20, v11
	.loc	1 436 16                        ; generate_amdgcn.py:436:16
	v_dual_mul_f32 v15, v15, v37 :: v_dual_mul_f32 v14, v17, v14
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_max_f32_e32 v1, 0, v1
	v_dual_max_f32 v5, 0, v5 :: v_dual_mul_f32 v0, v0, v0
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v13, v18, v13 :: v_dual_max_f32 v10, 0, v10
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_dual_max_f32 v14, 0, v14 :: v_dual_mul_f32 v1, v1, v1
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v9, v22, v9 :: v_dual_max_f32 v8, 0, v8
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v31.l, v0.h
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_mul_f32_e32 v10, v10, v10
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v40.l, v1.h
	.loc	1 436 9                         ; generate_amdgcn.py:436:9
	v_dual_mul_f32 v15, v16, v15 :: v_dual_max_f32 v12, 0, v12
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_dual_max_f32 v9, 0, v9 :: v_dual_mul_f32 v2, v2, v2
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_mul_f32_e32 v3, v3, v3
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_and_b32_e32 v17, 1, v31
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_dual_mul_f32 v7, v7, v7 :: v_dual_and_b32 v16, 1, v40
	v_dual_mul_f32 v5, v5, v5 :: v_dual_mul_f32 v14, v14, v14
	v_mul_f32_e32 v12, v12, v12
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v32.l, v2.h
	v_mov_b16_e32 v40.l, v3.h
	v_add3_u32 v0, v0, v17, 0x7fff
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_dual_max_f32 v13, 0, v13 :: v_dual_mul_f32 v6, v6, v6
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_add3_u32 v1, v1, v16, 0x7fff
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_dual_mul_f32 v9, v9, v9 :: v_dual_and_b32 v18, 1, v32
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v1.l, v0.h
	v_and_b32_e32 v16, 1, v40
	v_mov_b16_e32 v40.l, v5.h
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_dual_max_f32 v11, 0, v11 :: v_dual_mul_f32 v4, v4, v4
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v34.l, v6.h
	v_add3_u32 v2, v2, v18, 0x7fff
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_and_b32_e32 v17, 1, v40
	v_mov_b16_e32 v40.l, v7.h
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_dual_mul_f32 v11, v11, v11 :: v_dual_and_b32 v20, 1, v34
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_add3_u32 v0, v3, v16, 0x7fff
	v_mov_b16_e32 v0.l, v2.h
	v_add3_u32 v3, v5, v17, 0x7fff
	v_and_b32_e32 v2, 1, v40
	v_mov_b16_e32 v33.l, v4.h
	v_add3_u32 v6, v6, v20, 0x7fff
	.loc	1 443 15                        ; generate_amdgcn.py:443:15
	v_dual_max_f32 v15, 0, v15 :: v_dual_mul_f32 v8, v8, v8
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_add3_u32 v2, v7, v2, 0x7fff
	v_mov_b16_e32 v40.l, v9.h
	v_mov_b16_e32 v2.l, v6.h
	v_and_b32_e32 v19, 1, v33
	v_mov_b16_e32 v35.l, v8.h
	v_mov_b16_e32 v38.l, v10.h
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_mul_f32_e32 v13, v13, v13
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_cndmask_b32_e32 v6, 0x3276, v45, vcc_lo
	v_add3_u32 v4, v4, v19, 0x7fff
	v_and_b32_e32 v21, 1, v35
	v_and_b32_e32 v22, 1, v38
	v_mov_b16_e32 v42.l, v14.h
	.loc	1 444 15                        ; generate_amdgcn.py:444:15
	v_mul_f32_e32 v15, v15, v15
	.loc	1 448 9                         ; generate_amdgcn.py:448:9
	v_mov_b16_e32 v3.l, v4.h
	v_and_b32_e32 v4, 1, v40
	v_mov_b16_e32 v40.l, v11.h
	v_add3_u32 v8, v8, v21, 0x7fff
	v_add3_u32 v10, v10, v22, 0x7fff
	v_cndmask_b32_e32 v5, v3, v1, vcc_lo
	v_cndmask_b32_e32 v1, v1, v3, vcc_lo
	v_cndmask_b32_e32 v3, 0x1054, v44, vcc_lo
	v_and_b32_e32 v7, 1, v40
	v_mov_b16_e32 v40.l, v13.h
	v_add3_u32 v4, v9, v4, 0x7fff
	v_mov_b16_e32 v4.l, v8.h
	v_cndmask_b32_e32 v8, v2, v0, vcc_lo
	v_cndmask_b32_e32 v0, v0, v2, vcc_lo
	v_lshl_or_b32 v2, v3, 8, v3
	v_lshl_or_b32 v3, v6, 8, v6
	v_add3_u32 v6, v11, v7, 0x7fff
	v_mov_b16_e32 v39.l, v12.h
	v_mov_b16_e32 v6.l, v10.h
	v_and_b32_e32 v7, 1, v40
	v_and_b32_e32 v24, 1, v42
	v_mov_b16_e32 v40.l, v15.h
	v_permlanex16_b32 v9, v0, s3, 0xfedcba98 op_sel:[1,0]
	v_and_b32_e32 v0, 0x540054, v2
	v_add3_u32 v7, v13, v7, 0x7fff
	v_and_b32_e32 v23, 1, v39
	v_and_b32_e32 v2, 0x760076, v3
	v_and_b32_e32 v3, 1, v40
	v_add3_u32 v14, v14, v24, 0x7fff
	v_lshl_or_b32 v0, v0, 4, v0
	v_add3_u32 v12, v12, v23, 0x7fff
	v_lshl_or_b32 v2, v2, 4, v2
	v_add3_u32 v3, v15, v3, 0x7fff
	v_mov_b16_e32 v3.l, v14.h
	v_permlanex16_b32 v1, v1, s3, 0xfedcba98 op_sel:[1,0]
	v_mov_b16_e32 v7.l, v12.h
	v_and_b32_e32 v11, 0x7060706, v2
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_cndmask_b32_e32 v2, v6, v3, vcc_lo
	v_cndmask_b32_e32 v12, v3, v6, vcc_lo
	v_cndmask_b32_e32 v10, v7, v4, vcc_lo
	v_dual_cndmask_b32 v4, v4, v7 :: v_dual_and_b32 v7, 0x5040504, v0
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	v_permlanex16_b32 v13, v2, s3, 0xfedcba98 op_sel:[1,0]
	v_perm_b32 v3, v9, v8, v11
	v_permlanex16_b32 v6, v4, s3, 0xfedcba98 op_sel:[1,0]
	s_delay_alu instid0(VALU_DEP_4)
	v_perm_b32 v0, v1, v5, v7
	v_perm_b32 v1, v1, v5, v11
	v_perm_b32 v2, v9, v8, v7
	s_mov_b32 s3, s7
	v_perm_b32 v4, v6, v10, v7
	v_perm_b32 v5, v6, v10, v11
	v_perm_b32 v6, v13, v12, v7
	v_perm_b32 v7, v13, v12, v11
	s_clause 0x1
	buffer_store_b128 v[0:3], v36, s[0:3], 0 offen
	buffer_store_b128 v[4:7], v36, s[0:3], 0 offen offset:256
	.loc	1 338 1                         ; generate_amdgcn.py:338:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp16:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 35
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 63
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk, .Lfunc_end0-amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_vgpr, 171
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.numbered_sgpr, 35
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 8900
; TotalNumSgprs: 37
; NumVgprs: 171
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 37
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	5                               ; DW_FORM_data2
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x5f DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x39 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	347                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4e:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	346                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x5b:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	363                             ; DW_AT_call_line
	.byte	28                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk"             ; string offset=75 ; amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk
    .private_segment_fixed_size: 0
    .sgpr_count:     37
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int8xint8_tn_pc_relu2_bm16_bn256_bk64_gm4_w8_s3_weu2_sk1_evenk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     171
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
