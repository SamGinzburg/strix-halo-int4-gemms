	.amdgcn_target "amdgcn-amd-amdhsa--gfx1151"
	.amdhsa_code_object_version 5
	.text
	.globl	amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk               ; -- Begin function amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
	.p2align	8
	.type	amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk,@function
amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk:                      ; @amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
.Lfunc_begin0:
	.file	1 "<repo>/scripts" "generate_amdgcn.py"
	.loc	1 309 0                         ; generate_amdgcn.py:309:0
	.cfi_sections .debug_frame
	.cfi_startproc
; %bb.0:
	s_load_b64 s[10:11], s[0:1], 0x30
.Ltmp0:
	.loc	1 319 16 prologue_end           ; generate_amdgcn.py:319:16
	s_abs_i32 s7, s2
	.loc	1 325 32                        ; generate_amdgcn.py:325:32
	v_lshrrev_b32_e32 v2, 4, v0
	v_and_b32_e32 v19, 15, v0
	.loc	1 327 14                        ; generate_amdgcn.py:327:14
	v_lshrrev_b32_e32 v25, 3, v0
	s_load_b32 s21, s[0:1], 0x38
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
	v_lshlrev_b32_e32 v26, 2, v19
	v_or_b32_e32 v27, 32, v25
.Ltmp1:
	.file	2 "<triton>/python/triton/language" "standard.py"
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:317:17 ]
	s_waitcnt lgkmcnt(0)
	s_add_i32 s4, s11, 0x7f
.Ltmp2:
	.loc	2 43 13 is_stmt 0               ; standard.py:43:13 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s8, s10, 15
.Ltmp3:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s5, s4, 31
.Ltmp4:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s10, s8, 31
.Ltmp5:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_lshr_b32 s5, s5, 25
.Ltmp6:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_lshr_b32 s10, s10, 28
.Ltmp7:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_add_i32 s4, s4, s5
.Ltmp8:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_add_i32 s8, s8, s10
.Ltmp9:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:317:17 ]
	s_ashr_i32 s4, s4, 7
.Ltmp10:
	.loc	2 43 12                         ; standard.py:43:12 @[ generate_amdgcn.py:316:17 ]
	s_ashr_i32 s8, s8, 4
.Ltmp11:
	.loc	1 318 24 is_stmt 1              ; generate_amdgcn.py:318:24
	s_lshl_b32 s4, s4, 2
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_mul_lo_u32 v2, s21, v2
	.loc	1 319 16                        ; generate_amdgcn.py:319:16
	s_abs_i32 s5, s4
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_2)
	s_cvt_f32_u32 s6, s5
	s_sub_i32 s9, 0, s5
	v_rcp_iflag_f32_e32 v1, s6
	s_delay_alu instid0(TRANS32_DEP_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_3)
	v_readfirstlane_b32 s6, v1
	s_mul_f32 s6, s6, 0x4f7ffffe
	s_cvt_u32_f32 s6, s6
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_i32 s9, s9, s6
	s_mul_hi_u32 s9, s6, s9
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_4) | instid1(SALU_CYCLE_1)
	s_add_i32 s6, s6, s9
	s_xor_b32 s9, s2, s4
	s_mul_hi_u32 s6, s7, s6
	s_ashr_i32 s9, s9, 31
	s_mul_i32 s10, s6, s5
	s_sub_i32 s7, s7, s10
	s_add_i32 s10, s6, 1
	s_sub_i32 s12, s7, s5
	s_cmp_ge_u32 s7, s5
	s_cselect_b32 s6, s10, s6
	s_cselect_b32 s7, s12, s7
	s_add_i32 s10, s6, 1
	s_cmp_ge_u32 s7, s5
	s_load_b128 s[12:15], s[0:1], 0x0
	s_cselect_b32 s5, s10, s6
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_xor_b32 s5, s5, s9
	s_sub_i32 s5, s5, s9
	s_delay_alu instid0(SALU_CYCLE_1)
	.loc	1 320 19                        ; generate_amdgcn.py:320:19
	s_lshl_b32 s6, s5, 2
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_mul_i32 s5, s5, s4
	.loc	1 321 31                        ; generate_amdgcn.py:321:31
	s_sub_i32 s7, s8, s6
	.loc	1 322 29                        ; generate_amdgcn.py:322:29
	s_sub_i32 s2, s2, s5
	.loc	1 321 20                        ; generate_amdgcn.py:321:20
	s_min_i32 s7, s7, 4
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_abs_i32 s5, s2
	s_abs_i32 s8, s7
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_2)
	s_cvt_f32_u32 s9, s8
	s_sub_i32 s10, 0, s8
	v_rcp_iflag_f32_e32 v1, s9
	s_waitcnt lgkmcnt(0)
	s_mov_b64 s[16:17], s[14:15]
	s_mov_b32 s15, 0x31027000
	s_mov_b32 s14, 0x7ffffffe
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_mov_b32 s19, s15
	s_mov_b32 s18, s14
	s_delay_alu instid0(TRANS32_DEP_1) | instskip(SKIP_1) | instid1(SALU_CYCLE_3)
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	v_readfirstlane_b32 s9, v1
	s_mul_f32 s9, s9, 0x4f7ffffe
	s_cvt_u32_f32 s9, s9
	s_delay_alu instid0(SALU_CYCLE_3) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_mul_i32 s10, s10, s9
	s_mul_hi_u32 s4, s9, s10
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	s_add_i32 s9, s9, s4
	s_mul_hi_u32 s4, s5, s9
	s_xor_b32 s9, s2, s7
	s_mul_i32 s10, s4, s8
	s_ashr_i32 s9, s9, 31
	s_sub_i32 s5, s5, s10
	s_add_i32 s10, s4, 1
	s_sub_i32 s20, s5, s8
	s_cmp_ge_u32 s5, s8
	s_cselect_b32 s4, s10, s4
	s_cselect_b32 s5, s20, s5
	s_add_i32 s10, s4, 1
	s_cmp_ge_u32 s5, s8
	s_cselect_b32 s4, s10, s4
	.loc	1 368 22                        ; generate_amdgcn.py:368:22
	s_lshl_b32 s25, s3, 6
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_xor_b32 s3, s4, s9
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	v_or_b32_e32 v3, s25, v25
	.loc	1 323 13                        ; generate_amdgcn.py:323:13
	s_sub_i32 s3, s3, s9
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	v_or_b32_e32 v4, s25, v27
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_mul_i32 s5, s3, s7
	.loc	1 326 14                        ; generate_amdgcn.py:326:14
	s_lshl_b32 s10, s3, 7
	.loc	1 322 28                        ; generate_amdgcn.py:322:28
	s_sub_i32 s3, s2, s5
	.loc	1 370 23                        ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e64 s2, s21, v3
	.loc	1 322 13                        ; generate_amdgcn.py:322:13
	s_add_i32 s5, s3, s6
.Ltmp12:
	.loc	2 43 13                         ; standard.py:43:13 @[ generate_amdgcn.py:333:28 ]
	s_add_i32 s23, s21, 63
.Ltmp13:
	.loc	1 325 14                        ; generate_amdgcn.py:325:14
	s_lshl_b32 s20, s5, 4
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_mul_i32 s4, s11, s25
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	s_mul_i32 s5, s20, s21
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_gt_i32 s23, 63
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add3_u32 v29, v2, v26, s5
	.loc	1 370 23                        ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e64 s3, s21, v4
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s22, -1, 0
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_add_i32 s4, s10, s4
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_and_b32 s13, s13, 0xffff
	.loc	1 374 30 is_stmt 0              ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v3, s25, v29
	.loc	1 326 32 is_stmt 1              ; generate_amdgcn.py:326:32
	v_lshlrev_b32_e32 v24, 4, v0
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_and_b32 s17, s17, 0xffff
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 s3, s3, s22
	s_and_b32 s2, s2, s22
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_and_b32_e32 v1, 0x70, v24
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_mad_u64_u32 v[17:18], null, s11, v25, v[1:2]
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	v_or_b32_e32 v1, s25, v26
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_3)
	.loc	1 369 23                        ; generate_amdgcn.py:369:23
	v_cmp_gt_i32_e32 vcc_lo, s21, v1
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v1, s4, v17
	v_lshl_add_u32 v28, s11, 5, v17
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, vcc_lo, s22
	s_cmpk_gt_i32 s23, 0x7f
	s_delay_alu instid0(VALU_DEP_2)
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v1, 0x80000000, v1, s2
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s2, -1, 0
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s25, s25, 64
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v2, s4, v28
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	v_or_b32_e32 v12, s25, v26
	v_or_b32_e32 v13, s25, v25
	v_or_b32_e32 v14, s25, v27
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_cndmask_b32_e32 v11, 0x80000000, v3, vcc_lo
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v2, 0x80000000, v2, s3
	.loc	1 378 30 is_stmt 0              ; generate_amdgcn.py:378:30
	s_lshl_b32 s3, s11, 6
	.loc	1 369 23 is_stmt 1              ; generate_amdgcn.py:369:23
	v_cmp_gt_i32_e32 vcc_lo, s21, v12
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v15, s25, v29
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_add_i32 s4, s4, s3
	.loc	1 370 23                        ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e64 s3, s21, v13
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_clause 0x1
	buffer_load_b128 v[3:6], v1, s[16:19], 0 offen
	buffer_load_b128 v[7:10], v2, s[16:19], 0 offen
	.loc	1 378 30 is_stmt 0              ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v1, s4, v17
	v_add_nc_u32_e32 v2, s4, v28
	.loc	1 370 23 is_stmt 1              ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e64 s4, s21, v14
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, vcc_lo, s2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v16, v11, s[12:15], 0 offen
	v_cndmask_b32_e32 v11, 0x80000000, v15, vcc_lo
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s3, s2
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e32 v1, 0x80000000, v1, vcc_lo
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_and_b32 vcc_lo, s4, s2
	s_cmpk_gt_i32 s23, 0xbf
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e32 v2, 0x80000000, v2, vcc_lo
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v15, v11, s[12:15], 0 offen
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_clause 0x1
	buffer_load_b128 v[11:14], v1, s[16:19], 0 offen
	buffer_load_b128 v[20:23], v2, s[16:19], 0 offen
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v1, 1, v0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_lshlrev_b32_e32 v2, 2, v0
	s_mov_b32 s18, 0
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_and_b32_e32 v18, 48, v1
	v_xor_b32_e32 v30, v2, v18
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v18, 0, v24
	v_lshlrev_b32_e32 v2, 3, v0
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v31, 0, v30
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(4)
	ds_store_b128 v18, v[7:10] offset:4096
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt vmcnt(3)
	ds_store_b32 v31, v16 offset:16384
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	ds_store_b128 v18, v[3:6]
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_waitcnt vmcnt(2)
	ds_store_b32 v31, v15 offset:17408
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v18, v[11:14] offset:8192
	s_waitcnt vmcnt(0)
	ds_store_b128 v18, v[20:23] offset:12288
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_2
; %bb.1:                                ; %.._crit_edge_crit_edge
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_and_b32_e32 v3, 48, v2
	s_mov_b32 s3, 0
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_lshl_or_b32 v20, v19, 6, v3
	v_xor_b32_e32 v21, 16, v20
	v_xor_b32_e32 v22, 32, v20
	v_xor_b32_e32 v23, 48, v20
	s_branch .LBB0_3
.LBB0_2:
	.loc	1 0 22 is_stmt 0                ; generate_amdgcn.py:0:22
	s_mov_b32 s3, -1
                                        ; implicit-def: $vgpr20
                                        ; implicit-def: $vgpr21
                                        ; implicit-def: $vgpr22
                                        ; implicit-def: $vgpr23
.LBB0_3:                                ; %Flow35
	s_clause 0x1
	s_load_b128 s[4:7], s[0:1], 0x10
	s_load_b64 s[8:9], s[0:1], 0x20
	v_and_b32_e32 v18, 0x70, v1
	s_and_not1_b32 vcc_lo, exec_lo, s3
	s_mov_b32 s24, 0
	s_cbranch_vccnz .LBB0_7
; %bb.4:                                ; %.lr.ph
	v_dual_mov_b32 v10, 0 :: v_dual_and_b32 v1, 48, v2
.Ltmp14:
	.loc	2 43 12 is_stmt 1               ; standard.py:43:12 @[ generate_amdgcn.py:333:28 ]
	s_lshr_b32 s26, s23, 6
	s_add_i32 s25, s25, 64
	s_add_i32 s23, 0, 0x4000
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_3) | instid1(VALU_DEP_3)
	v_lshl_or_b32 v20, v19, 6, v1
	v_dual_mov_b32 v1, 0 :: v_dual_mov_b32 v12, 0
	v_mov_b32_e32 v9, 0
	s_add_i32 s3, 0, 0x4400
	v_xor_b32_e32 v21, 16, v20
	v_xor_b32_e32 v22, 32, v20
	v_xor_b32_e32 v23, 48, v20
	v_mov_b32_e32 v2, v1
	v_dual_mov_b32 v3, v1 :: v_dual_mov_b32 v16, 0
	v_dual_mov_b32 v4, v1 :: v_dual_mov_b32 v11, 0
	v_dual_mov_b32 v5, v1 :: v_dual_mov_b32 v14, 0
	v_dual_mov_b32 v6, v1 :: v_dual_mov_b32 v15, 0
	v_mov_b32_e32 v7, v1
	v_dual_mov_b32 v8, v1 :: v_dual_mov_b32 v13, 0
	s_add_i32 s1, 0, 0x2000
	s_add_i32 s26, s26, -3
	s_mov_b32 s27, 1
	s_mov_b32 s18, s14
	s_mov_b32 s19, s15
	s_mov_b32 s28, 0
.Ltmp15:
.LBB0_5:                                ; =>This Inner Loop Header: Depth=1
	.loc	2 0 12 is_stmt 0                ; standard.py:0:12
	s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(SALU_CYCLE_1)
	.loc	1 334 14 is_stmt 1              ; generate_amdgcn.py:334:14
	s_lshl_b32 s0, s28, 6
	.loc	1 368 17                        ; generate_amdgcn.py:368:17
	s_add_i32 s0, s25, s0
	s_delay_alu instid0(SALU_CYCLE_1)
	v_or_b32_e32 v31, s0, v26
	v_or_b32_e32 v32, s0, v25
	v_or_b32_e32 v33, s0, v27
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_mul_i32 s29, s0, s11
	.loc	1 374 30                        ; generate_amdgcn.py:374:30
	v_add_nc_u32_e32 v34, s0, v29
	.loc	1 369 23                        ; generate_amdgcn.py:369:23
	v_cmp_gt_i32_e32 vcc_lo, s21, v31
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	s_add_i32 s29, s29, s10
	.loc	1 370 23                        ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e64 s0, s21, v32
	.loc	1 378 30                        ; generate_amdgcn.py:378:30
	v_add_nc_u32_e32 v35, s29, v17
	v_add_nc_u32_e32 v36, s29, v28
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_cndmask_b32_e32 v31, 0x80000000, v34, vcc_lo
	.loc	1 370 23                        ; generate_amdgcn.py:370:23
	v_cmp_gt_i32_e32 vcc_lo, s21, v33
	s_delay_alu instid0(VALU_DEP_4)
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e64 v32, 0x80000000, v35, s0
	s_mov_b32 s0, s23
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	buffer_load_b32 v79, v31, s[12:15], 0 offen
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_cndmask_b32_e32 v33, 0x80000000, v36, vcc_lo
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_clause 0x1
	buffer_load_b128 v[39:42], v32, s[16:19], 0 offen
	buffer_load_b128 v[43:46], v33, s[16:19], 0 offen
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v31, s0, v20
	v_add_nc_u32_e32 v32, s0, v21
	s_mov_b32 s23, s3
	s_mov_b32 s3, s24
	v_add_nc_u32_e32 v33, s0, v22
	v_add_nc_u32_e32 v34, s0, v23
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add3_u32 v35, s3, v18, v19
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[47:50], v31
	ds_load_b128 v[51:54], v32
	ds_load_b128 v[55:58], v33
	ds_load_b128 v[59:62], v34
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v31, v35 offset:1664
	ds_load_u8 v32, v35 offset:1920
	ds_load_u8 v33, v35 offset:1792
	ds_load_u8 v34, v35 offset:1536
	ds_load_u8 v36, v35 offset:1152
	ds_load_u8 v37, v35 offset:1408
	ds_load_u8 v38, v35 offset:1280
	ds_load_u8 v63, v35 offset:1024
	ds_load_u8 v64, v35 offset:640
	ds_load_u8 v65, v35 offset:896
	ds_load_u8 v66, v35 offset:768
	ds_load_u8 v67, v35 offset:512
	ds_load_u8 v68, v35 offset:128
	ds_load_u8 v69, v35 offset:384
	ds_load_u8 v70, v35 offset:256
	ds_load_u8 v71, v35
	ds_load_u8 v72, v35 offset:3712
	ds_load_u8 v73, v35 offset:3968
	ds_load_u8 v74, v35 offset:3840
	ds_load_u8 v75, v35 offset:3584
	ds_load_u8 v76, v35 offset:3200
	ds_load_u8 v77, v35 offset:3456
	ds_load_u8 v78, v35 offset:3328
	ds_load_u8 v80, v35 offset:3072
	ds_load_u8 v81, v35 offset:2688
	ds_load_u8 v82, v35 offset:2944
	ds_load_u8 v83, v35 offset:2816
	ds_load_u8 v84, v35 offset:2560
	ds_load_u8 v85, v35 offset:2176
	ds_load_u8 v86, v35 offset:2432
	ds_load_u8 v87, v35 offset:2304
	ds_load_u8 v88, v35 offset:2048
	ds_load_u8 v89, v35 offset:5760
	ds_load_u8 v90, v35 offset:6016
	ds_load_u8 v91, v35 offset:5888
	ds_load_u8 v92, v35 offset:5632
	ds_load_u8 v93, v35 offset:5248
	ds_load_u8 v94, v35 offset:5504
	ds_load_u8 v95, v35 offset:5376
	ds_load_u8 v96, v35 offset:5120
	ds_load_u8 v97, v35 offset:4736
	ds_load_u8 v98, v35 offset:4992
	ds_load_u8 v99, v35 offset:4864
	ds_load_u8 v100, v35 offset:4608
	ds_load_u8 v101, v35 offset:4224
	ds_load_u8 v102, v35 offset:4480
	ds_load_u8 v103, v35 offset:4352
	ds_load_u8 v104, v35 offset:4096
	ds_load_u8 v105, v35 offset:7808
	ds_load_u8 v106, v35 offset:8064
	ds_load_u8 v107, v35 offset:7936
	ds_load_u8 v108, v35 offset:7680
	ds_load_u8 v109, v35 offset:7296
	ds_load_u8 v110, v35 offset:7552
	ds_load_u8 v111, v35 offset:7424
	ds_load_u8 v112, v35 offset:7168
	ds_load_u8 v113, v35 offset:6784
	ds_load_u8 v114, v35 offset:7040
	ds_load_u8 v115, v35 offset:6912
	ds_load_u8 v116, v35 offset:6656
	ds_load_u8 v117, v35 offset:6272
	ds_load_u8 v118, v35 offset:6528
	ds_load_u8 v119, v35 offset:6400
	ds_load_u8 v35, v35 offset:6144
	s_waitcnt lgkmcnt(60)
	v_perm_b32 v31, v34, v31, 0xc0c0004
	v_perm_b32 v32, v33, v32, 0xc0c0004
	s_waitcnt lgkmcnt(56)
	v_perm_b32 v33, v63, v36, 0xc0c0004
	v_perm_b32 v34, v38, v37, 0xc0c0004
	s_waitcnt lgkmcnt(52)
	v_perm_b32 v36, v67, v64, 0xc0c0004
	v_perm_b32 v37, v66, v65, 0xc0c0004
	s_waitcnt lgkmcnt(48)
	v_perm_b32 v38, v71, v68, 0xc0c0004
	v_perm_b32 v63, v70, v69, 0xc0c0004
	s_waitcnt lgkmcnt(44)
	v_perm_b32 v67, v75, v72, 0xc0c0004
	v_perm_b32 v68, v74, v73, 0xc0c0004
	s_waitcnt lgkmcnt(40)
	v_perm_b32 v69, v80, v76, 0xc0c0004
	v_perm_b32 v71, v78, v77, 0xc0c0004
	s_waitcnt lgkmcnt(36)
	v_perm_b32 v72, v84, v81, 0xc0c0004
	v_perm_b32 v73, v83, v82, 0xc0c0004
	s_waitcnt lgkmcnt(32)
	v_perm_b32 v74, v88, v85, 0xc0c0004
	v_perm_b32 v75, v87, v86, 0xc0c0004
	s_waitcnt lgkmcnt(29)
	v_perm_b32 v77, v91, v90, 0xc0c0004
	v_lshl_or_b32 v66, v32, 16, v31
	s_waitcnt lgkmcnt(1)
	v_perm_b32 v91, v119, v118, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v35, v35, v117, 0xc0c0004
	v_lshl_or_b32 v65, v34, 16, v33
	v_lshl_or_b32 v64, v37, 16, v36
	v_lshl_or_b32 v63, v63, 16, v38
	v_perm_b32 v76, v92, v89, 0xc0c0004
	v_perm_b32 v78, v96, v93, 0xc0c0004
	v_perm_b32 v80, v95, v94, 0xc0c0004
	v_perm_b32 v81, v100, v97, 0xc0c0004
	v_perm_b32 v82, v99, v98, 0xc0c0004
	v_perm_b32 v83, v104, v101, 0xc0c0004
	v_perm_b32 v84, v103, v102, 0xc0c0004
	v_lshl_or_b32 v70, v68, 16, v67
	v_lshl_or_b32 v69, v71, 16, v69
	v_lshl_or_b32 v68, v73, 16, v72
	v_lshl_or_b32 v67, v75, 16, v74
	v_lshl_or_b32 v75, v91, 16, v35
	v_wmma_i32_16x16x16_iu8 v[31:38], v[63:66], v[47:50], v[1:8] neg_lo:[1,1,0]
	v_perm_b32 v85, v108, v105, 0xc0c0004
	v_perm_b32 v86, v107, v106, 0xc0c0004
	v_perm_b32 v87, v112, v109, 0xc0c0004
	v_perm_b32 v88, v111, v110, 0xc0c0004
	v_perm_b32 v89, v116, v113, 0xc0c0004
	v_perm_b32 v90, v115, v114, 0xc0c0004
	v_lshl_or_b32 v74, v77, 16, v76
	v_lshl_or_b32 v73, v80, 16, v78
	v_lshl_or_b32 v72, v82, 16, v81
	v_lshl_or_b32 v71, v84, 16, v83
	v_wmma_i32_16x16x16_iu8 v[31:38], v[67:70], v[51:54], v[31:38] neg_lo:[1,1,0]
	v_lshl_or_b32 v78, v86, 16, v85
	v_lshl_or_b32 v77, v88, 16, v87
	v_lshl_or_b32 v76, v90, 16, v89
	s_mov_b32 s24, s1
	v_wmma_i32_16x16x16_iu8 v[31:38], v[71:74], v[55:58], v[31:38] neg_lo:[1,1,0]
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_add_i32 s1, s27, 1
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_lt_i32 s1, 2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_barrier
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_wmma_i32_16x16x16_iu8 v[31:38], v[75:78], v[59:62], v[31:38] neg_lo:[1,1,0]
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cselect_b32 s27, s1, 0
	s_add_i32 s0, s28, 1
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_lshl_b32 s1, s27, 10
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_lshl_b32 s3, s27, 13
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_cvt_f32_i32_e32 v31, v31
	v_cvt_f32_i32_e32 v32, v32
	v_cvt_f32_i32_e32 v33, v33
	v_cvt_f32_i32_e32 v34, v34
	v_cvt_f32_i32_e32 v35, v35
	v_cvt_f32_i32_e32 v36, v36
	v_cvt_f32_i32_e32 v37, v37
	v_cvt_f32_i32_e32 v38, v38
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_add_i32 s29, s1, 0
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_add_i32 s1, s3, 0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v120, s29, v30
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_add_f32 v10, v10, v32 :: v_dual_add_f32 v9, v9, v31
	v_dual_add_f32 v16, v16, v34 :: v_dual_add_f32 v15, v15, v33
	v_dual_add_f32 v12, v12, v36 :: v_dual_add_f32 v11, v11, v35
	v_dual_add_f32 v14, v14, v38 :: v_dual_add_f32 v13, v13, v37
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v121, s1, v24
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_add_i32 s3, s29, 0x4000
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cmp_lg_u32 s28, s26
	s_mov_b32 s28, s0
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt vmcnt(2)
	ds_store_b32 v120, v79 offset:16384
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	s_waitcnt vmcnt(1)
	ds_store_b128 v121, v[39:42]
	s_waitcnt vmcnt(0)
	ds_store_b128 v121, v[43:46] offset:4096
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	s_cbranch_scc1 .LBB0_5
; %bb.6:                                ; %Flow
	.loc	1 0 5 is_stmt 0                 ; generate_amdgcn.py:0:5
	s_mov_b32 s18, s24
	s_branch .LBB0_8
.LBB0_7:
	v_mov_b32_e32 v9, 0
	s_add_i32 s23, 0, 0x4000
	s_add_i32 s3, 0, 0x4400
	s_add_i32 s1, 0, 0x2000
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_mov_b32_e32 v10, v9
	v_dual_mov_b32 v12, v10 :: v_dual_mov_b32 v11, v9
	v_dual_mov_b32 v16, v10 :: v_dual_mov_b32 v15, v9
	v_dual_mov_b32 v14, v10 :: v_dual_mov_b32 v13, v9
.LBB0_8:                                ; %Flow36
	v_and_b32_e32 v1, 0xf0, v0
	.loc	1 378 22 is_stmt 1              ; generate_amdgcn.py:378:22
	v_or_b32_e32 v2, v18, v19
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s22
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_cbranch_vccnz .LBB0_10
; %bb.9:
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v7, s18, v2
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v8, v7 offset:1664
	ds_load_u8 v17, v7 offset:1920
	ds_load_u8 v24, v7 offset:1792
	ds_load_u8 v25, v7 offset:1536
	ds_load_u8 v26, v7 offset:1152
	ds_load_u8 v27, v7 offset:1408
	ds_load_u8 v28, v7 offset:1280
	ds_load_u8 v29, v7 offset:1024
	ds_load_u8 v30, v7 offset:640
	ds_load_u8 v31, v7 offset:896
	ds_load_u8 v44, v7 offset:768
	ds_load_u8 v45, v7 offset:512
	ds_load_u8 v46, v7 offset:128
	ds_load_u8 v47, v7 offset:256
	ds_load_u8 v48, v7
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v17, v24, v17, 0xc0c0004
	s_waitcnt lgkmcnt(11)
	v_perm_b32 v8, v25, v8, 0xc0c0004
	v_dual_mov_b32 v24, 0 :: v_dual_add_nc_u32 v3, s23, v23
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v32, s23, v22
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_waitcnt lgkmcnt(7)
	v_perm_b32 v25, v29, v26, 0xc0c0004
	v_perm_b32 v26, v28, v27, 0xc0c0004
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v36, s23, v21
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v28, v44, v31, 0xc0c0004
	s_waitcnt lgkmcnt(3)
	v_perm_b32 v27, v45, v30, 0xc0c0004
	v_dual_mov_b32 v31, v24 :: v_dual_add_nc_u32 v40, s23, v20
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v29, v48, v46, 0xc0c0004
	v_lshl_or_b32 v46, v26, 16, v25
	v_lshl_or_b32 v45, v28, 16, v27
	v_mov_b32_e32 v25, v24
	v_mov_b32_e32 v26, v24
	v_mov_b32_e32 v27, v24
	ds_load_u8 v49, v7 offset:384
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[3:6], v3
	ds_load_b128 v[32:35], v32
	ds_load_b128 v[36:39], v36
	ds_load_b128 v[40:43], v40
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v48, v7 offset:3712
	ds_load_u8 v50, v7 offset:3968
	ds_load_u8 v51, v7 offset:3840
	ds_load_u8 v52, v7 offset:3584
	ds_load_u8 v53, v7 offset:3200
	ds_load_u8 v54, v7 offset:3456
	ds_load_u8 v55, v7 offset:3328
	ds_load_u8 v56, v7 offset:3072
	ds_load_u8 v57, v7 offset:2688
	ds_load_u8 v58, v7 offset:2944
	ds_load_u8 v59, v7 offset:2816
	ds_load_u8 v60, v7 offset:2560
	ds_load_u8 v61, v7 offset:2176
	ds_load_u8 v62, v7 offset:2432
	ds_load_u8 v63, v7 offset:2304
	ds_load_u8 v64, v7 offset:2048
	v_mov_b32_e32 v28, v24
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v30, v47, v49, 0xc0c0004
	v_lshl_or_b32 v47, v17, 16, v8
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v8, v52, v48, 0xc0c0004
	v_perm_b32 v17, v51, v50, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v48, v56, v53, 0xc0c0004
	v_lshl_or_b32 v44, v30, 16, v29
	v_mov_b32_e32 v29, v24
	v_mov_b32_e32 v30, v24
	v_perm_b32 v49, v55, v54, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v50, v60, v57, 0xc0c0004
	v_perm_b32 v51, v59, v58, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v52, v64, v61, 0xc0c0004
	v_perm_b32 v53, v63, v62, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[24:31], v[44:47], v[40:43], v[24:31] neg_lo:[1,1,0]
	v_lshl_or_b32 v43, v17, 16, v8
	v_lshl_or_b32 v42, v49, 16, v48
	v_lshl_or_b32 v41, v51, 16, v50
	v_lshl_or_b32 v40, v53, 16, v52
	ds_load_u8 v8, v7 offset:5760
	ds_load_u8 v17, v7 offset:6016
	ds_load_u8 v44, v7 offset:5888
	ds_load_u8 v45, v7 offset:5632
	ds_load_u8 v46, v7 offset:5248
	ds_load_u8 v47, v7 offset:5504
	ds_load_u8 v48, v7 offset:5376
	ds_load_u8 v49, v7 offset:5120
	ds_load_u8 v50, v7 offset:4736
	ds_load_u8 v51, v7 offset:4992
	ds_load_u8 v52, v7 offset:4864
	ds_load_u8 v53, v7 offset:4608
	ds_load_u8 v54, v7 offset:4224
	ds_load_u8 v55, v7 offset:4480
	ds_load_u8 v56, v7 offset:4352
	ds_load_u8 v57, v7 offset:4096
	ds_load_u8 v58, v7 offset:7808
	ds_load_u8 v59, v7 offset:8064
	ds_load_u8 v60, v7 offset:7936
	ds_load_u8 v61, v7 offset:7680
	ds_load_u8 v62, v7 offset:7296
	ds_load_u8 v63, v7 offset:7552
	ds_load_u8 v64, v7 offset:7424
	ds_load_u8 v65, v7 offset:7168
	ds_load_u8 v66, v7 offset:6784
	ds_load_u8 v67, v7 offset:7040
	ds_load_u8 v68, v7 offset:6912
	ds_load_u8 v69, v7 offset:6656
	ds_load_u8 v70, v7 offset:6272
	ds_load_u8 v71, v7 offset:6528
	ds_load_u8 v72, v7 offset:6400
	ds_load_u8 v7, v7 offset:6144
	s_waitcnt lgkmcnt(28)
	v_perm_b32 v8, v45, v8, 0xc0c0004
	v_perm_b32 v17, v44, v17, 0xc0c0004
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v44, v49, v46, 0xc0c0004
	v_perm_b32 v45, v48, v47, 0xc0c0004
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v46, v53, v50, 0xc0c0004
	v_perm_b32 v47, v52, v51, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v48, v57, v54, 0xc0c0004
	v_perm_b32 v49, v56, v55, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[24:31], v[40:43], v[36:39], v[24:31] neg_lo:[1,1,0]
	v_lshl_or_b32 v39, v17, 16, v8
	v_lshl_or_b32 v38, v45, 16, v44
	v_lshl_or_b32 v37, v47, 16, v46
	v_lshl_or_b32 v36, v49, 16, v48
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v8, v61, v58, 0xc0c0004
	v_perm_b32 v17, v60, v59, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v40, v65, v62, 0xc0c0004
	v_perm_b32 v41, v64, v63, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v42, v69, v66, 0xc0c0004
	v_perm_b32 v43, v68, v67, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v7, v7, v70, 0xc0c0004
	v_perm_b32 v44, v72, v71, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[24:31], v[36:39], v[32:35], v[24:31] neg_lo:[1,1,0]
	v_lshl_or_b32 v35, v17, 16, v8
	v_lshl_or_b32 v34, v41, 16, v40
	v_lshl_or_b32 v33, v43, 16, v42
	v_lshl_or_b32 v32, v44, 16, v7
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[24:31], v[32:35], v[3:6], v[24:31] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v3, v27
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cvt_f32_i32_e32 v4, v26
	v_cvt_f32_i32_e32 v5, v29
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	v_cvt_f32_i32_e32 v6, v25
	v_cvt_f32_i32_e32 v7, v24
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_add_f32 v15, v15, v4 :: v_dual_add_f32 v12, v12, v5
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_cvt_f32_i32_e32 v4, v31
	v_cvt_f32_i32_e32 v5, v30
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_add_f32_e32 v16, v16, v3
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_cvt_f32_i32_e32 v3, v28
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_add_f32_e32 v10, v10, v6
	s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_1) | instid1(VALU_DEP_4)
	v_dual_add_f32 v14, v14, v4 :: v_dual_add_f32 v13, v13, v5
	v_add_f32_e32 v9, v9, v7
	v_add_f32_e32 v11, v11, v3
.LBB0_10:
	.loc	1 0 13 is_stmt 0                ; generate_amdgcn.py:0:13
	v_dual_mov_b32 v3, 0 :: v_dual_mov_b32 v4, 0
	v_dual_mov_b32 v5, 0 :: v_dual_mov_b32 v6, 0
	v_dual_mov_b32 v7, 0 :: v_dual_mov_b32 v8, 0
	v_dual_mov_b32 v17, 0 :: v_dual_mov_b32 v24, 0
	.loc	1 379 19 is_stmt 1              ; generate_amdgcn.py:379:19
	s_and_not1_b32 vcc_lo, exec_lo, s2
	s_cbranch_vccnz .LBB0_12
; %bb.11:
	.loc	1 378 22                        ; generate_amdgcn.py:378:22
	v_add_nc_u32_e32 v6, s1, v2
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	v_add_nc_u32_e32 v2, s3, v23
	v_add_nc_u32_e32 v22, s3, v22
	v_add_nc_u32_e32 v21, s3, v21
	v_add_nc_u32_e32 v20, s3, v20
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	ds_load_u8 v7, v6 offset:1664
	ds_load_u8 v8, v6 offset:1920
	ds_load_u8 v17, v6 offset:1792
	ds_load_u8 v24, v6 offset:1536
	ds_load_u8 v25, v6 offset:1152
	ds_load_u8 v26, v6 offset:1408
	ds_load_u8 v27, v6 offset:1280
	ds_load_u8 v40, v6 offset:1024
	ds_load_u8 v41, v6 offset:640
	ds_load_u8 v42, v6 offset:896
	ds_load_u8 v43, v6 offset:768
	ds_load_u8 v44, v6 offset:512
	ds_load_u8 v45, v6 offset:128
	ds_load_u8 v46, v6 offset:256
	ds_load_u8 v47, v6
	ds_load_u8 v23, v6 offset:384
	.loc	1 374 22                        ; generate_amdgcn.py:374:22
	ds_load_b128 v[2:5], v2
	ds_load_b128 v[28:31], v22
	ds_load_b128 v[32:35], v21
	ds_load_b128 v[36:39], v20
	.loc	1 379 19                        ; generate_amdgcn.py:379:19
	v_mov_b32_e32 v20, 0
	s_waitcnt lgkmcnt(17)
	v_perm_b32 v8, v17, v8, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v7, v24, v7, 0xc0c0004
	s_waitcnt lgkmcnt(13)
	v_perm_b32 v21, v27, v26, 0xc0c0004
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v17, v40, v25, 0xc0c0004
	v_mov_b32_e32 v26, v20
	v_mov_b32_e32 v27, v20
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v24, v43, v42, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v22, v44, v41, 0xc0c0004
	v_lshl_or_b32 v42, v21, 16, v17
	v_mov_b32_e32 v21, v20
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v25, v47, v45, 0xc0c0004
	ds_load_u8 v44, v6 offset:3712
	ds_load_u8 v45, v6 offset:3968
	ds_load_u8 v47, v6 offset:3840
	ds_load_u8 v48, v6 offset:3584
	ds_load_u8 v49, v6 offset:3200
	ds_load_u8 v50, v6 offset:3456
	ds_load_u8 v51, v6 offset:3328
	ds_load_u8 v52, v6 offset:3072
	ds_load_u8 v53, v6 offset:2688
	ds_load_u8 v54, v6 offset:2944
	ds_load_u8 v55, v6 offset:2816
	ds_load_u8 v56, v6 offset:2560
	ds_load_u8 v57, v6 offset:2176
	ds_load_u8 v58, v6 offset:2432
	ds_load_u8 v59, v6 offset:2304
	ds_load_u8 v60, v6 offset:2048
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v23, v46, v23, 0xc0c0004
	v_lshl_or_b32 v43, v8, 16, v7
	v_lshl_or_b32 v41, v24, 16, v22
	v_mov_b32_e32 v22, v20
	v_mov_b32_e32 v24, v20
	v_lshl_or_b32 v40, v23, 16, v25
	v_mov_b32_e32 v23, v20
	v_mov_b32_e32 v25, v20
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v7, v48, v44, 0xc0c0004
	v_perm_b32 v8, v47, v45, 0xc0c0004
	s_waitcnt lgkmcnt(9)
	v_perm_b32 v44, v51, v50, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v17, v52, v49, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[20:27], v[40:43], v[36:39], v[20:27] neg_lo:[1,1,0]
	s_waitcnt lgkmcnt(5)
	v_perm_b32 v46, v55, v54, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v45, v56, v53, 0xc0c0004
	v_lshl_or_b32 v39, v8, 16, v7
	v_lshl_or_b32 v38, v44, 16, v17
	s_waitcnt lgkmcnt(1)
	v_perm_b32 v48, v59, v58, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v47, v60, v57, 0xc0c0004
	v_lshl_or_b32 v37, v46, 16, v45
	s_delay_alu instid0(VALU_DEP_2)
	v_lshl_or_b32 v36, v48, 16, v47
	ds_load_u8 v7, v6 offset:5760
	ds_load_u8 v8, v6 offset:6016
	ds_load_u8 v17, v6 offset:5888
	ds_load_u8 v40, v6 offset:5632
	ds_load_u8 v41, v6 offset:5248
	ds_load_u8 v42, v6 offset:5504
	ds_load_u8 v43, v6 offset:5376
	ds_load_u8 v44, v6 offset:5120
	ds_load_u8 v45, v6 offset:4736
	ds_load_u8 v46, v6 offset:4992
	ds_load_u8 v47, v6 offset:4864
	ds_load_u8 v48, v6 offset:4608
	ds_load_u8 v49, v6 offset:4224
	ds_load_u8 v50, v6 offset:4480
	ds_load_u8 v51, v6 offset:4352
	ds_load_u8 v52, v6 offset:4096
	ds_load_u8 v53, v6 offset:7808
	ds_load_u8 v54, v6 offset:8064
	ds_load_u8 v55, v6 offset:7936
	ds_load_u8 v56, v6 offset:7680
	ds_load_u8 v57, v6 offset:7296
	ds_load_u8 v58, v6 offset:7552
	ds_load_u8 v59, v6 offset:7424
	ds_load_u8 v60, v6 offset:7168
	ds_load_u8 v61, v6 offset:6784
	ds_load_u8 v62, v6 offset:7040
	ds_load_u8 v63, v6 offset:6912
	ds_load_u8 v64, v6 offset:6656
	ds_load_u8 v65, v6 offset:6272
	ds_load_u8 v66, v6 offset:6528
	ds_load_u8 v67, v6 offset:6400
	ds_load_u8 v6, v6 offset:6144
	s_waitcnt lgkmcnt(28)
	v_perm_b32 v7, v40, v7, 0xc0c0004
	v_perm_b32 v8, v17, v8, 0xc0c0004
	s_waitcnt lgkmcnt(24)
	v_perm_b32 v17, v44, v41, 0xc0c0004
	v_perm_b32 v40, v43, v42, 0xc0c0004
	s_waitcnt lgkmcnt(20)
	v_perm_b32 v41, v48, v45, 0xc0c0004
	v_perm_b32 v42, v47, v46, 0xc0c0004
	s_waitcnt lgkmcnt(16)
	v_perm_b32 v43, v52, v49, 0xc0c0004
	v_perm_b32 v44, v51, v50, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[20:27], v[36:39], v[32:35], v[20:27] neg_lo:[1,1,0]
	v_lshl_or_b32 v35, v8, 16, v7
	v_lshl_or_b32 v34, v40, 16, v17
	v_lshl_or_b32 v33, v42, 16, v41
	v_lshl_or_b32 v32, v44, 16, v43
	s_waitcnt lgkmcnt(12)
	v_perm_b32 v7, v56, v53, 0xc0c0004
	v_perm_b32 v8, v55, v54, 0xc0c0004
	s_waitcnt lgkmcnt(8)
	v_perm_b32 v17, v60, v57, 0xc0c0004
	v_perm_b32 v36, v59, v58, 0xc0c0004
	s_waitcnt lgkmcnt(4)
	v_perm_b32 v37, v64, v61, 0xc0c0004
	v_perm_b32 v38, v63, v62, 0xc0c0004
	s_waitcnt lgkmcnt(0)
	v_perm_b32 v6, v6, v65, 0xc0c0004
	v_perm_b32 v39, v67, v66, 0xc0c0004
	v_wmma_i32_16x16x16_iu8 v[20:27], v[32:35], v[28:31], v[20:27] neg_lo:[1,1,0]
	v_lshl_or_b32 v31, v8, 16, v7
	v_lshl_or_b32 v30, v36, 16, v17
	v_lshl_or_b32 v29, v38, 16, v37
	v_lshl_or_b32 v28, v39, 16, v6
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_wmma_i32_16x16x16_iu8 v[20:27], v[28:31], v[2:5], v[20:27] neg_lo:[1,1,0]
	v_cvt_f32_i32_e32 v3, v20
	s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_cvt_f32_i32_e32 v4, v21
	v_cvt_f32_i32_e32 v7, v22
	s_delay_alu instid0(VALU_DEP_4)
	v_cvt_f32_i32_e32 v8, v23
	v_cvt_f32_i32_e32 v5, v24
	v_cvt_f32_i32_e32 v6, v25
	v_cvt_f32_i32_e32 v17, v26
	v_cvt_f32_i32_e32 v24, v27
.LBB0_12:
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_bfe_u32 v2, v0, 4, 1
	.loc	1 404 22                        ; generate_amdgcn.py:404:22
	v_or_b32_e32 v19, s20, v19
	.loc	1 404 14 is_stmt 0              ; generate_amdgcn.py:404:14
	s_and_b32 s13, s5, 0xffff
	s_mov_b32 s15, 0x31027000
	s_mov_b32 s14, 0x7ffffffe
	.loc	1 405 22 is_stmt 1              ; generate_amdgcn.py:405:22
	v_or3_b32 v2, v2, v18, s10
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_add_f32 v5, v11, v5 :: v_dual_lshlrev_b32 v18, 1, v19
	.loc	1 404 14                        ; generate_amdgcn.py:404:14
	s_mov_b32 s12, s4
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_add_f32 v4, v10, v4 :: v_dual_add_f32 v7, v15, v7
	s_delay_alu instid0(VALU_DEP_3)
	v_dual_add_f32 v3, v9, v3 :: v_dual_lshlrev_b32 v2, 1, v2
	.loc	1 404 14                        ; generate_amdgcn.py:404:14
	buffer_load_u16 v18, v18, s[12:15], 0 offen
	.loc	1 405 14                        ; generate_amdgcn.py:405:14
	s_and_b32 s13, s7, 0xffff
	s_mov_b32 s12, s6
	v_or_b32_e32 v21, 4, v2
	buffer_load_u16 v20, v2, s[12:15], 0 offen
	v_or_b32_e32 v22, 8, v2
	v_or_b32_e32 v23, 12, v2
	v_or_b32_e32 v25, 16, v2
	buffer_load_u16 v21, v21, s[12:15], 0 offen
	v_or_b32_e32 v26, 20, v2
	v_or_b32_e32 v27, 24, v2
	v_or_b32_e32 v2, 28, v2
	s_clause 0x5
	buffer_load_u16 v22, v22, s[12:15], 0 offen
	buffer_load_u16 v23, v23, s[12:15], 0 offen
	buffer_load_u16 v25, v25, s[12:15], 0 offen
	buffer_load_u16 v26, v26, s[12:15], 0 offen
	buffer_load_u16 v27, v27, s[12:15], 0 offen
	buffer_load_u16 v2, v2, s[12:15], 0 offen
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_dual_add_f32 v6, v12, v6 :: v_dual_mov_b32 v29, 0x5410
	v_dual_add_f32 v8, v16, v8 :: v_dual_add_f32 v17, v13, v17
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b32_e32 v30, 0x7632
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v3, v9, v3, s2
	v_cndmask_b32_e64 v7, v15, v7, s2
	v_cndmask_b32_e64 v6, v12, v6, s2
	.loc	1 399 13                        ; generate_amdgcn.py:399:13
	v_add_f32_e32 v24, v14, v24
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v13, v13, v17, s2
	v_cndmask_b32_e64 v5, v11, v5, s2
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v28.h, 0
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v4, v10, v4, s2
	v_cndmask_b32_e64 v8, v16, v8, s2
	.loc	1 326 32                        ; generate_amdgcn.py:326:32
	v_lshrrev_b32_e32 v1, 1, v1
	s_mov_b32 s7, 0x76543210
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v10.h, v28.h
	v_mov_b16_e32 v11.h, v28.h
	v_mov_b16_e32 v9.h, v28.h
	v_mov_b16_e32 v16.h, v28.h
	s_and_b32 s9, s9, 0xffff
	.loc	1 404 14                        ; generate_amdgcn.py:404:14
	s_waitcnt vmcnt(8)
	v_lshlrev_b32_e32 v12, 16, v18
	.loc	1 405 14                        ; generate_amdgcn.py:405:14
	s_waitcnt vmcnt(7)
	v_lshlrev_b32_e32 v15, 16, v20
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v0, 16, v0
	.loc	1 405 14                        ; generate_amdgcn.py:405:14
	s_waitcnt vmcnt(6)
	v_lshlrev_b32_e32 v17, 16, v21
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_2) | instid1(VALU_DEP_2)
	.loc	1 406 16                        ; generate_amdgcn.py:406:16
	v_mul_f32_e32 v15, v12, v15
	.loc	1 333 5                         ; generate_amdgcn.py:333:5
	v_cndmask_b32_e64 v14, v14, v24, s2
	.loc	1 406 9                         ; generate_amdgcn.py:406:9
	s_waitcnt vmcnt(5)
	v_dual_mul_f32 v3, v3, v15 :: v_dual_lshlrev_b32 v18, 16, v22
	.loc	1 406 16 is_stmt 0              ; generate_amdgcn.py:406:16
	v_mul_f32_e32 v17, v12, v17
	.loc	1 405 14 is_stmt 1              ; generate_amdgcn.py:405:14
	s_waitcnt vmcnt(3)
	v_lshlrev_b32_e32 v21, 16, v25
	s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_3)
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_mov_b16_e32 v28.l, v3.h
	.loc	1 406 16                        ; generate_amdgcn.py:406:16
	v_mul_f32_e32 v18, v12, v18
	v_dual_mul_f32 v21, v12, v21 :: v_dual_lshlrev_b32 v20, 16, v23
	.loc	1 405 14                        ; generate_amdgcn.py:405:14
	s_waitcnt vmcnt(2)
	v_lshlrev_b32_e32 v22, 16, v26
	s_waitcnt vmcnt(1)
	v_lshlrev_b32_e32 v23, 16, v27
	.loc	1 406 16                        ; generate_amdgcn.py:406:16
	s_waitcnt vmcnt(0)
	v_dual_mul_f32 v15, v12, v20 :: v_dual_lshlrev_b32 v2, 16, v2
	.loc	1 406 9 is_stmt 0               ; generate_amdgcn.py:406:9
	v_dual_mul_f32 v5, v5, v21 :: v_dual_mul_f32 v20, v12, v22
	v_mul_f32_e32 v7, v7, v18
	s_delay_alu instid0(VALU_DEP_3)
	.loc	1 406 16                        ; generate_amdgcn.py:406:16
	v_mul_f32_e32 v2, v12, v2
	v_mul_f32_e32 v12, v12, v23
	.loc	1 416 9 is_stmt 1               ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e32 vcc_lo, v3, v3
	v_mov_b16_e32 v11.l, v5.h
	v_mov_b16_e32 v10.l, v7.h
	.loc	1 406 9                         ; generate_amdgcn.py:406:9
	v_mul_f32_e32 v4, v4, v17
	v_mul_f32_e32 v12, v13, v12
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_and_b32_e32 v13, 1, v28
	v_cmp_o_f32_e64 s2, v7, v7
	v_and_b32_e32 v11, 1, v11
	v_mov_b16_e32 v9.l, v4.h
	.loc	1 406 9                         ; generate_amdgcn.py:406:9
	v_mul_f32_e32 v8, v8, v15
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_add3_u32 v3, v3, v13, 0x7fff
	v_cmp_o_f32_e64 s0, v4, v4
	v_cmp_o_f32_e64 s4, v5, v5
	v_and_b32_e32 v9, 1, v9
	v_add3_u32 v5, v5, v11, 0x7fff
	v_cndmask_b16 v3.l, 0x7fff, v3.h, vcc_lo
	v_cmp_eq_u32_e32 vcc_lo, 0, v0
	v_and_b32_e32 v10, 1, v10
	.loc	1 406 9                         ; generate_amdgcn.py:406:9
	v_mul_f32_e32 v6, v6, v20
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_add3_u32 v4, v4, v9, 0x7fff
	v_cndmask_b16 v4.l, 0x7fff, v5.h, s4
	v_mov_b16_e32 v28.l, v8.h
	v_add3_u32 v0, v7, v10, 0x7fff
	v_cndmask_b32_e32 v7, 0x1054, v29, vcc_lo
	v_cndmask_b16 v3.h, 0x7fff, v4.h, s0
	.loc	1 406 9                         ; generate_amdgcn.py:406:9
	v_mul_f32_e32 v2, v14, v2
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cmp_o_f32_e64 s1, v8, v8
	v_cmp_o_f32_e64 s3, v6, v6
	v_lshl_or_b32 v7, v7, 8, v7
	s_delay_alu instid0(VALU_DEP_1) | instskip(SKIP_2) | instid1(VALU_DEP_2)
	v_and_b32_e32 v7, 0x540054, v7
	v_mov_b16_e32 v16.l, v12.h
	v_cmp_o_f32_e64 s6, v12, v12
	v_and_b32_e32 v9, 1, v16
	s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
	v_add3_u32 v9, v12, v9, 0x7fff
	v_cndmask_b16 v0.l, 0x7fff, v9.h, s6
	v_and_b32_e32 v14, 1, v28
	v_mov_b16_e32 v28.l, v6.h
	.loc	1 416 18 is_stmt 0              ; generate_amdgcn.py:416:18
	v_mul_lo_u32 v9, v19, s11
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	s_mov_b32 s11, s15
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_add3_u32 v5, v8, v14, 0x7fff
	v_and_b32_e32 v15, 1, v28
	v_mov_b16_e32 v28.l, v2.h
	v_cndmask_b32_e32 v8, 0x3276, v30, vcc_lo
	v_cmp_o_f32_e64 s5, v2, v2
	v_cndmask_b16 v5.h, 0x7fff, v5.h, s1
	v_add3_u32 v6, v6, v15, 0x7fff
	v_and_b32_e32 v10, 1, v28
	v_lshl_or_b32 v8, v8, 8, v8
	v_cndmask_b16 v5.l, 0x7fff, v0.h, s2
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
	v_cndmask_b16 v4.h, 0x7fff, v6.h, s3
	v_add3_u32 v2, v2, v10, 0x7fff
	s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_and_b32_e32 v8, 0x760076, v8
	v_cndmask_b32_e32 v6, v4, v3, vcc_lo
	s_delay_alu instid0(VALU_DEP_3)
	v_cndmask_b16 v0.h, 0x7fff, v2.h, s5
	v_cndmask_b32_e32 v2, v3, v4, vcc_lo
	v_lshl_or_b32 v3, v7, 4, v7
	v_lshl_or_b32 v4, v8, 4, v8
	.loc	1 416 18                        ; generate_amdgcn.py:416:18
	v_or_b32_e32 v8, s10, v1
	.loc	1 416 9                         ; generate_amdgcn.py:416:9
	v_cndmask_b32_e32 v7, v5, v0, vcc_lo
	v_cndmask_b32_e32 v5, v0, v5, vcc_lo
	v_and_b32_e32 v3, 0x5040504, v3
	v_permlanex16_b32 v2, v2, s7, 0xfedcba98 op_sel:[1,0]
	v_and_b32_e32 v4, 0x7060706, v4
	v_permlanex16_b32 v7, v7, s7, 0xfedcba98 op_sel:[1,0]
	s_mov_b32 s10, s14
	s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
	v_perm_b32 v0, v2, v6, v3
	v_perm_b32 v1, v2, v6, v4
	s_delay_alu instid0(VALU_DEP_3)
	v_perm_b32 v2, v7, v5, v3
	v_perm_b32 v3, v7, v5, v4
	v_add_lshl_u32 v4, v8, v9, 1
	buffer_store_b128 v[0:3], v4, s[8:11], 0 offen
	.loc	1 309 1 is_stmt 1               ; generate_amdgcn.py:309:1
	s_nop 0
	s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
	s_endpgm
.Ltmp16:
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
		.amdhsa_group_segment_fixed_size 0
		.amdhsa_private_segment_fixed_size 0
		.amdhsa_kernarg_size 80
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_dispatch_ptr 0
		.amdhsa_user_sgpr_queue_ptr 0
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_user_sgpr_dispatch_id 0
		.amdhsa_user_sgpr_private_segment_size 0
		.amdhsa_wavefront_size32 1
		.amdhsa_uses_dynamic_stack 0
		.amdhsa_enable_private_segment 0
		.amdhsa_system_sgpr_workgroup_id_x 1
		.amdhsa_system_sgpr_workgroup_id_y 1
		.amdhsa_system_sgpr_workgroup_id_z 0
		.amdhsa_system_sgpr_workgroup_info 0
		.amdhsa_system_vgpr_workitem_id 0
		.amdhsa_next_free_vgpr 241
		.amdhsa_next_free_sgpr 30
		.amdhsa_reserve_vcc 1
		.amdhsa_float_round_mode_32 0
		.amdhsa_float_round_mode_16_64 0
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_float_denorm_mode_16_64 3
		.amdhsa_dx10_clamp 1
		.amdhsa_ieee_mode 1
		.amdhsa_fp16_overflow 0
		.amdhsa_workgroup_processor_mode 1
		.amdhsa_memory_ordered 1
		.amdhsa_forward_progress 1
		.amdhsa_shared_vgpr_count 0
		.amdhsa_inst_pref_size 50
		.amdhsa_exception_fp_ieee_invalid_op 0
		.amdhsa_exception_fp_denorm_src 0
		.amdhsa_exception_fp_ieee_div_zero 0
		.amdhsa_exception_fp_ieee_overflow 0
		.amdhsa_exception_fp_ieee_underflow 0
		.amdhsa_exception_fp_ieee_inexact 0
		.amdhsa_exception_int_div_zero 0
	.end_amdhsa_kernel
	.text
.Lfunc_end0:
	.size	amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk, .Lfunc_end0-amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
	.cfi_endproc
                                        ; -- End function
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.num_vgpr, 122
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.num_agpr, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.numbered_sgpr, 30
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.num_named_barrier, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.private_seg_size, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.uses_vcc, 1
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.uses_flat_scratch, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.has_dyn_sized_stack, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.has_recursion, 0
	.set amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.has_indirect_call, 0
	.section	.AMDGPU.csdata,"",@progbits
; Kernel info:
; codeLenInByte = 6332
; TotalNumSgprs: 32
; NumVgprs: 122
; ScratchSize: 0
; MemoryBound: 0
; FloatMode: 240
; IeeeMode: 1
; LDSByteSize: 0 bytes/workgroup (compile time only)
; SGPRBlocks: 0
; VGPRBlocks: 30
; NumSGPRsForWavesPerEU: 32
; NumVGPRsForWavesPerEU: 241
; Occupancy: 5
; WaveLimiterHint : 0
; COMPUTE_PGM_RSRC2:SCRATCH_EN: 0
; COMPUTE_PGM_RSRC2:USER_SGPR: 2
; COMPUTE_PGM_RSRC2:TRAP_HANDLER: 0
; COMPUTE_PGM_RSRC2:TGID_X_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Y_EN: 1
; COMPUTE_PGM_RSRC2:TGID_Z_EN: 0
; COMPUTE_PGM_RSRC2:TIDIG_COMP_CNT: 0
	.text
	.p2alignl 7, 3214868480
	.fill 96, 4, 3214868480
	.section	.AMDGPU.gpr_maximums,"",@progbits
	.set amdgpu.max_num_vgpr, 0
	.set amdgpu.max_num_agpr, 0
	.set amdgpu.max_num_sgpr, 0
	.set amdgpu.max_num_named_barrier, 0
	.text
	.section	.debug_abbrev,"",@progbits
	.byte	1                               ; Abbreviation Code
	.byte	17                              ; DW_TAG_compile_unit
	.byte	1                               ; DW_CHILDREN_yes
	.byte	37                              ; DW_AT_producer
	.byte	14                              ; DW_FORM_strp
	.byte	19                              ; DW_AT_language
	.byte	5                               ; DW_FORM_data2
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	16                              ; DW_AT_stmt_list
	.byte	23                              ; DW_FORM_sec_offset
	.byte	27                              ; DW_AT_comp_dir
	.byte	14                              ; DW_FORM_strp
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	2                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	0                               ; DW_CHILDREN_no
	.byte	3                               ; DW_AT_name
	.byte	14                              ; DW_FORM_strp
	.byte	32                              ; DW_AT_inline
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	3                               ; Abbreviation Code
	.byte	46                              ; DW_TAG_subprogram
	.byte	1                               ; DW_CHILDREN_yes
	.byte	17                              ; DW_AT_low_pc
	.byte	1                               ; DW_FORM_addr
	.byte	18                              ; DW_AT_high_pc
	.byte	6                               ; DW_FORM_data4
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	4                               ; Abbreviation Code
	.byte	29                              ; DW_TAG_inlined_subroutine
	.byte	0                               ; DW_CHILDREN_no
	.byte	49                              ; DW_AT_abstract_origin
	.byte	19                              ; DW_FORM_ref4
	.byte	85                              ; DW_AT_ranges
	.byte	23                              ; DW_FORM_sec_offset
	.byte	88                              ; DW_AT_call_file
	.byte	11                              ; DW_FORM_data1
	.byte	89                              ; DW_AT_call_line
	.byte	5                               ; DW_FORM_data2
	.byte	87                              ; DW_AT_call_column
	.byte	11                              ; DW_FORM_data1
	.byte	0                               ; EOM(1)
	.byte	0                               ; EOM(2)
	.byte	0                               ; EOM(3)
	.section	.debug_info,"",@progbits
.Lcu_begin0:
	.long	.Ldebug_info_end0-.Ldebug_info_start0 ; Length of Unit
.Ldebug_info_start0:
	.short	4                               ; DWARF version number
	.long	.debug_abbrev                   ; Offset Into Abbrev. Section
	.byte	8                               ; Address Size (in bytes)
	.byte	1                               ; Abbrev [1] 0xb:0x5f DW_TAG_compile_unit
	.long	.Linfo_string0                  ; DW_AT_producer
	.short	2                               ; DW_AT_language
	.long	.Linfo_string1                  ; DW_AT_name
	.long	.Lline_table_start0             ; DW_AT_stmt_list
	.long	.Linfo_string2                  ; DW_AT_comp_dir
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.byte	2                               ; Abbrev [2] 0x2a:0x6 DW_TAG_subprogram
	.long	.Linfo_string3                  ; DW_AT_name
	.byte	1                               ; DW_AT_inline
	.byte	3                               ; Abbrev [3] 0x30:0x39 DW_TAG_subprogram
	.quad	.Lfunc_begin0                   ; DW_AT_low_pc
	.long	.Lfunc_end0-.Lfunc_begin0       ; DW_AT_high_pc
	.long	42                              ; DW_AT_abstract_origin
	.byte	4                               ; Abbrev [4] 0x41:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges0                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	317                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x4e:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges1                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	316                             ; DW_AT_call_line
	.byte	17                              ; DW_AT_call_column
	.byte	4                               ; Abbrev [4] 0x5b:0xd DW_TAG_inlined_subroutine
	.long	42                              ; DW_AT_abstract_origin
	.long	.Ldebug_ranges2                 ; DW_AT_ranges
	.byte	1                               ; DW_AT_call_file
	.short	333                             ; DW_AT_call_line
	.byte	28                              ; DW_AT_call_column
	.byte	0                               ; End Of Children Mark
	.byte	0                               ; End Of Children Mark
.Ldebug_info_end0:
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.quad	.Ltmp1-.Lfunc_begin0
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges1:
	.quad	.Ltmp2-.Lfunc_begin0
	.quad	.Ltmp3-.Lfunc_begin0
	.quad	.Ltmp4-.Lfunc_begin0
	.quad	.Ltmp5-.Lfunc_begin0
	.quad	.Ltmp6-.Lfunc_begin0
	.quad	.Ltmp7-.Lfunc_begin0
	.quad	.Ltmp8-.Lfunc_begin0
	.quad	.Ltmp9-.Lfunc_begin0
	.quad	.Ltmp10-.Lfunc_begin0
	.quad	.Ltmp11-.Lfunc_begin0
	.quad	0
	.quad	0
.Ldebug_ranges2:
	.quad	.Ltmp12-.Lfunc_begin0
	.quad	.Ltmp13-.Lfunc_begin0
	.quad	.Ltmp14-.Lfunc_begin0
	.quad	.Ltmp15-.Lfunc_begin0
	.quad	0
	.quad	0
	.section	.debug_str,"MS",@progbits,1
.Linfo_string0:
	.asciz	"triton"                        ; string offset=0 ; triton
.Linfo_string1:
	.asciz	"generate_amdgcn.py"            ; string offset=7 ; generate_amdgcn.py
.Linfo_string2:
	.asciz	"<repo>/scripts" ; string offset=26 ; <repo>/scripts
.Linfo_string3:
	.asciz	"amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk"             ; string offset=75 ; amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
	.section	".note.GNU-stack","",@progbits
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - .address_space:  global
        .offset:         0
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         8
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         16
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         24
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         32
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         40
        .size:           8
        .value_kind:     global_buffer
      - .offset:         48
        .size:           4
        .value_kind:     by_value
      - .offset:         52
        .size:           4
        .value_kind:     by_value
      - .offset:         56
        .size:           4
        .value_kind:     by_value
      - .address_space:  global
        .offset:         64
        .size:           8
        .value_kind:     global_buffer
      - .address_space:  global
        .offset:         72
        .size:           8
        .value_kind:     global_buffer
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 80
    .max_flat_workgroup_size: 256
    .name:           amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk
    .private_segment_fixed_size: 0
    .sgpr_count:     32
    .sgpr_spill_count: 0
    .symbol:         amd_strix_halo_gfx1151_int8xint8_nn_pc_none_bm16_bn128_bk64_gm4_w8_s3_weu2_sk1_maskk.kd
    .uniform_work_group_size: 1
    .uses_dynamic_stack: false
    .vgpr_count:     122
    .vgpr_spill_count: 0
    .wavefront_size: 32
    .workgroup_processor_mode: 1
amdhsa.target:   amdgcn-amd-amdhsa--gfx1151
amdhsa.version:
  - 1
  - 2
...

	.end_amdgpu_metadata
	.section	.debug_line,"",@progbits
.Lline_table_start0:
