...
def make_inputs(kernel: KernelMetadata, shape: BenchmarkShape) -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    ...
    if kernel.scale.mode is ScaleMode.PER_CHANNEL:
        ...
        expected = torch.zeros((shape.m, shape.n), device="cuda", dtype=torch.float32)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "benchmarks" / "gemm_tuning.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not torch.cuda.is_available() or torch.version.hip is None:
        raise RuntimeError("GEMM tuning requires a ROCm torch CUDA/HIP device")
    if args.warmup_ms < 0:
    ...
    if not records:
        raise RuntimeError(f"all GEMM tuning candidates failed: {failures}")
